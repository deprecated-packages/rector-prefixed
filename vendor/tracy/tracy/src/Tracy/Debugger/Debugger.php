...
namespace RectorPrefix20210429\Tracy;
...
class Debugger
{
    ...
    public const DEBUG = \RectorPrefix20210429\Tracy\ILogger::DEBUG, INFO = \RectorPrefix20210429\Tracy\ILogger::INFO, WARNING = \RectorPrefix20210429\Tracy\ILogger::WARNING, ERROR = \RectorPrefix20210429\Tracy\ILogger::ERROR, EXCEPTION = \RectorPrefix20210429\Tracy\ILogger::EXCEPTION, CRITICAL = \RectorPrefix20210429\Tracy\ILogger::CRITICAL;
    /********************* misc ****************d*g**/
    ...
    public static function shutdownHandler() : void
    {
        ...
        if (\in_array($error['type'] ?? null, [\E_ERROR, \E_CORE_ERROR, \E_COMPILE_ERROR, \E_PARSE, \E_RECOVERABLE_ERROR, \E_USER_ERROR], \true)) {
            self::exceptionHandler(\RectorPrefix20210429\Tracy\Helpers::fixStack(new \ErrorException($error['message'], 0, $error['type'], $error['file'], $error['line'])));
    ...
    public static function exceptionHandler(\Throwable $exception) : void
    {
        ...
        \RectorPrefix20210429\Tracy\Helpers::improveException($exception);
        ...
        if (self::$productionMode || \connection_aborted()) {
            ...
            if (!$firstTime) {
                // nothing
            } elseif (\RectorPrefix20210429\Tracy\Helpers::isHtmlMode()) {
                if (!\headers_sent()) {
        ...
        } elseif ($firstTime && \RectorPrefix20210429\Tracy\Helpers::isHtmlMode() || \RectorPrefix20210429\Tracy\Helpers::isAjax()) {
    ...
    public static function errorHandler(int $severity, string $message, string $file, int $line, array $context = null) : ?bool
    {
        ...
        if ($severity === \E_RECOVERABLE_ERROR || $severity === \E_USER_ERROR) {
            if (\RectorPrefix20210429\Tracy\Helpers::findTrace(\debug_backtrace(\DEBUG_BACKTRACE_IGNORE_ARGS), '*::__toString')) {
            ...
            if (($severity & self::$logSeverity) === $severity) {
                $e = new \ErrorException($message, 0, $severity, $file, $line);
                $e->context = $context;
                \RectorPrefix20210429\Tracy\Helpers::improveException($e);
            } else {
                $e = 'PHP ' . \RectorPrefix20210429\Tracy\Helpers::errorTypeToString($severity) . ': ' . \RectorPrefix20210429\Tracy\Helpers::improveError($message, (array) $context) . " in {$file}:{$line}";
            }
            try {
            ...
            $message = 'PHP ' . \RectorPrefix20210429\Tracy\Helpers::errorTypeToString($severity) . ': ' . \RectorPrefix20210429\Tracy\Helpers::improveError($message, (array) $context);
            ...
            if ($count++) {
                ...
                return \RectorPrefix20210429\Tracy\Helpers::isHtmlMode() || \RectorPrefix20210429\Tracy\Helpers::isAjax() ? null : \false;
    ...
    /********************* services ****************d*g**/
    public static function getBlueScreen() : \RectorPrefix20210429\Tracy\BlueScreen
    {
        if (!self::$blueScreen) {
            self::$blueScreen = new \RectorPrefix20210429\Tracy\BlueScreen();
            self::$blueScreen->info = ['PHP ' . \PHP_VERSION, $_SERVER['SERVER_SOFTWARE'] ?? null, 'Tracy ' . self::VERSION];
        }
        return self::$blueScreen;
    }
    public static function getBar() : \RectorPrefix20210429\Tracy\Bar
    {
        if (!self::$bar) {
            self::$bar = new \RectorPrefix20210429\Tracy\Bar();
            self::$bar->addPanel($info = new \RectorPrefix20210429\Tracy\DefaultBarPanel('info'), 'Tracy:info');
            $info->cpuUsage = self::$cpuUsage;
            self::$bar->addPanel(new \RectorPrefix20210429\Tracy\DefaultBarPanel('errors'), 'Tracy:errors');
            // filled by errorHandler()
        }
        return self::$bar;
    }
    public static function setLogger(\RectorPrefix20210429\Tracy\ILogger $logger) : void
    {
        self::$logger = $logger;
    }
    public static function getLogger() : \RectorPrefix20210429\Tracy\ILogger
    {
        if (!self::$logger) {
            self::$logger = new \RectorPrefix20210429\Tracy\Logger(self::$logDirectory, self::$email, self::getBlueScreen());
    ...
    public static function getFireLogger() : \RectorPrefix20210429\Tracy\ILogger
    {
        if (!self::$fireLogger) {
            self::$fireLogger = new \RectorPrefix20210429\Tracy\FireLogger();
        }
    ...
    {
        if ($return) {
            $options = [\RectorPrefix20210429\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20210429\Tracy\Dumper::TRUNCATE => self::$maxLength];
            return \PHP_SAPI === 'cli' || \PHP_SAPI === 'phpdbg' ? \RectorPrefix20210429\Tracy\Dumper::toText($var) : \RectorPrefix20210429\Tracy\Helpers::capture(function () use($var, $options) {
                \RectorPrefix20210429\Tracy\Dumper::dump($var, $options);
            });
        } elseif (!self::$productionMode) {
            \RectorPrefix20210429\Tracy\Dumper::dump($var, [\RectorPrefix20210429\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20210429\Tracy\Dumper::TRUNCATE => self::$maxLength, \RectorPrefix20210429\Tracy\Dumper::LOCATION => self::$showLocation, \RectorPrefix20210429\Tracy\Dumper::THEME => self::$dumpTheme]);
        }
        return $var;
    }
    /**
    ...
    public static function barDump($var, string $title = null, array $options = [])
    {
        if (!self::$productionMode) {
            static $panel;
            if (!$panel) {
                self::getBar()->addPanel($panel = new \RectorPrefix20210429\Tracy\DefaultBarPanel('dumps'), 'Tracy:dumps');
            }
            $panel->data[] = ['title' => $title, 'dump' => \RectorPrefix20210429\Tracy\Dumper::toHtml($var, $options + [\RectorPrefix20210429\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20210429\Tracy\Dumper::TRUNCATE => self::$maxLength, \RectorPrefix20210429\Tracy\Dumper::LOCATION => self::$showLocation ?: \RectorPrefix20210429\Tracy\Dumper::LOCATION_CLASS | \RectorPrefix20210429\Tracy\Dumper::LOCATION_SOURCE, \RectorPrefix20210429\Tracy\Dumper::LAZY => \true])];
        }
        return $var;
    }
    /**
     ...
     * @return mixed
     */
    public static function log($message, string $level = \RectorPrefix20210429\Tracy\ILogger::INFO)
    {
