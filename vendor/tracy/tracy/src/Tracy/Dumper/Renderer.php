...
namespace RectorPrefix20210509\Tracy\Dumper;

use RectorPrefix20210509\Tracy\Helpers;
/**
...
final class Renderer
{
    ...
    public function renderAsHtml(\stdClass $model) : string
    {
        ...
        if ($model->location && $this->sourceLocation) {
            ...
            $uri = \RectorPrefix20210509\Tracy\Helpers::editorUri($file, $line);
            $location = \RectorPrefix20210509\Tracy\Helpers::formatHtml('<a href="%" class="tracy-dump-location" title="in file % on line %%">', $uri ?? '#', $file, $line, $uri ? "\nClick to open in editor" : '') . \RectorPrefix20210509\Tracy\Helpers::encodeString($code, 50) . " 📍</a\n>";
        }
    ...
    private function renderVar($value, int $depth = 0, $keyType = null) : string
    {
        switch (\true) {
            ...
            case \is_array($value):
            case $value->type === \RectorPrefix20210509\Tracy\Dumper\Value::TYPE_ARRAY:
                return $this->renderArray($value, $depth);
            case $value->type === \RectorPrefix20210509\Tracy\Dumper\Value::TYPE_REF:
                return $this->renderVar($this->snapshot[$value->value], $depth, $keyType);
            case $value->type === \RectorPrefix20210509\Tracy\Dumper\Value::TYPE_OBJECT:
                return $this->renderObject($value, $depth);
            case $value->type === \RectorPrefix20210509\Tracy\Dumper\Value::TYPE_NUMBER:
                return '<span class="tracy-dump-number">' . \RectorPrefix20210509\Tracy\Helpers::escapeHtml($value->value) . '</span>';
            case $value->type === \RectorPrefix20210509\Tracy\Dumper\Value::TYPE_TEXT:
                return '<span class="tracy-dump-virtual">' . \RectorPrefix20210509\Tracy\Helpers::escapeHtml($value->value) . '</span>';
            case $value->type === \RectorPrefix20210509\Tracy\Dumper\Value::TYPE_STRING_HTML:
            case $value->type === \RectorPrefix20210509\Tracy\Dumper\Value::TYPE_BINARY_HTML:
                return $this->renderString($value, $depth, $keyType);
            case $value->type === \RectorPrefix20210509\Tracy\Dumper\Value::TYPE_RESOURCE:
    ...
    private function renderString($str, int $depth, $keyType) : string
    {
        if ($keyType === self::TYPE_ARRAY_KEY) {
            ...
            return '<span class="tracy-dump-string">' . "<span class='tracy-dump-lq'>'</span>" . (\is_string($str) ? \RectorPrefix20210509\Tracy\Helpers::escapeHtml($str) : \str_replace("\n", "\n" . $indent, $str->value)) . "<span>'</span>" . '</span>';
        } elseif ($keyType !== null) {
            static $classes = [\RectorPrefix20210509\Tracy\Dumper\Value::PROP_PUBLIC => 'tracy-dump-public', \RectorPrefix20210509\Tracy\Dumper\Value::PROP_PROTECTED => 'tracy-dump-protected', \RectorPrefix20210509\Tracy\Dumper\Value::PROP_DYNAMIC => 'tracy-dump-dynamic', \RectorPrefix20210509\Tracy\Dumper\Value::PROP_VIRTUAL => 'tracy-dump-virtual'];
            $indent = '<span class="tracy-dump-indent">   ' . \str_repeat('|  ', $depth - 1) . ' </span>';
            $title = \is_string($keyType) ? ' title="declared in ' . \RectorPrefix20210509\Tracy\Helpers::escapeHtml($keyType) . '"' : null;
            return '<span class="' . ($title ? 'tracy-dump-private' : $classes[$keyType]) . '"' . $title . '>' . (\is_string($str) ? \RectorPrefix20210509\Tracy\Helpers::escapeHtml($str) : "<span class='tracy-dump-lq'>'</span>" . \str_replace("\n", "\n" . $indent, $str->value) . "<span>'</span>") . '</span>';
        } elseif (\is_string($str)) {
            $len = \strlen(\utf8_decode($str));
            return '<span class="tracy-dump-string"' . ($len > 1 ? ' title="' . $len . ' characters"' : '') . '>' . "<span>'</span>" . \RectorPrefix20210509\Tracy\Helpers::escapeHtml($str) . "<span>'</span>" . '</span>';
        } else {
            $unit = $str->type === \RectorPrefix20210509\Tracy\Dumper\Value::TYPE_STRING_HTML ? 'characters' : 'bytes';
    ...
    private function renderArray($array, int $depth) : string
    {
        ...
        if (\is_array($array)) {
            ...
            if ($array->id && isset($this->parents[$array->id])) {
                ...
                if ($this->lazy !== \false) {
                    $ref = new \RectorPrefix20210509\Tracy\Dumper\Value(\RectorPrefix20210509\Tracy\Dumper\Value::TYPE_REF, $array->id);
        ...
        if ($collapsed && $this->lazy !== \false) {
            $array = isset($array->id) ? new \RectorPrefix20210509\Tracy\Dumper\Value(\RectorPrefix20210509\Tracy\Dumper\Value::TYPE_REF, $array->id) : $array;
    ...
    private function renderObject(\RectorPrefix20210509\Tracy\Dumper\Value $object, int $depth) : string
    {
        $editorAttributes = '';
        if ($this->classLocation && $object->editor) {
            $editorAttributes = \RectorPrefix20210509\Tracy\Helpers::formatHtml(' title="Declared in file % on line %%%" data-tracy-href="%"', $object->editor->file, $object->editor->line, $object->editor->url ? "\nCtrl-Click to open in editor" : '', "\nAlt-Click to expand/collapse all child nodes", $object->editor->url);
        }
        $out = '<span class="tracy-dump-object"' . $editorAttributes . '>' . \RectorPrefix20210509\Tracy\Helpers::escapeHtml($object->value) . '</span>' . ($object->id ? ' <span class="tracy-dump-hash">#' . $object->id . '</span>' : '');
        if ($object->items === null) {
            ...
            if ($this->lazy !== \false) {
                $ref = new \RectorPrefix20210509\Tracy\Dumper\Value(\RectorPrefix20210509\Tracy\Dumper\Value::TYPE_REF, $object->id);
                $this->copySnapshot($ref);
                return '<span class="tracy-toggle tracy-collapsed" data-tracy-dump=\'' . \json_encode($ref) . "'>" . $out . '</span>';
            }
        ...
        $span = '<span class="tracy-toggle' . ($collapsed ? ' tracy-collapsed' : '') . '"';
        if ($collapsed && $this->lazy !== \false) {
            $value = $object->id ? new \RectorPrefix20210509\Tracy\Dumper\Value(\RectorPrefix20210509\Tracy\Dumper\Value::TYPE_REF, $object->id) : $object;
        ...
        foreach ($object->items as $info) {
            [$k, $v, $type, $ref] = $info + [2 => \RectorPrefix20210509\Tracy\Dumper\Value::PROP_VIRTUAL, null];
    ...
    private function renderResource(\RectorPrefix20210509\Tracy\Dumper\Value $resource, int $depth) : string
    {
        $out = '<span class="tracy-dump-resource">' . \RectorPrefix20210509\Tracy\Helpers::escapeHtml($resource->value) . '</span> ' . '<span class="tracy-dump-hash">@' . \substr($resource->id, 1) . '</span>';
        if (!$resource->items) {
            return $out;
        } elseif (isset($this->above[$resource->id])) {
            if ($this->lazy !== \false) {
                $ref = new \RectorPrefix20210509\Tracy\Dumper\Value(\RectorPrefix20210509\Tracy\Dumper\Value::TYPE_REF, $resource->id);
                $this->copySnapshot($ref);
                return '<span class="tracy-toggle tracy-collapsed" data-tracy-dump=\'' . \json_encode($ref) . "'>" . $out . '</span>';
            }
            ...
            foreach ($resource->items as [$k, $v]) {
                $out .= '<span class="tracy-dump-indent">   ' . \str_repeat('|  ', $depth) . '</span>' . $this->renderVar($k, $depth + 1, \RectorPrefix20210509\Tracy\Dumper\Value::PROP_VIRTUAL) . ': ' . ($tmp = $this->renderVar($v, $depth + 1)) . (\substr($tmp, -6) === '</div>' ? '' : "\n");
            }
            return $out . '</div>';
        }
    }
    private function copySnapshot($value) : void
    {
        ...
        } elseif ($value instanceof \RectorPrefix20210509\Tracy\Dumper\Value && $value->type === \RectorPrefix20210509\Tracy\Dumper\Value::TYPE_REF) {
        ...
        } elseif ($value instanceof \RectorPrefix20210509\Tracy\Dumper\Value && $value->items) {
