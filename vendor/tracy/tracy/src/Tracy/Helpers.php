...
class Helpers
{
    ...
    {
        $file = \strtr($origFile = $file, \RectorPrefix20210424\Tracy\Debugger::$editorMapping);
    ...
    {
        if (\RectorPrefix20210424\Tracy\Debugger::$editor && $file && ($action === 'create' || \is_file($file))) {
            $file = \strtr($file, '/', \DIRECTORY_SEPARATOR);
            $file = \strtr($file, \RectorPrefix20210424\Tracy\Debugger::$editorMapping);
            return \strtr(\RectorPrefix20210424\Tracy\Debugger::$editor, ['%action' => $action, '%file' => \rawurlencode($file), '%line' => $line ?: 1, '%search' => \rawurlencode($search), '%replace' => \rawurlencode($replace)]);
        }
        return null;
    }
