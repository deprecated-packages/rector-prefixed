...
class BlueScreen
{
    ...
    {
        if (\RectorPrefix20210429\Tracy\Helpers::isAjax() && \session_status() === \PHP_SESSION_ACTIVE) {
            $_SESSION['_tracy']['bluescreen'][$_SERVER['HTTP_X_TRACY_AJAX']] = ['content' => \RectorPrefix20210429\Tracy\Helpers::capture(function () use($exception) {
    ...
    private function renderTemplate(\Throwable $exception, string $template, $toScreen = \true) : void
    {
        ...
        $source = \RectorPrefix20210429\Tracy\Helpers::getSource();
        $title = $exception instanceof \ErrorException ? \RectorPrefix20210429\Tracy\Helpers::errorTypeToString($exception->getSeverity()) : \RectorPrefix20210429\Tracy\Helpers::getClass($exception);
        ...
        $snapshot = [];
        $dump = $this->getDumper();
        $css = \array_map('file_get_contents', \array_merge([__DIR__ . '/assets/bluescreen.css', __DIR__ . '/../Toggle/toggle.css', __DIR__ . '/../TableSort/table-sort.css', __DIR__ . '/../Dumper/assets/dumper-light.css'], \RectorPrefix20210429\Tracy\Debugger::$customCssFiles));
        $css = \RectorPrefix20210429\Tracy\Helpers::minifyCss(\implode($css));
        $nonce = $toScreen ? \RectorPrefix20210429\Tracy\Helpers::getNonce() : null;
    ...
    private function renderPanels(?\Throwable $ex) : array
    {
        ...
        foreach ($this->panels as $callback) {
            ...
            $res[] = (object) ['tab' => "Error in panel {$name}", 'panel' => \nl2br(\RectorPrefix20210429\Tracy\Helpers::escapeHtml($e))];
        }
        return $res;
    }
    /**
    ...
    private function renderActions(\Throwable $ex) : array
    {
        ...
        if (\preg_match('# ([\'"])(\\w{3,}(?:\\\\\\w{3,})+)\\1#i', $ex->getMessage(), $m)) {
            $class = $m[2];
            if (!\class_exists($class) && !\interface_exists($class) && !\trait_exists($class) && ($file = \RectorPrefix20210429\Tracy\Helpers::guessClassFile($class)) && !\is_file($file)) {
                $actions[] = ['link' => \RectorPrefix20210429\Tracy\Helpers::editorUri($file, 1, 'create'), 'label' => 'create class'];
            }
        }
        if (\preg_match('# ([\'"])((?:/|[a-z]:[/\\\\])\\w[^\'"]+\\.\\w{2,5})\\1#i', $ex->getMessage(), $m)) {
            $file = $m[2];
            $actions[] = ['link' => \RectorPrefix20210429\Tracy\Helpers::editorUri($file, 1, $label = \is_file($file) ? 'open' : 'create'), 'label' => $label . ' file'];
        }
        $query = ($ex instanceof \ErrorException ? '' : \RectorPrefix20210429\Tracy\Helpers::getClass($ex) . ' ') . \preg_replace('#\'.*\'|".*"#Us', '', $ex->getMessage());
        $actions[] = ['link' => 'https://www.google.com/search?sourceid=tracy&q=' . \urlencode($query), 'label' => 'search', 'external' => \true];
        if ($ex instanceof \ErrorException && !empty($ex->skippable) && \preg_match('#^https?://#', $source = \RectorPrefix20210429\Tracy\Helpers::getSource())) {
    ...
    public static function highlightFile(string $file, int $line, int $lines = 15) : ?string
    {
        ...
        if ($editor = \RectorPrefix20210429\Tracy\Helpers::editorUri($file, $line)) {
            $source = \substr_replace($source, ' title="Ctrl-Click to open in editor" data-tracy-href="' . \RectorPrefix20210429\Tracy\Helpers::escapeHtml($editor) . '"', 4, 0);
        }
        return $source;
    }
    /**
    ...
    public function getDumper() : \Closure
    {
        return function ($v, $k = null) : string {
            return \RectorPrefix20210429\Tracy\Dumper::toHtml($v, [\RectorPrefix20210429\Tracy\Dumper::DEPTH => $this->maxDepth, \RectorPrefix20210429\Tracy\Dumper::TRUNCATE => $this->maxLength, \RectorPrefix20210429\Tracy\Dumper::SNAPSHOT => &$this->snapshot, \RectorPrefix20210429\Tracy\Dumper::LOCATION => \RectorPrefix20210429\Tracy\Dumper::LOCATION_CLASS, \RectorPrefix20210429\Tracy\Dumper::SCRUBBER => $this->scrubber, \RectorPrefix20210429\Tracy\Dumper::KEYS_TO_HIDE => $this->keysToHide], $k);
        };
    }
    public function formatMessage(\Throwable $exception) : string
    {
        $msg = \RectorPrefix20210429\Tracy\Helpers::encodeString(\trim((string) $exception->getMessage()), self::MAX_MESSAGE_LENGTH);
        $msg = \str_replace('<i>\\n</i>', '', $msg);
        // highlight 'string'
        ...
        $msg = \preg_replace_callback('#(\\w+\\\\[\\w\\\\]+\\w)(?:::(\\w+))?#', function ($m) {
            ...
            return '<a href="' . \RectorPrefix20210429\Tracy\Helpers::escapeHtml(\RectorPrefix20210429\Tracy\Helpers::editorUri($r->getFileName(), $r->getStartLine())) . '" class="tracy-editor">' . $m[0] . '</a>';
        }, $msg);
        // clickable file name
        $msg = \preg_replace_callback('#([\\w\\\\/.:-]+\\.(?:php|phpt|phtml|latte|neon))(?|:(\\d+)| on line (\\d+))?#', function ($m) {
            return @\is_file($m[1]) ? '<a href="' . \RectorPrefix20210429\Tracy\Helpers::escapeHtml(\RectorPrefix20210429\Tracy\Helpers::editorUri($m[1], isset($m[2]) ? (int) $m[2] : null)) . '" class="tracy-editor">' . $m[0] . '</a>' : $m[0];
        }, $msg);
        return $msg;
    }
    private function renderPhpInfo() : void
    {
        ...
        if (\strpos($license, '<body') === \false) {
            echo '<pre class="tracy-dump tracy-light">', \RectorPrefix20210429\Tracy\Helpers::escapeHtml($info), '</pre>';
        } else {
