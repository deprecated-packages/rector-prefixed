 ...
 * Logger.
 */
class Logger implements \RectorPrefix20210501\Tracy\ILogger
{
     ...
     */
    public function __construct(?string $directory, $email = null, \RectorPrefix20210501\Tracy\BlueScreen $blueScreen = null)
    {
    ...
    public static function formatMessage($message) : string
    {
        if ($message instanceof \Throwable) {
            while ($message) {
                $tmp[] = ($message instanceof \ErrorException ? \RectorPrefix20210501\Tracy\Helpers::errorTypeToString($message->getSeverity()) . ': ' . $message->getMessage() : \RectorPrefix20210501\Tracy\Helpers::getClass($message) . ': ' . $message->getMessage() . ($message->getCode() ? ' #' . $message->getCode() : '')) . ' in ' . $message->getFile() . ':' . $message->getLine();
                $message = $message->getPrevious();
            }
            $message = \implode("\ncaused by ", $tmp);
        } elseif (!\is_string($message)) {
            $message = \RectorPrefix20210501\Tracy\Dumper::toText($message);
        }
        return \trim($message);
    }
    /**
    ...
    {
        return \implode(' ', [\date('[Y-m-d H-i-s]'), \preg_replace('#\\s*\\r?\\n\\s*#', ' ', static::formatMessage($message)), ' @  ' . \RectorPrefix20210501\Tracy\Helpers::getSource(), $exceptionFile ? ' @@  ' . \basename($exceptionFile) : null]);
    }
    ...
    protected function logException(\Throwable $exception, string $file = null) : string
    {
        ...
        $bs = $this->blueScreen ?: new \RectorPrefix20210501\Tracy\BlueScreen();
