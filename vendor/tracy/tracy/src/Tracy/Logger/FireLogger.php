 ...
 */
class FireLogger implements \RectorPrefix20210509\Tracy\ILogger
{
    /** @var int  */
    public $maxDepth = 3;
    /** @var int  */
    ...
    public function log($message, $level = self::DEBUG) : bool
    {
        ...
        $item = ['name' => 'PHP', 'level' => $level, 'order' => \count($this->payload['logs']), 'time' => \str_pad(\number_format((\microtime(\true) - \RectorPrefix20210509\Tracy\Debugger::$time) * 1000, 1, '.', ' '), 8, '0', \STR_PAD_LEFT) . ' ms', 'template' => '', 'message' => '', 'style' => 'background:#767ab6'];
        $args = \func_get_args();
        ...
        if (isset($args[0]) && $args[0] instanceof \Throwable) {
            $e = \array_shift($args);
            $trace = $e->getTrace();
            if (isset($trace[0]['class']) && $trace[0]['class'] === \RectorPrefix20210509\Tracy\Debugger::class && ($trace[0]['function'] === 'shutdownHandler' || $trace[0]['function'] === 'errorHandler')) {
                unset($trace[0]);
            }
            $file = \str_replace(\dirname($e->getFile(), 3), "…", $e->getFile());
            $item['template'] = ($e instanceof \ErrorException ? '' : \RectorPrefix20210509\Tracy\Helpers::getClass($e) . ': ') . $e->getMessage() . ($e->getCode() ? ' #' . $e->getCode() : '') . ' in ' . $file . ':' . $e->getLine();
            $item['pathname'] = $e->getFile();
            $item['lineno'] = $e->getLine();
        } else {
            $trace = \debug_backtrace();
            if (isset($trace[1]['class']) && $trace[1]['class'] === \RectorPrefix20210509\Tracy\Debugger::class && $trace[1]['function'] === 'fireLog') {
                unset($trace[0]);
            }
    ...
    private function jsonDump(&$var, int $level = 0)
    {
        if (\is_bool($var) || $var === null || \is_int($var) || \is_float($var)) {
            ...
            $var = \RectorPrefix20210509\Tracy\Helpers::encodeString($var, $this->maxLength);
            ...
            if (\in_array($var, $list, \true)) {
                ...
                $list[] = $var;
                $res = ["\0" => '(object) ' . \RectorPrefix20210509\Tracy\Helpers::getClass($var)];
                foreach ($arr as $k => &$v) {
                    if (isset($k[0]) && $k[0] === "\0") {
