...
use _PhpScoperabd03f0baf05\RingCentral\Psr7\StreamDecoratorTrait;
class Str extends \_PhpScoperabd03f0baf05\RingCentral\Psr7\StreamDecoratorTrait implements \_PhpScoperabd03f0baf05\Psr\Http\Message\StreamInterface
{
}
/**
 * @covers RingCentral\Psr7\StreamDecoratorTrait
 */
class StreamDecoratorTraitTest extends \_PhpScoperabd03f0baf05\PHPUnit_Framework_TestCase
{
    private $a;
    private $b;
    private $c;
    public function setUp()
    {
        ...
        \fwrite($this->c, 'foo');
        \fseek($this->c, 0);
        $this->a = \_PhpScoperabd03f0baf05\RingCentral\Psr7\stream_for($this->c);
        $this->b = new \_PhpScoperabd03f0baf05\RingCentral\Tests\Psr7\Str($this->a);
    }
    public function testCatchesExceptionsWhenCastingToString()
    {
        $s = $this->getMockBuilder('_PhpScoperabd03f0baf05\\Psr\\Http\\Message\\StreamInterface')->setMethods(array('read'))->getMockForAbstractClass();
        $s->expects($this->once())->method('read')->will($this->throwException(new \Exception('foo')));
        $msg = '';
        \set_error_handler(function ($errNo, $str) use(&$msg) {
            $msg = $str;
        });
        echo new \_PhpScoperabd03f0baf05\RingCentral\Tests\Psr7\Str($s);
