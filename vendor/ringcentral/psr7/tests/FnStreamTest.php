 ...
 */
class FnStreamTest extends \_PhpScoperabd03f0baf05\PHPUnit_Framework_TestCase
{
    /**
    ...
    {
        $stream = new \_PhpScoperabd03f0baf05\RingCentral\Psr7\FnStream(array());
        $stream->seek(1);
    }
    public function testProxiesToFunction()
    {
        $self = $this;
        $s = new \_PhpScoperabd03f0baf05\RingCentral\Psr7\FnStream(array('read' => function ($len) use($self) {
    ...
    {
        $called = \false;
        $s = new \_PhpScoperabd03f0baf05\RingCentral\Psr7\FnStream(array('close' => function () use(&$called) {
            $called = \true;
        }));
        unset($s);
    ...
    {
        $s = new \_PhpScoperabd03f0baf05\RingCentral\Psr7\FnStream(array());
        unset($s);
    }
    public function testDecoratesStream()
    {
        $a = \_PhpScoperabd03f0baf05\RingCentral\Psr7\stream_for('foo');
        $b = \_PhpScoperabd03f0baf05\RingCentral\Psr7\FnStream::decorate($a, array());
    ...
    public function testDecoratesWithCustomizations()
    {
        $called = \false;
        $a = \_PhpScoperabd03f0baf05\RingCentral\Psr7\stream_for('foo');
        $b = \_PhpScoperabd03f0baf05\RingCentral\Psr7\FnStream::decorate($a, array('read' => function ($len) use(&$called, $a) {
            $called = \true;
            return $a->read($len);
        }));
