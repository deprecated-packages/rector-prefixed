...
class Identifier
{
     ...
     */
    public function __construct(string $name, \_PhpScoperbd5d0c5f7638\Roave\BetterReflection\Identifier\IdentifierType $type)
    {
        $this->type = $type;
        if ($name === self::WILDCARD || $name === \_PhpScoperbd5d0c5f7638\Roave\BetterReflection\Reflection\ReflectionFunctionAbstract::CLOSURE_NAME || \strpos($name, \_PhpScoperbd5d0c5f7638\Roave\BetterReflection\Reflection\ReflectionClass::ANONYMOUS_CLASS_NAME_PREFIX) === 0) {
            $this->name = $name;
            return;
        }
        $name = \ltrim($name, '\\');
        if (!\preg_match(self::VALID_NAME_REGEXP, $name)) {
            throw \_PhpScoperbd5d0c5f7638\Roave\BetterReflection\Identifier\Exception\InvalidIdentifierName::fromInvalidName($name);
        }
        $this->name = $name;
    }
    ...
    public function getType() : \_PhpScoperbd5d0c5f7638\Roave\BetterReflection\Identifier\IdentifierType
    {
        return $this->type;
    }
