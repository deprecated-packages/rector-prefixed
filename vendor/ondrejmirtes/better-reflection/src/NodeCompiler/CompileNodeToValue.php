...
class CompileNodeToValue
{
     ...
     */
    public function __invoke(\PhpParser\Node $node, \_PhpScoper006a73f0e455\Roave\BetterReflection\NodeCompiler\CompilerContext $context)
    {
        ...
        $constExprEvaluator = new \PhpParser\ConstExprEvaluator(function (\PhpParser\Node\Expr $node) use($context) {
            ...
            throw \_PhpScoper006a73f0e455\Roave\BetterReflection\NodeCompiler\Exception\UnableToCompileNode::forUnRecognizedExpressionInContext($node, $context);
        });
     ...
     * @throws Exception\UnableToCompileNode
     */
    private function compileConstFetch(\PhpParser\Node\Expr\ConstFetch $constNode, \_PhpScoper006a73f0e455\Roave\BetterReflection\NodeCompiler\CompilerContext $context)
    {
        ...
        switch (\strtolower($constantName)) {
            ...
            default:
                if ($context->getNamespace() !== null && !$constNode->name->isFullyQualified()) {
                    ...
                    try {
                        return \_PhpScoper006a73f0e455\Roave\BetterReflection\Reflection\ReflectionConstant::createFromName($namespacedName)->getValue();
                ...
                try {
                    return \_PhpScoper006a73f0e455\Roave\BetterReflection\Reflection\ReflectionConstant::createFromName($constantName)->getValue();
                } catch (\_PhpScoper006a73f0e455\Roave\BetterReflection\Reflector\Exception\IdentifierNotFound $e) {
                    throw \_PhpScoper006a73f0e455\Roave\BetterReflection\NodeCompiler\Exception\UnableToCompileNode::becauseOfNotFoundConstantReference($context, $constNode);
                }
        }
    }
    /**
     ...
     */
    private function compileClassConstFetch(\PhpParser\Node\Expr\ClassConstFetch $node, \_PhpScoper006a73f0e455\Roave\BetterReflection\NodeCompiler\CompilerContext $context)
    {
        ...
        if ($classInfo === null) {
            ...
            \assert($classInfo instanceof \_PhpScoper006a73f0e455\Roave\BetterReflection\Reflection\ReflectionClass);
        }
        $reflectionConstant = $classInfo->getReflectionConstant($nodeName);
        if (!$reflectionConstant instanceof \_PhpScoper006a73f0e455\Roave\BetterReflection\Reflection\ReflectionClassConstant) {
            throw \_PhpScoper006a73f0e455\Roave\BetterReflection\NodeCompiler\Exception\UnableToCompileNode::becauseOfNotFoundClassConstantReference($context, $classInfo, $node);
        }
        return $this->__invoke($reflectionConstant->getAst()->consts[$reflectionConstant->getPositionInAst()]->value, new \_PhpScoper006a73f0e455\Roave\BetterReflection\NodeCompiler\CompilerContext($context->getReflector(), $context->hasFileName() ? $context->getFileName() : null, $classInfo, $context->getNamespace(), $context->getFunctionName()));
    }
    /**
     * Compile a __DIR__ node
     */
    private function compileDirConstant(\_PhpScoper006a73f0e455\Roave\BetterReflection\NodeCompiler\CompilerContext $context) : string
    {
        return \_PhpScoper006a73f0e455\Roave\BetterReflection\Util\FileHelper::normalizeWindowsPath(\dirname(\realpath($context->getFileName())));
    }
    /**
     * Compiles magic constant __CLASS__
     */
    private function compileClassConstant(\_PhpScoper006a73f0e455\Roave\BetterReflection\NodeCompiler\CompilerContext $context) : string
    {
        return $context->hasSelf() ? $context->getSelf()->getName() : '';
    }
    private function resolveClassNameForClassNameConstant(string $className, \_PhpScoper006a73f0e455\Roave\BetterReflection\NodeCompiler\CompilerContext $context) : string
    {
        if ($className === 'self' || $className === 'static') {
