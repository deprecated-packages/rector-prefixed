...
use PhpParser\Node;
use _PhpScoper006a73f0e455\Roave\BetterReflection\Util\Exception\InvalidNodePosition;
use _PhpScoper006a73f0e455\Roave\BetterReflection\Util\Exception\NoNodePosition;
use function strlen;
...
final class CalculateReflectionColum
{
    ...
    public static function getStartColumn(string $source, \PhpParser\Node $node) : int
    {
        if (!$node->hasAttribute('startFilePos')) {
            throw \_PhpScoper006a73f0e455\Roave\BetterReflection\Util\Exception\NoNodePosition::fromNode($node);
        }
    ...
    private static function calculateColumn(string $source, int $position) : int
    {
        ...
        if ($position > $sourceLength) {
            throw \_PhpScoper006a73f0e455\Roave\BetterReflection\Util\Exception\InvalidNodePosition::fromPosition($position);
        }
