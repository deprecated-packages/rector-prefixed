...
use RectorPrefix20210223\Nette\HtmlStringable;
 ...
 */
class Html implements \ArrayAccess, \Countable, \IteratorAggregate, \RectorPrefix20210223\Nette\HtmlStringable
{
    ...
    public static function el(string $name = null, $attrs = null)
    {
        ...
        if (isset($parts[1])) {
            foreach (\RectorPrefix20210223\Nette\Utils\Strings::matchAll($parts[1] . ' ', '#([a-z0-9:-]+)(?:=(["\'])?(.*?)(?(2)\\2|\\s))?#i') as $m) {
    ...
    {
        if (!$text instanceof \RectorPrefix20210223\Nette\HtmlStringable) {
        ...
        foreach ($attrs as $key => $value) {
            if ($value === null || $value === \false) {
                ...
                if (\strncmp($key, 'data-', 5) === 0) {
                    $value = \RectorPrefix20210223\Nette\Utils\Json::encode($value);
                } else {
                    $tmp = null;
