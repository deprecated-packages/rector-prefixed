...
final class Reflection
{
    ...
    private static function getType($reflection, ?\ReflectionType $type, bool $asArray = \false)
    {
        if ($type === null) {
            ...
            throw new \RectorPrefix20210414\Nette\InvalidStateException('The ' . self::toString($reflection) . ' is not expected to have a union type.');
        } else {
            throw new \RectorPrefix20210414\Nette\InvalidStateException('Unexpected type of ' . self::toString($reflection));
        }
    }
    /**
     * @param  \ReflectionFunction|\ReflectionMethod|\ReflectionParameter|\ReflectionProperty  $reflection
    ...
    public static function toString(\Reflector $ref) : string
    {
        if ($ref instanceof \ReflectionClass) {
            ...
            throw new \RectorPrefix20210414\Nette\InvalidArgumentException();
        }
    }
    /**
    ...
    public static function expandClassName(string $name, \ReflectionClass $context) : string
    {
        ...
        if (empty($name)) {
            throw new \RectorPrefix20210414\Nette\InvalidArgumentException('Class name must not be empty.');
    ...
    public static function getUseStatements(\ReflectionClass $class) : array
    {
        if ($class->isAnonymous()) {
            throw new \RectorPrefix20210414\Nette\NotImplementedException('Anonymous classes are not supported.');
        }
        static $cache = [];
