...
class Cache
{
    ...
    /** @var string */
    private $namespace;
    public function __construct(\RectorPrefix20210409\Nette\Caching\Storage $storage, string $namespace = null)
    {
     ...
     */
    public final function getStorage() : \RectorPrefix20210409\Nette\Caching\Storage
    {
        return $this->storage;
    }
    /**
    ...
    public function bulkLoad(array $keys, callable $generator = null) : array
    {
        ...
        foreach ($keys as $key) {
            if (!\is_scalar($key)) {
                throw new \RectorPrefix20210409\Nette\InvalidArgumentException('Only scalar keys are allowed in bulkLoad()');
            }
        }
        $result = [];
        if (!$this->storage instanceof \RectorPrefix20210409\Nette\Caching\BulkReader) {
            foreach ($keys as $key) {
    ...
    private function completeDependencies(?array $dp) : array
    {
        ...
        if (isset($dp[self::EXPIRATION])) {
            $dp[self::EXPIRATION] = \RectorPrefix20210409\Nette\Utils\DateTime::from($dp[self::EXPIRATION])->format('U') - \time();
        }
        // make list from TAGS
     ...
     * @param  mixed  $key
     */
    public function capture($key) : ?\RectorPrefix20210409\Nette\Caching\OutputHelper
    {
        $data = $this->load($key);
        if ($data === null) {
            return new \RectorPrefix20210409\Nette\Caching\OutputHelper($this, $key);
        }
        echo $data;
        return null;
    }
     ...
     */
    public function start($key) : ?\RectorPrefix20210409\Nette\Caching\OutputHelper
    {
