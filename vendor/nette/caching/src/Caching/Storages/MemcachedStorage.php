 ...
 */
class MemcachedStorage implements \RectorPrefix20210423\Nette\Caching\Storage, \RectorPrefix20210423\Nette\Caching\BulkReader
{
    ...
    public function __construct(string $host = 'localhost', int $port = 11211, string $prefix = '', \RectorPrefix20210423\Nette\Caching\Storages\Journal $journal = null)
    {
        if (!static::isAvailable()) {
            throw new \RectorPrefix20210423\Nette\NotSupportedException("PHP extension 'memcached' is not loaded.");
        }
        $this->prefix = $prefix;
    ...
    public function addServer(string $host = 'localhost', int $port = 11211) : void
    {
        if (@$this->memcached->addServer($host, $port, 1) === \false) {
            ...
            throw new \RectorPrefix20210423\Nette\InvalidStateException("Memcached::addServer(): {$error['message']}.");
        }
    }
    ...
    public function write(string $key, $data, array $dp) : void
    {
        ...
        if (isset($dp[\RectorPrefix20210423\Nette\Caching\Cache::EXPIRATION])) {
            $expire = (int) $dp[\RectorPrefix20210423\Nette\Caching\Cache::EXPIRATION];
