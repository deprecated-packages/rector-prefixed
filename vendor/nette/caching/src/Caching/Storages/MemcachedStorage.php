 ...
 */
class MemcachedStorage implements \RectorPrefix20210318\Nette\Caching\Storage, \RectorPrefix20210318\Nette\Caching\BulkReader
{
    ...
    public function __construct(string $host = 'localhost', int $port = 11211, string $prefix = '', \RectorPrefix20210318\Nette\Caching\Storages\Journal $journal = null)
    {
        if (!static::isAvailable()) {
            throw new \RectorPrefix20210318\Nette\NotSupportedException("PHP extension 'memcached' is not loaded.");
        }
        $this->prefix = $prefix;
    ...
    public function addServer($host = 'localhost', $port = 11211) : void
    {
        if (@$this->memcached->addServer($host, $port, 1) === \false) {
            ...
            throw new \RectorPrefix20210318\Nette\InvalidStateException("Memcached::addServer(): {$error['message']}.");
        }
    }
    ...
    public function write($key, $data, $dp) : void
    {
        ...
        if (isset($dp[\RectorPrefix20210318\Nette\Caching\Cache::EXPIRATION])) {
            $expire = (int) $dp[\RectorPrefix20210318\Nette\Caching\Cache::EXPIRATION];
