 ...
 */
class SQLiteJournal implements \RectorPrefix20210420\Nette\Caching\Storages\Journal
{
    ...
    public function __construct(string $path)
    {
        if (!\extension_loaded('pdo_sqlite')) {
            throw new \RectorPrefix20210420\Nette\NotSupportedException('SQLiteJournal requires PHP extension pdo_sqlite which is not loaded.');
        }
        $this->path = $path;
    }
    ...
    public function write(string $key, array $dependencies) : void
    {
        ...
        $this->pdo->exec('BEGIN');
        if (!empty($dependencies[\RectorPrefix20210420\Nette\Caching\Cache::TAGS])) {
            $this->pdo->prepare('DELETE FROM tags WHERE key = ?')->execute([$key]);
            foreach ($dependencies[\RectorPrefix20210420\Nette\Caching\Cache::TAGS] as $tag) {
                $arr[] = $key;
                $arr[] = $tag;
            }
            $this->pdo->prepare('INSERT INTO tags (key, tag) SELECT ?, ?' . \str_repeat('UNION SELECT ?, ?', \count($arr) / 2 - 1))->execute($arr);
        }
        if (!empty($dependencies[\RectorPrefix20210420\Nette\Caching\Cache::PRIORITY])) {
            $this->pdo->prepare('REPLACE INTO priorities (key, priority) VALUES (?, ?)')->execute([$key, (int) $dependencies[\RectorPrefix20210420\Nette\Caching\Cache::PRIORITY]]);
        }
    ...
    public function clean(array $conditions) : ?array
    {
        ...
        $unions = $args = [];
        if (!empty($conditions[\RectorPrefix20210420\Nette\Caching\Cache::TAGS])) {
            $tags = (array) $conditions[\RectorPrefix20210420\Nette\Caching\Cache::TAGS];
            $unions[] = 'SELECT DISTINCT key FROM tags WHERE tag IN (?' . \str_repeat(', ?', \count($tags) - 1) . ')';
            $args = $tags;
        }
        if (!empty($conditions[\RectorPrefix20210420\Nette\Caching\Cache::PRIORITY])) {
            $unions[] = 'SELECT DISTINCT key FROM priorities WHERE priority <= ?';
            $args[] = (int) $conditions[\RectorPrefix20210420\Nette\Caching\Cache::PRIORITY];
        }
        if (empty($unions)) {
            return [];
        }
