...
namespace RectorPrefix20210405\Nette\Caching\Storages;
 ...
 * Cache file storage.
 */
class FileStorage implements \RectorPrefix20210405\Nette\Caching\Storage
{
    ...
    /** @var array */
    private $locks;
    public function __construct(string $dir, \RectorPrefix20210405\Nette\Caching\Storages\Journal $journal = null)
    {
        if (!\is_dir($dir)) {
            throw new \RectorPrefix20210405\Nette\DirectoryNotFoundException("Directory '{$dir}' not found.");
        }
        $this->dir = $dir;
    ...
    private function verify(array $meta) : bool
    {
        do {
            ...
            if (!empty($meta[self::META_CALLBACKS]) && !\RectorPrefix20210405\Nette\Caching\Cache::checkCallbacks($meta[self::META_CALLBACKS])) {
                break;
            }
    ...
    public function write(string $key, $data, array $dp) : void
    {
        ...
        if (isset($dp[\RectorPrefix20210405\Nette\Caching\Cache::EXPIRATION])) {
            if (empty($dp[\RectorPrefix20210405\Nette\Caching\Cache::SLIDING])) {
                $meta[self::META_EXPIRE] = $dp[\RectorPrefix20210405\Nette\Caching\Cache::EXPIRATION] + \time();
                // absolute time
            } else {
                $meta[self::META_DELTA] = (int) $dp[\RectorPrefix20210405\Nette\Caching\Cache::EXPIRATION];
                // sliding time
            }
        }
        if (isset($dp[\RectorPrefix20210405\Nette\Caching\Cache::ITEMS])) {
            foreach ($dp[\RectorPrefix20210405\Nette\Caching\Cache::ITEMS] as $item) {
        ...
        if (isset($dp[\RectorPrefix20210405\Nette\Caching\Cache::CALLBACKS])) {
            $meta[self::META_CALLBACKS] = $dp[\RectorPrefix20210405\Nette\Caching\Cache::CALLBACKS];
        }
        ...
        $cacheFile = $this->getCacheFile($key);
        if (isset($dp[\RectorPrefix20210405\Nette\Caching\Cache::TAGS]) || isset($dp[\RectorPrefix20210405\Nette\Caching\Cache::PRIORITY])) {
            if (!$this->journal) {
                throw new \RectorPrefix20210405\Nette\InvalidStateException('CacheJournal has not been provided.');
            }
    ...
    {
        $all = !empty($conditions[\RectorPrefix20210405\Nette\Caching\Cache::ALL]);
        $collector = empty($conditions);
        $namespaces = $conditions[\RectorPrefix20210405\Nette\Caching\Cache::NAMESPACES] ?? null;
        // cleaning using file iterator
        if ($all || $collector) {
            $now = \time();
            foreach (\RectorPrefix20210405\Nette\Utils\Finder::find('_*')->from($this->dir)->childFirst() as $entry) {
                $path = (string) $entry;
                if ($entry->isDir()) {
            ...
            foreach ($namespaces as $namespace) {
                ...
                foreach (\RectorPrefix20210405\Nette\Utils\Finder::findFiles('_*')->in($dir) as $entry) {
