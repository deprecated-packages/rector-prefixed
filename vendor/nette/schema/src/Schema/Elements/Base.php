...
trait Base
{
    ...
    public function doNormalize($value, \_PhpScoper006a73f0e455\Nette\Schema\Context $context)
    {
        if ($this->before) {
    ...
    private function doValidate($value, string $expected, \_PhpScoper006a73f0e455\Nette\Schema\Context $context) : bool
    {
        try {
            \_PhpScoper006a73f0e455\Nette\Utils\Validators::assert($value, $expected, 'option %path%');
            return \true;
        } catch (\_PhpScoper006a73f0e455\Nette\Utils\AssertionException $e) {
            $context->addError($e->getMessage(), $expected);
            return \false;
        }
    }
    private function doFinalize($value, \_PhpScoper006a73f0e455\Nette\Schema\Context $context)
    {
        if ($this->castTo) {
            if (\_PhpScoper006a73f0e455\Nette\Utils\Reflection::isBuiltinType($this->castTo)) {
                \settype($value, $this->castTo);
            } else {
                $value = \_PhpScoper006a73f0e455\Nette\Utils\Arrays::toObject($value, new $this->castTo());
            }
        }
