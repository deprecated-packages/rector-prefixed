...
final class Structure implements \_PhpScoper006a73f0e455\Nette\Schema\Schema
{
    use Base;
    ...
    {
        (function (\_PhpScoper006a73f0e455\Nette\Schema\Schema ...$items) {
        })(...\array_values($items));
        $this->items = $items;
        $this->castTo = 'object';
    }
    ...
    {
        throw new \_PhpScoper006a73f0e455\Nette\InvalidStateException('Structure cannot have default value.');
    }
    ...
    {
        $this->otherItems = $type instanceof \_PhpScoper006a73f0e455\Nette\Schema\Schema ? $type : new \_PhpScoper006a73f0e455\Nette\Schema\Elements\Type($type);
        return $this;
    }
    /********************* processing ****************d*g**/
        ...
        if (\is_array($value) && \is_array($base)) {
            ...
            foreach ($value as $key => $val) {
                if ($key === $index) {
                    ...
                    $itemSchema = $this->items[$key] ?? $this->otherItems;
                    $base[$key] = $itemSchema ? $itemSchema->merge($val, $base[$key]) : \_PhpScoper006a73f0e455\Nette\Schema\Helpers::merge($val, $base[$key]);
                } else {
                    $base[$key] = $val;
                }
            }
    ...
    public function complete($value, \_PhpScoper006a73f0e455\Nette\Schema\Context $context)
    {
        ...
        if ($extraKeys = \array_keys(\array_diff_key($value, $items))) {
            if ($this->otherItems) {
                ...
                $hint = \_PhpScoper006a73f0e455\Nette\Utils\Helpers::getSuggestion(\array_map('strval', \array_keys($items)), (string) $extraKeys[0]);
