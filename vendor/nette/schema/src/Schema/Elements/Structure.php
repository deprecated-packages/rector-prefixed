...
final class Structure implements \_PhpScoperabd03f0baf05\Nette\Schema\Schema
{
    use Base;
    ...
    {
        (function (\_PhpScoperabd03f0baf05\Nette\Schema\Schema ...$items) {
        })(...\array_values($items));
        $this->items = $items;
        $this->castTo = 'object';
    }
    ...
    {
        throw new \_PhpScoperabd03f0baf05\Nette\InvalidStateException('Structure cannot have default value.');
    }
    ...
    {
        $this->otherItems = $type instanceof \_PhpScoperabd03f0baf05\Nette\Schema\Schema ? $type : new \_PhpScoperabd03f0baf05\Nette\Schema\Elements\Type($type);
        return $this;
    }
    /********************* processing ****************d*g**/
        ...
        if (\is_array($value) && \is_array($base)) {
            ...
            foreach ($value as $key => $val) {
                if ($key === $index) {
                    ...
                    $itemSchema = $this->items[$key] ?? $this->otherItems;
                    $base[$key] = $itemSchema ? $itemSchema->merge($val, $base[$key]) : \_PhpScoperabd03f0baf05\Nette\Schema\Helpers::merge($val, $base[$key]);
                } else {
                    $base[$key] = $val;
                }
            }
    ...
    public function complete($value, \_PhpScoperabd03f0baf05\Nette\Schema\Context $context)
    {
        ...
        if ($extraKeys = \array_keys(\array_diff_key($value, $items))) {
            if ($this->otherItems) {
                ...
                $hint = \_PhpScoperabd03f0baf05\Nette\Utils\Helpers::getSuggestion(\array_map('strval', \array_keys($items)), (string) $extraKeys[0]);
