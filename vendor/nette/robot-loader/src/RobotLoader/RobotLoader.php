...
namespace _PhpScoperbf340cb0be9d\Nette\Loaders;
...
class RobotLoader
{
    ...
    public function __construct()
    {
        if (!\extension_loaded('tokenizer')) {
            throw new \_PhpScoperbf340cb0be9d\Nette\NotSupportedException('PHP extension Tokenizer is not loaded.');
        }
    }
    /**
     * Register autoloader.
        ...
        foreach ($this->scanPaths as $path) {
            ...
            foreach ($iterator as $file) {
                ...
                foreach ($classes as $class) {
                    ...
                    if (isset($info['file'])) {
                        throw new \_PhpScoperbf340cb0be9d\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$info['file']} and in {$file}.");
                    }
     ...
     */
    private function createFileIterator(string $dir) : \_PhpScoperbf340cb0be9d\Nette\Utils\Finder
    {
        if (!\is_dir($dir)) {
            throw new \_PhpScoperbf340cb0be9d\Nette\IOException("File or directory '{$dir}' not found.");
        }
        ...
        $iterator = \_PhpScoperbf340cb0be9d\Nette\Utils\Finder::findFiles($acceptFiles)->filter(function (\SplFileInfo $file) use(&$disallow) {
    ...
    {
        \_PhpScoperbf340cb0be9d\Nette\Utils\FileSystem::createDir($dir);
