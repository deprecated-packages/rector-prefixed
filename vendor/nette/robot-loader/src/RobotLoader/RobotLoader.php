...
namespace RectorPrefix20201231\Nette\Loaders;
...
class RobotLoader
{
    ...
    public function __construct()
    {
        if (!\extension_loaded('tokenizer')) {
            throw new \RectorPrefix20201231\Nette\NotSupportedException('PHP extension Tokenizer is not loaded.');
        }
    }
    /**
     * Register autoloader.
        ...
        foreach ($this->scanPaths as $path) {
            ...
            foreach ($iterator as $file) {
                ...
                foreach ($classes as $class) {
                    ...
                    if (isset($info['file'])) {
                        throw new \RectorPrefix20201231\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$info['file']} and in {$file}.");
                    }
     ...
     */
    private function createFileIterator(string $dir) : \RectorPrefix20201231\Nette\Utils\Finder
    {
        if (!\is_dir($dir)) {
            throw new \RectorPrefix20201231\Nette\IOException("File or directory '{$dir}' not found.");
        }
        ...
        $iterator = \RectorPrefix20201231\Nette\Utils\Finder::findFiles($acceptFiles)->filter(function (\SplFileInfo $file) use(&$disallow) {
    ...
    {
        \RectorPrefix20201231\Nette\Utils\FileSystem::createDir($dir);
