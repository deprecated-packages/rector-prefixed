...
use _PhpScoper006a73f0e455\Nette\Neon;
/**
 * Reading and generating NEON files.
 */
final class NeonAdapter implements \_PhpScoper006a73f0e455\Nette\DI\Config\Adapter
{
    ...
    {
        return $this->process((array) \_PhpScoper006a73f0e455\Nette\Neon\Neon::decode(\_PhpScoper006a73f0e455\Nette\Utils\FileSystem::read($file)));
    }
        ...
        foreach ($arr as $key => $val) {
            if (\is_string($key) && \substr($key, -1) === self::PREVENT_MERGING_SUFFIX) {
                if (!\is_array($val) && $val !== null) {
                    throw new \_PhpScoper006a73f0e455\Nette\DI\InvalidConfigurationException("Replacing operator is available only for arrays, item '{$key}' is not array.");
                }
                $key = \substr($key, 0, -1);
            ...
            if (\is_array($val)) {
                ...
                if ($val->value === \_PhpScoper006a73f0e455\Nette\Neon\Neon::CHAIN) {
                    ...
                    $val = new \_PhpScoper006a73f0e455\Nette\DI\Definitions\Statement($tmp[0], $this->process($val->attributes));
                }
            }
            $res[$key] = $val;
        }
