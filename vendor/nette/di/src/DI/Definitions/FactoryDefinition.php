...
use _PhpScoper006a73f0e455\Nette\DI\ServiceCreationException;
use _PhpScoper006a73f0e455\Nette\Utils\Reflection;
/**
 * Definition of standard service.
 */
final class FactoryDefinition extends \_PhpScoper006a73f0e455\Nette\DI\Definitions\Definition
{
    ...
    {
        $this->resultDefinition = new \_PhpScoper006a73f0e455\Nette\DI\Definitions\ServiceDefinition();
    }
    /** @return static */
    public function setImplement(string $type)
    {
        if (!\interface_exists($type)) {
            throw new \_PhpScoper006a73f0e455\Nette\InvalidArgumentException("Service '{$this->getName()}': Interface '{$type}' not found.");
        }
        $rc = new \ReflectionClass($type);
        $method = $rc->getMethods()[0] ?? null;
        if (!$method || $method->isStatic() || $method->name !== self::METHOD_CREATE || \count($rc->getMethods()) > 1) {
            throw new \_PhpScoper006a73f0e455\Nette\InvalidArgumentException("Service '{$this->getName()}': Interface {$type} must have just one non-static method create().");
        }
    ...
    /** @return static */
    public function setResultDefinition(\_PhpScoper006a73f0e455\Nette\DI\Definitions\Definition $definition)
    {
        $this->resultDefinition = $definition;
        return $this;
    }
    /** @return ServiceDefinition */
    public function getResultDefinition() : \_PhpScoper006a73f0e455\Nette\DI\Definitions\Definition
    {
    ...
    public function getFactory() : ?\_PhpScoper006a73f0e455\Nette\DI\Definitions\Statement
    {
    ...
    public function resolveType(\_PhpScoper006a73f0e455\Nette\DI\Resolver $resolver) : void
    {
        ...
        if (!$resultDef->getType()) {
            ...
            if (!$interface) {
                throw new \_PhpScoper006a73f0e455\Nette\DI\ServiceCreationException('Type is missing in definition of service.');
            }
            $method = new \ReflectionMethod($interface, self::METHOD_CREATE);
            $returnType = \_PhpScoper006a73f0e455\Nette\DI\Helpers::getReturnType($method);
            if (!$returnType) {
                throw new \_PhpScoper006a73f0e455\Nette\DI\ServiceCreationException("Method {$interface}::create() has not return type hint or annotation @return.");
            } elseif (!\class_exists($returnType) && !\interface_exists($returnType)) {
                throw new \_PhpScoper006a73f0e455\Nette\DI\ServiceCreationException("Check a type hint or annotation @return of the {$interface}::create() method, class '{$returnType}' cannot be found.");
            }
    ...
    {
        $resultDef = $this->resultDefinition;
        if ($resultDef instanceof \_PhpScoper006a73f0e455\Nette\DI\Definitions\ServiceDefinition) {
            if (!$this->parameters) {
                $this->completeParameters($resolver);
            }
            if ($resultDef->getEntity() instanceof \_PhpScoper006a73f0e455\Nette\DI\Definitions\Reference && !$resultDef->getFactory()->arguments) {
                $resultDef->setFactory([
                    // render as $container->createMethod()
                    new \_PhpScoper006a73f0e455\Nette\DI\Definitions\Reference(\_PhpScoper006a73f0e455\Nette\DI\ContainerBuilder::THIS_CONTAINER),
                    \_PhpScoper006a73f0e455\Nette\DI\Container::getMethodName($resultDef->getEntity()->getValue()),
                ]);
            }
        }
        $resolver->completeDefinition($resultDef);
    }
    private function completeParameters(\_PhpScoper006a73f0e455\Nette\DI\Resolver $resolver) : void
    {
        $interface = $this->getType();
        $method = new \ReflectionMethod($interface, self::METHOD_CREATE);
        $ctorParams = [];
        ...
        foreach ($method->getParameters() as $param) {
            $methodHint = \_PhpScoper006a73f0e455\Nette\Utils\Reflection::getParameterTypes($param);
            if (isset($ctorParams[$param->name])) {
                $ctorParam = $ctorParams[$param->name];
                $ctorHint = \_PhpScoper006a73f0e455\Nette\Utils\Reflection::getParameterTypes($ctorParam);
                if ($methodHint !== $ctorHint && !\is_a((string) \reset($methodHint), (string) \reset($ctorHint), \true)) {
                    throw new \_PhpScoper006a73f0e455\Nette\DI\ServiceCreationException("Type hint for \${$param->name} in {$interface}::create() doesn't match type hint in {$class} constructor.");
                }
                $this->resultDefinition->getFactory()->arguments[$ctorParam->getPosition()] = \_PhpScoper006a73f0e455\Nette\DI\ContainerBuilder::literal('$' . $ctorParam->name);
            } elseif (!$this->resultDefinition->getSetup()) {
                $hint = \_PhpScoper006a73f0e455\Nette\Utils\Helpers::getSuggestion(\array_keys($ctorParams), $param->name);
                throw new \_PhpScoper006a73f0e455\Nette\DI\ServiceCreationException("Unused parameter \${$param->name} when implementing method {$interface}::create()" . ($hint ? ", did you mean \${$hint}?" : '.'));
            }
            $paramDef = \PHP_VERSION_ID < 80000 ? ($methodHint && $param->allowsNull() ? '?' : '') . \reset($methodHint) : \implode('|', $methodHint);
            $paramDef .= ' ' . $param->name;
            if ($param->isDefaultValueAvailable()) {
                $this->parameters[$paramDef] = \_PhpScoper006a73f0e455\Nette\Utils\Reflection::getParameterDefaultValue($param);
            } else {
    ...
    {
        $class = (new \_PhpScoper006a73f0e455\Nette\PhpGenerator\ClassType())->addImplement($this->getType());
        ...
        $methodCreate->setParameters($generator->convertParameters($this->parameters))->setReturnType(\_PhpScoper006a73f0e455\Nette\Utils\Reflection::getReturnType($rm) ?: $this->getResultType())->setBody($body);
