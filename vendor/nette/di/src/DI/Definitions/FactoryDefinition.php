 ...
 */
final class FactoryDefinition extends \_PhpScoperabd03f0baf05\Nette\DI\Definitions\Definition
{
    ...
    {
        $this->resultDefinition = new \_PhpScoperabd03f0baf05\Nette\DI\Definitions\ServiceDefinition();
    }
    /** @return static */
    public function setImplement(string $type)
    {
        ...
        if (!$method || $method->isStatic() || $method->name !== self::METHOD_CREATE || \count($rc->getMethods()) > 1) {
            throw new \_PhpScoperabd03f0baf05\Nette\InvalidArgumentException("Service '{$this->getName()}': Interface {$type} must have just one non-static method create().");
        }
    ...
    /** @return static */
    public function setResultDefinition(\_PhpScoperabd03f0baf05\Nette\DI\Definitions\Definition $definition)
    {
        $this->resultDefinition = $definition;
        return $this;
    }
    /** @return ServiceDefinition */
    public function getResultDefinition() : \_PhpScoperabd03f0baf05\Nette\DI\Definitions\Definition
    {
    ...
    public function resolveType(\_PhpScoperabd03f0baf05\Nette\DI\Resolver $resolver) : void
    {
        ...
        if (!$resultDef->getType()) {
            ...
            if (!$interface) {
                throw new \_PhpScoperabd03f0baf05\Nette\DI\ServiceCreationException('Type is missing in definition of service.');
            }
            ...
            if (!$returnType) {
                throw new \_PhpScoperabd03f0baf05\Nette\DI\ServiceCreationException("Method {$interface}::create() has not return type hint or annotation @return.");
            } elseif (!\class_exists($returnType) && !\interface_exists($returnType)) {
                throw new \_PhpScoperabd03f0baf05\Nette\DI\ServiceCreationException("Check a type hint or annotation @return of the {$interface}::create() method, class '{$returnType}' cannot be found.");
            }
    ...
    {
        $resultDef = $this->resultDefinition;
        if ($resultDef instanceof \_PhpScoperabd03f0baf05\Nette\DI\Definitions\ServiceDefinition) {
            if (!$this->parameters) {
                $this->completeParameters($resolver);
            }
            if ($resultDef->getEntity() instanceof \_PhpScoperabd03f0baf05\Nette\DI\Definitions\Reference && !$resultDef->getFactory()->arguments) {
                $resultDef->setFactory([
                    // render as $container->createMethod()
                    new \_PhpScoperabd03f0baf05\Nette\DI\Definitions\Reference(\_PhpScoperabd03f0baf05\Nette\DI\ContainerBuilder::THIS_CONTAINER),
                    \_PhpScoperabd03f0baf05\Nette\DI\Container::getMethodName($resultDef->getEntity()->getValue()),
                ]);
            }
        }
        $resolver->completeDefinition($resultDef);
    }
    private function completeParameters(\_PhpScoperabd03f0baf05\Nette\DI\Resolver $resolver) : void
    {
        $interface = $this->getType();
        $method = new \ReflectionMethod($interface, self::METHOD_CREATE);
        $ctorParams = [];
        ...
        foreach ($method->getParameters() as $param) {
            $methodHint = \_PhpScoperabd03f0baf05\Nette\Utils\Reflection::getParameterTypes($param);
            if (isset($ctorParams[$param->name])) {
                $ctorParam = $ctorParams[$param->name];
                $ctorHint = \_PhpScoperabd03f0baf05\Nette\Utils\Reflection::getParameterTypes($ctorParam);
                if ($methodHint !== $ctorHint && !\is_a((string) \reset($methodHint), (string) \reset($ctorHint), \true)) {
                    throw new \_PhpScoperabd03f0baf05\Nette\DI\ServiceCreationException("Type hint for \${$param->name} in {$interface}::create() doesn't match type hint in {$class} constructor.");
                }
                $this->resultDefinition->getFactory()->arguments[$ctorParam->getPosition()] = \_PhpScoperabd03f0baf05\Nette\DI\ContainerBuilder::literal('$' . $ctorParam->name);
            } elseif (!$this->resultDefinition->getSetup()) {
                $hint = \_PhpScoperabd03f0baf05\Nette\Utils\Helpers::getSuggestion(\array_keys($ctorParams), $param->name);
                throw new \_PhpScoperabd03f0baf05\Nette\DI\ServiceCreationException("Unused parameter \${$param->name} when implementing method {$interface}::create()" . ($hint ? ", did you mean \${$hint}?" : '.'));
            }
            $paramDef = \PHP_VERSION_ID < 80000 ? ($methodHint && $param->allowsNull() ? '?' : '') . \reset($methodHint) : \implode('|', $methodHint);
            $paramDef .= ' ' . $param->name;
            if ($param->isDefaultValueAvailable()) {
                $this->parameters[$paramDef] = \_PhpScoperabd03f0baf05\Nette\Utils\Reflection::getParameterDefaultValue($param);
            } else {
    ...
    public function generateMethod(\_PhpScoperabd03f0baf05\Nette\PhpGenerator\Method $method, \_PhpScoperabd03f0baf05\Nette\DI\PhpGenerator $generator) : void
    {
        ...
        $methodCreate->setParameters($generator->convertParameters($this->parameters))->setReturnType(\_PhpScoperabd03f0baf05\Nette\Utils\Reflection::getReturnType($rm) ?: $this->getResultType())->setBody($body);
