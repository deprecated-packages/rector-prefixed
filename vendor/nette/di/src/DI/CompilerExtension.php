...
abstract class CompilerExtension
{
    ...
    /** @return static */
    public function setCompiler(\_PhpScopera143bcca66cb\Nette\DI\Compiler $compiler, string $name)
    {
        $this->initialization = new \_PhpScopera143bcca66cb\Nette\PhpGenerator\Closure();
    ...
    public function setConfig($config)
    {
        if (!\is_array($config) && !\is_object($config)) {
            throw new \_PhpScopera143bcca66cb\Nette\InvalidArgumentException();
        }
        $this->config = $config;
        return $this;
    }
    ...
    {
        return \is_object($this->config) ? \_PhpScopera143bcca66cb\Nette\Schema\Expect::from($this->config) : \_PhpScopera143bcca66cb\Nette\Schema\Expect::array();
    }
    /**
    ...
    public function validateConfig(array $expected, array $config = null, string $name = null) : array
    {
        ...
        if ($extra = \array_diff_key((array) $config, $expected)) {
            ...
            $hint = \_PhpScopera143bcca66cb\Nette\Utils\Helpers::getSuggestion(\array_keys($expected), \key($extra));
            $extra = $hint ? \key($extra) : \implode("', '{$name} › ", \array_keys($extra));
            throw new \_PhpScopera143bcca66cb\Nette\DI\InvalidConfigurationException("Unknown configuration option '{$name} › {$extra}'" . ($hint ? ", did you mean '{$name} › {$hint}'?" : '.'));
        }
        return \_PhpScopera143bcca66cb\Nette\Schema\Helpers::merge($config, $expected);
    }
    ...
    public function loadDefinitionsFromConfig(array $configList) : void
    {
        ...
        foreach ($configList as $key => $config) {
            ...
            $res[$key] = \_PhpScopera143bcca66cb\Nette\DI\Helpers::prefixServiceName($config, $this->name);
        }
    ...
    public function getInitialization() : \_PhpScopera143bcca66cb\Nette\PhpGenerator\Closure
    {
