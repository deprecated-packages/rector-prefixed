...
final class Helpers
{
    ...
    public static function expand($var, array $params, $recursive = \false)
    {
        if (\is_array($var)) {
            ...
            return $res;
        } elseif ($var instanceof \_PhpScoperbd5d0c5f7638\Nette\DI\Definitions\Statement) {
            return new \_PhpScoperbd5d0c5f7638\Nette\DI\Definitions\Statement(self::expand($var->getEntity(), $params, $recursive), self::expand($var->arguments, $params, $recursive));
        ...
        foreach ($parts as $n => $part) {
            if ($n % 2 === 0) {
                ...
                throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidArgumentException(\sprintf('Circular reference detected for variables: %s.', \implode(', ', \array_keys($recursive))));
            } else {
                $val = $params;
                foreach (\explode('.', $part) as $key) {
                    if (\is_array($val) && \array_key_exists($key, $val)) {
                        $val = $val[$key];
                    } elseif ($val instanceof \_PhpScoperbd5d0c5f7638\Nette\DI\DynamicParameter) {
                        $val = new \_PhpScoperbd5d0c5f7638\Nette\DI\DynamicParameter($val . '[' . \var_export($key, \true) . ']');
                    } else {
                        throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidArgumentException("Missing parameter '{$part}'.");
                    }
                }
                if ($recursive) {
                ...
                if ($val instanceof \_PhpScoperbd5d0c5f7638\Nette\DI\DynamicParameter) {
                    $php = \true;
                } elseif (!\is_scalar($val)) {
                    throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidArgumentException("Unable to concatenate non-scalar parameter '{$part}' into '{$var}'.");
                }
                $res[] = $val;
            }
        }
        if ($php) {
            ...
            $res = \array_map(function ($val) : string {
                return $val instanceof \_PhpScoperbd5d0c5f7638\Nette\DI\DynamicParameter ? "({$val})" : \var_export((string) $val, \true);
            }, $res);
            return new \_PhpScoperbd5d0c5f7638\Nette\DI\DynamicParameter(\implode(' . ', $res));
        }
        return \implode('', $res);
    }
    /**
    ...
    public static function filterArguments(array $args) : array
    {
        foreach ($args as $k => $v) {
            if ($v === '...') {
                ...
                $args[$k] = new \_PhpScoperbd5d0c5f7638\Nette\DI\Definitions\Reference(\substr($v, 1));
            } elseif (\is_array($v)) {
                $args[$k] = self::filterArguments($v);
            } elseif ($v instanceof \_PhpScoperbd5d0c5f7638\Nette\DI\Definitions\Statement) {
                [$tmp] = self::filterArguments([$v->getEntity()]);
                $args[$k] = new \_PhpScoperbd5d0c5f7638\Nette\DI\Definitions\Statement($tmp, self::filterArguments($v->arguments));
            }
        }
        return $args;
    }
    ...
    public static function prefixServiceName($config, string $namespace)
    {
        ...
        } elseif ($config instanceof \_PhpScoperbd5d0c5f7638\Nette\DI\Definitions\Reference) {
            if (\strncmp($config->getValue(), 'extension.', 9) === 0) {
                $config = new \_PhpScoperbd5d0c5f7638\Nette\DI\Definitions\Reference($namespace . '.' . \substr($config->getValue(), 10));
            }
        } elseif ($config instanceof \_PhpScoperbd5d0c5f7638\Nette\DI\Definitions\Statement) {
            return new \_PhpScoperbd5d0c5f7638\Nette\DI\Definitions\Statement(self::prefixServiceName($config->getEntity(), $namespace), self::prefixServiceName($config->arguments, $namespace));
    ...
    {
        if ($type = \_PhpScoperbd5d0c5f7638\Nette\Utils\Reflection::getReturnType($func)) {
            return $type;
        } elseif ($type = \preg_replace('#[|\\s].*#', '', (string) self::parseAnnotation($func, 'return'))) {
            if ($type === 'object' || $type === 'mixed') {
                return null;
            } elseif ($func instanceof \ReflectionMethod) {
                return $type === 'static' || $type === '$this' ? $func->getDeclaringClass()->name : \_PhpScoperbd5d0c5f7638\Nette\Utils\Reflection::expandClassName($type, $func->getDeclaringClass());
            } else {
                return $type;
            }
        }
    ...
    public static function convertType($value, string $type)
    {
        ...
        throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidStateException("Cannot convert {$value} to {$type}.");
    }
}
