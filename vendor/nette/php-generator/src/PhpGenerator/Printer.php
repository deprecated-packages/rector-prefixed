...
class Printer
{
    ...
    public function printFunction(\_PhpScoper006a73f0e455\Nette\PhpGenerator\GlobalFunction $function, \_PhpScoper006a73f0e455\Nette\PhpGenerator\PhpNamespace $namespace = null) : string
    {
        return \_PhpScoper006a73f0e455\Nette\PhpGenerator\Helpers::formatDocComment($function->getComment() . "\n") . self::printAttributes($function->getAttributes(), $namespace) . 'function ' . ($function->getReturnReference() ? '&' : '') . $function->getName() . $this->printParameters($function, $namespace) . $this->printReturnType($function, $namespace) . "\n{\n" . $this->indent(\ltrim(\rtrim($function->getBody()) . "\n")) . "}\n";
    }
    public function printClosure(\_PhpScoper006a73f0e455\Nette\PhpGenerator\Closure $closure) : string
    {
        $uses = [];
        foreach ($closure->getUses() as $param) {
            $uses[] = ($param->isReference() ? '&' : '') . '$' . $param->getName();
        }
        $useStr = \strlen($tmp = \implode(', ', $uses)) > (new \_PhpScoper006a73f0e455\Nette\PhpGenerator\Dumper())->wrapLength && \count($uses) > 1 ? "\n" . $this->indentation . \implode(",\n" . $this->indentation, $uses) . "\n" : $tmp;
    ...
    public function printArrowFunction(\_PhpScoper006a73f0e455\Nette\PhpGenerator\Closure $closure) : string
    {
        foreach ($closure->getUses() as $use) {
            if ($use->isReference()) {
                throw new \_PhpScoper006a73f0e455\Nette\InvalidArgumentException('Arrow function cannot bind variables by-reference.');
            }
        }
    ...
    public function printMethod(\_PhpScoper006a73f0e455\Nette\PhpGenerator\Method $method, \_PhpScoper006a73f0e455\Nette\PhpGenerator\PhpNamespace $namespace = null) : string
    {
        $method->validate();
        return \_PhpScoper006a73f0e455\Nette\PhpGenerator\Helpers::formatDocComment($method->getComment() . "\n") . self::printAttributes($method->getAttributes(), $namespace) . ($method->isAbstract() ? 'abstract ' : '') . ($method->isFinal() ? 'final ' : '') . ($method->getVisibility() ? $method->getVisibility() . ' ' : '') . ($method->isStatic() ? 'static ' : '') . 'function ' . ($method->getReturnReference() ? '&' : '') . $method->getName() . ($params = $this->printParameters($method, $namespace)) . $this->printReturnType($method, $namespace) . ($method->isAbstract() || $method->getBody() === null ? ";\n" : (\strpos($params, "\n") === \false ? "\n" : ' ') . "{\n" . $this->indent(\ltrim(\rtrim($method->getBody()) . "\n")) . "}\n");
    }
    public function printClass(\_PhpScoper006a73f0e455\Nette\PhpGenerator\ClassType $class, \_PhpScoper006a73f0e455\Nette\PhpGenerator\PhpNamespace $namespace = null) : string
    {
        $class->validate();
        ...
        foreach ($class->getConstants() as $const) {
            ...
            $consts[] = \_PhpScoper006a73f0e455\Nette\PhpGenerator\Helpers::formatDocComment((string) $const->getComment()) . self::printAttributes($const->getAttributes(), $namespace) . $def . $this->dump($const->getValue(), \strlen($def)) . ";\n";
        }
        $properties = [];
        foreach ($class->getProperties() as $property) {
            ...
            $properties[] = \_PhpScoper006a73f0e455\Nette\PhpGenerator\Helpers::formatDocComment((string) $property->getComment()) . self::printAttributes($property->getAttributes(), $namespace) . $def . ($property->getValue() === null && !$property->isInitialized() ? '' : ' = ' . $this->dump($property->getValue(), \strlen($def) + 3)) . ";\n";
        }
        $methods = [];
    ...
    public function printFile(\_PhpScoper006a73f0e455\Nette\PhpGenerator\PhpFile $file) : string
    {
        $namespaces = [];
        foreach ($file->getNamespaces() as $namespace) {
            $namespaces[] = $this->printNamespace($namespace);
        }
        return \_PhpScoper006a73f0e455\Nette\Utils\Strings::normalize("<?php\n" . ($file->getComment() ? "\n" . \_PhpScoper006a73f0e455\Nette\PhpGenerator\Helpers::formatDocComment($file->getComment() . "\n") : '') . "\n" . ($file->hasStrictTypes() ? "declare(strict_types=1);\n\n" : '') . \implode("\n\n", $namespaces)) . "\n";
    }
    /** @return static */
    ...
    protected function indent(string $s) : string
    {
        ...
        return \_PhpScoper006a73f0e455\Nette\Utils\Strings::indent($s, 1, $this->indentation);
    }
    protected function dump($var, int $column = 0) : string
    {
        return (new \_PhpScoper006a73f0e455\Nette\PhpGenerator\Dumper())->dump($var, $column);
    }
    protected function printUses(\_PhpScoper006a73f0e455\Nette\PhpGenerator\PhpNamespace $namespace) : string
    {
        $name = $namespace->getName();
        $uses = [];
     ...
     */
    public function printParameters($function, \_PhpScoper006a73f0e455\Nette\PhpGenerator\PhpNamespace $namespace = null) : string
    {
        $params = [];
        ...
        foreach ($list as $param) {
            ...
            $type = $param->getType();
            $promoted = $param instanceof \_PhpScoper006a73f0e455\Nette\PhpGenerator\PromotedParameter ? $param : null;
            $params[] = ($promoted ? \_PhpScoper006a73f0e455\Nette\PhpGenerator\Helpers::formatDocComment((string) $promoted->getComment()) : '') . ($attrs = self::printAttributes($param->getAttributes(), $namespace, \true)) . ($promoted ? ($promoted->getVisibility() ?: 'public') . ' ' : '') . \ltrim($this->printType($type, $param->isNullable(), $namespace) . ' ') . ($param->isReference() ? '&' : '') . ($variadic ? '...' : '') . '$' . $param->getName() . ($param->hasDefaultValue() && !$variadic ? ' = ' . $this->dump($param->getDefaultValue()) : '');
            $special = $special || $promoted || $attrs;
        }
        $line = \implode(', ', $params);
        return \count($params) > 1 && ($special || \strlen($line) > (new \_PhpScoper006a73f0e455\Nette\PhpGenerator\Dumper())->wrapLength) ? "(\n" . $this->indent(\implode(",\n", $params)) . ($special ? ',' : '') . "\n)" : "({$line})";
    }
    public function printType(?string $type, bool $nullable = \false, \_PhpScoper006a73f0e455\Nette\PhpGenerator\PhpNamespace $namespace = null) : string
    {
     ...
     * @param Closure|GlobalFunction|Method  $function
     */
    private function printReturnType($function, ?\_PhpScoper006a73f0e455\Nette\PhpGenerator\PhpNamespace $namespace) : string
    {
        return ($tmp = $this->printType($function->getReturnType(), $function->isReturnNullable(), $namespace)) ? $this->returnTypeColon . $tmp : '';
    }
    private function printAttributes(array $attrs, ?\_PhpScoper006a73f0e455\Nette\PhpGenerator\PhpNamespace $namespace, bool $inline = \false) : string
    {
        if (!$attrs) {
            return '';
        }
        ...
        foreach ($attrs as $attr) {
            $args = (new \_PhpScoper006a73f0e455\Nette\PhpGenerator\Dumper())->format('...?:', $attr->getArguments());
