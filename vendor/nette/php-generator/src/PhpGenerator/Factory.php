...
use _PhpScoperabd03f0baf05\PhpParser;
use PhpParser\Node;
...
final class Factory
{
    ...
    public function fromClassReflection(\ReflectionClass $from, bool $withBodies = \false) : \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType
    {
        $class = $from->isAnonymous() ? new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType() : new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType($from->getShortName(), new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\PhpNamespace($from->getNamespaceName()));
        ...
        $class->setComment(\_PhpScoperabd03f0baf05\Nette\PhpGenerator\Helpers::unformatDocComment((string) $from->getDocComment()));
        ...
        foreach ($from->getMethods() as $method) {
            if ($method->getDeclaringClass()->name === $from->name) {
                ...
                if ($withBodies) {
                    $srcMethod = \_PhpScoperabd03f0baf05\Nette\Utils\Reflection::getMethodDeclaringMethod($method);
    ...
    public function fromMethodReflection(\ReflectionMethod $from) : \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Method
    {
        $method = new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Method($from->name);
        $method->setParameters(\array_map([$this, 'fromParameterReflection'], $from->getParameters()));
        $method->setStatic($from->isStatic());
        $isInterface = $from->getDeclaringClass()->isInterface();
        $method->setVisibility($from->isPrivate() ? \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType::VISIBILITY_PRIVATE : ($from->isProtected() ? \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType::VISIBILITY_PROTECTED : ($isInterface ? null : \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType::VISIBILITY_PUBLIC)));
        $method->setFinal($from->isFinal());
        $method->setAbstract($from->isAbstract() && !$isInterface);
        $method->setBody($from->isAbstract() ? null : '');
        $method->setReturnReference($from->returnsReference());
        $method->setVariadic($from->isVariadic());
        $method->setComment(\_PhpScoperabd03f0baf05\Nette\PhpGenerator\Helpers::unformatDocComment((string) $from->getDocComment()));
    ...
    {
        $function = $from->isClosure() ? new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Closure() : new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\GlobalFunction($from->name);
        ...
        if (!$from->isClosure()) {
            $function->setComment(\_PhpScoperabd03f0baf05\Nette\PhpGenerator\Helpers::unformatDocComment((string) $from->getDocComment()));
        }
    ...
    {
        $ref = \_PhpScoperabd03f0baf05\Nette\Utils\Callback::toReflection($from);
        return $ref instanceof \ReflectionMethod ? self::fromMethodReflection($ref) : self::fromFunctionReflection($ref);
    }
    public function fromParameterReflection(\ReflectionParameter $from) : \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Parameter
    {
        $param = \PHP_VERSION_ID >= 80000 && $from->isPromoted() ? new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\PromotedParameter($from->name) : new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Parameter($from->name);
        ...
        if ($from->isDefaultValueAvailable()) {
            $param->setDefaultValue($from->isDefaultValueConstant() ? new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Literal($from->getDefaultValueConstantName()) : $from->getDefaultValue());
            $param->setNullable($param->isNullable() && $param->getDefaultValue() !== null);
        }
        $param->setAttributes(self::getAttributes($from));
        return $param;
    }
    public function fromConstantReflection(\ReflectionClassConstant $from) : \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Constant
    {
        $const = new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Constant($from->name);
        $const->setValue($from->getValue());
        $const->setVisibility($from->isPrivate() ? \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType::VISIBILITY_PRIVATE : ($from->isProtected() ? \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType::VISIBILITY_PROTECTED : \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType::VISIBILITY_PUBLIC));
        $const->setComment(\_PhpScoperabd03f0baf05\Nette\PhpGenerator\Helpers::unformatDocComment((string) $from->getDocComment()));
        $const->setAttributes(self::getAttributes($from));
        return $const;
    }
    public function fromPropertyReflection(\ReflectionProperty $from) : \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Property
    {
        $defaults = $from->getDeclaringClass()->getDefaultProperties();
        $prop = new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Property($from->name);
        $prop->setValue($defaults[$prop->getName()] ?? null);
        $prop->setStatic($from->isStatic());
        $prop->setVisibility($from->isPrivate() ? \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType::VISIBILITY_PRIVATE : ($from->isProtected() ? \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType::VISIBILITY_PROTECTED : \_PhpScoperabd03f0baf05\Nette\PhpGenerator\ClassType::VISIBILITY_PUBLIC));
        ...
        $prop->setComment(\_PhpScoperabd03f0baf05\Nette\PhpGenerator\Helpers::unformatDocComment((string) $from->getDocComment()));
    ...
    {
        if ($from->isAnonymous()) {
            throw new \_PhpScoperabd03f0baf05\Nette\NotSupportedException('Anonymous classes are not supported.');
        }
        ...
        foreach ($nodeFinder->findInstanceOf($class, \PhpParser\Node\Stmt\ClassMethod::class) as $method) {
            ...
            if ($method->stmts) {
                ...
                $bodies[$method->name->toString()] = \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Helpers::unindent($body, 2);
            }
        }
        return $bodies;
    }
    ...
    {
        if ($from->isClosure()) {
            throw new \_PhpScoperabd03f0baf05\Nette\NotSupportedException('Closures are not supported.');
        }
        [$code, $stmts] = $this->parse($from);
        $nodeFinder = new \PhpParser\NodeFinder();
        ...
        return \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Helpers::unindent($body, 1);
    }
    /**
    ...
    private function parse($from) : array
    {
        ...
        if (!\class_exists(\PhpParser\ParserFactory::class)) {
            throw new \_PhpScoperabd03f0baf05\Nette\NotSupportedException("PHP-Parser is required to load method bodies, install package 'nikic/php-parser'.");
        } elseif (!$file) {
            throw new \_PhpScoperabd03f0baf05\Nette\InvalidStateException("Source code of {$from->name} not found.");
        }
    ...
    private function getAttributes($from) : array
    {
        ...
        foreach ($from->getAttributes() as $attr) {
            $res[] = new \_PhpScoperabd03f0baf05\Nette\PhpGenerator\Attribute($attr->getName(), $attr->getArguments());
        }
        return $res;
    }
}
