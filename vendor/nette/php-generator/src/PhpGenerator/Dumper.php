...
final class Dumper
{
    ...
    {
        if ($var instanceof \_PhpScoperbd5d0c5f7638\Nette\PhpGenerator\Literal) {
            return \ltrim(\_PhpScoperbd5d0c5f7638\Nette\Utils\Strings::indent(\trim((string) $var), $level), "\t");
        } elseif ($var === null) {
            return 'null';
            ...
            throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidArgumentException('Cannot dump resource.');
        } else {
    ...
    private function dumpArray(array &$var, array $parents, int $level, int $column) : string
    {
        if (empty($var)) {
            ...
            throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidArgumentException('Nesting level too deep or recursive dependency.');
        }
    ...
    private function dumpObject(&$var, array $parents, int $level) : string
    {
        if ($var instanceof \Serializable) {
            ...
            throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidArgumentException('Cannot dump closure.');
        }
        $class = \get_class($var);
        if ((new \ReflectionObject($var))->isAnonymous()) {
            throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidArgumentException('Cannot dump anonymous class.');
    ...
    public function format(string $statement, ...$args) : string
    {
        ...
        foreach ($tokens as $n => $token) {
            if ($n % 2 === 0) {
                ...
                throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidArgumentException('Insufficient number of arguments.');
            } elseif ($token === '?') {
                $res .= $this->dump(\array_shift($args), \strlen($res) - \strrpos($res, "\n"));
            } elseif ($token === '...?' || $token === '...?:' || $token === '?*') {
                $arg = \array_shift($args);
                if (!\is_array($arg)) {
                    throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidArgumentException('Argument must be an array.');
                }
                $res .= $this->dumpArguments($arg, \strlen($res) - \strrpos($res, "\n"), $token === '...?:');
            } else {
                // $  ->  ::
                $arg = \array_shift($args);
                if ($arg instanceof \_PhpScoperbd5d0c5f7638\Nette\PhpGenerator\Literal || !\_PhpScoperbd5d0c5f7638\Nette\PhpGenerator\Helpers::isIdentifier($arg)) {
        ...
        if ($args) {
            throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidArgumentException('Insufficient number of placeholders.');
        }
        return $res;
    }
