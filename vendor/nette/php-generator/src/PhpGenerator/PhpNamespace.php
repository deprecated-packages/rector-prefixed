...
use _PhpScopera143bcca66cb\Nette\InvalidStateException;
...
final class PhpNamespace
{
    ...
    {
        if ($name !== '' && !\_PhpScopera143bcca66cb\Nette\PhpGenerator\Helpers::isNamespaceIdentifier($name)) {
    ...
    {
        $name = \ltrim($name, '\\');
        if ($alias === null && $this->name === \_PhpScopera143bcca66cb\Nette\PhpGenerator\Helpers::extractNamespace($name)) {
            $alias = \_PhpScopera143bcca66cb\Nette\PhpGenerator\Helpers::extractShortName($name);
        }
        if ($alias === null) {
            $path = \explode('\\', $name);
            $counter = null;
            ...
            throw new \_PhpScopera143bcca66cb\Nette\InvalidStateException("Alias '{$alias}' used already for '{$this->uses[$alias]}', cannot use for '{$name}'.");
        }
        $aliasOut = $alias;
    ...
    public function unresolveName(string $name) : string
    {
        ...
        foreach ($this->uses as $alias => $original) {
            if (\_PhpScopera143bcca66cb\Nette\Utils\Strings::startsWith($lower . '\\', \strtolower($original) . '\\')) {
        ...
        if (!$res && \_PhpScopera143bcca66cb\Nette\Utils\Strings::startsWith($lower, \strtolower($this->name) . '\\')) {
    ...
    /** @return static */
    public function add(\_PhpScopera143bcca66cb\Nette\PhpGenerator\ClassType $class) : self
    {
        $name = $class->getName();
        if ($name === null) {
            throw new \_PhpScopera143bcca66cb\Nette\InvalidArgumentException('Class does not have a name.');
        }
    ...
    {
        $this->add($class = new \_PhpScopera143bcca66cb\Nette\PhpGenerator\ClassType($name, $this));
        return $class;
    }
    ...
    {
        try {
            return (new \_PhpScopera143bcca66cb\Nette\PhpGenerator\Printer())->printNamespace($this);
        } catch (\Throwable $e) {
