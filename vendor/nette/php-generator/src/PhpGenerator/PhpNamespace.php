...
use _PhpScoper88fe6e0ad041\Nette\InvalidStateException;
...
final class PhpNamespace
{
    ...
    {
        if ($name !== '' && !\_PhpScoper88fe6e0ad041\Nette\PhpGenerator\Helpers::isNamespaceIdentifier($name)) {
    ...
    {
        $name = \ltrim($name, '\\');
        if ($alias === null && $this->name === \_PhpScoper88fe6e0ad041\Nette\PhpGenerator\Helpers::extractNamespace($name)) {
            $alias = \_PhpScoper88fe6e0ad041\Nette\PhpGenerator\Helpers::extractShortName($name);
        }
        if ($alias === null) {
            $path = \explode('\\', $name);
            $counter = null;
            ...
            throw new \_PhpScoper88fe6e0ad041\Nette\InvalidStateException("Alias '{$alias}' used already for '{$this->uses[$alias]}', cannot use for '{$name}'.");
        }
        $aliasOut = $alias;
    ...
    public function unresolveName(string $name) : string
    {
        ...
        foreach ($this->uses as $alias => $original) {
            if (\_PhpScoper88fe6e0ad041\Nette\Utils\Strings::startsWith($lower . '\\', \strtolower($original) . '\\')) {
        ...
        if (!$res && \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::startsWith($lower, \strtolower($this->name) . '\\')) {
    ...
    /** @return static */
    public function add(\_PhpScoper88fe6e0ad041\Nette\PhpGenerator\ClassType $class) : self
    {
        $name = $class->getName();
        if ($name === null) {
            throw new \_PhpScoper88fe6e0ad041\Nette\InvalidArgumentException('Class does not have a name.');
        }
    ...
    {
        $this->add($class = new \_PhpScoper88fe6e0ad041\Nette\PhpGenerator\ClassType($name, $this));
        return $class;
    }
    ...
    {
        try {
            return (new \_PhpScoper88fe6e0ad041\Nette\PhpGenerator\Printer())->printNamespace($this);
        } catch (\Throwable $e) {
