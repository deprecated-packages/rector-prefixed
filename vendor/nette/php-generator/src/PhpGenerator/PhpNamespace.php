...
use _PhpScoperbd5d0c5f7638\Nette\InvalidStateException;
...
final class PhpNamespace
{
    ...
    {
        if ($name !== '' && !\_PhpScoperbd5d0c5f7638\Nette\PhpGenerator\Helpers::isNamespaceIdentifier($name)) {
    ...
    {
        $name = \ltrim($name, '\\');
        if ($alias === null && $this->name === \_PhpScoperbd5d0c5f7638\Nette\PhpGenerator\Helpers::extractNamespace($name)) {
            $alias = \_PhpScoperbd5d0c5f7638\Nette\PhpGenerator\Helpers::extractShortName($name);
        }
        if ($alias === null) {
            $path = \explode('\\', $name);
            $counter = null;
            ...
            throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidStateException("Alias '{$alias}' used already for '{$this->uses[$alias]}', cannot use for '{$name}'.");
        }
        $aliasOut = $alias;
    ...
    public function unresolveName(string $name) : string
    {
        ...
        foreach ($this->uses as $alias => $original) {
            if (\_PhpScoperbd5d0c5f7638\Nette\Utils\Strings::startsWith($lower . '\\', \strtolower($original) . '\\')) {
        ...
        if (!$res && \_PhpScoperbd5d0c5f7638\Nette\Utils\Strings::startsWith($lower, \strtolower($this->name) . '\\')) {
    ...
    /** @return static */
    public function add(\_PhpScoperbd5d0c5f7638\Nette\PhpGenerator\ClassType $class) : self
    {
        $name = $class->getName();
        if ($name === null) {
            throw new \_PhpScoperbd5d0c5f7638\Nette\InvalidArgumentException('Class does not have a name.');
        }
    ...
    {
        $this->add($class = new \_PhpScoperbd5d0c5f7638\Nette\PhpGenerator\ClassType($name, $this));
        return $class;
    }
    ...
    {
        try {
            return (new \_PhpScoperbd5d0c5f7638\Nette\PhpGenerator\Printer())->printNamespace($this);
        } catch (\Throwable $e) {
