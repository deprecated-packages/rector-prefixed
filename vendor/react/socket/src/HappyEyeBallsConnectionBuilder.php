...
use _PhpScopera143bcca66cb\React\EventLoop\TimerInterface;
...
final class HappyEyeBallsConnectionBuilder
{
    ...
    public $uri;
    public $host;
    public $resolved = array(\_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_A => \false, \_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_AAAA => \false);
    ...
    public $lastError6;
    public $lastError4;
    public function __construct(\_PhpScopera143bcca66cb\React\EventLoop\LoopInterface $loop, \_PhpScopera143bcca66cb\React\Socket\ConnectorInterface $connector, \_PhpScopera143bcca66cb\React\Dns\Resolver\ResolverInterface $resolver, $uri, $host, $parts)
    {
        $this->loop = $loop;
    ...
    public function connect()
    {
        $timer = null;
        $that = $this;
        return new \_PhpScopera143bcca66cb\React\Promise\Promise(function ($resolve, $reject) use($that, &$timer) {
            ...
            $that->resolverPromises[\_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_AAAA] = $that->resolve(\_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_AAAA, $reject)->then($lookupResolve(\_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_AAAA));
            $that->resolverPromises[\_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_A] = $that->resolve(\_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_A, $reject)->then(function (array $ips) use($that, &$timer) {
                // happy path: IPv6 has resolved already, continue with IPv4 addresses
                if ($that->resolved[\_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_AAAA] === \true) {
                    return $ips;
                }
                // Otherwise delay processing IPv4 lookup until short timer passes or IPv6 resolves in the meantime
                $deferred = new \_PhpScopera143bcca66cb\React\Promise\Deferred();
                $timer = $that->loop->addTimer($that::RESOLUTION_DELAY, function () use($deferred, $ips) {
                    $deferred->resolve($ips);
                });
                $that->resolverPromises[\_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_AAAA]->then(function () use($that, $timer, $deferred, $ips) {
                    $that->loop->cancelTimer($timer);
                    $deferred->resolve($ips);
                });
                return $deferred->promise();
            })->then($lookupResolve(\_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_A));
            ...
            $_ = $reject = null;
            $that->cleanUp();
            if ($timer instanceof \_PhpScopera143bcca66cb\React\EventLoop\TimerInterface) {
                $that->loop->cancelTimer($timer);
            }
        });
    }
    ...
    public function resolve($type, $reject)
    {
        ...
        return $that->resolver->resolveAll($that->host, $type)->then(null, function (\Exception $e) use($type, $reject, $that) {
            unset($that->resolverPromises[$type]);
            $that->resolved[$type] = \true;
            if ($type === \_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_A) {
    ...
    public function check($resolve, $reject)
    {
        ...
        if ($this->nextAttemptTimer === null && (\count($this->connectQueue) > 0 || $this->resolved[\_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_A] === \false || $this->resolved[\_PhpScopera143bcca66cb\React\Dns\Model\Message::TYPE_AAAA] === \false)) {
    ...
    public function cleanUp()
    {
        ...
        foreach ($this->connectionPromises as $connectionPromise) {
            if ($connectionPromise instanceof \_PhpScopera143bcca66cb\React\Promise\CancellablePromiseInterface) {
                $connectionPromise->cancel();
            }
        }
        foreach ($this->resolverPromises as $resolverPromise) {
            if ($resolverPromise instanceof \_PhpScopera143bcca66cb\React\Promise\CancellablePromiseInterface) {
                $resolverPromise->cancel();
            }
        }
        if ($this->nextAttemptTimer instanceof \_PhpScopera143bcca66cb\React\EventLoop\TimerInterface) {
