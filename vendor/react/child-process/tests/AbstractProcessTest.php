...
use _PhpScoper006a73f0e455\SebastianBergmann\Environment\Runtime;
abstract class AbstractProcessTest extends \_PhpScoper006a73f0e455\PHPUnit\Framework\TestCase
{
    public abstract function createLoop();
    public function testGetCommand()
    {
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('echo foo', null, null, array());
    ...
    public function testPipesWillBeUnsetBeforeStarting()
    {
        ...
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('echo foo');
    ...
    public function testStartWillAssignPipes()
    {
        ...
        $this->assertInstanceOf('_PhpScoper006a73f0e455\\React\\Stream\\WritableStreamInterface', $process->stdin);
        $this->assertInstanceOf('_PhpScoper006a73f0e455\\React\\Stream\\ReadableStreamInterface', $process->stdout);
        $this->assertInstanceOf('_PhpScoper006a73f0e455\\React\\Stream\\ReadableStreamInterface', $process->stderr);
    ...
    {
        if (\DIRECTORY_SEPARATOR === '\\') {
            $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('cmd /c exit 0', null, null, array());
        } else {
            $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('exit 0', null, null, array());
        }
        $process->start($this->createLoop());
        $this->assertNull($process->stdin);
        $this->assertNull($process->stdout);
    ...
    public function testStartWithCustomPipesWillAssignPipes()
    {
        ...
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('exit 0', null, null, array(0 => array('pipe', 'w'), 3 => array('pipe', 'r')));
        $process->start($this->createLoop());
        $this->assertInstanceOf('_PhpScoper006a73f0e455\\React\\Stream\\ReadableStreamInterface', $process->stdin);
        $this->assertNull($process->stdout);
        $this->assertNull($process->stderr);
        $this->assertCount(2, $process->pipes);
        $this->assertSame($process->stdin, $process->pipes[0]);
        $this->assertInstanceOf('_PhpScoper006a73f0e455\\React\\Stream\\WritableStreamInterface', $process->pipes[3]);
    }
    /**
    ...
    public function testStartWithInvalidFileDescriptorPathWillThrow()
    {
        ...
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('exit 0', null, null, $fds);
        $process->start($this->createLoop());
    }
    public function testStartWithExcessiveNumberOfFileDescriptorsWillThrow()
    {
        ...
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('ping example.com', null, null, $new);
        try {
            $process->start($loop);
    ...
    public function testIsRunning()
    {
        if (\DIRECTORY_SEPARATOR === '\\') {
            ...
            $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process($this->getPhpBinary() . ' -r ' . \escapeshellarg('sleep(1);'), null, null, array());
        } else {
            $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('sleep 1');
        }
    ...
    public function testCommandWithEnhancedSigchildCompatibilityReceivesExitCode()
    {
        ...
        $loop = $this->createLoop();
        $old = \_PhpScoper006a73f0e455\React\ChildProcess\Process::isSigchildEnabled();
        \_PhpScoper006a73f0e455\React\ChildProcess\Process::setSigchildEnabled(\true);
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('echo foo');
        $process->start($loop);
        \_PhpScoper006a73f0e455\React\ChildProcess\Process::setSigchildEnabled($old);
        $loop->run();
    ...
    public function testReceivesProcessStdoutFromEcho()
    {
        ...
        $cmd = 'echo test';
        $loop = $this->createLoop();
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process($cmd);
        $process->start($loop);
        $buffer = '';
    ...
    public function testReceivesProcessOutputFromStdoutRedirectedToFile()
    {
        ...
        $loop = $this->createLoop();
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process($cmd, null, null, array(1 => $tmp));
        $process->start($loop);
        $loop->run();
        \rewind($tmp);
    ...
    public function testReceivesProcessOutputFromStdoutAttachedToSocket()
    {
        ...
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process($cmd, null, null, array(1 => $client));
        $process->start($loop);
        \fclose($client);
        $loop->run();
    ...
    public function testReceivesProcessOutputFromStdoutRedirectedToSocketProcess()
    {
        ...
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process($cmd, null, null, array());
        $process->start($loop);
    ...
    public function testProcessWithEnv()
    {
        ...
        $loop = $this->createLoop();
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process($cmd, null, array('foo' => 'bar'));
        $process->start($loop);
        $output = '';
        $process->stdout->on('data', function () use(&$output) {
            $output .= \func_get_arg(0);
    ...
    public function testStartAndAllowProcessToExitSuccessfullyUsingEventLoop()
    {
        ...
        $loop = $this->createLoop();
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('exit 0');
        $called = \false;
        $exitCode = 'initial';
        $termSignal = 'initial';
    ...
    public function testProcessWillExitFasterThanExitInterval()
    {
        ...
        $loop = $this->createLoop();
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('echo hi');
    ...
    public function testTerminateWillExit()
    {
        if (\DIRECTORY_SEPARATOR === '\\') {
            // Windows doesn't have a sleep command and also does not support process pipes
            $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process($this->getPhpBinary() . ' -r ' . \escapeshellarg('sleep(10);'), null, null, array());
        } else {
            $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('sleep 10');
        }
        $loop = $this->createloop();
        $process->start($loop);
        $called = \false;
    ...
    public function testTerminateWithDefaultTermSignalUsingEventLoop()
    {
        ...
        $loop = $this->createloop();
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process('sleep 1; exit 0');
        $called = \false;
        $exitCode = 'initial';
        $termSignal = 'initial';
        $process->on('exit', function () use(&$called, &$exitCode, &$termSignal) {
    ...
    public function testIssue18()
    {
        ...
        $loop = $this->createLoop();
        $testString = 'x';
        $process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process($this->getPhpBinary() . " -r 'echo \"{$testString}\";'");
        $stdOut = '';
        $stdErr = '';
        $that = $this;
    ...
    public function assertSoon(\Closure $callback, $timeout = 20000, $interval = 200)
    {
        ...
        while (1) {
            ...
            } catch (\_PhpScoper006a73f0e455\PHPUnit_Framework_ExpectationFailedException $e) {
    ...
    {
        $runtime = new \_PhpScoper006a73f0e455\SebastianBergmann\Environment\Runtime();
