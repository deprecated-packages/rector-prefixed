 ...
 * @internal
 */
class RequestHeaderParser extends \_PhpScoperabd03f0baf05\Evenement\EventEmitter
{
    private $maxSize = 8192;
    public function handle(\_PhpScoperabd03f0baf05\React\Socket\ConnectionInterface $conn)
    {
        $buffer = '';
        ...
        $conn->on('data', $fn = function ($data) use(&$buffer, &$fn, $conn, $maxSize, $that) {
            ...
            if ($contentLength === 0) {
                ...
                $stream = new \_PhpScoperabd03f0baf05\React\Http\Io\EmptyBodyStream();
                $request = $request->withBody($stream);
            } else {
                // otherwise body is present => delimit using Content-Length or ChunkedDecoder
                $stream = new \_PhpScoperabd03f0baf05\React\Http\Io\CloseProtectionStream($conn);
                if ($contentLength !== null) {
                    $stream = new \_PhpScoperabd03f0baf05\React\Http\Io\LengthLimitedStream($stream, $contentLength);
                } else {
                    $stream = new \_PhpScoperabd03f0baf05\React\Http\Io\ChunkedDecoder($stream);
                }
                $request = $request->withBody(new \_PhpScoperabd03f0baf05\React\Http\Io\HttpBodyStream($stream, $contentLength));
            }
    ...
    public function parseRequest($headers, $remoteSocketUri, $localSocketUri)
    {
        ...
        $request = new \_PhpScoperabd03f0baf05\React\Http\Message\ServerRequest($start['method'], $uri, $fields, '', $start['version'], $serverParams);
