...
use _PhpScoper26e51eeacccf\Psr\Http\Message\RequestInterface;
...
use _PhpScoper26e51eeacccf\React\Http\Message\ResponseException;
...
class Transaction
{
    ...
    // 16 MiB = 2^24 bytes
    public function __construct(\_PhpScoper26e51eeacccf\React\Http\Io\Sender $sender, \_PhpScoper26e51eeacccf\React\EventLoop\LoopInterface $loop)
    {
        $this->sender = $sender;
        $this->loop = $loop;
    }
    ...
    public function send(\_PhpScoper26e51eeacccf\Psr\Http\Message\RequestInterface $request)
    {
        $deferred = new \_PhpScoper26e51eeacccf\React\Promise\Deferred(function () use(&$deferred) {
        ...
        $loop = $this->loop;
        $this->next($request, $deferred)->then(function (\_PhpScoper26e51eeacccf\Psr\Http\Message\ResponseInterface $response) use($deferred, $loop, &$timeout) {
        ...
        if ($body instanceof \_PhpScoper26e51eeacccf\React\Stream\ReadableStreamInterface && $body->isReadable()) {
            $that = $this;
     ...
     * @return void
     */
    public function applyTimeout(\_PhpScoper26e51eeacccf\React\Promise\Deferred $deferred, $timeout)
    {
    ...
    private function next(\_PhpScoper26e51eeacccf\Psr\Http\Message\RequestInterface $request, \_PhpScoper26e51eeacccf\React\Promise\Deferred $deferred)
    {
        ...
        return $promise->then(function (\_PhpScoper26e51eeacccf\Psr\Http\Message\ResponseInterface $response) use($request, $that, $deferred) {
     ...
     */
    public function bufferResponse(\_PhpScoper26e51eeacccf\Psr\Http\Message\ResponseInterface $response, $deferred)
    {
        $stream = $response->getBody();
        $size = $stream->getSize();
        if ($size !== null && $size > $this->maximumSize) {
            $stream->close();
            return \_PhpScoper26e51eeacccf\React\Promise\reject(new \OverflowException('Response body size of ' . $size . ' bytes exceeds maximum of ' . $this->maximumSize . ' bytes', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 0));
        }
        // body is not streaming => already buffered
        if (!$stream instanceof \_PhpScoper26e51eeacccf\React\Stream\ReadableStreamInterface) {
            return \_PhpScoper26e51eeacccf\React\Promise\resolve($response);
        }
        // buffer stream and resolve with buffered body
        $maximumSize = $this->maximumSize;
        $promise = \_PhpScoper26e51eeacccf\React\Promise\Stream\buffer($stream, $maximumSize)->then(function ($body) use($response) {
            return $response->withBody(\_PhpScoper26e51eeacccf\RingCentral\Psr7\stream_for($body));
     ...
     */
    public function onResponse(\_PhpScoper26e51eeacccf\Psr\Http\Message\ResponseInterface $response, \_PhpScoper26e51eeacccf\Psr\Http\Message\RequestInterface $request, $deferred)
    {
        ...
        if ($this->obeySuccessCode && ($response->getStatusCode() < 200 || $response->getStatusCode() >= 400)) {
            throw new \_PhpScoper26e51eeacccf\React\Http\Message\ResponseException($response);
        }
     ...
     */
    private function onResponseRedirect(\_PhpScoper26e51eeacccf\Psr\Http\Message\ResponseInterface $response, \_PhpScoper26e51eeacccf\Psr\Http\Message\RequestInterface $request, \_PhpScoper26e51eeacccf\React\Promise\Deferred $deferred)
    {
        // resolve location relative to last request URI
        $location = \_PhpScoper26e51eeacccf\RingCentral\Psr7\Uri::resolve($request->getUri(), $response->getHeaderLine('Location'));
     ...
     */
    private function makeRedirectRequest(\_PhpScoper26e51eeacccf\Psr\Http\Message\RequestInterface $request, \_PhpScoper26e51eeacccf\Psr\Http\Message\UriInterface $location)
    {
        ...
        return new \_PhpScoper26e51eeacccf\RingCentral\Psr7\Request($method, $location, $request->getHeaders());
    }
    private function progress($name, array $args = array())
    {
        ...
        foreach ($args as $arg) {
            echo ' ';
            if ($arg instanceof \_PhpScoper26e51eeacccf\Psr\Http\Message\ResponseInterface) {
                echo 'HTTP/' . $arg->getProtocolVersion() . ' ' . $arg->getStatusCode() . ' ' . $arg->getReasonPhrase();
            } elseif ($arg instanceof \_PhpScoper26e51eeacccf\Psr\Http\Message\RequestInterface) {
