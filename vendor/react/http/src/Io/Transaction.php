...
use _PhpScopera143bcca66cb\Psr\Http\Message\RequestInterface;
use _PhpScopera143bcca66cb\Psr\Http\Message\ResponseInterface;
use _PhpScopera143bcca66cb\Psr\Http\Message\UriInterface;
...
use _PhpScopera143bcca66cb\React\Http\Message\ResponseException;
...
class Transaction
{
    ...
    // 16 MiB = 2^24 bytes
    public function __construct(\_PhpScopera143bcca66cb\React\Http\Io\Sender $sender, \_PhpScopera143bcca66cb\React\EventLoop\LoopInterface $loop)
    {
        $this->sender = $sender;
        $this->loop = $loop;
    }
    ...
    public function send(\_PhpScopera143bcca66cb\Psr\Http\Message\RequestInterface $request)
    {
        $deferred = new \_PhpScopera143bcca66cb\React\Promise\Deferred(function () use(&$deferred) {
        ...
        $loop = $this->loop;
        $this->next($request, $deferred)->then(function (\_PhpScopera143bcca66cb\Psr\Http\Message\ResponseInterface $response) use($deferred, $loop, &$timeout) {
        ...
        if ($body instanceof \_PhpScopera143bcca66cb\React\Stream\ReadableStreamInterface && $body->isReadable()) {
            $that = $this;
     ...
     * @return void
     */
    public function applyTimeout(\_PhpScopera143bcca66cb\React\Promise\Deferred $deferred, $timeout)
    {
    ...
    private function next(\_PhpScopera143bcca66cb\Psr\Http\Message\RequestInterface $request, \_PhpScopera143bcca66cb\React\Promise\Deferred $deferred)
    {
        ...
        return $promise->then(function (\_PhpScopera143bcca66cb\Psr\Http\Message\ResponseInterface $response) use($request, $that, $deferred) {
     ...
     */
    public function bufferResponse(\_PhpScopera143bcca66cb\Psr\Http\Message\ResponseInterface $response, $deferred)
    {
        $stream = $response->getBody();
        $size = $stream->getSize();
        if ($size !== null && $size > $this->maximumSize) {
            $stream->close();
            return \_PhpScopera143bcca66cb\React\Promise\reject(new \OverflowException('Response body size of ' . $size . ' bytes exceeds maximum of ' . $this->maximumSize . ' bytes', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 0));
        }
        // body is not streaming => already buffered
        if (!$stream instanceof \_PhpScopera143bcca66cb\React\Stream\ReadableStreamInterface) {
            return \_PhpScopera143bcca66cb\React\Promise\resolve($response);
        }
        // buffer stream and resolve with buffered body
        $maximumSize = $this->maximumSize;
        $promise = \_PhpScopera143bcca66cb\React\Promise\Stream\buffer($stream, $maximumSize)->then(function ($body) use($response) {
            return $response->withBody(\_PhpScopera143bcca66cb\RingCentral\Psr7\stream_for($body));
     ...
     */
    public function onResponse(\_PhpScopera143bcca66cb\Psr\Http\Message\ResponseInterface $response, \_PhpScopera143bcca66cb\Psr\Http\Message\RequestInterface $request, $deferred)
    {
        ...
        if ($this->obeySuccessCode && ($response->getStatusCode() < 200 || $response->getStatusCode() >= 400)) {
            throw new \_PhpScopera143bcca66cb\React\Http\Message\ResponseException($response);
        }
     ...
     */
    private function onResponseRedirect(\_PhpScopera143bcca66cb\Psr\Http\Message\ResponseInterface $response, \_PhpScopera143bcca66cb\Psr\Http\Message\RequestInterface $request, \_PhpScopera143bcca66cb\React\Promise\Deferred $deferred)
    {
        // resolve location relative to last request URI
        $location = \_PhpScopera143bcca66cb\RingCentral\Psr7\Uri::resolve($request->getUri(), $response->getHeaderLine('Location'));
     ...
     */
    private function makeRedirectRequest(\_PhpScopera143bcca66cb\Psr\Http\Message\RequestInterface $request, \_PhpScopera143bcca66cb\Psr\Http\Message\UriInterface $location)
    {
        ...
        return new \_PhpScopera143bcca66cb\RingCentral\Psr7\Request($method, $location, $request->getHeaders());
    }
    private function progress($name, array $args = array())
    {
        ...
        foreach ($args as $arg) {
            echo ' ';
            if ($arg instanceof \_PhpScopera143bcca66cb\Psr\Http\Message\ResponseInterface) {
                echo 'HTTP/' . $arg->getProtocolVersion() . ' ' . $arg->getStatusCode() . ' ' . $arg->getReasonPhrase();
            } elseif ($arg instanceof \_PhpScopera143bcca66cb\Psr\Http\Message\RequestInterface) {
