...
use _PhpScoper006a73f0e455\RingCentral\Psr7\Request;
use _PhpScoper006a73f0e455\RingCentral\Psr7\Uri;
use _PhpScoper006a73f0e455\React\EventLoop\LoopInterface;
use _PhpScoper006a73f0e455\React\Http\Message\ResponseException;
...
class Transaction
{
    ...
    // 16 MiB = 2^24 bytes
    public function __construct(\_PhpScoper006a73f0e455\React\Http\Io\Sender $sender, \_PhpScoper006a73f0e455\React\EventLoop\LoopInterface $loop)
    {
        $this->sender = $sender;
        $this->loop = $loop;
    }
    ...
    public function send(\_PhpScoper006a73f0e455\Psr\Http\Message\RequestInterface $request)
    {
        $deferred = new \_PhpScoper006a73f0e455\React\Promise\Deferred(function () use(&$deferred) {
        ...
        $loop = $this->loop;
        $this->next($request, $deferred)->then(function (\_PhpScoper006a73f0e455\Psr\Http\Message\ResponseInterface $response) use($deferred, $loop, &$timeout) {
     ...
     * @return void
     */
    public function applyTimeout(\_PhpScoper006a73f0e455\React\Promise\Deferred $deferred, $timeout)
    {
    ...
    private function next(\_PhpScoper006a73f0e455\Psr\Http\Message\RequestInterface $request, \_PhpScoper006a73f0e455\React\Promise\Deferred $deferred)
    {
        ...
        return $promise->then(function (\_PhpScoper006a73f0e455\Psr\Http\Message\ResponseInterface $response) use($request, $that, $deferred) {
     ...
     */
    public function bufferResponse(\_PhpScoper006a73f0e455\Psr\Http\Message\ResponseInterface $response, $deferred)
    {
        $stream = $response->getBody();
        $size = $stream->getSize();
        if ($size !== null && $size > $this->maximumSize) {
            $stream->close();
            return \_PhpScoper006a73f0e455\React\Promise\reject(new \OverflowException('Response body size of ' . $size . ' bytes exceeds maximum of ' . $this->maximumSize . ' bytes', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 0));
        }
        // body is not streaming => already buffered
        if (!$stream instanceof \_PhpScoper006a73f0e455\React\Stream\ReadableStreamInterface) {
            return \_PhpScoper006a73f0e455\React\Promise\resolve($response);
        }
        // buffer stream and resolve with buffered body
        $maximumSize = $this->maximumSize;
        $promise = \_PhpScoper006a73f0e455\React\Promise\Stream\buffer($stream, $maximumSize)->then(function ($body) use($response) {
            return $response->withBody(\_PhpScoper006a73f0e455\RingCentral\Psr7\stream_for($body));
     ...
     */
    public function onResponse(\_PhpScoper006a73f0e455\Psr\Http\Message\ResponseInterface $response, \_PhpScoper006a73f0e455\Psr\Http\Message\RequestInterface $request, $deferred)
    {
        ...
        if ($this->obeySuccessCode && ($response->getStatusCode() < 200 || $response->getStatusCode() >= 400)) {
            throw new \_PhpScoper006a73f0e455\React\Http\Message\ResponseException($response);
        }
     ...
     */
    private function onResponseRedirect(\_PhpScoper006a73f0e455\Psr\Http\Message\ResponseInterface $response, \_PhpScoper006a73f0e455\Psr\Http\Message\RequestInterface $request, \_PhpScoper006a73f0e455\React\Promise\Deferred $deferred)
    {
        // resolve location relative to last request URI
        $location = \_PhpScoper006a73f0e455\RingCentral\Psr7\Uri::resolve($request->getUri(), $response->getHeaderLine('Location'));
     ...
     */
    private function makeRedirectRequest(\_PhpScoper006a73f0e455\Psr\Http\Message\RequestInterface $request, \_PhpScoper006a73f0e455\Psr\Http\Message\UriInterface $location)
    {
        ...
        return new \_PhpScoper006a73f0e455\RingCentral\Psr7\Request($method, $location, $request->getHeaders());
    }
    private function progress($name, array $args = array())
    {
        ...
        foreach ($args as $arg) {
            echo ' ';
            if ($arg instanceof \_PhpScoper006a73f0e455\Psr\Http\Message\ResponseInterface) {
                echo 'HTTP/' . $arg->getProtocolVersion() . ' ' . $arg->getStatusCode() . ' ' . $arg->getReasonPhrase();
            } elseif ($arg instanceof \_PhpScoper006a73f0e455\Psr\Http\Message\RequestInterface) {
