...
final class MultipartParser
{
    ...
    public function __construct($uploadMaxFilesize = null, $maxFileUploads = null)
    {
        ...
        $this->uploadMaxFilesize = \_PhpScoper26e51eeacccf\React\Http\Io\IniUtil::iniSizeToBytes($uploadMaxFilesize);
        $this->maxFileUploads = $maxFileUploads === null ? \ini_get('file_uploads') === '' ? 0 : (int) \ini_get('max_file_uploads') : (int) $maxFileUploads;
    }
    public function parse(\_PhpScoper26e51eeacccf\Psr\Http\Message\ServerRequestInterface $request)
    {
    ...
    private function parseUploadedFile($filename, $contentType, $contents)
    {
        ...
        if ($size === 0 && $filename === '') {
            ...
            return new \_PhpScoper26e51eeacccf\React\Http\Io\UploadedFile(\_PhpScoper26e51eeacccf\RingCentral\Psr7\stream_for(), $size, \UPLOAD_ERR_NO_FILE, $filename, $contentType);
        }
        ...
        if ($size > $this->uploadMaxFilesize) {
            return new \_PhpScoper26e51eeacccf\React\Http\Io\UploadedFile(\_PhpScoper26e51eeacccf\RingCentral\Psr7\stream_for(), $size, \UPLOAD_ERR_INI_SIZE, $filename, $contentType);
        }
        // file exceeds MAX_FILE_SIZE value
        if ($this->maxFileSize !== null && $size > $this->maxFileSize) {
            return new \_PhpScoper26e51eeacccf\React\Http\Io\UploadedFile(\_PhpScoper26e51eeacccf\RingCentral\Psr7\stream_for(), $size, \UPLOAD_ERR_FORM_SIZE, $filename, $contentType);
        }
        return new \_PhpScoper26e51eeacccf\React\Http\Io\UploadedFile(\_PhpScoper26e51eeacccf\RingCentral\Psr7\stream_for($contents), $size, \UPLOAD_ERR_OK, $filename, $contentType);
    }
