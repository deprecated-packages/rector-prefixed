...
use _PhpScoperbd5d0c5f7638\React\Http\Client\Client as HttpClient;
...
class Sender
{
     ...
     * @return self
     */
    public static function createFromLoop(\_PhpScoperbd5d0c5f7638\React\EventLoop\LoopInterface $loop, \_PhpScoperbd5d0c5f7638\React\Socket\ConnectorInterface $connector = null)
    {
        return new self(new \_PhpScoperbd5d0c5f7638\React\Http\Client\Client($loop, $connector));
    }
    private $http;
    /**
     ...
     * @internal
     */
    public function __construct(\_PhpScoperbd5d0c5f7638\React\Http\Client\Client $http)
    {
        $this->http = $http;
    }
    /**
    ...
    public function send(\_PhpScoperbd5d0c5f7638\Psr\Http\Message\RequestInterface $request)
    {
        ...
        } elseif ($body instanceof \_PhpScoperbd5d0c5f7638\React\Stream\ReadableStreamInterface && $body->isReadable() && !$request->hasHeader('Content-Length')) {
        ...
        $deferred = new \_PhpScoperbd5d0c5f7638\React\Promise\Deferred(function ($_, $reject) use($requestStream) {
        ...
        $requestStream->on('response', function (\_PhpScoperbd5d0c5f7638\Psr\Http\Message\ResponseInterface $response, \_PhpScoperbd5d0c5f7638\React\Stream\ReadableStreamInterface $body) use($deferred, $request) {
            $length = null;
            ...
            if ($request->getMethod() === 'HEAD' || $code >= 100 && $code < 200 || $code == 204 || $code == 304) {
                ...
                $body = new \_PhpScoperbd5d0c5f7638\React\Http\Io\ChunkedDecoder($body);
            } elseif ($response->hasHeader('Content-Length')) {
                $length = (int) $response->getHeaderLine('Content-Length');
            }
            $deferred->resolve($response->withBody(new \_PhpScoperbd5d0c5f7638\React\Http\Io\ReadableBodyStream($body, $length)));
        });
