...
use _PhpScopera143bcca66cb\React\Http\Client\Client as HttpClient;
...
class Sender
{
     ...
     * @return self
     */
    public static function createFromLoop(\_PhpScopera143bcca66cb\React\EventLoop\LoopInterface $loop, \_PhpScopera143bcca66cb\React\Socket\ConnectorInterface $connector = null)
    {
        return new self(new \_PhpScopera143bcca66cb\React\Http\Client\Client($loop, $connector));
    }
    private $http;
    /**
     ...
     * @internal
     */
    public function __construct(\_PhpScopera143bcca66cb\React\Http\Client\Client $http)
    {
        $this->http = $http;
    }
    /**
    ...
    public function send(\_PhpScopera143bcca66cb\Psr\Http\Message\RequestInterface $request)
    {
        ...
        } elseif ($body instanceof \_PhpScopera143bcca66cb\React\Stream\ReadableStreamInterface && $body->isReadable() && !$request->hasHeader('Content-Length')) {
        ...
        $deferred = new \_PhpScopera143bcca66cb\React\Promise\Deferred(function ($_, $reject) use($requestStream) {
        ...
        $requestStream->on('response', function (\_PhpScopera143bcca66cb\Psr\Http\Message\ResponseInterface $response, \_PhpScopera143bcca66cb\React\Stream\ReadableStreamInterface $body) use($deferred, $request) {
            $length = null;
            ...
            if ($request->getMethod() === 'HEAD' || $code >= 100 && $code < 200 || $code == 204 || $code == 304) {
                ...
                $body = new \_PhpScopera143bcca66cb\React\Http\Io\ChunkedDecoder($body);
            } elseif ($response->hasHeader('Content-Length')) {
                $length = (int) $response->getHeaderLine('Content-Length');
            }
            $deferred->resolve($response->withBody(new \_PhpScopera143bcca66cb\React\Http\Io\ReadableBodyStream($body, $length)));
        });
