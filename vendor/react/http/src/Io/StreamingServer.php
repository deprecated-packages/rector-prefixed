...
use _PhpScoper88fe6e0ad041\React\Http\Message\Response;
use _PhpScoper88fe6e0ad041\React\Http\Message\ServerRequest;
...
use _PhpScoper88fe6e0ad041\React\Socket\ServerInterface;
 ...
 * @internal
 */
final class StreamingServer extends \_PhpScoper88fe6e0ad041\Evenement\EventEmitter
{
    private $callback;
    private $parser;
    private $loop;
     ...
     * @see self::listen()
     */
    public function __construct(\_PhpScoper88fe6e0ad041\React\EventLoop\LoopInterface $loop, $requestHandler)
    {
        ...
        $this->parser = new \_PhpScoper88fe6e0ad041\React\Http\Io\RequestHeaderParser();
        $that = $this;
        $this->parser->on('headers', function (\_PhpScoper88fe6e0ad041\Psr\Http\Message\ServerRequestInterface $request, \_PhpScoper88fe6e0ad041\React\Socket\ConnectionInterface $conn) use($that) {
            $that->handleRequest($conn, $request);
        });
        $this->parser->on('error', function (\Exception $e, \_PhpScoper88fe6e0ad041\React\Socket\ConnectionInterface $conn) use($that) {
            $that->emit('error', array($e));
            // parsing failed => assume dummy request and send appropriate error
            $that->writeError($conn, $e->getCode() !== 0 ? $e->getCode() : 400, new \_PhpScoper88fe6e0ad041\React\Http\Message\ServerRequest('GET', '/'));
        });
    }
    /**
     ...
     */
    public function listen(\_PhpScoper88fe6e0ad041\React\Socket\ServerInterface $socket)
    {
        $socket->on('connection', array($this->parser, 'handle'));
    }
    /** @internal */
    public function handleRequest(\_PhpScoper88fe6e0ad041\React\Socket\ConnectionInterface $conn, \_PhpScoper88fe6e0ad041\Psr\Http\Message\ServerRequestInterface $request)
    {
        ...
        try {
            ...
            $response = \_PhpScoper88fe6e0ad041\React\Promise\reject($error);
        ...
        if ($response instanceof \_PhpScoper88fe6e0ad041\React\Promise\CancellablePromiseInterface) {
            $conn->on('close', function () use($response) {
                $response->cancel();
            });
        }
        // happy path: response returned, handle and return immediately
        if ($response instanceof \_PhpScoper88fe6e0ad041\Psr\Http\Message\ResponseInterface) {
        ...
        if (!$response instanceof \_PhpScoper88fe6e0ad041\React\Promise\PromiseInterface) {
            $response = \_PhpScoper88fe6e0ad041\React\Promise\resolve($response);
        }
        $that = $this;
    ...
    /** @internal */
    public function writeError(\_PhpScoper88fe6e0ad041\React\Socket\ConnectionInterface $conn, $code, \_PhpScoper88fe6e0ad041\Psr\Http\Message\ServerRequestInterface $request)
    {
        $response = new \_PhpScoper88fe6e0ad041\React\Http\Message\Response($code, array('Content-Type' => 'text/plain'), 'Error ' . $code);
    ...
    /** @internal */
    public function handleResponse(\_PhpScoper88fe6e0ad041\React\Socket\ConnectionInterface $connection, \_PhpScoper88fe6e0ad041\Psr\Http\Message\ServerRequestInterface $request, \_PhpScoper88fe6e0ad041\Psr\Http\Message\ResponseInterface $response)
    {
        ...
        if (($code === 101 || $method === 'CONNECT' && $code >= 200 && $code < 300) && $body instanceof \_PhpScoper88fe6e0ad041\React\Http\Io\HttpBodyStream && $body->input instanceof \_PhpScoper88fe6e0ad041\React\Stream\WritableStreamInterface) {
        ...
        if (!$body instanceof \_PhpScoper88fe6e0ad041\React\Stream\ReadableStreamInterface || !$body->isReadable()) {
            ...
            if ($body instanceof \_PhpScoper88fe6e0ad041\React\Stream\ReadableStreamInterface && $chunked) {
                $body = "0\r\n\r\n";
            }
        ...
        $connection->write($headers . "\r\n");
        if ($chunked) {
            $body = new \_PhpScoper88fe6e0ad041\React\Http\Io\ChunkedEncoder($body);
        }
