...
use _PhpScoperabd03f0baf05\React\Http\Message\Response;
use _PhpScoperabd03f0baf05\React\Http\Message\ServerRequest;
...
use _PhpScoperabd03f0baf05\React\Socket\ServerInterface;
 ...
 * @internal
 */
final class StreamingServer extends \_PhpScoperabd03f0baf05\Evenement\EventEmitter
{
    private $callback;
    private $parser;
    private $loop;
     ...
     * @see self::listen()
     */
    public function __construct(\_PhpScoperabd03f0baf05\React\EventLoop\LoopInterface $loop, $requestHandler)
    {
        ...
        $this->parser = new \_PhpScoperabd03f0baf05\React\Http\Io\RequestHeaderParser();
        $that = $this;
        $this->parser->on('headers', function (\_PhpScoperabd03f0baf05\Psr\Http\Message\ServerRequestInterface $request, \_PhpScoperabd03f0baf05\React\Socket\ConnectionInterface $conn) use($that) {
            $that->handleRequest($conn, $request);
        });
        $this->parser->on('error', function (\Exception $e, \_PhpScoperabd03f0baf05\React\Socket\ConnectionInterface $conn) use($that) {
            $that->emit('error', array($e));
            // parsing failed => assume dummy request and send appropriate error
            $that->writeError($conn, $e->getCode() !== 0 ? $e->getCode() : 400, new \_PhpScoperabd03f0baf05\React\Http\Message\ServerRequest('GET', '/'));
        });
    }
    /**
     ...
     */
    public function listen(\_PhpScoperabd03f0baf05\React\Socket\ServerInterface $socket)
    {
        $socket->on('connection', array($this->parser, 'handle'));
    }
    /** @internal */
    public function handleRequest(\_PhpScoperabd03f0baf05\React\Socket\ConnectionInterface $conn, \_PhpScoperabd03f0baf05\Psr\Http\Message\ServerRequestInterface $request)
    {
        ...
        try {
            ...
            $response = \_PhpScoperabd03f0baf05\React\Promise\reject($error);
        ...
        if ($response instanceof \_PhpScoperabd03f0baf05\React\Promise\CancellablePromiseInterface) {
            $conn->on('close', function () use($response) {
                $response->cancel();
            });
        }
        // happy path: response returned, handle and return immediately
        if ($response instanceof \_PhpScoperabd03f0baf05\Psr\Http\Message\ResponseInterface) {
        ...
        if (!$response instanceof \_PhpScoperabd03f0baf05\React\Promise\PromiseInterface) {
            $response = \_PhpScoperabd03f0baf05\React\Promise\resolve($response);
        }
        $that = $this;
    ...
    /** @internal */
    public function writeError(\_PhpScoperabd03f0baf05\React\Socket\ConnectionInterface $conn, $code, \_PhpScoperabd03f0baf05\Psr\Http\Message\ServerRequestInterface $request)
    {
        $response = new \_PhpScoperabd03f0baf05\React\Http\Message\Response($code, array('Content-Type' => 'text/plain'), 'Error ' . $code);
    ...
    /** @internal */
    public function handleResponse(\_PhpScoperabd03f0baf05\React\Socket\ConnectionInterface $connection, \_PhpScoperabd03f0baf05\Psr\Http\Message\ServerRequestInterface $request, \_PhpScoperabd03f0baf05\Psr\Http\Message\ResponseInterface $response)
    {
        ...
        if (($code === 101 || $method === 'CONNECT' && $code >= 200 && $code < 300) && $body instanceof \_PhpScoperabd03f0baf05\React\Http\Io\HttpBodyStream && $body->input instanceof \_PhpScoperabd03f0baf05\React\Stream\WritableStreamInterface) {
        ...
        if (!$body instanceof \_PhpScoperabd03f0baf05\React\Stream\ReadableStreamInterface || !$body->isReadable()) {
            ...
            if ($body instanceof \_PhpScoperabd03f0baf05\React\Stream\ReadableStreamInterface && $chunked) {
                $body = "0\r\n\r\n";
            }
        ...
        $connection->write($headers . "\r\n");
        if ($chunked) {
            $body = new \_PhpScoperabd03f0baf05\React\Http\Io\ChunkedEncoder($body);
        }
