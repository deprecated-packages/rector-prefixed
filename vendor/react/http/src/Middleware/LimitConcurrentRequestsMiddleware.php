...
use _PhpScoperabd03f0baf05\React\Http\Io\PauseBufferStream;
...
final class LimitConcurrentRequestsMiddleware
{
    ...
    public function __invoke(\_PhpScoperabd03f0baf05\Psr\Http\Message\ServerRequestInterface $request, $next)
    {
        ...
        if ($this->pending < $this->limit) {
            ...
            return $this->await(\_PhpScoperabd03f0baf05\React\Promise\resolve($response));
        }
        ...
        if ($body instanceof \_PhpScoperabd03f0baf05\React\Stream\ReadableStreamInterface) {
            ...
            $size = $body->getSize();
            $body = new \_PhpScoperabd03f0baf05\React\Http\Io\PauseBufferStream($body);
            $body->pauseImplicit();
            // replace with buffering body to ensure any readable events will be buffered
            $request = $request->withBody(new \_PhpScoperabd03f0baf05\React\Http\Io\HttpBodyStream($body, $size));
        }
        ...
        \end($queue);
        $id = \key($queue);
        $deferred = new \_PhpScoperabd03f0baf05\React\Promise\Deferred(function ($_, $reject) use(&$queue, $id) {
        ...
        return $deferred->promise()->then(function () use($request, $next, $body, &$pending, $that) {
            ...
            if ($body instanceof \_PhpScoperabd03f0baf05\React\Http\Io\PauseBufferStream) {
                $body->resumeImplicit();
            }
            // if the next handler returns a pending promise, we have to
            // await its resolution before invoking next queued request
            return $that->await(\_PhpScoperabd03f0baf05\React\Promise\resolve($response));
        });
    }
    /**
     * @internal
     ...
     */
    public function await(\_PhpScoperabd03f0baf05\React\Promise\PromiseInterface $promise)
    {
        $that = $this;
        return $promise->then(function ($response) use($that) {
            ...
            $that->processQueue();
            return \_PhpScoperabd03f0baf05\React\Promise\reject($error);
        });
    }
    /**
     * @internal
