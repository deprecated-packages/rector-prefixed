...
final class RequestBodyBufferMiddleware
{
    ...
    public function __construct($sizeLimit = null)
    {
        ...
        $this->sizeLimit = \_PhpScoper26e51eeacccf\React\Http\Io\IniUtil::iniSizeToBytes($sizeLimit);
    }
    public function __invoke(\_PhpScoper26e51eeacccf\Psr\Http\Message\ServerRequestInterface $request, $stack)
    {
        $body = $request->getBody();
        $size = $body->getSize();
        // happy path: skip if body is known to be empty (or is already buffered)
        if ($size === 0 || !$body instanceof \_PhpScoper26e51eeacccf\React\Stream\ReadableStreamInterface) {
            // replace with empty body if body is streaming (or buffered size exceeds limit)
            if ($body instanceof \_PhpScoper26e51eeacccf\React\Stream\ReadableStreamInterface || $size > $this->sizeLimit) {
                $request = $request->withBody(new \_PhpScoper26e51eeacccf\RingCentral\Psr7\BufferStream(0));
            }
        ...
        return \_PhpScoper26e51eeacccf\React\Promise\Stream\buffer($body, $sizeLimit)->then(function ($buffer) use($request, $stack) {
            $stream = new \_PhpScoper26e51eeacccf\RingCentral\Psr7\BufferStream(\strlen($buffer));
            $stream->write($buffer);
            ...
            if ($error instanceof \OverflowException) {
                return \_PhpScoper26e51eeacccf\React\Promise\Stream\first($body, 'close')->then(function () use($stack, $request) {
                    return $stack($request);
                });
            }
            throw $error;
