 ...
 */
class UnwrapWritableStream extends \_PhpScoperabd03f0baf05\Evenement\EventEmitter implements \_PhpScoperabd03f0baf05\React\Stream\WritableStreamInterface
{
    private $promise;
    private $stream;
    ...
    public function __construct(\_PhpScoperabd03f0baf05\React\Promise\PromiseInterface $promise)
    {
        ...
        $this->promise = $promise->then(function ($stream) {
            if (!$stream instanceof \_PhpScoperabd03f0baf05\React\Stream\WritableStreamInterface) {
                throw new \InvalidArgumentException('Not a writable stream');
            }
            return $stream;
        })->then(function (\_PhpScoperabd03f0baf05\React\Stream\WritableStreamInterface $stream) use($out, &$store, &$buffer, &$ending, &$closed) {
