 ...
 */
final class HostsFileExecutor implements \_PhpScoperbd5d0c5f7638\React\Dns\Query\ExecutorInterface
{
    private $hosts;
    private $fallback;
    public function __construct(\_PhpScoperbd5d0c5f7638\React\Dns\Config\HostsFile $hosts, \_PhpScoperbd5d0c5f7638\React\Dns\Query\ExecutorInterface $fallback)
    {
        $this->hosts = $hosts;
    ...
    {
        if ($query->class === \_PhpScoperbd5d0c5f7638\React\Dns\Model\Message::CLASS_IN && ($query->type === \_PhpScoperbd5d0c5f7638\React\Dns\Model\Message::TYPE_A || $query->type === \_PhpScoperbd5d0c5f7638\React\Dns\Model\Message::TYPE_AAAA)) {
            // forward lookup for type A or AAAA
            $records = array();
            $expectsColon = $query->type === \_PhpScoperbd5d0c5f7638\React\Dns\Model\Message::TYPE_AAAA;
            foreach ($this->hosts->getIpsForHost($query->name) as $ip) {
                ...
                if ((\strpos($ip, ':') !== \false) === $expectsColon) {
                    $records[] = new \_PhpScoperbd5d0c5f7638\React\Dns\Model\Record($query->name, $query->type, $query->class, 0, $ip);
                }
            }
            if ($records) {
                return \_PhpScoperbd5d0c5f7638\React\Promise\resolve(\_PhpScoperbd5d0c5f7638\React\Dns\Model\Message::createResponseWithAnswersForQuery($query, $records));
            }
        } elseif ($query->class === \_PhpScoperbd5d0c5f7638\React\Dns\Model\Message::CLASS_IN && $query->type === \_PhpScoperbd5d0c5f7638\React\Dns\Model\Message::TYPE_PTR) {
            ...
            if ($ip !== null) {
                ...
                foreach ($this->hosts->getHostsForIp($ip) as $host) {
                    $records[] = new \_PhpScoperbd5d0c5f7638\React\Dns\Model\Record($query->name, $query->type, $query->class, 0, $host);
                }
                if ($records) {
                    return \_PhpScoperbd5d0c5f7638\React\Promise\resolve(\_PhpScoperbd5d0c5f7638\React\Dns\Model\Message::createResponseWithAnswersForQuery($query, $records));
                }
