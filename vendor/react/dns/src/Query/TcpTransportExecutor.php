 ...
 */
class TcpTransportExecutor implements \_PhpScoper88fe6e0ad041\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
    ...
    public function query(\_PhpScoper88fe6e0ad041\React\Dns\Query\Query $query)
    {
        ...
        if ($length > 0xffff) {
            return \_PhpScoper88fe6e0ad041\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->name . ' failed: Query too large for TCP transport'));
        }
        ...
        $names =& $this->names;
        $that = $this;
        $deferred = new \_PhpScoper88fe6e0ad041\React\Promise\Deferred(function () use($that, &$names, $request) {
            // remove from list of pending names, but remember pending query
            $name = $names[$request->id];
            unset($names[$request->id]);
            $that->checkIdle();
            throw new \_PhpScoper88fe6e0ad041\React\Dns\Query\CancellationException('DNS query for ' . $name . ' has been cancelled');
        });
