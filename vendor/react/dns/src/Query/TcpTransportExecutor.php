 ...
 */
class TcpTransportExecutor implements \_PhpScopera143bcca66cb\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
    ...
    public function query(\_PhpScopera143bcca66cb\React\Dns\Query\Query $query)
    {
        ...
        if ($length > 0xffff) {
            return \_PhpScopera143bcca66cb\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->name . ' failed: Query too large for TCP transport'));
        }
        ...
        $names =& $this->names;
        $that = $this;
        $deferred = new \_PhpScopera143bcca66cb\React\Promise\Deferred(function () use($that, &$names, $request) {
            // remove from list of pending names, but remember pending query
            $name = $names[$request->id];
            unset($names[$request->id]);
            $that->checkIdle();
            throw new \_PhpScopera143bcca66cb\React\Dns\Query\CancellationException('DNS query for ' . $name . ' has been cancelled');
        });
