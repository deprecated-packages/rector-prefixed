...
function timeout(\_PhpScoperabd03f0baf05\React\Promise\PromiseInterface $promise, $time, \_PhpScoperabd03f0baf05\React\EventLoop\LoopInterface $loop)
{
    // cancelling this promise will only try to cancel the input promise,
    // thus leaving responsibility to the input promise.
    $canceller = null;
    if ($promise instanceof \_PhpScoperabd03f0baf05\React\Promise\CancellablePromiseInterface || !\interface_exists('_PhpScoperabd03f0baf05\\React\\Promise\\CancellablePromiseInterface') && \method_exists($promise, 'cancel')) {
    ...
    return new \_PhpScoperabd03f0baf05\React\Promise\Promise(function ($resolve, $reject) use($loop, $time, $promise) {
        $timer = null;
        ...
        $timer = $loop->addTimer($time, function () use($time, &$promise, $reject) {
            $reject(new \_PhpScoperabd03f0baf05\React\Promise\Timer\TimeoutException($time, 'Timed out after ' . $time . ' seconds'));
...
function resolve($time, \_PhpScoperabd03f0baf05\React\EventLoop\LoopInterface $loop)
{
    return new \_PhpScoperabd03f0baf05\React\Promise\Promise(function ($resolve) use($loop, $time, &$timer) {
...
function reject($time, \_PhpScoperabd03f0baf05\React\EventLoop\LoopInterface $loop)
{
    return resolve($time, $loop)->then(function ($time) {
        throw new \_PhpScoperabd03f0baf05\React\Promise\Timer\TimeoutException($time, 'Timer expired after ' . $time . ' seconds');
    });
}
