 ...
 */
final class ExtUvLoop implements \_PhpScoperabd03f0baf05\React\EventLoop\LoopInterface
{
    private $uv;
    ...
    public function addSignal($signal, $listener)
    {
        ...
        if (!isset($this->signalEvents[$signal])) {
            ...
            $this->signalEvents[$signal] = \_PhpScoperabd03f0baf05\uv_signal_init($this->uv);
            \_PhpScoperabd03f0baf05\uv_signal_start($this->signalEvents[$signal], function () use($signals, $signal) {
                $signals->call($signal);
            }, $signal);
        }
    }
    ...
    {
        if (!isset($this->streamEvents[(int) $stream])) {
            $this->streamEvents[(int) $stream] = \_PhpScoperabd03f0baf05\uv_poll_init_socket($this->uv, $stream);
        }
