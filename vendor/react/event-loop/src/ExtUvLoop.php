 ...
 */
final class ExtUvLoop implements \_PhpScoper88fe6e0ad041\React\EventLoop\LoopInterface
{
    private $uv;
    ...
    public function addSignal($signal, $listener)
    {
        ...
        if (!isset($this->signalEvents[$signal])) {
            ...
            $this->signalEvents[$signal] = \_PhpScoper88fe6e0ad041\uv_signal_init($this->uv);
            \_PhpScoper88fe6e0ad041\uv_signal_start($this->signalEvents[$signal], function () use($signals, $signal) {
                $signals->call($signal);
            }, $signal);
        }
    }
    ...
    {
        if (!isset($this->streamEvents[(int) $stream])) {
            $this->streamEvents[(int) $stream] = \_PhpScoper88fe6e0ad041\uv_poll_init_socket($this->uv, $stream);
        }
