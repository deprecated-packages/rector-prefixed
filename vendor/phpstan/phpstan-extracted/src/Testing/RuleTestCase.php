...
use _PhpScoperb75b35f52b74\PHPStan\Dependency\ExportedNodeResolver;
...
use _PhpScoperb75b35f52b74\PHPStan\Reflection\ReflectionProvider\DirectReflectionProviderProvider;
 ...
 */
abstract class RuleTestCase extends \_PhpScoperb75b35f52b74\PHPStan\Testing\TestCase
{
     ...
     */
    protected abstract function getRule() : \_PhpScoperb75b35f52b74\PHPStan\Rules\Rule;
    protected function getTypeSpecifier() : \_PhpScoperb75b35f52b74\PHPStan\Analyser\TypeSpecifier
    {
        return $this->createTypeSpecifier(new \_PhpScoperb75b35f52b74\PhpParser\PrettyPrinter\Standard(), $this->createReflectionProvider(), $this->getMethodTypeSpecifyingExtensions(), $this->getStaticMethodTypeSpecifyingExtensions());
    }
    private function getAnalyser() : \_PhpScoperb75b35f52b74\PHPStan\Analyser\Analyser
    {
        if ($this->analyser === null) {
            $registry = new \_PhpScoperb75b35f52b74\PHPStan\Rules\Registry([$this->getRule()]);
            ...
            $anonymousClassNameHelper = new \_PhpScoperb75b35f52b74\PHPStan\Broker\AnonymousClassNameHelper($fileHelper, $relativePathHelper);
            $fileTypeMapper = new \_PhpScoperb75b35f52b74\PHPStan\Type\FileTypeMapper(new \_PhpScoperb75b35f52b74\PHPStan\Reflection\ReflectionProvider\DirectReflectionProviderProvider($broker), $this->getParser(), self::getContainer()->getByType(\_PhpScoperb75b35f52b74\PHPStan\PhpDoc\PhpDocStringResolver::class), self::getContainer()->getByType(\_PhpScoperb75b35f52b74\PHPStan\PhpDoc\PhpDocNodeResolver::class), $this->createMock(\_PhpScoperb75b35f52b74\PHPStan\Cache\Cache::class), $anonymousClassNameHelper);
            $phpDocInheritanceResolver = new \_PhpScoperb75b35f52b74\PHPStan\PhpDoc\PhpDocInheritanceResolver($fileTypeMapper);
            $nodeScopeResolver = new \_PhpScoperb75b35f52b74\PHPStan\Analyser\NodeScopeResolver($broker, self::getReflectors()[0], $this->getClassReflectionExtensionRegistryProvider(), $this->getParser(), $fileTypeMapper, self::getContainer()->getByType(\_PhpScoperb75b35f52b74\PHPStan\Php\PhpVersion::class), $phpDocInheritanceResolver, $fileHelper, $typeSpecifier, $this->shouldPolluteScopeWithLoopInitialAssignments(), $this->shouldPolluteCatchScopeWithTryAssignments(), $this->shouldPolluteScopeWithAlwaysIterableForeach(), [], []);
            $fileAnalyser = new \_PhpScoperb75b35f52b74\PHPStan\Analyser\FileAnalyser($this->createScopeFactory($broker, $typeSpecifier), $nodeScopeResolver, $this->getParser(), new \_PhpScoperb75b35f52b74\PHPStan\Dependency\DependencyResolver($fileHelper, $broker, new \_PhpScoperb75b35f52b74\PHPStan\Dependency\ExportedNodeResolver($fileTypeMapper, $printer)), \true);
            $this->analyser = new \_PhpScoperb75b35f52b74\PHPStan\Analyser\Analyser($fileAnalyser, $registry, $nodeScopeResolver, 50);
        }
    ...
    public function analyse(array $files, array $expectedErrors) : void
    {
        ...
        $actualErrors = \array_map(static function (\_PhpScoperb75b35f52b74\PHPStan\Analyser\Error $error) use($strictlyTypedSprintf) : string {
            $line = $error->getLine();
            if ($line === null) {
