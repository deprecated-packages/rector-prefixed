...
use _PhpScoper2a4e7ab1ecbc\PHPStan\Command\IgnoredRegexValidator;
...
class IgnoredErrorHelper
{
     ...
     */
    public function __construct(\_PhpScoper2a4e7ab1ecbc\PHPStan\Command\IgnoredRegexValidator $ignoredRegexValidator, \_PhpScoper2a4e7ab1ecbc\PHPStan\File\FileHelper $fileHelper, array $ignoreErrors, bool $reportUnmatchedIgnoredErrors)
    {
    ...
    public function initialize() : \_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\IgnoredErrorHelperResult
    {
        $otherIgnoreErrors = [];
        $ignoreErrorsByFile = [];
        $warnings = [];
            ...
            try {
                if (\is_array($ignoreError)) {
                    if (!isset($ignoreError['message'])) {
                        $errors[] = \sprintf('Ignored error %s is missing a message.', \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\Utils\Json::encode($ignoreError));
                        continue;
                    }
                    if (!isset($ignoreError['path'])) {
                        if (!isset($ignoreError['paths'])) {
                            $errors[] = \sprintf('Ignored error %s is missing a path.', \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\Utils\Json::encode($ignoreError));
                        }
                    ...
                    \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\Utils\Strings::match('', $ignoreMessage);
        ...
        return new \_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\IgnoredErrorHelperResult($this->fileHelper, $errors, $warnings, $otherIgnoreErrors, $ignoreErrorsByFile, $this->ignoreErrors, $this->reportUnmatchedIgnoredErrors);
    }
    /**
     * @param string $regex
