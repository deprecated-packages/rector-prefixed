...
use _PhpScoper0a2ac50786fa\PHPStan\Dependency\ExportedNodeFetcher;
...
class ResultCacheManager
{
     ...
     */
    public function __construct(\_PhpScoper0a2ac50786fa\PHPStan\Dependency\ExportedNodeFetcher $exportedNodeFetcher, string $cacheFilePath, string $tempResultCachePath, array $analysedPaths, array $composerAutoloaderProjectPaths, array $stubFiles, string $usedLevel, ?string $cliAutoloadFile, array $fileReplacements)
    {
     ...
     * @return ResultCache
     */
    public function restore(array $allAnalysedFiles, bool $debug, bool $onlyFiles, ?array $projectConfigArray, \_PhpScoper0a2ac50786fa\PHPStan\Command\Output $output, ?string $resultCacheName = null) : \_PhpScoper0a2ac50786fa\PHPStan\Analyser\ResultCache\ResultCache
    {
        if ($debug) {
            if ($output->isDebug()) {
                $output->writeLineFormatted('Result cache not used because of debug mode.');
            }
            return new \_PhpScoper0a2ac50786fa\PHPStan\Analyser\ResultCache\ResultCache($allAnalysedFiles, \true, \time(), [], [], []);
        }
        if ($onlyFiles) {
            if ($output->isDebug()) {
        ...
        return new \_PhpScoper0a2ac50786fa\PHPStan\Analyser\ResultCache\ResultCache(\array_unique($filesToAnalyse), \false, $data['lastFullAnalysisTime'], $filteredErrors, $invertedDependenciesToReturn, $filteredExportedNodes);
    }
    /**
     ...
     */
    public function process(\_PhpScoper0a2ac50786fa\PHPStan\Analyser\AnalyserResult $analyserResult, \_PhpScoper0a2ac50786fa\PHPStan\Analyser\ResultCache\ResultCache $resultCache, \_PhpScoper0a2ac50786fa\PHPStan\Command\Output $output, bool $onlyFiles, ?array $projectConfigArray, $save) : \_PhpScoper0a2ac50786fa\PHPStan\Analyser\ResultCache\ResultCacheProcessResult
    {
        ...
        if ($resultCache->isFullAnalysis()) {
            ...
            return new \_PhpScoper0a2ac50786fa\PHPStan\Analyser\ResultCache\ResultCacheProcessResult($analyserResult, $saved);
        }
        ...
        return new \_PhpScoper0a2ac50786fa\PHPStan\Analyser\ResultCache\ResultCacheProcessResult(new \_PhpScoper0a2ac50786fa\PHPStan\Analyser\AnalyserResult($flatErrors, $internalErrors, $dependencies, $exportedNodes, $analyserResult->hasReachedInternalErrorsCountLimit()), $saved);
    }
    /**
     * @param ResultCache $resultCache
     ...
     */
    private function mergeErrors(\_PhpScoper0a2ac50786fa\PHPStan\Analyser\ResultCache\ResultCache $resultCache, array $freshErrorsByFile) : array
    {
     ...
     */
    private function mergeDependencies(\_PhpScoper0a2ac50786fa\PHPStan\Analyser\ResultCache\ResultCache $resultCache, ?array $freshDependencies) : ?array
    {
     ...
     */
    private function mergeExportedNodes(\_PhpScoper0a2ac50786fa\PHPStan\Analyser\ResultCache\ResultCache $resultCache, array $freshExportedNodes) : array
    {
...
php;
        ...
        \_PhpScoper0a2ac50786fa\PHPStan\File\FileWriter::write($file, \sprintf($template, \var_export($lastFullAnalysisTime, \true), \var_export($this->getMeta($projectConfigArray), \true), \var_export($errors, \true), \var_export($invertedDependencies, \true), \var_export($exportedNodes, \true)));
    }
    /**
     * @param mixed[]|null $projectConfigArray
     * @return mixed[]
    ...
    private function getMeta(?array $projectConfigArray) : array
    {
        ...
        if ($projectConfigArray !== null) {
            ...
            $projectConfigArray = \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Nette\Neon\Neon::encode($projectConfigArray);
        }
    ...
    private function getFileHash(string $path) : string
    {
        ...
        $contents = \_PhpScoper0a2ac50786fa\PHPStan\File\FileReader::read($path);
