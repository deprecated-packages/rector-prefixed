...
use _PhpScoperb75b35f52b74\PHPStan\Dependency\ExportedNodeFetcher;
...
class ResultCacheManager
{
     ...
     */
    public function __construct(\_PhpScoperb75b35f52b74\PHPStan\Dependency\ExportedNodeFetcher $exportedNodeFetcher, string $cacheFilePath, string $tempResultCachePath, array $analysedPaths, array $composerAutoloaderProjectPaths, array $stubFiles, string $usedLevel, ?string $cliAutoloadFile, array $fileReplacements)
    {
     ...
     * @return ResultCache
     */
    public function restore(array $allAnalysedFiles, bool $debug, bool $onlyFiles, ?array $projectConfigArray, \_PhpScoperb75b35f52b74\PHPStan\Command\Output $output, ?string $resultCacheName = null) : \_PhpScoperb75b35f52b74\PHPStan\Analyser\ResultCache\ResultCache
    {
        if ($debug) {
            if ($output->isDebug()) {
                $output->writeLineFormatted('Result cache not used because of debug mode.');
            }
            return new \_PhpScoperb75b35f52b74\PHPStan\Analyser\ResultCache\ResultCache($allAnalysedFiles, \true, \time(), [], [], []);
        }
        if ($onlyFiles) {
            if ($output->isDebug()) {
        ...
        return new \_PhpScoperb75b35f52b74\PHPStan\Analyser\ResultCache\ResultCache(\array_unique($filesToAnalyse), \false, $data['lastFullAnalysisTime'], $filteredErrors, $invertedDependenciesToReturn, $filteredExportedNodes);
    }
    /**
     ...
     */
    public function process(\_PhpScoperb75b35f52b74\PHPStan\Analyser\AnalyserResult $analyserResult, \_PhpScoperb75b35f52b74\PHPStan\Analyser\ResultCache\ResultCache $resultCache, \_PhpScoperb75b35f52b74\PHPStan\Command\Output $output, bool $onlyFiles, ?array $projectConfigArray, $save) : \_PhpScoperb75b35f52b74\PHPStan\Analyser\ResultCache\ResultCacheProcessResult
    {
        ...
        if ($resultCache->isFullAnalysis()) {
            ...
            return new \_PhpScoperb75b35f52b74\PHPStan\Analyser\ResultCache\ResultCacheProcessResult($analyserResult, $saved);
        }
        ...
        return new \_PhpScoperb75b35f52b74\PHPStan\Analyser\ResultCache\ResultCacheProcessResult(new \_PhpScoperb75b35f52b74\PHPStan\Analyser\AnalyserResult($flatErrors, $internalErrors, $dependencies, $exportedNodes, $analyserResult->hasReachedInternalErrorsCountLimit()), $saved);
    }
    /**
     * @param ResultCache $resultCache
     ...
     */
    private function mergeErrors(\_PhpScoperb75b35f52b74\PHPStan\Analyser\ResultCache\ResultCache $resultCache, array $freshErrorsByFile) : array
    {
     ...
     */
    private function mergeDependencies(\_PhpScoperb75b35f52b74\PHPStan\Analyser\ResultCache\ResultCache $resultCache, ?array $freshDependencies) : ?array
    {
     ...
     */
    private function mergeExportedNodes(\_PhpScoperb75b35f52b74\PHPStan\Analyser\ResultCache\ResultCache $resultCache, array $freshExportedNodes) : array
    {
...
php;
        ...
        \_PhpScoperb75b35f52b74\PHPStan\File\FileWriter::write($file, \sprintf($template, \var_export($lastFullAnalysisTime, \true), \var_export($this->getMeta($projectConfigArray), \true), \var_export($errors, \true), \var_export($invertedDependencies, \true), \var_export($exportedNodes, \true)));
    }
    /**
     * @param mixed[]|null $projectConfigArray
     * @return mixed[]
    ...
    private function getMeta(?array $projectConfigArray) : array
    {
        ...
        if ($projectConfigArray !== null) {
            ...
            $projectConfigArray = \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Neon\Neon::encode($projectConfigArray);
        }
    ...
    private function getFileHash(string $path) : string
    {
        ...
        $contents = \_PhpScoperb75b35f52b74\PHPStan\File\FileReader::read($path);
