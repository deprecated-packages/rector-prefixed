...
use RectorPrefix20201227\PHPStan\Dependency\ExportedNodeFetcher;
...
class ResultCacheManager
{
     ...
     */
    public function __construct(\RectorPrefix20201227\PHPStan\Dependency\ExportedNodeFetcher $exportedNodeFetcher, string $cacheFilePath, string $tempResultCachePath, array $analysedPaths, array $composerAutoloaderProjectPaths, array $stubFiles, string $usedLevel, ?string $cliAutoloadFile, array $fileReplacements)
    {
     ...
     * @return ResultCache
     */
    public function restore(array $allAnalysedFiles, bool $debug, bool $onlyFiles, ?array $projectConfigArray, \RectorPrefix20201227\PHPStan\Command\Output $output, ?string $resultCacheName = null) : \RectorPrefix20201227\PHPStan\Analyser\ResultCache\ResultCache
    {
        if ($debug) {
            if ($output->isDebug()) {
                $output->writeLineFormatted('Result cache not used because of debug mode.');
            }
            return new \RectorPrefix20201227\PHPStan\Analyser\ResultCache\ResultCache($allAnalysedFiles, \true, \time(), [], [], []);
        }
        if ($onlyFiles) {
            if ($output->isDebug()) {
        ...
        return new \RectorPrefix20201227\PHPStan\Analyser\ResultCache\ResultCache(\array_unique($filesToAnalyse), \false, $data['lastFullAnalysisTime'], $filteredErrors, $invertedDependenciesToReturn, $filteredExportedNodes);
    }
    /**
     ...
     */
    public function process(\RectorPrefix20201227\PHPStan\Analyser\AnalyserResult $analyserResult, \RectorPrefix20201227\PHPStan\Analyser\ResultCache\ResultCache $resultCache, \RectorPrefix20201227\PHPStan\Command\Output $output, bool $onlyFiles, ?array $projectConfigArray, $save) : \RectorPrefix20201227\PHPStan\Analyser\ResultCache\ResultCacheProcessResult
    {
        ...
        if ($resultCache->isFullAnalysis()) {
            ...
            return new \RectorPrefix20201227\PHPStan\Analyser\ResultCache\ResultCacheProcessResult($analyserResult, $saved);
        }
        ...
        return new \RectorPrefix20201227\PHPStan\Analyser\ResultCache\ResultCacheProcessResult(new \RectorPrefix20201227\PHPStan\Analyser\AnalyserResult($flatErrors, $internalErrors, $dependencies, $exportedNodes, $analyserResult->hasReachedInternalErrorsCountLimit()), $saved);
    }
    /**
     * @param ResultCache $resultCache
     ...
     */
    private function mergeErrors(\RectorPrefix20201227\PHPStan\Analyser\ResultCache\ResultCache $resultCache, array $freshErrorsByFile) : array
    {
     ...
     */
    private function mergeDependencies(\RectorPrefix20201227\PHPStan\Analyser\ResultCache\ResultCache $resultCache, ?array $freshDependencies) : ?array
    {
     ...
     */
    private function mergeExportedNodes(\RectorPrefix20201227\PHPStan\Analyser\ResultCache\ResultCache $resultCache, array $freshExportedNodes) : array
    {
...
php;
        ...
        \RectorPrefix20201227\PHPStan\File\FileWriter::write($file, \sprintf($template, \var_export($lastFullAnalysisTime, \true), \var_export($this->getMeta($projectConfigArray), \true), \var_export($errors, \true), \var_export($invertedDependencies, \true), \var_export($exportedNodes, \true)));
    }
    /**
     * @param mixed[]|null $projectConfigArray
     * @return mixed[]
    ...
    private function getMeta(?array $projectConfigArray) : array
    {
        ...
        if ($projectConfigArray !== null) {
            ...
            $projectConfigArray = \RectorPrefix20201227\_HumbugBox221ad6f1b81f__UniqueRector\Nette\Neon\Neon::encode($projectConfigArray);
        }
    ...
    private function getFileHash(string $path) : string
    {
        ...
        $contents = \RectorPrefix20201227\PHPStan\File\FileReader::read($path);
