...
use _PhpScoper2a4e7ab1ecbc\PHPStan\Dependency\ExportedNodeFetcher;
...
class ResultCacheManager
{
     ...
     */
    public function __construct(\_PhpScoper2a4e7ab1ecbc\PHPStan\Dependency\ExportedNodeFetcher $exportedNodeFetcher, string $cacheFilePath, string $tempResultCachePath, array $analysedPaths, array $composerAutoloaderProjectPaths, array $stubFiles, string $usedLevel, ?string $cliAutoloadFile, array $fileReplacements)
    {
     ...
     * @return ResultCache
     */
    public function restore(array $allAnalysedFiles, bool $debug, bool $onlyFiles, ?array $projectConfigArray, \_PhpScoper2a4e7ab1ecbc\PHPStan\Command\Output $output, ?string $resultCacheName = null) : \_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\ResultCache\ResultCache
    {
        if ($debug) {
            if ($output->isDebug()) {
                $output->writeLineFormatted('Result cache not used because of debug mode.');
            }
            return new \_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\ResultCache\ResultCache($allAnalysedFiles, \true, \time(), [], [], []);
        }
        if ($onlyFiles) {
            if ($output->isDebug()) {
        ...
        return new \_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\ResultCache\ResultCache(\array_unique($filesToAnalyse), \false, $data['lastFullAnalysisTime'], $filteredErrors, $invertedDependenciesToReturn, $filteredExportedNodes);
    }
    /**
     ...
     */
    public function process(\_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\AnalyserResult $analyserResult, \_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\ResultCache\ResultCache $resultCache, \_PhpScoper2a4e7ab1ecbc\PHPStan\Command\Output $output, bool $onlyFiles, ?array $projectConfigArray, $save) : \_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\ResultCache\ResultCacheProcessResult
    {
        ...
        if ($resultCache->isFullAnalysis()) {
            ...
            return new \_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\ResultCache\ResultCacheProcessResult($analyserResult, $saved);
        }
        ...
        return new \_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\ResultCache\ResultCacheProcessResult(new \_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\AnalyserResult($flatErrors, $internalErrors, $dependencies, $exportedNodes, $analyserResult->hasReachedInternalErrorsCountLimit()), $saved);
    }
    /**
     * @param ResultCache $resultCache
     ...
     */
    private function mergeErrors(\_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\ResultCache\ResultCache $resultCache, array $freshErrorsByFile) : array
    {
     ...
     */
    private function mergeDependencies(\_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\ResultCache\ResultCache $resultCache, ?array $freshDependencies) : ?array
    {
     ...
     */
    private function mergeExportedNodes(\_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\ResultCache\ResultCache $resultCache, array $freshExportedNodes) : array
    {
...
php;
        ...
        \_PhpScoper2a4e7ab1ecbc\PHPStan\File\FileWriter::write($file, \sprintf($template, \var_export($lastFullAnalysisTime, \true), \var_export($this->getMeta($projectConfigArray), \true), \var_export($errors, \true), \var_export($invertedDependencies, \true), \var_export($exportedNodes, \true)));
    }
    /**
     * @param mixed[]|null $projectConfigArray
     * @return mixed[]
    ...
    private function getMeta(?array $projectConfigArray) : array
    {
        ...
        if ($projectConfigArray !== null) {
            ...
            $projectConfigArray = \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\Neon\Neon::encode($projectConfigArray);
        }
    ...
    private function getFileHash(string $path) : string
    {
        ...
        $contents = \_PhpScoper2a4e7ab1ecbc\PHPStan\File\FileReader::read($path);
