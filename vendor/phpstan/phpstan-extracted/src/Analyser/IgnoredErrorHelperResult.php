...
class IgnoredErrorHelperResult
{
     ...
     */
    public function __construct(\_PhpScoper0a2ac50786fa\PHPStan\File\FileHelper $fileHelper, array $errors, array $warnings, array $otherIgnoreErrors, array $ignoreErrorsByFile, array $ignoreErrors, bool $reportUnmatchedIgnoredErrors)
    {
    ...
    public function process(array $errors, bool $onlyFiles, array $analysedFiles, bool $hasInternalErrors) : array
    {
        ...
        $addErrors = [];
        $processIgnoreError = function (\_PhpScoper0a2ac50786fa\PHPStan\Analyser\Error $error, int $i, $ignore) use(&$unmatchedIgnoredErrors, &$addErrors) : bool {
            $shouldBeIgnored = \false;
            if (\is_string($ignore)) {
                $shouldBeIgnored = \_PhpScoper0a2ac50786fa\PHPStan\Analyser\IgnoredError::shouldIgnore($this->fileHelper, $error, $ignore, null);
                if ($shouldBeIgnored) {
                    unset($unmatchedIgnoredErrors[$i]);
                }
            } else {
                if (isset($ignore['path'])) {
                    $shouldBeIgnored = \_PhpScoper0a2ac50786fa\PHPStan\Analyser\IgnoredError::shouldIgnore($this->fileHelper, $error, $ignore['message'], $ignore['path']);
                    if ($shouldBeIgnored) {
                    ...
                    foreach ($ignore['paths'] as $j => $ignorePath) {
                        $shouldBeIgnored = \_PhpScoper0a2ac50786fa\PHPStan\Analyser\IgnoredError::shouldIgnore($this->fileHelper, $error, $ignore['message'], $ignorePath);
                        if (!$shouldBeIgnored) {
                            continue;
                        }
        ...
        $errors = \array_values(\array_filter($errors, function (\_PhpScoper0a2ac50786fa\PHPStan\Analyser\Error $error) use($processIgnoreError) : bool {
        ...
        foreach ($unmatchedIgnoredErrors as $unmatchedIgnoredError) {
            ...
            $addErrors[] = new \_PhpScoper0a2ac50786fa\PHPStan\Analyser\Error(\sprintf('Ignored error pattern %s is expected to occur %d %s, but occurred %d %s.', \_PhpScoper0a2ac50786fa\PHPStan\Analyser\IgnoredError::stringifyPattern($unmatchedIgnoredError), $unmatchedIgnoredError['count'], $unmatchedIgnoredError['count'] === 1 ? 'time' : 'times', $unmatchedIgnoredError['realCount'], $unmatchedIgnoredError['realCount'] === 1 ? 'time' : 'times'), $unmatchedIgnoredError['file'], $unmatchedIgnoredError['line'], \false);
        }
        ...
        if ($this->reportUnmatchedIgnoredErrors && !$hasInternalErrors) {
            foreach ($unmatchedIgnoredErrors as $unmatchedIgnoredError) {
                if (isset($unmatchedIgnoredError['count']) && isset($unmatchedIgnoredError['realCount']) && (isset($unmatchedIgnoredError['realPath']) || !$onlyFiles)) {
                    if ($unmatchedIgnoredError['realCount'] < $unmatchedIgnoredError['count']) {
                        $errors[] = new \_PhpScoper0a2ac50786fa\PHPStan\Analyser\Error(\sprintf('Ignored error pattern %s is expected to occur %d %s, but occurred only %d %s.', \_PhpScoper0a2ac50786fa\PHPStan\Analyser\IgnoredError::stringifyPattern($unmatchedIgnoredError), $unmatchedIgnoredError['count'], $unmatchedIgnoredError['count'] === 1 ? 'time' : 'times', $unmatchedIgnoredError['realCount'], $unmatchedIgnoredError['realCount'] === 1 ? 'time' : 'times'), $unmatchedIgnoredError['file'], $unmatchedIgnoredError['line'], \false);
                    }
                } elseif (isset($unmatchedIgnoredError['realPath'])) {
                    if (!\array_key_exists($unmatchedIgnoredError['realPath'], $analysedFilesKeys)) {
                        continue;
                    }
                    $errors[] = new \_PhpScoper0a2ac50786fa\PHPStan\Analyser\Error(\sprintf('Ignored error pattern %s was not matched in reported errors.', \_PhpScoper0a2ac50786fa\PHPStan\Analyser\IgnoredError::stringifyPattern($unmatchedIgnoredError)), $unmatchedIgnoredError['realPath'], null, \false);
                } elseif (!$onlyFiles) {
                    $errors[] = \sprintf('Ignored error pattern %s was not matched in reported errors.', \_PhpScoper0a2ac50786fa\PHPStan\Analyser\IgnoredError::stringifyPattern($unmatchedIgnoredError));
                }
            }
        }
        return $errors;
