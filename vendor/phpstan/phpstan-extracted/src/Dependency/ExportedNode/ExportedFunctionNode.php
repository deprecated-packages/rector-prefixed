...
class ExportedFunctionNode implements \_PhpScoper0a2ac50786fa\PHPStan\Dependency\ExportedNode, \JsonSerializable
{
    /** @var string */
    private $name;
     ...
     */
    public function __construct(string $name, ?\_PhpScoper0a2ac50786fa\PHPStan\Dependency\ExportedNode\ExportedPhpDocNode $phpDoc, bool $byRef, ?string $returnType, array $parameters)
    {
        $this->name = $name;
        $this->phpDoc = $phpDoc;
        $this->byRef = $byRef;
    ...
    {
        return new self($data['name'], $data['phpDoc'] !== null ? \_PhpScoper0a2ac50786fa\PHPStan\Dependency\ExportedNode\ExportedPhpDocNode::decode($data['phpDoc']['data']) : null, $data['byRef'], $data['returnType'], \array_map(static function (array $parameterData) : ExportedParameterNode {
            if ($parameterData['type'] !== \_PhpScoper0a2ac50786fa\PHPStan\Dependency\ExportedNode\ExportedParameterNode::class) {
                throw new \_PhpScoper0a2ac50786fa\PHPStan\ShouldNotHappenException();
            }
            return \_PhpScoper0a2ac50786fa\PHPStan\Dependency\ExportedNode\ExportedParameterNode::decode($parameterData['data']);
        }, $data['parameters']));
    }
}
