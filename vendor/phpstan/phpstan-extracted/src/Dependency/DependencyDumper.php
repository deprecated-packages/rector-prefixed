...
class DependencyDumper
{
    ...
    public function dumpDependencies(array $files, callable $countCallback, callable $progressCallback, ?array $analysedPaths) : array
    {
        ...
        foreach ($files as $file) {
            ...
            $fileDependencies = [];
            try {
                $this->nodeScopeResolver->processNodes($parserNodes, $this->scopeFactory->create(\_PhpScoper0a2ac50786fa\PHPStan\Analyser\ScopeContext::create($file)), function (\_PhpScoper0a2ac50786fa\PhpParser\Node $node, \_PhpScoper0a2ac50786fa\PHPStan\Analyser\Scope $scope) use($analysedFiles, &$fileDependencies) : void {
