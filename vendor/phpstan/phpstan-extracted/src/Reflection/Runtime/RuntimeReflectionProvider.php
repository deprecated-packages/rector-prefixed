...
namespace _PhpScoper2a4e7ab1ecbc\PHPStan\Reflection\Runtime;
...
class RuntimeReflectionProvider implements \_PhpScoper2a4e7ab1ecbc\PHPStan\Reflection\ReflectionProvider
{
     ...
     */
    private function getClassFromReflection(\ReflectionClass $reflectionClass, string $displayName, ?string $anonymousFilename) : \_PhpScoper2a4e7ab1ecbc\PHPStan\Reflection\ClassReflection
    {
    ...
    public function hasClass(string $className) : bool
    {
        ...
        \spl_autoload_register($autoloader = function (string $autoloadedClassName) use($className) : void {
            ...
            if ($autoloadedClassName !== $className && !$this->isExistsCheckCall()) {
                throw new \_PhpScoper2a4e7ab1ecbc\PHPStan\Broker\ClassAutoloadingException($autoloadedClassName);
            }
        });
        try {
            return $this->hasClassCache[$className] = \class_exists($className) || \interface_exists($className) || \trait_exists($className);
        } catch (\_PhpScoper2a4e7ab1ecbc\PHPStan\Broker\ClassAutoloadingException $e) {
            throw $e;
        } catch (\Throwable $t) {
            throw new \_PhpScoper2a4e7ab1ecbc\PHPStan\Broker\ClassAutoloadingException($className, $t);
        } finally {
    ...
    private function hasCustomFunction(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Name $nameNode, ?\_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\Scope $scope) : bool
    {
        $functionName = $this->resolveFunctionName($nameNode, $scope);
        if ($functionName === null) {
            return \false;
    ...
    private function getCustomFunction(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Name $nameNode, ?\_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\Scope $scope) : \_PhpScoper2a4e7ab1ecbc\PHPStan\Reflection\Php\PhpFunctionReflection
    {
        ...
        if (!\function_exists($functionName)) {
            throw new \_PhpScoper2a4e7ab1ecbc\PHPStan\Broker\FunctionNotFoundException($functionName);
        }
        $lowerCasedFunctionName = \strtolower($functionName);
        ...
        $functionReflection = $this->functionReflectionFactory->create($reflectionFunction, $templateTypeMap, \array_map(static function (\_PhpScoper2a4e7ab1ecbc\PHPStan\PhpDoc\Tag\ParamTag $paramTag) : Type {
    ...
    public function getConstant(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Name $nameNode, ?\_PhpScoper2a4e7ab1ecbc\PHPStan\Analyser\Scope $scope) : \_PhpScoper2a4e7ab1ecbc\PHPStan\Reflection\GlobalConstantReflection
    {
        ...
        return new \_PhpScoper2a4e7ab1ecbc\PHPStan\Reflection\Constant\RuntimeConstantReflection($constantName, \_PhpScoper2a4e7ab1ecbc\PHPStan\Type\ConstantTypeHelper::getTypeFromValue(\constant($constantName)), null);
    }
