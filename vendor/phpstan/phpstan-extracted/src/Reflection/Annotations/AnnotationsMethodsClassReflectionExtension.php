...
class AnnotationsMethodsClassReflectionExtension implements \_PhpScoper0a2ac50786fa\PHPStan\Reflection\MethodsClassReflectionExtension
{
     ...
     */
    private function createMethods(\_PhpScoper0a2ac50786fa\PHPStan\Reflection\ClassReflection $classReflection, \_PhpScoper0a2ac50786fa\PHPStan\Reflection\ClassReflection $declaringClass) : array
    {
        $methods = [];
        ...
        foreach ($methodTags as $methodName => $methodTag) {
            ...
            foreach ($methodTag->getParameters() as $parameterName => $parameterTag) {
                $parameters[] = new \_PhpScoper0a2ac50786fa\PHPStan\Reflection\Annotations\AnnotationsMethodParameterReflection($parameterName, $parameterTag->getType(), $parameterTag->passedByReference(), $parameterTag->isOptional(), $parameterTag->isVariadic(), $parameterTag->getDefaultValue());
            }
            $methods[$methodName] = new \_PhpScoper0a2ac50786fa\PHPStan\Reflection\Annotations\AnnotationMethodReflection($methodName, $declaringClass, \_PhpScoper0a2ac50786fa\PHPStan\Type\Generic\TemplateTypeHelper::resolveTemplateTypes($methodTag->getReturnType(), $classReflection->getActiveTemplateTypeMap()), $parameters, $methodTag->isStatic(), $this->detectMethodVariadic($parameters));
        }
        return $methods;
    }
    /**
