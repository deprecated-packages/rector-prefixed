...
class Php8SignatureMapProvider implements \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\SignatureMapProvider
{
    ...
    private function findMethodNode(string $className, string $methodName) : ?array
    {
        ...
        $stubFile = self::DIRECTORY . '/' . \_PhpScoper0a2ac50786fa\PHPStan\Php8StubsMap::CLASSES[$lowerClassName];
        $nodes = $this->fileNodesFetcher->fetchNodes($stubFile);
        $classes = $nodes->getClassNodes();
        if (\count($classes) !== 1) {
            throw new \_PhpScoper0a2ac50786fa\PHPStan\ShouldNotHappenException(\sprintf('Class %s stub not found in %s.', $className, $stubFile));
        }
    ...
    public function hasFunctionSignature(string $name, int $variant = 0) : bool
    {
        ...
        if (!\array_key_exists($lowerName, \_PhpScoper0a2ac50786fa\PHPStan\Php8StubsMap::FUNCTIONS)) {
    ...
    public function getMethodSignature(string $className, string $methodName, ?\ReflectionMethod $reflectionMethod, int $variant = 0) : \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\FunctionSignature
    {
        $lowerClassName = \strtolower($className);
        if (!\array_key_exists($lowerClassName, \_PhpScoper0a2ac50786fa\PHPStan\Php8StubsMap::CLASSES)) {
    ...
    public function getFunctionSignature(string $functionName, ?string $className, int $variant = 0) : \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\FunctionSignature
    {
        ...
        $stubFile = self::DIRECTORY . '/' . \_PhpScoper0a2ac50786fa\PHPStan\Php8StubsMap::FUNCTIONS[$lowerName];
        $nodes = $this->fileNodesFetcher->fetchNodes($stubFile);
        $functions = $nodes->getFunctionNodes();
        if (\count($functions) !== 1) {
            throw new \_PhpScoper0a2ac50786fa\PHPStan\ShouldNotHappenException(\sprintf('Function %s stub not found in %s.', $functionName, $stubFile));
        }
    ...
    private function mergeSignatures(\_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\FunctionSignature $nativeSignature, \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\FunctionSignature $functionMapSignature) : \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\FunctionSignature
    {
        $parameters = [];
        foreach ($nativeSignature->getParameters() as $i => $nativeParameter) {
            ...
            $parameters[] = new \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\ParameterSignature($nativeParameter->getName(), $nativeParameter->isOptional(), \_PhpScoper0a2ac50786fa\PHPStan\Type\TypehintHelper::decideType($nativeParameterType, \_PhpScoper0a2ac50786fa\PHPStan\Type\TypehintHelper::decideType($nativeParameter->getType(), $functionMapParameter->getType())), $nativeParameterType, $nativeParameter->passedByReference()->yes() ? $functionMapParameter->passedByReference() : $nativeParameter->passedByReference(), $nativeParameter->isVariadic());
        }
        $nativeReturnType = $nativeSignature->getNativeReturnType();
        if ($nativeReturnType instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\MixedType && !$nativeReturnType->isExplicitMixed()) {
            $returnType = $functionMapSignature->getReturnType();
        } else {
            $returnType = \_PhpScoper0a2ac50786fa\PHPStan\Type\TypehintHelper::decideType($nativeReturnType, \_PhpScoper0a2ac50786fa\PHPStan\Type\TypehintHelper::decideType($nativeSignature->getReturnType(), $functionMapSignature->getReturnType()));
        }
        return new \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\FunctionSignature($parameters, $returnType, $nativeReturnType, $nativeSignature->isVariadic());
    }
     ...
     */
    private function getSignature(\_PhpScoper0a2ac50786fa\PhpParser\Node\FunctionLike $function, ?string $className, string $stubFile) : \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\FunctionSignature
    {
        $phpDocParameterTypes = null;
        $phpDocReturnType = null;
        if ($function->getDocComment() !== null) {
            $phpDoc = $this->fileTypeMapper->getResolvedPhpDoc($stubFile, $className, null, $function instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\ClassMethod ? $function->name->toString() : $function->namespacedName->toString(), $function->getDocComment()->getText());
            $phpDocParameterTypes = \array_map(static function (\_PhpScoper0a2ac50786fa\PHPStan\PhpDoc\Tag\ParamTag $param) : Type {
        ...
        foreach ($function->getParams() as $param) {
            $name = $param->var;
            if (!$name instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Variable || !\is_string($name->name)) {
                throw new \_PhpScoper0a2ac50786fa\PHPStan\ShouldNotHappenException();
            }
            if ($name->name === 'array') {
                $parameterType = new \_PhpScoper0a2ac50786fa\PHPStan\Type\ArrayType(new \_PhpScoper0a2ac50786fa\PHPStan\Type\MixedType(), new \_PhpScoper0a2ac50786fa\PHPStan\Type\MixedType());
            } else {
                $parameterType = \_PhpScoper0a2ac50786fa\PHPStan\Type\ParserNodeTypeToPHPStanType::resolve($param->type, null);
            }
            $parameters[] = new \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\ParameterSignature($name->name, $param->default !== null || $param->variadic, \_PhpScoper0a2ac50786fa\PHPStan\Type\TypehintHelper::decideType($parameterType, $phpDocParameterTypes[$name->name] ?? null), $parameterType, $param->byRef ? \_PhpScoper0a2ac50786fa\PHPStan\Reflection\PassedByReference::createCreatesNewVariable() : \_PhpScoper0a2ac50786fa\PHPStan\Reflection\PassedByReference::createNo(), $param->variadic);
            $variadic = $variadic || $param->variadic;
        }
        $returnType = \_PhpScoper0a2ac50786fa\PHPStan\Type\ParserNodeTypeToPHPStanType::resolve($function->getReturnType(), null);
        return new \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\FunctionSignature($parameters, \_PhpScoper0a2ac50786fa\PHPStan\Type\TypehintHelper::decideType($returnType, $phpDocReturnType ?? null), $returnType, $variadic);
    }
}
