...
class FunctionSignatureMapProvider implements \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\SignatureMapProvider
{
    ...
    public function __construct(\_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\SignatureMapParser $parser, \_PhpScoper0a2ac50786fa\PHPStan\Php\PhpVersion $phpVersion)
    {
        $this->parser = $parser;
    ...
    public function getMethodSignature(string $className, string $methodName, ?\ReflectionMethod $reflectionMethod, int $variant = 0) : \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\FunctionSignature
    {
        ...
        foreach ($signature->getParameters() as $i => $parameter) {
            ...
            $parameters[] = new \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\ParameterSignature($parameter->getName(), $parameter->isOptional(), $parameter->getType(), \_PhpScoper0a2ac50786fa\PHPStan\Type\TypehintHelper::decideTypeFromReflection($nativeParameters[$i]->getType()), $parameter->passedByReference(), $parameter->isVariadic());
        }
        if ($reflectionMethod === null) {
            $nativeReturnType = new \_PhpScoper0a2ac50786fa\PHPStan\Type\MixedType();
        } else {
            $nativeReturnType = \_PhpScoper0a2ac50786fa\PHPStan\Type\TypehintHelper::decideTypeFromReflection($reflectionMethod->getReturnType());
        }
        return new \_PhpScoper0a2ac50786fa\PHPStan\Reflection\SignatureMap\FunctionSignature($parameters, $signature->getReturnType(), $nativeReturnType, $signature->isVariadic());
    }
    ...
    public function getSignatureMap() : array
    {
        if ($this->signatureMap === null) {
            ...
            if (!\is_array($signatureMap)) {
                throw new \_PhpScoper0a2ac50786fa\PHPStan\ShouldNotHappenException('Signature map could not be loaded.');
            }
