...
class FunctionSignatureMapProvider implements \RectorPrefix20201227\PHPStan\Reflection\SignatureMap\SignatureMapProvider
{
    ...
    public function __construct(\RectorPrefix20201227\PHPStan\Reflection\SignatureMap\SignatureMapParser $parser, \RectorPrefix20201227\PHPStan\Php\PhpVersion $phpVersion)
    {
        $this->parser = $parser;
    ...
    public function getMethodSignature(string $className, string $methodName, ?\ReflectionMethod $reflectionMethod, int $variant = 0) : \RectorPrefix20201227\PHPStan\Reflection\SignatureMap\FunctionSignature
    {
        ...
        foreach ($signature->getParameters() as $i => $parameter) {
            ...
            $parameters[] = new \RectorPrefix20201227\PHPStan\Reflection\SignatureMap\ParameterSignature($parameter->getName(), $parameter->isOptional(), $parameter->getType(), \PHPStan\Type\TypehintHelper::decideTypeFromReflection($nativeParameters[$i]->getType()), $parameter->passedByReference(), $parameter->isVariadic());
        }
        if ($reflectionMethod === null) {
        ...
        return new \RectorPrefix20201227\PHPStan\Reflection\SignatureMap\FunctionSignature($parameters, $signature->getReturnType(), $nativeReturnType, $signature->isVariadic());
    }
    ...
    public function getSignatureMap() : array
    {
        if ($this->signatureMap === null) {
            ...
            if (!\is_array($signatureMap)) {
                throw new \RectorPrefix20201227\PHPStan\ShouldNotHappenException('Signature map could not be loaded.');
            }
