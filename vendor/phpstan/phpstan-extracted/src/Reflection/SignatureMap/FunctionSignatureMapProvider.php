...
class FunctionSignatureMapProvider implements \_PhpScopere8e811afab72\PHPStan\Reflection\SignatureMap\SignatureMapProvider
{
    ...
    public function __construct(\_PhpScopere8e811afab72\PHPStan\Reflection\SignatureMap\SignatureMapParser $parser, \_PhpScopere8e811afab72\PHPStan\Php\PhpVersion $phpVersion)
    {
        $this->parser = $parser;
    ...
    public function getMethodSignature(string $className, string $methodName, ?\ReflectionMethod $reflectionMethod, int $variant = 0) : \_PhpScopere8e811afab72\PHPStan\Reflection\SignatureMap\FunctionSignature
    {
        ...
        foreach ($signature->getParameters() as $i => $parameter) {
            ...
            $parameters[] = new \_PhpScopere8e811afab72\PHPStan\Reflection\SignatureMap\ParameterSignature($parameter->getName(), $parameter->isOptional(), $parameter->getType(), \_PhpScopere8e811afab72\PHPStan\Type\TypehintHelper::decideTypeFromReflection($nativeParameters[$i]->getType()), $parameter->passedByReference(), $parameter->isVariadic());
        }
        if ($reflectionMethod === null) {
            $nativeReturnType = new \_PhpScopere8e811afab72\PHPStan\Type\MixedType();
        } else {
            $nativeReturnType = \_PhpScopere8e811afab72\PHPStan\Type\TypehintHelper::decideTypeFromReflection($reflectionMethod->getReturnType());
        }
        return new \_PhpScopere8e811afab72\PHPStan\Reflection\SignatureMap\FunctionSignature($parameters, $signature->getReturnType(), $nativeReturnType, $signature->isVariadic());
    }
    ...
    public function getSignatureMap() : array
    {
        if ($this->signatureMap === null) {
            ...
            if (!\is_array($signatureMap)) {
                throw new \_PhpScopere8e811afab72\PHPStan\ShouldNotHappenException('Signature map could not be loaded.');
            }
