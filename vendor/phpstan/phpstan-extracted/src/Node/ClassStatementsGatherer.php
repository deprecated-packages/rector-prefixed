...
use _PhpScoper0a2ac50786fa\PHPStan\Node\Constant\ClassConstantFetch;
...
class ClassStatementsGatherer
{
     ...
     */
    public function __construct(\_PhpScoper0a2ac50786fa\PHPStan\Reflection\ClassReflection $classReflection, callable $nodeCallback)
    {
    ...
    public function __invoke(\_PhpScoper0a2ac50786fa\PhpParser\Node $node, \_PhpScoper0a2ac50786fa\PHPStan\Analyser\Scope $scope) : void
    {
        $nodeCallback = $this->nodeCallback;
        $nodeCallback($node, $scope);
        $this->gatherNodes($node, $scope);
    }
    private function gatherNodes(\_PhpScoper0a2ac50786fa\PhpParser\Node $node, \_PhpScoper0a2ac50786fa\PHPStan\Analyser\Scope $scope) : void
    {
        if (!$scope->isInClass()) {
        ...
        if ($node instanceof \_PhpScoper0a2ac50786fa\PHPStan\Node\ClassPropertyNode && !$scope->isInTrait()) {
            $this->properties[] = $node;
            if ($node->isPromoted()) {
                $this->propertyUsages[] = new \_PhpScoper0a2ac50786fa\PHPStan\Node\Property\PropertyWrite(new \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\PropertyFetch(new \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Variable('this'), new \_PhpScoper0a2ac50786fa\PhpParser\Node\Identifier($node->getName())), $scope);
            }
            return;
        }
        if ($node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\ClassMethod && !$scope->isInTrait()) {
            $this->methods[] = $node;
            return;
        }
        ...
        if ($node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\MethodCall || $node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\StaticCall) {
            $this->methodCalls[] = new \_PhpScoper0a2ac50786fa\PHPStan\Node\Method\MethodCall($node, $scope);
            return;
        }
        if ($node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Array_ && \count($node->items) === 2) {
            $this->methodCalls[] = new \_PhpScoper0a2ac50786fa\PHPStan\Node\Method\MethodCall($node, $scope);
            return;
        }
        if ($node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\ClassConstFetch) {
            $this->constantFetches[] = new \_PhpScoper0a2ac50786fa\PHPStan\Node\Constant\ClassConstantFetch($node, $scope);
            return;
        }
        ...
        if ($node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Scalar\EncapsedStringPart) {
            return;
        }
        $inAssign = $scope->isInExpressionAssign($node);
        while ($node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\ArrayDimFetch) {
            $node = $node->var;
        }
        ...
        if ($inAssign) {
            $this->propertyUsages[] = new \_PhpScoper0a2ac50786fa\PHPStan\Node\Property\PropertyWrite($node, $scope);
        } else {
            $this->propertyUsages[] = new \_PhpScoper0a2ac50786fa\PHPStan\Node\Property\PropertyRead($node, $scope);
        }
    }
}
