...
use _PhpScoper0a6b37af0871\PHPStan\Process\Runnable\Runnable;
...
class ProcessPromise implements \_PhpScoper0a6b37af0871\PHPStan\Process\Runnable\Runnable
{
    /** @var LoopInterface */
    private $loop;
    /** @var string */
    ...
    public function __construct(\_PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\React\EventLoop\LoopInterface $loop, string $name, string $command)
    {
        $this->loop = $loop;
        $this->name = $name;
        $this->command = $command;
        $this->deferred = new \_PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\React\Promise\Deferred();
    }
    ...
    public function run() : \_PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\React\Promise\CancellablePromiseInterface
    {
        ...
        $this->process = new \_PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\React\ChildProcess\Process($this->command, null, null, [1 => $tmpStdOutResource, 2 => $tmpStdErrResource]);
        ...
        $this->process->on('exit', function ($exitCode) use($tmpStdOutResource, $tmpStdErrResource) : void {
            ...
            \fclose($tmpStdErrResource);
            if ($exitCode === null) {
                $this->deferred->reject(new \_PhpScoper0a6b37af0871\PHPStan\Process\ProcessCrashedException($stdOut . $stdErr));
                return;
            }
            if ($exitCode === 0) {
    ...
    public function cancel() : void
    {
        if ($this->process === null) {
            throw new \_PhpScoper0a6b37af0871\PHPStan\ShouldNotHappenException('Cancelling process before running');
        }
        $this->canceled = \true;
        $this->process->terminate();
        $this->deferred->reject(new \_PhpScoper0a6b37af0871\PHPStan\Process\ProcessCanceledException());
    }
}
