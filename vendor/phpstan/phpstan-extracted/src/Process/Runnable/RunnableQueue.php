...
class RunnableQueue
{
    ...
    private $running;
    public function __construct(\_PhpScoperb75b35f52b74\PHPStan\Process\Runnable\RunnableQueueLogger $logger, int $maxSize)
    {
        $this->logger = $logger;
    ...
    public function queue(\_PhpScoperb75b35f52b74\PHPStan\Process\Runnable\Runnable $runnable, int $size) : \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\React\Promise\CancellablePromiseInterface
    {
        if ($size > $this->maxSize) {
            throw new \_PhpScoperb75b35f52b74\PHPStan\ShouldNotHappenException('Runnable size exceeds queue maxSize.');
        }
        $deferred = new \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\React\Promise\Deferred(static function () use($runnable) : void {
            $runnable->cancel();
        });
    ...
    private function drainQueue() : void
    {
        ...
        if ($currentQueueSize > $this->maxSize) {
            throw new \_PhpScoperb75b35f52b74\PHPStan\ShouldNotHappenException('Running overflow');
        }
        if ($currentQueueSize === $this->maxSize) {
