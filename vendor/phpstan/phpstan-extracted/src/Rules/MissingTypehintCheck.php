...
class MissingTypehintCheck
{
    ...
    public function __construct(\_PhpScoper0a2ac50786fa\PHPStan\Reflection\ReflectionProvider $reflectionProvider, bool $checkMissingIterableValueType, bool $checkGenericClassInNonGenericObjectType)
    {
     ...
     */
    public function getIterableTypesWithMissingValueTypehint(\_PhpScoper0a2ac50786fa\PHPStan\Type\Type $type) : array
    {
        if (!$this->checkMissingIterableValueType) {
            return [];
        }
        $iterablesWithMissingValueTypehint = [];
        \_PhpScoper0a2ac50786fa\PHPStan\Type\TypeTraverser::map($type, function (\_PhpScoper0a2ac50786fa\PHPStan\Type\Type $type, callable $traverse) use(&$iterablesWithMissingValueTypehint) : Type {
            if ($type instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\Generic\TemplateType) {
                return $type;
            }
            if ($type->isIterable()->yes()) {
                $iterableValue = $type->getIterableValueType();
                if ($iterableValue instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\MixedType && !$iterableValue->isExplicitMixed()) {
                    if ($type instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\TypeWithClassName && !\in_array($type->getClassName(), self::ITERABLE_GENERIC_CLASS_NAMES, \true) && $this->reflectionProvider->hasClass($type->getClassName())) {
     ...
     */
    public function getNonGenericObjectTypesWithGenericClass(\_PhpScoper0a2ac50786fa\PHPStan\Type\Type $type) : array
    {
        if (!$this->checkGenericClassInNonGenericObjectType) {
            return [];
        }
        $objectTypes = [];
        \_PhpScoper0a2ac50786fa\PHPStan\Type\TypeTraverser::map($type, static function (\_PhpScoper0a2ac50786fa\PHPStan\Type\Type $type, callable $traverse) use(&$objectTypes) : Type {
            ...
            if ($type instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\ObjectType) {
                ...
                if (!$resolvedType instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\ObjectType) {
