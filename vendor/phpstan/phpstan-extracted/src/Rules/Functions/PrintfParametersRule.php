 ...
 */
class PrintfParametersRule implements \RectorPrefix20201227\PHPStan\Rules\Rule
{
    ...
    public function processNode(\PhpParser\Node $node, \RectorPrefix20201227\PHPStan\Analyser\Scope $scope) : array
    {
        ...
        if ($argsCount !== $placeHoldersCount + 1) {
            return [\RectorPrefix20201227\PHPStan\Rules\RuleErrorBuilder::message(\sprintf(\sprintf('%s, %s.', $placeHoldersCount === 1 ? 'Call to %s contains %d placeholder' : 'Call to %s contains %d placeholders', $argsCount - 1 === 1 ? '%d value given' : '%d values given'), $name, $placeHoldersCount, $argsCount - 1))->build()];
        }
        return [];
    }
    private function getPlaceholdersCount(string $functionName, string $format) : int
    {
        ...
        $matches = \RectorPrefix20201227\_HumbugBox221ad6f1b81f__UniqueRector\Nette\Utils\Strings::matchAll($format, $pattern, \PREG_SET_ORDER);
        if (\count($matches) === 0) {
            return 0;
        }
