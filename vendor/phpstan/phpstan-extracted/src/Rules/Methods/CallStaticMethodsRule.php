 ...
 */
class CallStaticMethodsRule implements \_PhpScoper0a2ac50786fa\PHPStan\Rules\Rule
{
    ...
    public function processNode(\_PhpScoper0a2ac50786fa\PhpParser\Node $node, \_PhpScoper0a2ac50786fa\PHPStan\Analyser\Scope $scope) : array
    {
        ...
        if ($class instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Name) {
            ...
            if (\in_array($lowercasedClassName, ['self', 'static'], \true)) {
                if (!$scope->isInClass()) {
                    return [\_PhpScoper0a2ac50786fa\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Calling %s::%s() outside of class scope.', $className, $methodName))->build()];
                }
                ...
                if ($currentClassReflection->getParentClass() === \false) {
                    return [\_PhpScoper0a2ac50786fa\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('%s::%s() calls parent::%s() but %s does not extend any class.', $scope->getClassReflection()->getDisplayName(), $scope->getFunctionName(), $methodName, $scope->getClassReflection()->getDisplayName()))->build()];
                }
                ...
                if (!$this->reflectionProvider->hasClass($className)) {
                    ...
                    return [\_PhpScoper0a2ac50786fa\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Call to static method %s() on an unknown class %s.', $methodName, $className))->discoveringSymbolsTip()->build()];
                } else {
                    $errors = $this->classCaseSensitivityCheck->checkClassNames([new \_PhpScoper0a2ac50786fa\PHPStan\Rules\ClassNameNodePair($className, $class)]);
                }
            ...
            if ($classReflection->hasNativeMethod($methodName) && $lowercasedClassName !== 'static') {
                ...
                if ($nativeMethodReflection instanceof \_PhpScoper0a2ac50786fa\PHPStan\Reflection\Php\PhpMethodReflection || $nativeMethodReflection instanceof \_PhpScoper0a2ac50786fa\PHPStan\Reflection\Native\NativeMethodReflection) {
                    $isAbstract = $nativeMethodReflection->isAbstract();
                }
            }
        } else {
            $classTypeResult = $this->ruleLevelHelper->findTypeToCheck($scope, $class, \sprintf('Call to static method %s() on an unknown class %%s.', $methodName), static function (\_PhpScoper0a2ac50786fa\PHPStan\Type\Type $type) use($methodName) : bool {
        ...
        if ($classType instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\Generic\GenericClassStringType) {
            $classType = $classType->getGenericType();
        } elseif ((new \_PhpScoper0a2ac50786fa\PHPStan\Type\StringType())->isSuperTypeOf($classType)->yes()) {
            return [];
        }
        ...
        if (!$classType->canCallMethods()->yes()) {
            return \array_merge($errors, [\_PhpScoper0a2ac50786fa\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Cannot call static method %s() on %s.', $methodName, $typeForDescribe->describe(\_PhpScoper0a2ac50786fa\PHPStan\Type\VerbosityLevel::typeOnly())))->build()]);
        }
        if (!$classType->hasMethod($methodName)->yes()) {
            if (!$this->reportMagicMethods) {
                $directClassNames = \_PhpScoper0a2ac50786fa\PHPStan\Type\TypeUtils::getDirectClassNames($classType);
            ...
            return \array_merge($errors, [\_PhpScoper0a2ac50786fa\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Call to an undefined static method %s::%s().', $typeForDescribe->describe(\_PhpScoper0a2ac50786fa\PHPStan\Type\VerbosityLevel::typeOnly()), $methodName))->build()]);
        }
        $method = $classType->getMethod($methodName, $scope);
        if (!$method->isStatic()) {
            $function = $scope->getFunction();
            if (!$function instanceof \_PhpScoper0a2ac50786fa\PHPStan\Reflection\MethodReflection || $function->isStatic() || !$scope->isInClass() || $classType instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\TypeWithClassName && $scope->getClassReflection()->getName() !== $classType->getClassName() && !$scope->getClassReflection()->isSubclassOf($classType->getClassName())) {
                return \array_merge($errors, [\_PhpScoper0a2ac50786fa\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Static call to instance method %s::%s().', $method->getDeclaringClass()->getDisplayName(), $method->getName()))->build()]);
            }
        }
        if (!$scope->canCallMethod($method)) {
            $errors = \array_merge($errors, [\_PhpScoper0a2ac50786fa\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Call to %s %s %s() of class %s.', $method->isPrivate() ? 'private' : 'protected', $method->isStatic() ? 'static method' : 'method', $method->getName(), $method->getDeclaringClass()->getDisplayName()))->build()]);
        }
        if ($isAbstract) {
            return [\_PhpScoper0a2ac50786fa\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Cannot call abstract%s method %s::%s().', $method->isStatic() ? ' static' : '', $method->getDeclaringClass()->getDisplayName(), $method->getName()))->build()];
        }
        ...
        if ($this->checkFunctionNameCase && $method->getName() !== $methodName) {
            $errors[] = \_PhpScoper0a2ac50786fa\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Call to %s with incorrect case: %s', $lowercasedMethodName, $methodName))->build();
        }
        return $errors;
    }
}
