 ...
 */
class ExistingNamesInUseRule implements \_PhpScoper0a6b37af0871\PHPStan\Rules\Rule
{
    ...
    {
        if ($node->type === \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Use_::TYPE_UNKNOWN) {
            throw new \_PhpScoper0a6b37af0871\PHPStan\ShouldNotHappenException();
        }
        foreach ($node->uses as $use) {
            if ($use->type !== \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Use_::TYPE_UNKNOWN) {
                throw new \_PhpScoper0a6b37af0871\PHPStan\ShouldNotHappenException();
            }
        }
        if ($node->type === \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Use_::TYPE_CONSTANT) {
            return $this->checkConstants($node->uses);
        }
        if ($node->type === \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Use_::TYPE_FUNCTION) {
    ...
    private function checkConstants(array $uses) : array
    {
        ...
        foreach ($uses as $use) {
            ...
            $errors[] = \_PhpScoper0a6b37af0871\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Used constant %s not found.', (string) $use->name))->line($use->name->getLine())->discoveringSymbolsTip()->build();
        }
        return $errors;
    }
    /**
    ...
    private function checkFunctions(array $uses) : array
    {
        ...
        foreach ($uses as $use) {
            if (!$this->reflectionProvider->hasFunction($use->name, null)) {
                $errors[] = \_PhpScoper0a6b37af0871\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Used function %s not found.', (string) $use->name))->line($use->name->getLine())->discoveringSymbolsTip()->build();
                ...
                if (\strtolower($realName) === \strtolower($usedName) && $realName !== $usedName) {
                    $errors[] = \_PhpScoper0a6b37af0871\PHPStan\Rules\RuleErrorBuilder::message(\sprintf('Function %s used with incorrect case: %s.', $realName, $usedName))->line($use->name->getLine())->build();
                }
            }
        }
        return $errors;
    ...
    {
        return $this->classCaseSensitivityCheck->checkClassNames(\array_map(static function (\_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\UseUse $use) : ClassNameNodePair {
            return new \_PhpScoper0a6b37af0871\PHPStan\Rules\ClassNameNodePair((string) $use->name, $use->name);
        }, $uses));
    }
}
