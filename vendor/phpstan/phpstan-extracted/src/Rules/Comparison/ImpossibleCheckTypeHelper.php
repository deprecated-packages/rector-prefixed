...
class ImpossibleCheckTypeHelper
{
     ...
     */
    public function __construct(\_PhpScopere8e811afab72\PHPStan\Reflection\ReflectionProvider $reflectionProvider, \_PhpScopere8e811afab72\PHPStan\Analyser\TypeSpecifier $typeSpecifier, array $universalObjectCratesClasses, bool $treatPhpDocTypesAsCertain)
    {
    ...
    public function findSpecifiedType(\_PhpScopere8e811afab72\PHPStan\Analyser\Scope $scope, \_PhpScopere8e811afab72\PhpParser\Node\Expr $node) : ?bool
    {
        if ($node instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\FuncCall && \count($node->args) > 0) {
            if ($node->name instanceof \_PhpScopere8e811afab72\PhpParser\Node\Name) {
                ...
                if ($functionName === 'assert') {
                    ...
                    if (!$assertValue instanceof \_PhpScopere8e811afab72\PHPStan\Type\Constant\ConstantBooleanType) {
                        return null;
                    }
                ...
                if ($functionName === 'count') {
                    ...
                    if ($haystackType instanceof \_PhpScopere8e811afab72\PHPStan\Type\MixedType) {
                        return null;
                    }
                    if (!$haystackType->isArray()->yes()) {
                        return null;
                    }
                    if (!$haystackType instanceof \_PhpScopere8e811afab72\PHPStan\Type\Constant\ConstantArrayType || \count($haystackType->getValueTypes()) > 0) {
                        $needleType = $scope->getType($node->args[0]->value);
                        $haystackArrayTypes = \_PhpScopere8e811afab72\PHPStan\Type\TypeUtils::getArrays($haystackType);
                        if (\count($haystackArrayTypes) === 1 && $haystackArrayTypes[0]->getIterableValueType() instanceof \_PhpScopere8e811afab72\PHPStan\Type\NeverType) {
                            return null;
                        }
                        ...
                        if ($isNeedleSupertype->maybe() || $isNeedleSupertype->yes()) {
                            foreach ($haystackArrayTypes as $haystackArrayType) {
                                foreach (\_PhpScopere8e811afab72\PHPStan\Type\TypeUtils::getConstantScalars($haystackArrayType->getIterableValueType()) as $constantScalarType) {
                        ...
                        if ($isNeedleSupertype->yes()) {
                            $hasConstantNeedleTypes = \count(\_PhpScopere8e811afab72\PHPStan\Type\TypeUtils::getConstantScalars($needleType)) > 0;
                            $hasConstantHaystackTypes = \count(\_PhpScopere8e811afab72\PHPStan\Type\TypeUtils::getConstantScalars($valueType)) > 0;
                    ...
                    if ($objectType instanceof \_PhpScopere8e811afab72\PHPStan\Type\Constant\ConstantStringType && !$this->reflectionProvider->hasClass($objectType->getValue())) {
                        return \false;
                    }
                    if ($methodType instanceof \_PhpScopere8e811afab72\PHPStan\Type\Constant\ConstantStringType) {
                        if ($objectType instanceof \_PhpScopere8e811afab72\PHPStan\Type\Constant\ConstantStringType) {
                            $objectType = new \_PhpScopere8e811afab72\PHPStan\Type\ObjectType($objectType->getValue());
                        }
                        if ($objectType instanceof \_PhpScopere8e811afab72\PHPStan\Type\TypeWithClassName) {
        ...
        $specifiedTypes = $this->typeSpecifier->specifyTypesInCondition($scope, $node, \_PhpScopere8e811afab72\PHPStan\Analyser\TypeSpecifierContext::createTruthy());
        $sureTypes = $specifiedTypes->getSureTypes();
        $sureNotTypes = $specifiedTypes->getSureNotTypes();
        $isSpecified = static function (\_PhpScopere8e811afab72\PhpParser\Node\Expr $expr) use($scope, $node) : bool {
            return ($node instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\FuncCall || $node instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\MethodCall || $node instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\StaticCall) && $scope->isSpecified($expr);
        };
        ...
        if (\count($sureTypes) > 0) {
            ...
            $types = \_PhpScopere8e811afab72\PHPStan\Type\TypeCombinator::union(...\array_column($sureTypes, 1));
            if ($types instanceof \_PhpScopere8e811afab72\PHPStan\Type\NeverType) {
                return \false;
            }
        }
        if (\count($sureNotTypes) > 0) {
            ...
            $types = \_PhpScopere8e811afab72\PHPStan\Type\TypeCombinator::union(...\array_column($sureNotTypes, 1));
            if ($types instanceof \_PhpScopere8e811afab72\PHPStan\Type\NeverType) {
                return \true;
            }
        }
     ...
     * @return string
     */
    public function getArgumentsDescription(\_PhpScopere8e811afab72\PHPStan\Analyser\Scope $scope, array $args) : string
    {
        if (\count($args) === 0) {
            return '';
        }
        $descriptions = \array_map(static function (\_PhpScopere8e811afab72\PhpParser\Node\Arg $arg) use($scope) : string {
            return $scope->getType($arg->value)->describe(\_PhpScopere8e811afab72\PHPStan\Type\VerbosityLevel::value());
        }, $args);
