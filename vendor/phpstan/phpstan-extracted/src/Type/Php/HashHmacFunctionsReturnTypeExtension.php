...
final class HashHmacFunctionsReturnTypeExtension implements \_PhpScoper0a6b37af0871\PHPStan\Type\DynamicFunctionReturnTypeExtension
{
    ...
    public function getTypeFromFunctionCall(\_PhpScoper0a6b37af0871\PHPStan\Reflection\FunctionReflection $functionReflection, \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\FuncCall $functionCall, \_PhpScoper0a6b37af0871\PHPStan\Analyser\Scope $scope) : \_PhpScoper0a6b37af0871\PHPStan\Type\Type
    {
        if ($functionReflection->getName() === 'hash_hmac') {
            $defaultReturnType = new \_PhpScoper0a6b37af0871\PHPStan\Type\StringType();
        } else {
        ...
        $string = $values[0];
        return \in_array($string->getValue(), self::HMAC_ALGORITHMS, \true) ? $defaultReturnType : new \_PhpScoper0a6b37af0871\PHPStan\Type\Constant\ConstantBooleanType(\false);
    }
}
