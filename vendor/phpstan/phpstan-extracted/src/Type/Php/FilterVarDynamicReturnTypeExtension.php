...
class FilterVarDynamicReturnTypeExtension implements \_PhpScoper0a2ac50786fa\PHPStan\Type\DynamicFunctionReturnTypeExtension
{
    ...
    public function __construct(\_PhpScoper0a2ac50786fa\PHPStan\Reflection\ReflectionProvider $reflectionProvider)
    {
        ...
        $this->flagsString = new \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantStringType('flags');
    }
    /**
    ...
    private function getFilterTypeMap() : array
    {
        ...
        $booleanType = new \_PhpScoper0a2ac50786fa\PHPStan\Type\BooleanType();
        $floatType = new \_PhpScoper0a2ac50786fa\PHPStan\Type\FloatType();
        $intType = new \_PhpScoper0a2ac50786fa\PHPStan\Type\IntegerType();
        ...
        if ($this->reflectionProvider->hasConstant(new \_PhpScoper0a2ac50786fa\PhpParser\Node\Name('FILTER_SANITIZE_MAGIC_QUOTES'), null)) {
            $this->filterTypeMap[$this->getConstant('FILTER_SANITIZE_MAGIC_QUOTES')] = $stringType;
        }
        if ($this->reflectionProvider->hasConstant(new \_PhpScoper0a2ac50786fa\PhpParser\Node\Name('FILTER_SANITIZE_ADD_SLASHES'), null)) {
    ...
    public function getTypeFromFunctionCall(\_PhpScoper0a2ac50786fa\PHPStan\Reflection\FunctionReflection $functionReflection, \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\FuncCall $functionCall, \_PhpScoper0a2ac50786fa\PHPStan\Analyser\Scope $scope) : \_PhpScoper0a2ac50786fa\PHPStan\Type\Type
    {
        ...
        if ($filterArg === null) {
            ...
            if (!$filterType instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantIntegerType) {
                return $mixedType;
            }
        ...
        if ($exactType !== null) {
            ...
            if ($otherType->isSuperTypeOf($type)->no()) {
                $type = new \_PhpScoper0a2ac50786fa\PHPStan\Type\UnionType([$type, $otherType]);
            }
        }
        if ($this->hasFlag($this->getConstant('FILTER_FORCE_ARRAY'), $flagsArg, $scope)) {
            return new \_PhpScoper0a2ac50786fa\PHPStan\Type\ArrayType(new \_PhpScoper0a2ac50786fa\PHPStan\Type\MixedType(), $type);
        }
        return $type;
    }
    private function determineExactType(\_PhpScoper0a2ac50786fa\PHPStan\Type\Type $in, int $filterValue) : ?\_PhpScoper0a2ac50786fa\PHPStan\Type\Type
    {
        if ($filterValue === $this->getConstant('FILTER_VALIDATE_BOOLEAN') && $in instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\BooleanType || $filterValue === $this->getConstant('FILTER_VALIDATE_INT') && $in instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\IntegerType || $filterValue === $this->getConstant('FILTER_VALIDATE_FLOAT') && $in instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\FloatType) {
            return $in;
        }
        if ($filterValue === $this->getConstant('FILTER_VALIDATE_FLOAT') && $in instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\IntegerType) {
            return $in->toFloat();
        }
        return null;
    }
    private function getOtherType(?\_PhpScoper0a2ac50786fa\PhpParser\Node\Arg $flagsArg, \_PhpScoper0a2ac50786fa\PHPStan\Analyser\Scope $scope) : \_PhpScoper0a2ac50786fa\PHPStan\Type\Type
    {
        $falseType = new \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantBooleanType(\false);
        if ($flagsArg === null) {
            return $falseType;
        }
    ...
    private function getDefault(\_PhpScoper0a2ac50786fa\PhpParser\Node\Arg $expression, \_PhpScoper0a2ac50786fa\PHPStan\Analyser\Scope $scope) : ?\_PhpScoper0a2ac50786fa\PHPStan\Type\Type
    {
        $exprType = $scope->getType($expression->value);
        if (!$exprType instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantArrayType) {
            return null;
        }
        $optionsType = $exprType->getOffsetValueType(new \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantStringType('options'));
        if (!$optionsType instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantArrayType) {
            return null;
        }
        $defaultType = $optionsType->getOffsetValueType(new \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantStringType('default'));
        if (!$defaultType instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\ErrorType) {
            return $defaultType;
        }
        return null;
    }
    ...
    private function getFlagsValue(\_PhpScoper0a2ac50786fa\PHPStan\Type\Type $exprType) : \_PhpScoper0a2ac50786fa\PHPStan\Type\Type
    {
        if (!$exprType instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantArrayType) {
            return $exprType;
        }
