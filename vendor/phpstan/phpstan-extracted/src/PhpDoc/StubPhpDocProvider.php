...
class StubPhpDocProvider
{
     ...
     */
    public function __construct(\_PhpScoper0a2ac50786fa\PHPStan\Parser\Parser $parser, \_PhpScoper0a2ac50786fa\PHPStan\Type\FileTypeMapper $fileTypeMapper, array $stubFiles)
    {
        $this->parser = $parser;
        $this->fileTypeMapper = $fileTypeMapper;
        $this->stubFiles = $stubFiles;
    }
    public function findClassPhpDoc(string $className) : ?\_PhpScoper0a2ac50786fa\PHPStan\PhpDoc\ResolvedPhpDocBlock
    {
    ...
    public function findPropertyPhpDoc(string $className, string $propertyName) : ?\_PhpScoper0a2ac50786fa\PHPStan\PhpDoc\ResolvedPhpDocBlock
    {
        if (!$this->isKnownClass($className)) {
            return null;
        }
     ...
     */
    public function findMethodPhpDoc(string $className, string $methodName, array $positionalParameterNames) : ?\_PhpScoper0a2ac50786fa\PHPStan\PhpDoc\ResolvedPhpDocBlock
    {
        if (!$this->isKnownClass($className)) {
            return null;
        }
    ...
    public function findFunctionPhpDoc(string $functionName) : ?\_PhpScoper0a2ac50786fa\PHPStan\PhpDoc\ResolvedPhpDocBlock
    {
    ...
    private function initializeKnownElementNode(string $stubFile, \_PhpScoper0a2ac50786fa\PhpParser\Node $node) : void
    {
        ...
        if (!$node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\Class_ && !$node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\Interface_ && !$node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\Trait_) {
            return;
        }
        ...
        foreach ($node->stmts as $stmt) {
            ...
            if ($stmt instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\Property) {
                ...
                $this->knownMethodsParameterNames[$className][$methodName] = \array_map(static function (\_PhpScoper0a2ac50786fa\PhpParser\Node\Param $param) : string {
