...
namespace _PhpScoper0a2ac50786fa\PHPStan\Parallel;
...
use _PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Nette\Utils\Random;
...
class ParallelAnalyser
{
    ...
    public function analyse(\_PhpScoper0a2ac50786fa\PHPStan\Parallel\Schedule $schedule, string $mainScript, ?\Closure $postFileCallback, ?string $projectConfigFile, ?string $tmpFile, ?string $insteadOfFile, \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Input\InputInterface $input) : \_PhpScoper0a2ac50786fa\PHPStan\Analyser\AnalyserResult
    {
        ...
        $this->processPool = new \_PhpScoper0a2ac50786fa\PHPStan\Parallel\ProcessPool($server);
        $server->on('connection', function (\_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Socket\ConnectionInterface $connection) use(&$jobs) : void {
            $decoder = new \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Clue\React\NDJson\Decoder($connection, \true, 512, \defined('JSON_INVALID_UTF8_IGNORE') ? \JSON_INVALID_UTF8_IGNORE : 0, $this->decoderBufferSize);
        ...
        for ($i = 0; $i < $numberOfProcesses; $i++) {
            ...
            $processIdentifier = \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Nette\Utils\Random::generate();
            ...
            $process = new \_PhpScoper0a2ac50786fa\PHPStan\Parallel\Process(\_PhpScoper0a2ac50786fa\PHPStan\Process\ProcessHelper::getWorkerCommand($mainScript, 'worker', $projectConfigFile, $commandOptions, $input), $loop, $this->processTimeout);
            $process->start(function (array $json) use($process, &$internalErrors, &$errors, &$dependencies, &$exportedNodes, &$jobs, $postFileCallback, &$internalErrorsCount, &$reachedInternalErrorsCountLimit, $processIdentifier) : void {
                foreach ($json['errors'] as $jsonError) {
                    ...
                    $errors[] = \_PhpScoper0a2ac50786fa\PHPStan\Analyser\Error::decode($jsonError);
                }
                /**
                 * @var string $file
        ...
        return new \_PhpScoper0a2ac50786fa\PHPStan\Analyser\AnalyserResult($errors, $internalErrors, $internalErrorsCount === 0 ? $dependencies : null, $exportedNodes, $reachedInternalErrorsCountLimit);
    }
}
