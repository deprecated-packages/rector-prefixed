...
namespace _PhpScopere8e811afab72\PHPStan\Parallel;
...
class ParallelAnalyser
{
    ...
    public function analyse(\_PhpScopere8e811afab72\PHPStan\Parallel\Schedule $schedule, string $mainScript, ?\Closure $postFileCallback, ?string $projectConfigFile, ?string $tmpFile, ?string $insteadOfFile, \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Input\InputInterface $input) : \_PhpScopere8e811afab72\PHPStan\Analyser\AnalyserResult
    {
        ...
        $this->processPool = new \_PhpScopere8e811afab72\PHPStan\Parallel\ProcessPool($server);
        $server->on('connection', function (\_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\React\Socket\ConnectionInterface $connection) use(&$jobs) : void {
            $decoder = new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Clue\React\NDJson\Decoder($connection, \true, 512, \defined('JSON_INVALID_UTF8_IGNORE') ? \JSON_INVALID_UTF8_IGNORE : 0, $this->decoderBufferSize);
        ...
        for ($i = 0; $i < $numberOfProcesses; $i++) {
            ...
            $processIdentifier = \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\Utils\Random::generate();
            ...
            $process = new \_PhpScopere8e811afab72\PHPStan\Parallel\Process(\_PhpScopere8e811afab72\PHPStan\Process\ProcessHelper::getWorkerCommand($mainScript, 'worker', $projectConfigFile, $commandOptions, $input), $loop, $this->processTimeout);
            $process->start(function (array $json) use($process, &$internalErrors, &$errors, &$dependencies, &$exportedNodes, &$jobs, $postFileCallback, &$internalErrorsCount, &$reachedInternalErrorsCountLimit, $processIdentifier) : void {
                foreach ($json['errors'] as $jsonError) {
                    ...
                    $errors[] = \_PhpScopere8e811afab72\PHPStan\Analyser\Error::decode($jsonError);
                }
                /**
                 * @var string $file
        ...
        return new \_PhpScopere8e811afab72\PHPStan\Analyser\AnalyserResult($errors, $internalErrors, $internalErrorsCount === 0 ? $dependencies : null, $exportedNodes, $reachedInternalErrorsCountLimit);
    }
}
