...
use _PhpScoper2a4e7ab1ecbc\PHPStan\Command\AnalyseCommand;
...
class TableErrorFormatter implements \_PhpScoper2a4e7ab1ecbc\PHPStan\Command\ErrorFormatter\ErrorFormatter
{
    /** @var RelativePathHelper */
    private $relativePathHelper;
    /** @var bool */
    private $showTipsOfTheDay;
    public function __construct(\_PhpScoper2a4e7ab1ecbc\PHPStan\File\RelativePathHelper $relativePathHelper, bool $showTipsOfTheDay)
    {
        ...
        if (!$analysisResult->hasErrors() && !$analysisResult->hasWarnings()) {
            ...
            if ($this->showTipsOfTheDay) {
                if ($analysisResult->isDefaultLevelUsed()) {
                    ...
                    $output->writeLineFormatted(\sprintf("PHPStan is performing only the most basic checks.\nYou can pass a higher rule level through the <fg=cyan>--%s</> option\n(the default and current level is %d) to analyse code more thoroughly.", \_PhpScoper2a4e7ab1ecbc\PHPStan\Command\AnalyseCommand::OPTION_LEVEL, \_PhpScoper2a4e7ab1ecbc\PHPStan\Command\AnalyseCommand::DEFAULT_LEVEL));
