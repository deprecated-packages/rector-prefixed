...
use _PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Composer\XdebugHandler\XdebugHandler;
use _PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\DI\Config\Adapters\PhpAdapter;
use _PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\DI\Helpers;
use _PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Schema\Context as SchemaContext;
use _PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Schema\Processor;
...
use _PhpScoperb75b35f52b74\PHPStan\DependencyInjection\LoaderFactory;
use _PhpScoperb75b35f52b74\PHPStan\DependencyInjection\NeonAdapter;
...
class CommandHelper
{
     ...
     */
    public static function begin(\_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Input\InputInterface $input, \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Output\OutputInterface $output, array $paths, ?string $pathsFile, ?string $memoryLimit, ?string $autoloadFile, array $composerAutoloaderProjectPaths, ?string $projectConfigFile, ?string $generateBaselineFile, ?string $level, bool $allowXdebug, bool $manageMemoryLimitFile = \true, bool $debugEnabled = \false, ?string $singleReflectionFile = null) : \_PhpScoperb75b35f52b74\PHPStan\Command\InceptionResult
    {
        if (!$allowXdebug) {
            $xdebug = new \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Composer\XdebugHandler\XdebugHandler('phpstan', '--ansi');
            $xdebug->check();
            unset($xdebug);
        }
        $stdOutput = new \_PhpScoperb75b35f52b74\PHPStan\Command\Symfony\SymfonyOutput($output, new \_PhpScoperb75b35f52b74\PHPStan\Command\Symfony\SymfonyStyle(new \_PhpScoperb75b35f52b74\PHPStan\Command\ErrorsConsoleStyle($input, $output)));
        /** @var \PHPStan\Command\Output $errorOutput */
        $errorOutput = (static function () use($input, $output) : Output {
            $symfonyErrorOutput = $output instanceof \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Output\ConsoleOutputInterface ? $output->getErrorOutput() : $output;
            return new \_PhpScoperb75b35f52b74\PHPStan\Command\Symfony\SymfonyOutput($symfonyErrorOutput, new \_PhpScoperb75b35f52b74\PHPStan\Command\Symfony\SymfonyStyle(new \_PhpScoperb75b35f52b74\PHPStan\Command\ErrorsConsoleStyle($input, $symfonyErrorOutput)));
        })();
        if ($memoryLimit !== null) {
            if (\_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Utils\Strings::match($memoryLimit, '#^-?\\d+[kMG]?$#i') === null) {
                $errorOutput->writeLineFormatted(\sprintf('Invalid memory limit format "%s".', $memoryLimit));
                throw new \_PhpScoperb75b35f52b74\PHPStan\Command\InceptionNotSuccessfulException();
            }
        ...
        $currentWorkingDirectoryFileHelper = new \_PhpScoperb75b35f52b74\PHPStan\File\FileHelper($currentWorkingDirectory);
        ...
        if (\count($paths) === 0 && $pathsFile !== null) {
            ...
            try {
                $pathsString = \_PhpScoperb75b35f52b74\PHPStan\File\FileReader::read($pathsFile);
            ...
            $pathsFileFileHelper = new \_PhpScoperb75b35f52b74\PHPStan\File\FileHelper(\dirname($pathsFile));
        ...
        if ($projectConfigFile !== null) {
            ...
            $loader = (new \_PhpScoperb75b35f52b74\PHPStan\DependencyInjection\LoaderFactory($currentWorkingDirectoryFileHelper, $containerFactory->getRootDirectory(), $containerFactory->getCurrentWorkingDirectory(), $generateBaselineFile))->createLoader();
            try {
                $projectConfig = $loader->load($projectConfigFile, null);
            } catch (\_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\InvalidStateException|\_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\FileNotFoundException $e) {
                $errorOutput->writeLineFormatted($e->getMessage());
                throw new \_PhpScoperb75b35f52b74\PHPStan\Command\InceptionNotSuccessfulException();
            }
            ...
            if (isset($projectConfig['parameters']['tmpDir'])) {
                $tmpDir = \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\DI\Helpers::expand($projectConfig['parameters']['tmpDir'], $defaultParameters);
            }
            ...
            if (\count($paths) === 0 && isset($projectConfig['parameters']['paths'])) {
                $analysedPathsFromConfig = \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\DI\Helpers::expand($projectConfig['parameters']['paths'], $defaultParameters);
        ...
        if (\class_exists('_PhpScoperb75b35f52b74\\PHPStan\\ExtensionInstaller\\GeneratedConfig')) {
            $generatedConfigReflection = new \ReflectionClass('_PhpScoperb75b35f52b74\\PHPStan\\ExtensionInstaller\\GeneratedConfig');
            $generatedConfigDirectory = \dirname($generatedConfigReflection->getFileName());
            foreach (\_PhpScoperb75b35f52b74\PHPStan\ExtensionInstaller\GeneratedConfig::EXTENSIONS as $name => $extensionConfig) {
        ...
        } catch (\_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\DI\InvalidConfigurationException|\_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Utils\AssertionException $e) {
        ...
        if ($manageMemoryLimitFile && \file_exists($memoryLimitFile)) {
            $memoryLimitFileContents = \_PhpScoperb75b35f52b74\PHPStan\File\FileReader::read($memoryLimitFile);
        ...
        $processor = new \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Schema\Processor();
        $processor->onNewContext[] = static function (\_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Schema\Context $context) : void {
            $context->path = ['parameters'];
        };
        try {
            $processor->process($schema, $container->getParameters());
        } catch (\_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Schema\ValidationException $e) {
        ...
        $fileFinder = $container->getByType(\_PhpScoperb75b35f52b74\PHPStan\File\FileFinder::class);
        ...
        return new \_PhpScoperb75b35f52b74\PHPStan\Command\InceptionResult($filesCallback, $stdOutput, $errorOutput, $container, $defaultLevelUsed, $memoryLimitFile, $projectConfigFile, $projectConfig, $generateBaselineFile);
    }
    private static function executeBootstrapFile(string $file, \_PhpScoperb75b35f52b74\PHPStan\DependencyInjection\Container $container, \_PhpScoperb75b35f52b74\PHPStan\Command\Output $errorOutput, bool $debugEnabled) : void
    {
        if (!\is_file($file)) {
    ...
    private static function setUpSignalHandler(\_PhpScoperb75b35f52b74\PHPStan\Command\Output $output, ?string $memoryLimitFile) : void
    {
     ...
     */
    private static function detectDuplicateIncludedFiles(\_PhpScoperb75b35f52b74\PHPStan\Command\Output $output, \_PhpScoperb75b35f52b74\PHPStan\File\FileHelper $fileHelper, array $configFiles, array $loaderParameters) : void
    {
        $neonAdapter = new \_PhpScoperb75b35f52b74\PHPStan\DependencyInjection\NeonAdapter();
        $phpAdapter = new \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\DI\Config\Adapters\PhpAdapter();
        $allConfigFiles = [];
     ...
     * @return string[]
     */
    private static function getConfigFiles(\_PhpScoperb75b35f52b74\PHPStan\File\FileHelper $fileHelper, \_PhpScoperb75b35f52b74\PHPStan\DependencyInjection\NeonAdapter $neonAdapter, \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\DI\Config\Adapters\PhpAdapter $phpAdapter, string $configFile, array $loaderParameters, ?string $generateBaselineFile) : array
    {
        ...
        if (\_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Utils\Strings::endsWith($configFile, '.php')) {
        ...
        if (isset($data['includes'])) {
            \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Utils\Validators::assert($data['includes'], 'list', \sprintf("section 'includes' in file '%s'", $configFile));
            $includes = \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\DI\Helpers::expand($data['includes'], $loaderParameters);
    ...
    {
        return \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Utils\Strings::match($includedFile, '#([a-z]+:)?[/\\\\]#Ai') !== null ? $includedFile : \dirname($mainFile) . '/' . $includedFile;
    }
}
