...
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\Composer\CaBundle\CaBundle;
...
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\Psr\Http\Message\ResponseInterface;
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\ChildProcess\Process;
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\EventLoop\LoopInterface;
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\EventLoop\StreamSelectLoop;
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Http\Browser;
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Promise\CancellablePromiseInterface;
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Promise\ExtendedPromiseInterface;
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Promise\PromiseInterface;
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Socket\ConnectionInterface;
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Socket\Connector;
use RectorPrefix20201227\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Helper\ProgressBar;
...
use function RectorPrefix20201227\_HumbugBox221ad6f1b81f\Clue\React\Block\await;
use function escapeshellarg;
use function file_exists;
use function RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Promise\resolve;
class FixerApplication
{
     ...
     * @return int
     */
    public function run(?string $projectConfigFile, \RectorPrefix20201227\PHPStan\Command\InceptionResult $inceptionResult, \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Output\OutputInterface $output, array $fileSpecificErrors, array $notFileSpecificErrors, int $filesCount, string $mainScript) : int
    {
        ...
        $server->on('connection', function (\RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Socket\ConnectionInterface $connection) use($loop, $projectConfigFile, $input, $output, $fileSpecificErrors, $notFileSpecificErrors, $mainScript, $filesCount, $reanalyseProcessQueue, $inceptionResult) : void {
            $decoder = new \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Clue\React\NDJson\Decoder($connection, \true, 512, \defined('JSON_INVALID_UTF8_IGNORE') ? \JSON_INVALID_UTF8_IGNORE : 0, 128 * 1024 * 1024);
            ...
            $decoder->on('data', function (array $data) use($loop, $encoder, $projectConfigFile, $input, $output, $mainScript, $reanalyseProcessQueue, $inceptionResult) : void {
                ...
                $this->reanalyseWithTmpFile($loop, $inceptionResult, $mainScript, $reanalyseProcessQueue, $projectConfigFile, $data['data']['tmpFile'], $data['data']['insteadOfFile'], $data['data']['fixerSuggestionId'], $input)->done(static function (string $output) use($encoder, $id) : void {
                    $encoder->write(['id' => $id, 'response' => \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Nette\Utils\Json::decode($output, \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Nette\Utils\Json::FORCE_ARRAY)]);
    ...
    private function getFixerProcess(\RectorPrefix20201227\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Output\OutputInterface $output, int $serverPort) : \RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\ChildProcess\Process
    {
        ...
        return new \RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\ChildProcess\Process(\sprintf('%s -d memory_limit=%s %s --port %d', \PHP_BINARY, \escapeshellarg(\ini_get('memory_limit')), \escapeshellarg($pharPath), $serverPort), null, $env, []);
    }
    private function downloadPhar(\RectorPrefix20201227\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Output\OutputInterface $output, string $pharPath, string $infoPath) : void
    {
        $currentVersion = null;
        if (\file_exists($pharPath) && \file_exists($infoPath)) {
            /** @var array{version: string, date: string} $currentInfo */
            $currentInfo = \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Nette\Utils\Json::decode(\RectorPrefix20201227\PHPStan\File\FileReader::read($infoPath), \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Nette\Utils\Json::FORCE_ARRAY);
        ...
        $loop = new \RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\EventLoop\StreamSelectLoop();
        $client = new \RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Http\Browser($loop, new \RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Socket\Connector($loop, ['timeout' => 5, 'tls' => ['cafile' => \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Composer\CaBundle\CaBundle::getBundledCaBundlePath()], 'dns' => '1.1.1.1']));
        /** @var array{url: string, version: string} $latestInfo */
        $latestInfo = \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Nette\Utils\Json::decode((string) \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Clue\React\Block\await($client->get('https://fixer-download-api.phpstan.com/latest'), $loop, 5.0)->getBody(), \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Nette\Utils\Json::FORCE_ARRAY);
        ...
        $progressBar = new \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Helper\ProgressBar($output);
        $client->requestStreaming('GET', $latestInfo['url'])->done(static function (\RectorPrefix20201227\_HumbugBox221ad6f1b81f\Psr\Http\Message\ResponseInterface $response) use($progressBar, $pharPathResource) : void {
            $body = $response->getBody();
            if (!$body instanceof \RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Stream\ReadableStreamInterface) {
                throw new \RectorPrefix20201227\PHPStan\ShouldNotHappenException();
            }
    ...
    {
        \RectorPrefix20201227\PHPStan\File\FileWriter::write($infoPath, \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Nette\Utils\Json::encode(['version' => $version, 'date' => (new \DateTimeImmutable('', new \DateTimeZone('UTC')))->format(\DateTime::ATOM)]));
    }
    /**
     * @param LoopInterface $loop
     * @param callable(FileMonitorResult): void $hasChangesCallback
     */
    private function monitorFileChanges(\RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\EventLoop\LoopInterface $loop, callable $hasChangesCallback) : void
    {
    ...
    private function reanalyseWithTmpFile(\RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\EventLoop\LoopInterface $loop, \RectorPrefix20201227\PHPStan\Command\InceptionResult $inceptionResult, string $mainScript, \RectorPrefix20201227\PHPStan\Process\Runnable\RunnableQueue $runnableQueue, ?string $projectConfigFile, string $tmpFile, string $insteadOfFile, string $fixerSuggestionId, \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Input\InputInterface $input) : \RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Promise\PromiseInterface
    {
    ...
    private function reanalyseAfterFileChanges(\RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\EventLoop\LoopInterface $loop, \RectorPrefix20201227\PHPStan\Command\InceptionResult $inceptionResult, string $mainScript, ?string $projectConfigFile, ?string $fixerSuggestionId, \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Symfony\Component\Console\Input\InputInterface $input) : \RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Promise\PromiseInterface
    {
        ...
        if (\count($resultCache->getFilesToAnalyse()) === 0) {
            ...
            return \RectorPrefix20201227\_HumbugBox221ad6f1b81f\React\Promise\resolve(['fileSpecificErrors' => $finalFileSpecificErrors, 'notFileSpecificErrors' => $finalNotFileSpecificErrors]);
        }
        ...
        return $this->processInProgress->then(static function (string $output) : array {
            return \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Nette\Utils\Json::decode($output, \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Nette\Utils\Json::FORCE_ARRAY);
        });
    }
    private function getPhpstanVersion() : string
    {
        try {
            return \RectorPrefix20201227\_HumbugBox221ad6f1b81f\Jean85\PrettyVersions::getVersion('phpstan/phpstan')->getPrettyVersion();
