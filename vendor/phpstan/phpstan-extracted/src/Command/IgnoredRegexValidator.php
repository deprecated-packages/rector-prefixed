...
use _PhpScoper0a2ac50786fa\Hoa\Compiler\Llk\Parser;
use _PhpScoper0a2ac50786fa\Hoa\Compiler\Llk\TreeNode;
...
class IgnoredRegexValidator
{
    ...
    public function __construct(\_PhpScoper0a2ac50786fa\Hoa\Compiler\Llk\Parser $parser, \_PhpScoper0a2ac50786fa\PHPStan\PhpDoc\TypeStringResolver $typeStringResolver)
    {
        $this->parser = $parser;
        $this->typeStringResolver = $typeStringResolver;
    }
    public function validate(string $regex) : \_PhpScoper0a2ac50786fa\PHPStan\Command\IgnoredRegexValidatorResult
    {
        $regex = $this->removeDelimiters($regex);
        try {
            /** @var TreeNode $ast */
            $ast = $this->parser->parse($regex);
        } catch (\_PhpScoper0a2ac50786fa\Hoa\Exception\Exception $e) {
            if (\strpos($e->getMessage(), 'Unexpected token "|" (alternation) at line 1') === 0) {
                return new \_PhpScoper0a2ac50786fa\PHPStan\Command\IgnoredRegexValidatorResult([], \false, \true, '||', '\\|\\|');
            }
            if (\strpos($regex, '()') !== \false && \strpos($e->getMessage(), 'Unexpected token ")" (_capturing) at line 1') === 0) {
                return new \_PhpScoper0a2ac50786fa\PHPStan\Command\IgnoredRegexValidatorResult([], \false, \true, '()', '\\(\\)');
            }
            return new \_PhpScoper0a2ac50786fa\PHPStan\Command\IgnoredRegexValidatorResult([], \false, \false);
        }
        return new \_PhpScoper0a2ac50786fa\PHPStan\Command\IgnoredRegexValidatorResult($this->getIgnoredTypes($ast), $this->hasAnchorsInTheMiddle($ast), \false);
    }
    /**
     * @param TreeNode $ast
     * @return array<string, string>
     */
    private function getIgnoredTypes(\_PhpScoper0a2ac50786fa\Hoa\Compiler\Llk\TreeNode $ast) : array
    {
        ...
        foreach ($alternation->getChildren() as $child) {
            ...
            $matches = \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Nette\Utils\Strings::match($text, '#^([a-zA-Z0-9]+)[,]?\\s*#');
            if ($matches === null) {
                continue;
            }
            try {
            ...
            if ($type->describe(\_PhpScoper0a2ac50786fa\PHPStan\Type\VerbosityLevel::typeOnly()) !== $matches[1]) {
                continue;
            }
            if ($type instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\ObjectType) {
                continue;
            }
            $types[$type->describe(\_PhpScoper0a2ac50786fa\PHPStan\Type\VerbosityLevel::typeOnly())] = $text;
        }
        return $types;
    }
    ...
    private function getText(\_PhpScoper0a2ac50786fa\Hoa\Compiler\Llk\TreeNode $treeNode) : ?string
    {
    ...
    private function hasAnchorsInTheMiddle(\_PhpScoper0a2ac50786fa\Hoa\Compiler\Llk\TreeNode $ast) : bool
    {
        if ($ast->getId() === 'token') {
