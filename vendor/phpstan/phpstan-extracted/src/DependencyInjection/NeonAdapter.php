...
use _PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\Nette\DI\Config\Adapter;
...
use _PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\Nette\Neon\Entity;
...
class NeonAdapter implements \_PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\Nette\DI\Config\Adapter
{
    ...
    {
        $contents = \_PhpScoper0a6b37af0871\PHPStan\File\FileReader::read($file);
        try {
            return $this->process((array) \_PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\Nette\Neon\Neon::decode($contents), '', $file);
        } catch (\_PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\Nette\Neon\Exception $e) {
            throw new \_PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\Nette\Neon\Exception(\sprintf('Error while loading %s: %s', $file, $e->getMessage()));
        }
    }
    /**
     * @param mixed[] $arr
        ...
        foreach ($arr as $key => $val) {
            if (\is_string($key) && \substr($key, -1) === self::PREVENT_MERGING_SUFFIX) {
                if (!\is_array($val) && $val !== null) {
                    throw new \_PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\Nette\DI\InvalidConfigurationException(\sprintf('Replacing operator is available only for arrays, item \'%s\' is not array.', $key));
                }
                $key = \substr($key, 0, -1);
            ...
            if (\is_array($val)) {
                ...
                if ($val->value === \_PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\Nette\Neon\Neon::CHAIN) {
                    ...
                    $val = new \_PhpScoper0a6b37af0871\_HumbugBox221ad6f1b81f\Nette\DI\Definitions\Statement($tmp[0], $this->process($val->attributes, $fileKeyToPass, $file));
                }
            }
    ...
    private function createFileHelperByFile(string $file) : \_PhpScoper0a6b37af0871\PHPStan\File\FileHelper
    {
        $dir = \dirname($file);
        if (!isset($this->fileHelpers[$dir])) {
            $this->fileHelpers[$dir] = new \_PhpScoper0a6b37af0871\PHPStan\File\FileHelper($dir);
        }
