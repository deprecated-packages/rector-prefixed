...
class FuzzyRelativePathHelper implements \_PhpScoper2a4e7ab1ecbc\PHPStan\File\RelativePathHelper
{
     ...
     */
    public function __construct(\_PhpScoper2a4e7ab1ecbc\PHPStan\File\RelativePathHelper $fallbackRelativePathHelper, string $currentWorkingDirectory, array $analysedPaths, ?string $directorySeparator = null)
    {
        ...
        foreach ($analysedPaths as $pathNumber => $path) {
            ...
            foreach ($pathArray as $i => $pathPart) {
                if (\_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\Utils\Strings::endsWith($pathPart, '.php')) {
                    continue;
                }
