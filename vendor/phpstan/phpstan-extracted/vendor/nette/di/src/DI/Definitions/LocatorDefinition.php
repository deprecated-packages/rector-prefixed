 ...
 */
final class LocatorDefinition extends \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\DI\Definitions\Definition
{
    /** @var Reference[] */
    private $references = [];
    /** @var string|null */
    ...
    public function setImplement(string $type)
    {
        if (!\interface_exists($type)) {
            throw new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\InvalidArgumentException(\sprintf("Service '%s': Interface '%s' not found.", $this->getName(), $type));
        }
        $methods = (new \ReflectionClass($type))->getMethods();
        if (!$methods) {
            throw new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\InvalidArgumentException(\sprintf("Service '%s': Interface %s must have at least one method.", $this->getName(), $type));
        }
        foreach ($methods as $method) {
            if ($method->isStatic() || !(\preg_match('#^(get|create)$#', $method->name) && $method->getNumberOfParameters() === 1 || \preg_match('#^(get|create)[A-Z]#', $method->name) && $method->getNumberOfParameters() === 0)) {
                throw new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\InvalidArgumentException(\sprintf("Service '%s': Method %s::%s() does not meet the requirements: is create(\$name), get(\$name), create*() or get*() and is non-static.", $this->getName(), $type, $method->name));
            }
        }
    ...
    public function setReferences(array $references)
    {
        ...
        foreach ($references as $name => $ref) {
            $this->references[$name] = \substr($ref, 0, 1) === '@' ? new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\DI\Definitions\Reference(\substr($ref, 1)) : \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\DI\Definitions\Reference::fromType($ref);
        }
        return $this;
    }
    /** @return Reference[] */
    ...
    public function complete(\_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\DI\Resolver $resolver) : void
    {
        if ($this->tagged !== null) {
            ...
            foreach ($resolver->getContainerBuilder()->findByTag($this->tagged) as $name => $tag) {
                ...
                $this->references[$tag] = new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\DI\Definitions\Reference($name);
            }
        }
    ...
    public function generateMethod(\_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\PhpGenerator\Method $method, \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\DI\PhpGenerator $generator) : void
    {
        ...
        foreach ((new \ReflectionClass($this->getType()))->getMethods() as $rm) {
            ...
            $methodInner = $class->addMethod($rm->name)->setReturnType(\_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\Utils\Reflection::getReturnType($rm))->setReturnNullable($nullable);
            if (!$name) {
...
return $this->container->' . $m[1] . 'Service($this->mapping[$name]);')->addParameter('name');
                ...
                if ($m[1] === 'get') {
                    ...
                    $methodInner->setBody('return $this->container->?();', [\_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\DI\Container::getMethodName($ref)]);
                }
            } else {
