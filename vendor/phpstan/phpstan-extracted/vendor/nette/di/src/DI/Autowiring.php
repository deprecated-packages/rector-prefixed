...
class Autowiring
{
    ...
    public function getByType(string $type, bool $throw = \false) : ?string
    {
        ...
        if (empty($types[$type])) {
            if ($throw) {
                throw new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\DI\MissingServiceException("Service of type '{$type}' not found.");
            }
            return null;
            ...
            throw new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\DI\ServiceCreationException("Multiple services of type {$type} found: " . \implode(', ', $list) . $hint);
        }
    }
    /**
        ...
        foreach ($this->builder->getDefinitions() as $name => $def) {
            ...
            if (\is_array($autowired)) {
                foreach ($autowired as $k => $autowiredType) {
                    if ($autowiredType === \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\DI\ContainerBuilder::THIS_SERVICE) {
                        $autowired[$k] = $type;
                    } elseif (!\is_a($type, $autowiredType, \true)) {
                        throw new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\DI\ServiceCreationException("Incompatible class {$autowiredType} in autowiring definition of service '{$name}'.");
                    }
                }
            }
