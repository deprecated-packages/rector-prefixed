...
use _HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\ServiceCreationException;
use _HumbugBox221ad6f1b81f__UniqueRector\Nette\Utils\Reflection;
/**
 * Definition of standard service.
 */
final class FactoryDefinition extends \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Definitions\Definition
{
    ...
    {
        $this->resultDefinition = new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Definitions\ServiceDefinition();
    }
    /** @return static */
    public function setImplement(string $type)
    {
        if (!\interface_exists($type)) {
            throw new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\InvalidArgumentException("Service '{$this->getName()}': Interface '{$type}' not found.");
        }
        $rc = new \ReflectionClass($type);
        $method = $rc->getMethods()[0] ?? null;
        if (!$method || $method->isStatic() || $method->name !== self::METHOD_CREATE || \count($rc->getMethods()) > 1) {
            throw new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\InvalidArgumentException("Service '{$this->getName()}': Interface {$type} must have just one non-static method create().");
        }
    ...
    /** @return static */
    public function setResultDefinition(\_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Definitions\Definition $definition)
    {
        $this->resultDefinition = $definition;
        return $this;
    }
    /** @return ServiceDefinition */
    public function getResultDefinition() : \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Definitions\Definition
    {
    ...
    public function getFactory() : ?\_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Definitions\Statement
    {
    ...
    public function resolveType(\_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Resolver $resolver) : void
    {
        ...
        if (!$resultDef->getType()) {
            ...
            if (!$interface) {
                throw new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\ServiceCreationException('Type is missing in definition of service.');
            }
            $method = new \ReflectionMethod($interface, self::METHOD_CREATE);
            $returnType = \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Helpers::getReturnType($method);
            if (!$returnType) {
                throw new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\ServiceCreationException("Method {$interface}::create() has not return type hint or annotation @return.");
            } elseif (!\class_exists($returnType) && !\interface_exists($returnType)) {
                throw new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\ServiceCreationException("Check a type hint or annotation @return of the {$interface}::create() method, class '{$returnType}' cannot be found.");
            }
    ...
    {
        $resultDef = $this->resultDefinition;
        if ($resultDef instanceof \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Definitions\ServiceDefinition) {
            if (!$this->parameters) {
                $this->completeParameters($resolver);
            }
            if ($resultDef->getEntity() instanceof \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Definitions\Reference && !$resultDef->getFactory()->arguments) {
                $resultDef->setFactory([
                    // render as $container->createMethod()
                    new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Definitions\Reference(\_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\ContainerBuilder::THIS_CONTAINER),
                    \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Container::getMethodName($resultDef->getEntity()->getValue()),
                ]);
            }
        }
        $resolver->completeDefinition($resultDef);
    }
    private function completeParameters(\_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Resolver $resolver) : void
    {
        $interface = $this->getType();
        $method = new \ReflectionMethod($interface, self::METHOD_CREATE);
        $ctorParams = [];
        ...
        foreach ($method->getParameters() as $param) {
            $hint = \_HumbugBox221ad6f1b81f__UniqueRector\Nette\Utils\Reflection::getParameterType($param);
            if (isset($ctorParams[$param->name])) {
                $arg = $ctorParams[$param->name];
                $argHint = \_HumbugBox221ad6f1b81f__UniqueRector\Nette\Utils\Reflection::getParameterType($arg);
                if ($hint !== $argHint && !\is_a($hint, (string) $argHint, \true)) {
                    throw new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\ServiceCreationException("Type hint for \${$param->name} in {$interface}::create() doesn't match type hint in {$class} constructor.");
                }
                $this->resultDefinition->getFactory()->arguments[$arg->getPosition()] = \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\ContainerBuilder::literal('$' . $arg->name);
            } elseif (!$this->resultDefinition->getSetup()) {
                $hint = \_HumbugBox221ad6f1b81f__UniqueRector\Nette\Utils\Helpers::getSuggestion(\array_keys($ctorParams), $param->name);
                throw new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\ServiceCreationException("Unused parameter \${$param->name} when implementing method {$interface}::create()" . ($hint ? ", did you mean \${$hint}?" : '.'));
            }
            ...
            if ($param->isDefaultValueAvailable()) {
                $this->parameters[$paramDef] = \_HumbugBox221ad6f1b81f__UniqueRector\Nette\Utils\Reflection::getParameterDefaultValue($param);
            } else {
    ...
    {
        $class = (new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\PhpGenerator\ClassType())->addImplement($this->getType());
        ...
        $methodCreate->setParameters($generator->convertParameters($this->parameters))->setReturnType(\_HumbugBox221ad6f1b81f__UniqueRector\Nette\Utils\Reflection::getReturnType($rm) ?: $this->getResultType())->setBody($body);
