...
class ContainerLoader
{
    ...
    private function loadFile(string $class, callable $generator) : void
    {
        ...
        \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Utils\FileSystem::createDir($this->tempDirectory);
        $handle = @\fopen("{$file}.lock", 'c+');
        // @ is escalated to exception
        if (!$handle) {
            throw new \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\IOException("Unable to create file '{$file}.lock'. " . \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Utils\Helpers::getLastError());
        } elseif (!@\flock($handle, \LOCK_EX)) {
            // @ is escalated to exception
            throw new \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\IOException("Unable to acquire exclusive lock on '{$file}.lock'. " . \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\Utils\Helpers::getLastError());
        }
        if (!\is_file($file) || $this->isExpired($file, $updatedMeta)) {
            ...
            foreach ($toWrite as $name => $content) {
                if (\file_put_contents("{$name}.tmp", $content) !== \strlen($content) || !\rename("{$name}.tmp", $name)) {
                    @\unlink("{$name}.tmp");
                    // @ - file may not exist
                    throw new \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\IOException("Unable to create file '{$name}'.");
        ...
        if (@(include $file) === \false) {
            ...
            throw new \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\IOException("Unable to include '{$file}'.");
        }
    ...
    private function isExpired(string $file, string &$updatedMeta = null) : bool
    {
        if ($this->autoRebuild) {
            ...
            // @ - file may not exist
            $orig = $meta[2] ?? null;
            return empty($meta[0]) || \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\DI\DependencyChecker::isExpired(...$meta) || $orig !== $meta[2] && ($updatedMeta = \serialize($meta));
        }
        return \false;
    }
    ...
    {
        $compiler = new \_PhpScoperb75b35f52b74\_HumbugBox221ad6f1b81f\Nette\DI\Compiler();
