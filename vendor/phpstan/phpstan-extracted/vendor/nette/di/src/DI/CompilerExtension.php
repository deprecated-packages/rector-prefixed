...
abstract class CompilerExtension
{
    ...
    /** @return static */
    public function setCompiler(\_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\DI\Compiler $compiler, string $name)
    {
        $this->initialization = new \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\PhpGenerator\Closure();
    ...
    {
        return \is_object($this->config) ? \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\Schema\Expect::from($this->config) : \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\Schema\Expect::array();
    }
    /**
    ...
    public function validateConfig(array $expected, array $config = null, string $name = null) : array
    {
        ...
        if ($extra = \array_diff_key((array) $config, $expected)) {
            ...
            $hint = \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\Utils\Helpers::getSuggestion(\array_keys($expected), \key($extra));
            $extra = $hint ? \key($extra) : \implode("', '{$name} › ", \array_keys($extra));
            throw new \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\DI\InvalidConfigurationException("Unknown configuration option '{$name} › {$extra}'" . ($hint ? ", did you mean '{$name} › {$hint}'?" : '.'));
        }
        return \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\Schema\Helpers::merge($config, $expected);
    }
    ...
    public function loadDefinitionsFromConfig(array $configList) : void
    {
        ...
        foreach ($configList as $key => $config) {
            ...
            $res[$key] = \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\DI\Helpers::prefixServiceName($config, $this->name);
        }
    ...
    public function getInitialization() : \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\Nette\PhpGenerator\Closure
    {
