...
namespace _HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Config\Adapters;
...
use _HumbugBox221ad6f1b81f__UniqueRector\Nette\Neon;
/**
 * Reading and generating NEON files.
 */
final class NeonAdapter implements \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Config\Adapter
{
    ...
    {
        return $this->process((array) \_HumbugBox221ad6f1b81f__UniqueRector\Nette\Neon\Neon::decode(\_HumbugBox221ad6f1b81f__UniqueRector\Nette\Utils\FileSystem::read($file)));
    }
        ...
        foreach ($arr as $key => $val) {
            if (\is_string($key) && \substr($key, -1) === self::PREVENT_MERGING_SUFFIX) {
                if (!\is_array($val) && $val !== null) {
                    throw new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\InvalidConfigurationException("Replacing operator is available only for arrays, item '{$key}' is not array.");
                }
                $key = \substr($key, 0, -1);
            ...
            if (\is_array($val)) {
                ...
                if ($val->value === \_HumbugBox221ad6f1b81f__UniqueRector\Nette\Neon\Neon::CHAIN) {
                    ...
                    $val = new \_HumbugBox221ad6f1b81f__UniqueRector\Nette\DI\Definitions\Statement($tmp[0], $this->process($val->attributes));
                }
            }
            $res[$key] = $val;
        }
