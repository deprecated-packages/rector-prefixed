...
use _PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\InvalidStateException;
...
final class PhpNamespace
{
    ...
    {
        if ($name !== '' && !\_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\PhpGenerator\Helpers::isNamespaceIdentifier($name)) {
    ...
    {
        $name = \ltrim($name, '\\');
        if ($alias === null && $this->name === \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\PhpGenerator\Helpers::extractNamespace($name)) {
            $alias = \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\PhpGenerator\Helpers::extractShortName($name);
        }
        if ($alias === null) {
            $path = \explode('\\', $name);
            $counter = null;
            ...
            throw new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\InvalidStateException("Alias '{$alias}' used already for '{$this->uses[$alias]}', cannot use for '{$name}'.");
        }
        $aliasOut = $alias;
    ...
    public function unresolveName(string $name) : string
    {
        ...
        foreach ($this->uses as $alias => $original) {
            if (\_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\Utils\Strings::startsWith($lower . '\\', \strtolower($original) . '\\')) {
        ...
        if (!$res && \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\Utils\Strings::startsWith($lower, \strtolower($this->name) . '\\')) {
    ...
    /** @return static */
    public function add(\_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\PhpGenerator\ClassType $class) : self
    {
        $name = $class->getName();
        if ($name === null) {
            throw new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\InvalidArgumentException('Class does not have a name.');
        }
    ...
    {
        $this->add($class = new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\PhpGenerator\ClassType($name, $this));
        return $class;
    }
    ...
    {
        try {
            return (new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\Nette\PhpGenerator\Printer())->printNamespace($this);
        } catch (\Throwable $e) {
