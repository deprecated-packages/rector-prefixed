...
final class Structure implements \_HumbugBox221ad6f1b81f\Nette\Schema\Schema
{
    use Base;
    ...
    {
        (function (\_HumbugBox221ad6f1b81f\Nette\Schema\Schema ...$items) {
        })(...\array_values($items));
        $this->items = $items;
        $this->castTo = 'object';
    }
    ...
    {
        throw new \_HumbugBox221ad6f1b81f\Nette\InvalidStateException('Structure cannot have default value.');
    }
    ...
    {
        $this->otherItems = $type instanceof \_HumbugBox221ad6f1b81f\Nette\Schema\Schema ? $type : new \_HumbugBox221ad6f1b81f\Nette\Schema\Elements\Type($type);
        return $this;
    }
    /********************* processing ****************d*g**/
        ...
        if (\is_array($value) && \is_array($base)) {
            ...
            foreach ($value as $key => $val) {
                if ($key === $index) {
                    ...
                    $itemSchema = $this->items[$key] ?? $this->otherItems;
                    $base[$key] = $itemSchema ? $itemSchema->merge($val, $base[$key]) : \_HumbugBox221ad6f1b81f\Nette\Schema\Helpers::merge($val, $base[$key]);
                } else {
                    $base[$key] = $val;
                }
            }
    ...
    public function complete($value, \_HumbugBox221ad6f1b81f\Nette\Schema\Context $context)
    {
        ...
        if ($extraKeys = \array_keys(\array_diff_key($value, $items))) {
            if ($this->otherItems) {
                ...
                $hint = \_HumbugBox221ad6f1b81f\Nette\Utils\Helpers::getSuggestion(\array_map('strval', \array_keys($items)), (string) $extraKeys[0]);
