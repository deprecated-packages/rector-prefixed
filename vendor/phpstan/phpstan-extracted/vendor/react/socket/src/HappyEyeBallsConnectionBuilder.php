...
use _HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message;
use _HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Resolver\ResolverInterface;
use _HumbugBox221ad6f1b81f__UniqueRector\React\EventLoop\LoopInterface;
use _HumbugBox221ad6f1b81f__UniqueRector\React\EventLoop\TimerInterface;
...
final class HappyEyeBallsConnectionBuilder
{
    ...
    public $uri;
    public $host;
    public $resolved = array(\_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_A => \false, \_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_AAAA => \false);
    ...
    public $lastError6;
    public $lastError4;
    public function __construct(\_HumbugBox221ad6f1b81f__UniqueRector\React\EventLoop\LoopInterface $loop, \_HumbugBox221ad6f1b81f__UniqueRector\React\Socket\ConnectorInterface $connector, \_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Resolver\ResolverInterface $resolver, $uri, $host, $parts)
    {
        $this->loop = $loop;
    ...
    public function connect()
    {
        $timer = null;
        $that = $this;
        return new \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Promise(function ($resolve, $reject) use($that, &$timer) {
            ...
            $that->resolverPromises[\_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_AAAA] = $that->resolve(\_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_AAAA, $reject)->then($lookupResolve(\_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_AAAA));
            $that->resolverPromises[\_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_A] = $that->resolve(\_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_A, $reject)->then(function (array $ips) use($that, &$timer) {
                // happy path: IPv6 has resolved already, continue with IPv4 addresses
                if ($that->resolved[\_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_AAAA] === \true) {
                    return $ips;
                }
                // Otherwise delay processing IPv4 lookup until short timer passes or IPv6 resolves in the meantime
                $deferred = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Deferred();
                $timer = $that->loop->addTimer($that::RESOLUTION_DELAY, function () use($deferred, $ips) {
                    $deferred->resolve($ips);
                });
                $that->resolverPromises[\_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_AAAA]->then(function () use($that, $timer, $deferred, $ips) {
                    $that->loop->cancelTimer($timer);
                    $deferred->resolve($ips);
                });
                return $deferred->promise();
            })->then($lookupResolve(\_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_A));
            ...
            $_ = $reject = null;
            $that->cleanUp();
            if ($timer instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\EventLoop\TimerInterface) {
                $that->loop->cancelTimer($timer);
            }
        });
    }
    ...
    public function resolve($type, $reject)
    {
        ...
        return $that->resolver->resolveAll($that->host, $type)->then(null, function (\Exception $e) use($type, $reject, $that) {
            unset($that->resolverPromises[$type]);
            $that->resolved[$type] = \true;
            if ($type === \_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_A) {
    ...
    public function check($resolve, $reject)
    {
        ...
        if ($this->nextAttemptTimer === null && (\count($this->connectQueue) > 0 || $this->resolved[\_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_A] === \false || $this->resolved[\_HumbugBox221ad6f1b81f__UniqueRector\React\Dns\Model\Message::TYPE_AAAA] === \false)) {
    ...
    public function cleanUp()
    {
        ...
        foreach ($this->connectionPromises as $connectionPromise) {
            if ($connectionPromise instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\CancellablePromiseInterface) {
                $connectionPromise->cancel();
            }
        }
        foreach ($this->resolverPromises as $resolverPromise) {
            if ($resolverPromise instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\CancellablePromiseInterface) {
                $resolverPromise->cancel();
            }
        }
        if ($this->nextAttemptTimer instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\EventLoop\TimerInterface) {
