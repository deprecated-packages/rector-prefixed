...
final class TcpConnector implements \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\React\Socket\ConnectorInterface
{
    private $loop;
    private $context;
    public function __construct(\_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\React\EventLoop\LoopInterface $loop, array $context = array())
    {
        $this->loop = $loop;
    ...
    public function connect($uri)
    {
        ...
        if (\false === \filter_var($ip, \FILTER_VALIDATE_IP)) {
            return \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\React\Promise\reject(new \InvalidArgumentException('Given URI "' . $ip . '" does not contain a valid host IP'));
        }
        ...
        if (\false === $stream) {
            return \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\React\Promise\reject(new \RuntimeException(\sprintf("Connection to %s failed: %s", $uri, $errstr), $errno));
        }
        // wait for connection
        $loop = $this->loop;
        return new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\React\Promise\Promise(function ($resolve, $reject) use($loop, $stream, $uri) {
            $loop->addWriteStream($stream, function ($stream) use($loop, $resolve, $reject, $uri) {
                ...
                if (\false === \stream_socket_get_name($stream, \true)) {
                    ...
                    $resolve(new \_PhpScopere8e811afab72\_HumbugBox221ad6f1b81f\React\Socket\Connection($stream, $loop));
                }
            });
