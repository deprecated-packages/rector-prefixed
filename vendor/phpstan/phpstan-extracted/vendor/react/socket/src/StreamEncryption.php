...
class StreamEncryption
{
    ...
    private $method;
    private $server;
    public function __construct(\_HumbugBox221ad6f1b81f__UniqueRector\React\EventLoop\LoopInterface $loop, $server = \true)
    {
        $this->loop = $loop;
        $this->server = $server;
    ...
    public function enable(\_HumbugBox221ad6f1b81f__UniqueRector\React\Socket\Connection $stream)
    {
        return $this->toggle($stream, \true);
    }
    public function toggle(\_HumbugBox221ad6f1b81f__UniqueRector\React\Socket\Connection $stream, $toggle)
    {
        // pause actual stream instance to continue operation on raw stream socket
        $stream->pause();
        // TODO: add write() event to make sure we're not sending any excessive data
        // cancelling this leaves this stream in an inconsistent state…
        $deferred = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Deferred(function () {
    ...
    public function toggleCrypto($socket, \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Deferred $deferred, $toggle, $method)
    {
        $error = null;
