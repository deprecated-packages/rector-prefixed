 ...
 */
function buffer(\_HumbugBox221ad6f1b81f__UniqueRector\React\Stream\ReadableStreamInterface $stream, $maxLength = null)
{
    // stream already ended => resolve with empty buffer
    if (!$stream->isReadable()) {
        return \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\resolve('');
    }
    $buffer = '';
    $promise = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Promise(function ($resolve, $reject) use($stream, $maxLength, &$buffer, &$bufferer) {
 ...
 */
function first(\_HumbugBox221ad6f1b81f__UniqueRector\Evenement\EventEmitterInterface $stream, $event = 'data')
{
    if ($stream instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Stream\ReadableStreamInterface) {
        ...
        if (!$stream->isReadable()) {
            return \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\reject(new \RuntimeException('Stream already closed'));
        }
    } elseif ($stream instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Stream\WritableStreamInterface) {
        // writable-only stream (not duplex) not writable => already closed
        if (!$stream->isWritable()) {
            return \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\reject(new \RuntimeException('Stream already closed'));
        }
    }
    return new \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Promise(function ($resolve, $reject) use($stream, $event, &$listener) {
 ...
 */
function all(\_HumbugBox221ad6f1b81f__UniqueRector\Evenement\EventEmitterInterface $stream, $event = 'data')
{
    // stream already ended => resolve with empty buffer
    if ($stream instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Stream\ReadableStreamInterface) {
        // readable or duplex stream not readable => already closed
        // a half-open duplex stream is considered closed if its readable side is closed
        if (!$stream->isReadable()) {
            return \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\resolve(array());
        }
    } elseif ($stream instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Stream\WritableStreamInterface) {
        // writable-only stream (not duplex) not writable => already closed
        if (!$stream->isWritable()) {
    ...
    $promise = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Promise(function ($resolve, $reject) use($stream, &$buffer) {
        $stream->on('error', function ($error) use($reject) {
            $reject(new \RuntimeException('An error occured on the underlying stream while buffering', 0, $error));
        });
        $stream->on('close', function () use($resolve, &$buffer) {
 ...
 */
function unwrapReadable(\_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\PromiseInterface $promise)
{
    return new \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Stream\UnwrapReadableStream($promise);
}
/**
 ...
 */
function unwrapWritable(\_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\PromiseInterface $promise)
{
    return new \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Stream\UnwrapWritableStream($promise);
}
