 ...
 */
class TcpTransportExecutor implements \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
    ...
    public function query(\_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\React\Dns\Query\Query $query)
    {
        ...
        if ($length > 0xffff) {
            return \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->name . ' failed: Query too large for TCP transport'));
        }
        ...
        $names =& $this->names;
        $that = $this;
        $deferred = new \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\React\Promise\Deferred(function () use($that, &$names, $request) {
            // remove from list of pending names, but remember pending query
            $name = $names[$request->id];
            unset($names[$request->id]);
            $that->checkIdle();
            throw new \_PhpScoper2a4e7ab1ecbc\_HumbugBox221ad6f1b81f\React\Dns\Query\CancellationException('DNS query for ' . $name . ' has been cancelled');
        });
