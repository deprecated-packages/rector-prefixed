...
use _PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Dns\Protocol\BinaryDumper;
use _PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Dns\Protocol\Parser;
 ...
 */
final class UdpTransportExecutor implements \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
     ...
     */
    public function __construct($nameserver, \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\EventLoop\LoopInterface $loop)
    {
        ...
        $this->loop = $loop;
        $this->parser = new \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Dns\Protocol\Parser();
        $this->dumper = new \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Dns\Protocol\BinaryDumper();
    }
    public function query(\_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Dns\Query\Query $query)
    {
        $request = \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Dns\Model\Message::createRequestForQuery($query);
        $queryData = $this->dumper->toBinary($request);
        if (isset($queryData[512])) {
            return \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->name . ' failed: Query too large for UDP transport', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        ...
        if ($socket === \false) {
            return \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->name . ' failed: Unable to connect to DNS server (' . $errstr . ')', $errno));
        }
        // set socket to non-blocking and immediately try to send (fill write buffer)
        \stream_set_blocking($socket, \false);
        \fwrite($socket, $queryData);
        $loop = $this->loop;
        $deferred = new \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Promise\Deferred(function () use($loop, $socket, $query) {
            // cancellation should remove socket from loop and close socket
            $loop->removeReadStream($socket);
            \fclose($socket);
            throw new \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\React\Dns\Query\CancellationException('DNS query for ' . $query->name . ' has been cancelled');
        });
        $parser = $this->parser;
