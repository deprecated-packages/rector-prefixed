...
use _HumbugBox221ad6f1b81f__UniqueRector\React\Http\Message\ResponseException;
...
class Transaction
{
    ...
    // 16 MiB = 2^24 bytes
    public function __construct(\_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\Sender $sender, \_HumbugBox221ad6f1b81f__UniqueRector\React\EventLoop\LoopInterface $loop)
    {
        $this->sender = $sender;
        $this->loop = $loop;
    }
    ...
    {
        $deferred = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Deferred(function () use(&$deferred) {
        ...
        $loop = $this->loop;
        $this->next($request, $deferred)->then(function (\_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ResponseInterface $response) use($deferred, $loop, &$timeout) {
        ...
        if ($body instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Stream\ReadableStreamInterface && $body->isReadable()) {
            $that = $this;
     ...
     * @return void
     */
    public function applyTimeout(\_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Deferred $deferred, $timeout)
    {
    ...
    private function next(\_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\RequestInterface $request, \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Deferred $deferred)
    {
        ...
        return $promise->then(function (\_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ResponseInterface $response) use($request, $that, $deferred) {
     ...
     */
    public function bufferResponse(\_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ResponseInterface $response, $deferred)
    {
        $stream = $response->getBody();
        $size = $stream->getSize();
        if ($size !== null && $size > $this->maximumSize) {
            $stream->close();
            return \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\reject(new \OverflowException('Response body size of ' . $size . ' bytes exceeds maximum of ' . $this->maximumSize . ' bytes', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 0));
        }
        // body is not streaming => already buffered
        if (!$stream instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Stream\ReadableStreamInterface) {
            return \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\resolve($response);
        }
        // buffer stream and resolve with buffered body
        $maximumSize = $this->maximumSize;
        $promise = \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Stream\buffer($stream, $maximumSize)->then(function ($body) use($response) {
            return $response->withBody(\_HumbugBox221ad6f1b81f__UniqueRector\RingCentral\Psr7\stream_for($body));
     ...
     */
    public function onResponse(\_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ResponseInterface $response, \_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\RequestInterface $request, $deferred)
    {
        ...
        if ($this->obeySuccessCode && ($response->getStatusCode() < 200 || $response->getStatusCode() >= 400)) {
            throw new \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Message\ResponseException($response);
        }
     ...
     */
    private function onResponseRedirect(\_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ResponseInterface $response, \_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\RequestInterface $request, \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Deferred $deferred)
    {
        // resolve location relative to last request URI
        $location = \_HumbugBox221ad6f1b81f__UniqueRector\RingCentral\Psr7\Uri::resolve($request->getUri(), $response->getHeaderLine('Location'));
     ...
     */
    private function makeRedirectRequest(\_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\RequestInterface $request, \_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\UriInterface $location)
    {
        ...
        return new \_HumbugBox221ad6f1b81f__UniqueRector\RingCentral\Psr7\Request($method, $location, $request->getHeaders());
    }
    private function progress($name, array $args = array())
    {
        ...
        foreach ($args as $arg) {
            echo ' ';
            if ($arg instanceof \_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ResponseInterface) {
                echo 'HTTP/' . $arg->getProtocolVersion() . ' ' . $arg->getStatusCode() . ' ' . $arg->getReasonPhrase();
            } elseif ($arg instanceof \_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\RequestInterface) {
