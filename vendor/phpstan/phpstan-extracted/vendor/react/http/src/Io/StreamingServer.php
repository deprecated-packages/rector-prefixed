...
use _HumbugBox221ad6f1b81f__UniqueRector\React\Http\Message\Response;
use _HumbugBox221ad6f1b81f__UniqueRector\React\Http\Message\ServerRequest;
use _HumbugBox221ad6f1b81f__UniqueRector\React\Promise;
...
use _HumbugBox221ad6f1b81f__UniqueRector\React\Socket\ServerInterface;
 ...
 * @internal
 */
final class StreamingServer extends \_HumbugBox221ad6f1b81f__UniqueRector\Evenement\EventEmitter
{
    private $callback;
    private $parser;
    private $loop;
     ...
     * @see self::listen()
     */
    public function __construct(\_HumbugBox221ad6f1b81f__UniqueRector\React\EventLoop\LoopInterface $loop, $requestHandler)
    {
        ...
        $this->parser = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\RequestHeaderParser();
        $that = $this;
        $this->parser->on('headers', function (\_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ServerRequestInterface $request, \_HumbugBox221ad6f1b81f__UniqueRector\React\Socket\ConnectionInterface $conn) use($that) {
            $that->handleRequest($conn, $request);
        });
        $this->parser->on('error', function (\Exception $e, \_HumbugBox221ad6f1b81f__UniqueRector\React\Socket\ConnectionInterface $conn) use($that) {
            $that->emit('error', array($e));
            // parsing failed => assume dummy request and send appropriate error
            $that->writeError($conn, $e->getCode() !== 0 ? $e->getCode() : 400, new \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Message\ServerRequest('GET', '/'));
        });
    }
    /**
     ...
     */
    public function listen(\_HumbugBox221ad6f1b81f__UniqueRector\React\Socket\ServerInterface $socket)
    {
        $socket->on('connection', array($this->parser, 'handle'));
    }
    /** @internal */
    public function handleRequest(\_HumbugBox221ad6f1b81f__UniqueRector\React\Socket\ConnectionInterface $conn, \_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ServerRequestInterface $request)
    {
        ...
        try {
            ...
            $response = \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\reject($error);
        ...
        if ($response instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\CancellablePromiseInterface) {
            $conn->on('close', function () use($response) {
                $response->cancel();
            });
        }
        // happy path: response returned, handle and return immediately
        if ($response instanceof \_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ResponseInterface) {
        ...
        if (!$response instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\PromiseInterface) {
            $response = \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\resolve($response);
        }
        $that = $this;
    ...
    /** @internal */
    public function writeError(\_HumbugBox221ad6f1b81f__UniqueRector\React\Socket\ConnectionInterface $conn, $code, \_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ServerRequestInterface $request)
    {
        $response = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Message\Response($code, array('Content-Type' => 'text/plain'), 'Error ' . $code);
    ...
    /** @internal */
    public function handleResponse(\_HumbugBox221ad6f1b81f__UniqueRector\React\Socket\ConnectionInterface $connection, \_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ServerRequestInterface $request, \_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ResponseInterface $response)
    {
        ...
        if (($code === 101 || $method === 'CONNECT' && $code >= 200 && $code < 300) && $body instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\HttpBodyStream && $body->input instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Stream\WritableStreamInterface) {
        ...
        if (!$body instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Stream\ReadableStreamInterface || !$body->isReadable()) {
            ...
            if ($body instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Stream\ReadableStreamInterface && $chunked) {
                $body = "0\r\n\r\n";
            }
