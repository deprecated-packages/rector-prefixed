 ...
 * @internal
 */
class RequestHeaderParser extends \_HumbugBox221ad6f1b81f\Evenement\EventEmitter
{
    private $maxSize = 8192;
    public function handle(\_HumbugBox221ad6f1b81f\React\Socket\ConnectionInterface $conn)
    {
        $buffer = '';
        ...
        $conn->on('data', $fn = function ($data) use(&$buffer, &$fn, $conn, $maxSize, $that) {
            ...
            if ($contentLength === 0) {
                ...
                $stream = new \_HumbugBox221ad6f1b81f\React\Http\Io\EmptyBodyStream();
                $request = $request->withBody($stream);
            } else {
                // otherwise body is present => delimit using Content-Length or ChunkedDecoder
                $stream = new \_HumbugBox221ad6f1b81f\React\Http\Io\CloseProtectionStream($conn);
                if ($contentLength !== null) {
                    $stream = new \_HumbugBox221ad6f1b81f\React\Http\Io\LengthLimitedStream($stream, $contentLength);
                } else {
                    $stream = new \_HumbugBox221ad6f1b81f\React\Http\Io\ChunkedDecoder($stream);
                }
                $request = $request->withBody(new \_HumbugBox221ad6f1b81f\React\Http\Io\HttpBodyStream($stream, $contentLength));
            }
    ...
    public function parseRequest($headers, $remoteSocketUri, $localSocketUri)
    {
        ...
        $request = new \_HumbugBox221ad6f1b81f\React\Http\Message\ServerRequest($start['method'], $uri, $fields, '', $start['version'], $serverParams);
