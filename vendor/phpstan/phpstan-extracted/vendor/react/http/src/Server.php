...
use _HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\MiddlewareRunner;
use _HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\StreamingServer;
use _HumbugBox221ad6f1b81f__UniqueRector\React\Http\Middleware\LimitConcurrentRequestsMiddleware;
use _HumbugBox221ad6f1b81f__UniqueRector\React\Http\Middleware\StreamingRequestMiddleware;
use _HumbugBox221ad6f1b81f__UniqueRector\React\Http\Middleware\RequestBodyBufferMiddleware;
use _HumbugBox221ad6f1b81f__UniqueRector\React\Http\Middleware\RequestBodyParserMiddleware;
 ...
 */
final class Server extends \_HumbugBox221ad6f1b81f__UniqueRector\Evenement\EventEmitter
{
    /**
     ...
     * @see self::listen()
     */
    public function __construct(\_HumbugBox221ad6f1b81f__UniqueRector\React\EventLoop\LoopInterface $loop)
    {
        ...
        foreach ((array) $requestHandlers as $handler) {
            if ($handler instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Middleware\StreamingRequestMiddleware) {
                $streaming = \true;
                break;
            }
        }
        ...
        if (!$streaming) {
            ...
            if ($concurrency !== null) {
                $middleware[] = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Middleware\LimitConcurrentRequestsMiddleware($concurrency);
            }
            $middleware[] = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Middleware\RequestBodyBufferMiddleware($maxSize);
            ...
            if ($enablePostDataReading !== '') {
                $middleware[] = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Middleware\RequestBodyParserMiddleware();
            }
        }
        $middleware = \array_merge($middleware, $requestHandlers);
        $this->streamingServer = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\StreamingServer($loop, new \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\MiddlewareRunner($middleware));
        $that = $this;
    ...
    private function getConcurrentRequestsLimit($memory_limit, $post_max_size)
    {
        ...
        $availableMemory = \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\IniUtil::iniSizeToBytes($memory_limit) / 2;
        $concurrentRequests = (int) \ceil($availableMemory / \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\IniUtil::iniSizeToBytes($post_max_size));
    ...
    {
        $maxSize = \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\IniUtil::iniSizeToBytes($post_max_size === null ? \ini_get('post_max_size') : $post_max_size);
