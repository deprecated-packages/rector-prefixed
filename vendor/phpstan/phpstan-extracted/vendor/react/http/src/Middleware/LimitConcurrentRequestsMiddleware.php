...
use _HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\PauseBufferStream;
...
final class LimitConcurrentRequestsMiddleware
{
    ...
    public function __invoke(\_HumbugBox221ad6f1b81f__UniqueRector\Psr\Http\Message\ServerRequestInterface $request, $next)
    {
        ...
        if ($this->pending < $this->limit) {
            ...
            return $this->await(\_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\resolve($response));
        }
        ...
        if ($body instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Stream\ReadableStreamInterface) {
            ...
            $size = $body->getSize();
            $body = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\PauseBufferStream($body);
            $body->pauseImplicit();
            // replace with buffering body to ensure any readable events will be buffered
            $request = $request->withBody(new \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\HttpBodyStream($body, $size));
        }
        ...
        \end($queue);
        $id = \key($queue);
        $deferred = new \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\Deferred(function ($_, $reject) use(&$queue, $id) {
        ...
        return $deferred->promise()->then(function () use($request, $next, $body, &$pending, $that) {
            ...
            if ($body instanceof \_HumbugBox221ad6f1b81f__UniqueRector\React\Http\Io\PauseBufferStream) {
                $body->resumeImplicit();
            }
            // if the next handler returns a pending promise, we have to
            // await its resolution before invoking next queued request
            return $that->await(\_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\resolve($response));
        });
    }
    /**
     * @internal
     ...
     */
    public function await(\_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\PromiseInterface $promise)
    {
        $that = $this;
        return $promise->then(function ($response) use($that) {
            ...
            $that->processQueue();
            return \_HumbugBox221ad6f1b81f__UniqueRector\React\Promise\reject($error);
        });
    }
    /**
     * @internal
