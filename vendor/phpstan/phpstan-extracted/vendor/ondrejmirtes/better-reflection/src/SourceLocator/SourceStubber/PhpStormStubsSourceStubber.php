...
use Error;
use _PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\JetBrains\PHPStormStub\PhpStormStubsMap;
use ParseError;
...
use _PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Roave\BetterReflection\SourceLocator\SourceStubber\Exception\CouldNotFindPhpStormStubs;
 ...
 * @internal
 */
final class PhpStormStubsSourceStubber implements \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Roave\BetterReflection\SourceLocator\SourceStubber\SourceStubber
{
    ...
    /** @var array<lowercase-string, string> */
    private $constantMap;
    public function __construct(\_PhpScoper0a2ac50786fa\PhpParser\Parser $phpParser, ?int $phpVersionId = null)
    {
        ...
        $this->nodeTraverser = new \_PhpScoper0a2ac50786fa\PhpParser\NodeTraverser();
        $this->nodeTraverser->addVisitor(new \_PhpScoper0a2ac50786fa\PhpParser\NodeVisitor\NameResolver());
        $this->nodeTraverser->addVisitor($this->cachingVisitor);
        $this->classMap = \array_change_key_case(\_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\JetBrains\PHPStormStub\PhpStormStubsMap::CLASSES);
        $this->functionMap = \array_change_key_case(\_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\JetBrains\PHPStormStub\PhpStormStubsMap::FUNCTIONS);
        $this->constantMap = \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\JetBrains\PHPStormStub\PhpStormStubsMap::CONSTANTS;
    }
    ...
    public function generateClassStub(string $className) : ?\_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Roave\BetterReflection\SourceLocator\SourceStubber\StubData
    {
        ...
        if ($classNode instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\Class_) {
            if ($classNode->name instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Identifier && $classNode->name->toString() === \ParseError::class && $this->phpVersionId < 70300) {
                $classNode->extends = new \_PhpScoper0a2ac50786fa\PhpParser\Node\Name\FullyQualified(\Error::class);
        ...
        } elseif ($classNode instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\Interface_) {
        ...
        return new \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Roave\BetterReflection\SourceLocator\SourceStubber\StubData($stub, $this->getExtensionFromFilePath($filePath), $this->getAbsoluteFilePath($filePath));
    }
    private function findClassNode(string $filePath, string $lowercaseName) : ?\_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\ClassLike
    {
    ...
    public function generateFunctionStub(string $functionName) : ?\_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Roave\BetterReflection\SourceLocator\SourceStubber\StubData
    {
        ...
        return new \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Roave\BetterReflection\SourceLocator\SourceStubber\StubData($this->createStub($functionNode), $this->getExtensionFromFilePath($filePath));
    }
    private function findFunctionNode(string $filePath, string $lowercaseFunctionName) : ?\_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\Function_
    {
    ...
    public function generateConstantStub(string $constantName) : ?\_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Roave\BetterReflection\SourceLocator\SourceStubber\StubData
    {
        ...
        return new \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Roave\BetterReflection\SourceLocator\SourceStubber\StubData($this->createStub($constantNode), $this->getExtensionFromFilePath($filePath));
    }
    private function parseFile(string $filePath) : void
    {
        $absoluteFilePath = $this->getAbsoluteFilePath($filePath);
        \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Roave\BetterReflection\SourceLocator\FileChecker::assertReadableFile($absoluteFilePath);
        ...
        foreach ($this->cachingVisitor->getClassNodes() as $className => $classNode) {
            ...
            \assert($classNode instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\ClassLike);
        ...
        foreach ($this->cachingVisitor->getFunctionNodes() as $functionName => $functionNode) {
            ...
            \assert($functionNode instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\Function_);
        ...
        foreach ($this->cachingVisitor->getConstantNodes() as $constantName => $constantNode) {
            ...
            \assert($constantNode instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\Const_ || $constantNode instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\FuncCall);
    ...
    private function hasLaterSinceVersion(\_PhpScoper0a2ac50786fa\PhpParser\Node $node) : bool
    {
     ...
     */
    private function createStub(\_PhpScoper0a2ac50786fa\PhpParser\Node $node) : string
    {
        $nodeWithNamespaceName = $node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\Const_ ? $node->consts[0] : $node;
        ...
        return "<?php\n\n" . $printed . ($node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\FuncCall ? ';' : '') . "\n";
    }
    private function createCachingVisitor() : \_PhpScoper0a2ac50786fa\PhpParser\NodeVisitorAbstract
    {
        return new class extends \_PhpScoper0a2ac50786fa\PhpParser\NodeVisitorAbstract
        {
            ...
            public function enterNode(\_PhpScoper0a2ac50786fa\PhpParser\Node $node) : ?int
            {
                ...
                if ($node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\FuncCall) {
                    ...
                    \assert($nameNode instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Scalar\String_);
                    ...
                    if (\count($node->args) === 3 && $node->args[2]->value instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\ConstFetch && $node->args[2]->value->name->toLowerString() === 'true') {
    ...
    private function getStubsDirectory() : string
    {
        ...
        throw \_PhpScoper0a2ac50786fa\_HumbugBox221ad6f1b81f\Roave\BetterReflection\SourceLocator\SourceStubber\Exception\CouldNotFindPhpStormStubs::create();
    }
}
