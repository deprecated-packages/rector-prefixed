...
class ShouldCallParentMethodsRule implements \PHPStan\Rules\Rule
{
    ...
    public function processNode(\PhpParser\Node $node, \PHPStan\Analyser\Scope $scope) : array
    {
        ...
        if (!$scope->getClassReflection()->isSubclassOf(\RectorPrefix2020DecSat\PHPUnit\Framework\TestCase::class)) {
            return [];
        }
        ...
        if ($parentMethod->getDeclaringClass()->getName() === \RectorPrefix2020DecSat\PHPUnit\Framework\TestCase::class) {
            return [];
        }
