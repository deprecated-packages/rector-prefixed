...
class PostGenerator
{
    private \RectorPrefix20210504\SebastianBergmann\Diff\Differ $differ;
    private string $latestCommit;
    public function __construct(\RectorPrefix20210504\SebastianBergmann\Diff\Differ $differ, string $latestCommit)
    {
        $this->differ = $differ;
     ...
     * @return string|null
     */
    public function createText(\RectorPrefix20210504\App\Playground\PlaygroundResult $result, array $botComments) : ?string
    {
        $maxDigit = 1;
        ...
        $text = \sprintf("%s After [the latest commit in dev-master](https://github.com/phpstan/phpstan-src/commit/%s), PHPStan now reports different result with your [code snippet](https://phpstan.org/r/%s):\n\n```diff\n%s```", \implode(' ', \array_map(static function (string $user) : string {
            return \sprintf('@%s', $user);
        }, $result->getUsers())), $this->latestCommit, $result->getHash(), $diff);
        ...
        foreach ($result->getCurrentTabs() as $tab) {
            ...
            $detail .= "|---|---|\n";
            foreach ($tab->getErrors() as $error) {
                $errorText = \RectorPrefix20210504\Nette\Utils\Strings::replace($error->getMessage(), "/\r|\n/", '');
