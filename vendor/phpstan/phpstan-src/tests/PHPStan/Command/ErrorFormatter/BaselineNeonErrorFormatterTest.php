...
class BaselineNeonErrorFormatterTest extends \PHPStan\Testing\ErrorFormatterTestCase
{
    ...
    public function testFormatErrors(string $message, int $exitCode, int $numFileErrors, int $numGenericErrors, array $expected) : void
    {
        ...
        $this->assertSame(\trim(\_PhpScoperabd03f0baf05\Nette\Neon\Neon::encode(['parameters' => ['ignoreErrors' => $expected]], \_PhpScoperabd03f0baf05\Nette\Neon\Neon::BLOCK)), \trim($this->getOutputContent()), \sprintf('%s: output do not match', $message));
    }
    public function testFormatErrorMessagesRegexEscape() : void
    {
        ...
        self::assertSame(\trim(\_PhpScoperabd03f0baf05\Nette\Neon\Neon::encode(['parameters' => ['ignoreErrors' => [['message' => "#^Escape Regex with file \\# ~ ' \\(\\)\$#", 'count' => 1, 'path' => 'Testfile']]]], \_PhpScoperabd03f0baf05\Nette\Neon\Neon::BLOCK)), \trim($this->getOutputContent()));
    }
    public function testEscapeDiNeon() : void
    {
        ...
        $formatter->formatErrors($result, $this->getOutput());
        self::assertSame(\trim(\_PhpScoperabd03f0baf05\Nette\Neon\Neon::encode(['parameters' => ['ignoreErrors' => [['message' => '#^Test %%value%%$#', 'count' => 1, 'path' => 'Testfile']]]], \_PhpScoperabd03f0baf05\Nette\Neon\Neon::BLOCK)), \trim($this->getOutputContent()));
    }
}
