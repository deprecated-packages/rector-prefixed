...
class AnalyserTest extends \PHPStan\Testing\TestCase
{
    ...
    public function testIgnoringBrokenConfigurationDoesNotWork() : void
    {
        ...
        \assert($result[0] instanceof \PHPStan\Analyser\Error);
    ...
    public function testIgnoreErrorByPathAndCountMoreThanExpected(bool $onlyFiles) : void
    {
        ...
        $this->assertInstanceOf(\PHPStan\Analyser\Error::class, $result[0]);
        ...
        $this->assertInstanceOf(\PHPStan\Analyser\Error::class, $result[1]);
        ...
        $this->assertInstanceOf(\PHPStan\Analyser\Error::class, $result[2]);
    ...
    public function testIgnoreErrorByPathAndCountMissing() : void
    {
        ...
        $this->assertSamePaths(__DIR__ . '/data/two-fails.php', $result[2]->getFile());
        $this->assertInstanceOf(\PHPStan\Analyser\Error::class, $result[3]);
    ...
    public function testIgnoreNextLine(bool $reportUnmatchedIgnoredErrors) : void
    {
        ...
        foreach ([10, 30, 34] as $i => $line) {
            ...
            $this->assertInstanceOf(\PHPStan\Analyser\Error::class, $result[$i]);
    ...
    private function runAnalyser(array $ignoreErrors, bool $reportUnmatchedIgnoredErrors, $filePaths, bool $onlyFiles) : array
    {
        ...
        $ignoredErrorHelper = new \PHPStan\Analyser\IgnoredErrorHelper(self::getContainer()->getByType(\PHPStan\Command\IgnoredRegexValidator::class), $this->getFileHelper(), $ignoreErrors, $reportUnmatchedIgnoredErrors);
    ...
    {
        $registry = new \PHPStan\Rules\Registry([new \PHPStan\Rules\AlwaysFailRule()]);
        ...
        $fileTypeMapper = new \PHPStan\Type\FileTypeMapper(new \PHPStan\Reflection\ReflectionProvider\DirectReflectionProviderProvider($broker), $this->getParser(), $phpDocStringResolver, $phpDocNodeResolver, $this->createMock(\PHPStan\Cache\Cache::class), new \PHPStan\Broker\AnonymousClassNameHelper($fileHelper, $relativePathHelper));
        $phpDocInheritanceResolver = new \PHPStan\PhpDoc\PhpDocInheritanceResolver($fileTypeMapper);
        $nodeScopeResolver = new \PHPStan\Analyser\NodeScopeResolver($broker, self::getReflectors()[0], $this->getClassReflectionExtensionRegistryProvider(), $this->getParser(), $fileTypeMapper, self::getContainer()->getByType(\PHPStan\Php\PhpVersion::class), $phpDocInheritanceResolver, $fileHelper, $typeSpecifier, \false, \false, \true, [], []);
        $lexer = new \PhpParser\Lexer(['usedAttributes' => ['comments', 'startLine', 'endLine', 'startTokenPos', 'endTokenPos']]);
        $fileAnalyser = new \PHPStan\Analyser\FileAnalyser($this->createScopeFactory($broker, $typeSpecifier), $nodeScopeResolver, new \PHPStan\Parser\RichParser(new \PhpParser\Parser\Php7($lexer), $lexer, new \PhpParser\NodeVisitor\NameResolver(), new \PhpParser\NodeVisitor\NodeConnectingVisitor(), new \PHPStan\NodeVisitor\StatementOrderVisitor(), new \PHPStan\Parser\NodeChildrenVisitor(), self::getContainer()->getByType(\PHPStan\Php\PhpVersion::class)), new \PHPStan\Dependency\DependencyResolver($fileHelper, $broker, new \PHPStan\Dependency\ExportedNodeResolver($fileTypeMapper, $printer)), $reportUnmatchedIgnoredErrors);
        return new \PHPStan\Analyser\Analyser($fileAnalyser, $registry, $nodeScopeResolver, 50);
    }
}
