...
class RunnableQueueTest extends \_PhpScoperbd5d0c5f7638\PHPUnit\Framework\TestCase
{
    public function testQueuedProcessIsRun() : void
    {
        $logger = new \PHPStan\Process\Runnable\RunnableQueueLoggerStub();
        $queue = new \PHPStan\Process\Runnable\RunnableQueue($logger, 8);
        $one = new \PHPStan\Process\Runnable\RunnableStub('1');
        $queue->queue($one, 1);
    ...
    public function testComplexScenario() : void
    {
        ...
        $queue = new \PHPStan\Process\Runnable\RunnableQueue($logger, 8);
        $one = new \PHPStan\Process\Runnable\RunnableStub('1');
        $two = new \PHPStan\Process\Runnable\RunnableStub('2');
        $three = new \PHPStan\Process\Runnable\RunnableStub('3');
        $four = new \PHPStan\Process\Runnable\RunnableStub('4');
        $queue->queue($one, 4);
        $this->assertSame(0, $queue->getQueueSize());
    ...
    {
        $logger = new \PHPStan\Process\Runnable\RunnableQueueLoggerStub();
        $queue = new \PHPStan\Process\Runnable\RunnableQueue($logger, 6);
        $one = new \PHPStan\Process\Runnable\RunnableStub('1');
        $two = new \PHPStan\Process\Runnable\RunnableStub('2');
        $three = new \PHPStan\Process\Runnable\RunnableStub('3');
        $queue->queue($one, 3);
