 ...
 * @extends A<\DateTime>
 */
class AOfDateTime extends \PHPStan\Generics\Classes\A
{
 ...
 * @extends A<T>
 */
class B extends \PHPStan\Generics\Classes\A
{
    /**
     * B::__construct()
     *
 ...
 * @implements I<int>
 */
class C implements \PHPStan\Generics\Classes\I
{
    /** @var int */
    private $a;
    public function set($a) : void
 ...
 */
interface IfaceAB extends \PHPStan\Generics\Classes\SuperIfaceA, \PHPStan\Generics\Classes\SuperIfaceB
{
}
/**
 * ABImpl
 ...
 */
class ABImpl implements \PHPStan\Generics\Classes\IfaceAB
{
    public function getA()
    {
        throw new \Exception();
 ...
 */
class SomeRule implements \PHPStan\Generics\Classes\GenericRule
{
    public function getNodeType() : string
    {
...
{
    $a = new \PHPStan\Generics\Classes\A(1);
    $a->set(2);
    $a->set($a->get(0));
    $a->set('');
    $a = new \PHPStan\Generics\Classes\AOfDateTime();
    $a->set(new \DateTime());
    $a->set($a->get(0));
    $a->set(1);
    $b = new \PHPStan\Generics\Classes\B(1);
    $b->set(2);
    $b->set($b->get(0));
    $b->set('');
    $c = new \PHPStan\Generics\Classes\C();
    $c->set(2);
    $c->set($c->get());
    $c->set('');
    $ab = new \PHPStan\Generics\Classes\ABImpl();
    acceptSuperIFaceAOfInt($ab);
    acceptSuperIFaceBOfDateTime($ab);
    acceptSuperIFaceBOfInt($ab);
    new \PHPStan\Generics\Classes\SomeRule();
}
