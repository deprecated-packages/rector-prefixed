...
class ImpossibleCheckTypeMethodCallRuleTest extends \PHPStan\Testing\RuleTestCase
{
    ...
    {
        return new \PHPStan\Rules\Comparison\ImpossibleCheckTypeMethodCallRule(new \PHPStan\Rules\Comparison\ImpossibleCheckTypeHelper($this->createReflectionProvider(), $this->getTypeSpecifier(), [], $this->treatPhpDocTypesAsCertain), \true, $this->treatPhpDocTypesAsCertain);
    }
    ...
    {
        return [new \PHPStan\Tests\AssertionClassMethodTypeSpecifyingExtension(null), new class implements \PHPStan\Type\MethodTypeSpecifyingExtension, \PHPStan\Analyser\TypeSpecifierAwareExtension
        {
            /** @var TypeSpecifier */
            ...
            {
                return $this->typeSpecifier->specifyTypesInCondition($scope, new \PhpParser\Node\Expr\BooleanNot(new \PhpParser\Node\Expr\FuncCall(new \PhpParser\Node\Name('is_int'), [$node->args[0]])), \PHPStan\Analyser\TypeSpecifierContext::createTruthy());
            }
        }, new class implements \PHPStan\Type\MethodTypeSpecifyingExtension, \PHPStan\Analyser\TypeSpecifierAwareExtension
        {
            /** @var TypeSpecifier */
            private $typeSpecifier;
            public function setTypeSpecifier(\PHPStan\Analyser\TypeSpecifier $typeSpecifier) : void
            ...
            public function getClass() : string
            {
                return \_PhpScoper006a73f0e455\ImpossibleMethodCall\Foo::class;
            }
            public function isMethodSupported(\PHPStan\Reflection\MethodReflection $methodReflection, \PhpParser\Node\Expr\MethodCall $node, \PHPStan\Analyser\TypeSpecifierContext $context) : bool
            {
            ...
            public function specifyTypes(\PHPStan\Reflection\MethodReflection $methodReflection, \PhpParser\Node\Expr\MethodCall $node, \PHPStan\Analyser\Scope $scope, \PHPStan\Analyser\TypeSpecifierContext $context) : \PHPStan\Analyser\SpecifiedTypes
            {
                return $this->typeSpecifier->specifyTypesInCondition($scope, new \PhpParser\Node\Expr\BinaryOp\Identical($node->args[0]->value, $node->args[1]->value), \PHPStan\Analyser\TypeSpecifierContext::createTruthy());
            }
        }, new class implements \PHPStan\Type\MethodTypeSpecifyingExtension, \PHPStan\Analyser\TypeSpecifierAwareExtension
        {
            /** @var TypeSpecifier */
            ...
            public function specifyTypes(\PHPStan\Reflection\MethodReflection $methodReflection, \PhpParser\Node\Expr\MethodCall $node, \PHPStan\Analyser\Scope $scope, \PHPStan\Analyser\TypeSpecifierContext $context) : \PHPStan\Analyser\SpecifiedTypes
            {
                return $this->typeSpecifier->specifyTypesInCondition($scope, new \PhpParser\Node\Expr\BinaryOp\NotIdentical($node->args[0]->value, $node->args[1]->value), \PHPStan\Analyser\TypeSpecifierContext::createTruthy());
            }
        }];
    }
