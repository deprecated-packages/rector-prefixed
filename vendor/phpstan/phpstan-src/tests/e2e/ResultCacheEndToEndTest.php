 ...
 * @group exec
 */
class ResultCacheEndToEndTest extends \_PhpScoperbd5d0c5f7638\PHPUnit\Framework\TestCase
{
    ...
    public function testResultCacheDeleteFile() : void
    {
        ...
        $fileHelper = new \PHPStan\File\FileHelper(__DIR__);
        $result = $this->runPhpstan(1);
        $this->assertSame(4, $result['totals']['file_errors'], \_PhpScoperbd5d0c5f7638\Nette\Utils\Json::encode($result));
        $this->assertSame(0, $result['totals']['errors'], \_PhpScoperbd5d0c5f7638\Nette\Utils\Json::encode($result));
    ...
    private function runPhpstan(int $expectedExitCode) : array
    {
        ...
        $output = \implode("\n", $outputLines);
        try {
            $json = \_PhpScoperbd5d0c5f7638\Nette\Utils\Json::decode($output, \_PhpScoperbd5d0c5f7638\Nette\Utils\Json::FORCE_ARRAY);
