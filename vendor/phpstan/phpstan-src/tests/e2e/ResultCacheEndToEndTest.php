 ...
 * @group exec
 */
class ResultCacheEndToEndTest extends \_PhpScoper26e51eeacccf\PHPUnit\Framework\TestCase
{
    public function setUp() : void
    {
        ...
        $baselineContents = \PHPStan\File\FileReader::read($baselinePath);
    ...
    public function testResultCache() : void
    {
        ...
        $lexerCode = \PHPStan\File\FileReader::read($lexerPath);
        ...
        $errorHandlerContents = \PHPStan\File\FileReader::read($errorHandlerPath);
        ...
        $originalBootstrapContents = \PHPStan\File\FileReader::read($bootstrapPath);
    ...
    public function testResultCacheDeleteFile() : void
    {
        ...
        $serializerCode = \PHPStan\File\FileReader::read($serializerPath);
        $originalSerializerCode = $serializerCode;
        \unlink($serializerPath);
        $fileHelper = new \PHPStan\File\FileHelper(__DIR__);
        $result = $this->runPhpstan(1);
        $this->assertSame(4, $result['totals']['file_errors'], \_PhpScoper26e51eeacccf\Nette\Utils\Json::encode($result));
        $this->assertSame(0, $result['totals']['errors'], \_PhpScoper26e51eeacccf\Nette\Utils\Json::encode($result));
    ...
    private function runPhpstan(int $expectedExitCode) : array
    {
        ...
        $output = \implode("\n", $outputLines);
        try {
            $json = \_PhpScoper26e51eeacccf\Nette\Utils\Json::decode($output, \_PhpScoper26e51eeacccf\Nette\Utils\Json::FORCE_ARRAY);
    ...
    private function relativizePath(string $path) : string
    {
        ...
        $helper = new \PHPStan\File\SimpleRelativePathHelper(\str_replace('\\', '/', __DIR__ . '/PHP-Parser'));
