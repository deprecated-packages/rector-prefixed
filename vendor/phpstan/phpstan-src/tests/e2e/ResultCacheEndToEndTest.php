 ...
 * @group exec
 */
class ResultCacheEndToEndTest extends \_PhpScoper006a73f0e455\PHPUnit\Framework\TestCase
{
    ...
    public function testResultCacheDeleteFile() : void
    {
        ...
        $fileHelper = new \PHPStan\File\FileHelper(__DIR__);
        $result = $this->runPhpstan(1);
        $this->assertSame(4, $result['totals']['file_errors'], \_PhpScoper006a73f0e455\Nette\Utils\Json::encode($result));
        $this->assertSame(0, $result['totals']['errors'], \_PhpScoper006a73f0e455\Nette\Utils\Json::encode($result));
    ...
    private function runPhpstan(int $expectedExitCode) : array
    {
        ...
        $output = \implode("\n", $outputLines);
        try {
            $json = \_PhpScoper006a73f0e455\Nette\Utils\Json::decode($output, \_PhpScoper006a73f0e455\Nette\Utils\Json::FORCE_ARRAY);
