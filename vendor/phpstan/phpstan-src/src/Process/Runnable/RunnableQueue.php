...
class RunnableQueue
{
    ...
    public function queue(\PHPStan\Process\Runnable\Runnable $runnable, int $size) : \_PhpScoper006a73f0e455\React\Promise\CancellablePromiseInterface
    {
        if ($size > $this->maxSize) {
            throw new \PHPStan\ShouldNotHappenException('Runnable size exceeds queue maxSize.');
        }
        $deferred = new \_PhpScoper006a73f0e455\React\Promise\Deferred(static function () use($runnable) : void {
            $runnable->cancel();
        });
