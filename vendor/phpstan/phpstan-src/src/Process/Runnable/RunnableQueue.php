...
class RunnableQueue
{
    ...
    public function queue(\PHPStan\Process\Runnable\Runnable $runnable, int $size) : \_PhpScopera143bcca66cb\React\Promise\CancellablePromiseInterface
    {
        if ($size > $this->maxSize) {
            throw new \PHPStan\ShouldNotHappenException('Runnable size exceeds queue maxSize.');
        }
        $deferred = new \_PhpScopera143bcca66cb\React\Promise\Deferred(static function () use($runnable) : void {
            $runnable->cancel();
        });
