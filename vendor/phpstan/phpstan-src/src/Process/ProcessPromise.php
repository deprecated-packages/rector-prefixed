...
class ProcessPromise implements \PHPStan\Process\Runnable\Runnable
{
    ...
    public function __construct(\_PhpScoper006a73f0e455\React\EventLoop\LoopInterface $loop, string $name, string $command)
    {
        $this->loop = $loop;
        $this->name = $name;
    ...
    public function run() : \_PhpScoper006a73f0e455\React\Promise\CancellablePromiseInterface
    {
        ...
        $this->process = new \_PhpScoper006a73f0e455\React\ChildProcess\Process($this->command, null, null, [1 => $tmpStdOutResource, 2 => $tmpStdErrResource]);
