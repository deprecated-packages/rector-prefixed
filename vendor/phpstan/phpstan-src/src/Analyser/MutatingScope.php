...
class MutatingScope implements \PHPStan\Analyser\Scope
{
    ...
    /**
     * @var \PHPStan\Type\OperatorTypeSpecifyingExtensionRegistry
     */
    ...
    private function resolveType(\PhpParser\Node\Expr $node) : \PHPStan\Type\Type
    {
        ...
        if ($node instanceof \PhpParser\Node\Expr\BinaryOp\Div || $node instanceof \PhpParser\Node\Expr\AssignOp\Div || $node instanceof \PhpParser\Node\Expr\BinaryOp\Mod || $node instanceof \PhpParser\Node\Expr\AssignOp\Mod) {
            ...
            foreach ($rightTypes as $rightType) {
                if ($rightType->getValue() === 0 || $rightType->getValue() === 0) {
                    return new \PHPStan\Type\ErrorType();
                }
            }
        }
    ...
    private function promoteNativeTypes() : self
    {
        ...
        return $this->scopeFactory->create($this->context, $this->declareStrictTypes, $this->constantTypes, $this->function, $this->namespace, $variableTypes, $this->moreSpecificTypes, $this->inClosureBindScopeClass, $this->anonymousFunctionReflection, $this->inFirstLevelStatement, $this->currentlyAssignedExpressions, []);
    }
    /**
    ...
    public function enterClosureBind(?\PHPStan\Type\Type $thisType, string $scopeClass) : self
    {
        ...
        return $this->scopeFactory->create($this->context, $this->isDeclareStrictTypes(), $this->constantTypes, $this->getFunction(), $this->getNamespace(), $variableTypes, $this->moreSpecificTypes, $scopeClass, $this->anonymousFunctionReflection);
    }
    public function restoreOriginalScopeAfterClosureBind(self $originalScope) : self
    {
        ...
        return $this->scopeFactory->create($this->context, $this->isDeclareStrictTypes(), $this->constantTypes, $this->getFunction(), $this->getNamespace(), $variableTypes, $this->moreSpecificTypes, $originalScope->inClosureBindScopeClass, $this->anonymousFunctionReflection);
    }
    public function enterClosureCall(\PHPStan\Type\Type $thisType) : self
    {
        $variableTypes = $this->getVariableTypes();
        $variableTypes['this'] = \PHPStan\Analyser\VariableTypeHolder::createYes($thisType);
        return $this->scopeFactory->create($this->context, $this->isDeclareStrictTypes(), $this->constantTypes, $this->getFunction(), $this->getNamespace(), $variableTypes, $this->moreSpecificTypes, $thisType instanceof \PHPStan\Type\TypeWithClassName ? $thisType->getClassName() : null, $this->anonymousFunctionReflection);
    }
    ...
    public function enterAnonymousFunction(\PhpParser\Node\Expr\Closure $closure, ?array $callableParameters = null) : self
    {
        ...
        foreach ($closure->uses as $use) {
            ...
            foreach ($this->moreSpecificTypes as $exprString => $moreSpecificType) {
                $matches = \_PhpScoper26e51eeacccf\Nette\Utils\Strings::matchAll((string) $exprString, '#^\\$([a-zA-Z_\\x7f-\\xff][a-zA-Z_0-9\\x7f-\\xff]*)#');
                if ($matches === []) {
                    continue;
                }
    ...
    public function enterArrowFunction(\PhpParser\Node\Expr\ArrowFunction $arrowFunction) : self
    {
        ...
        return $this->scopeFactory->create($this->context, $this->isDeclareStrictTypes(), $this->constantTypes, $this->getFunction(), $this->getNamespace(), $variableTypes, $this->moreSpecificTypes, $this->inClosureBindScopeClass, $anonymousFunctionReflection, \true, [], [], [], $this->afterExtractCall, $this->parentScope);
    }
    ...
    public function assignVariable(string $variableName, \PHPStan\Type\Type $type, ?\PHPStan\TrinaryLogic $certainty = null) : self
    {
        ...
        foreach (\array_keys($moreSpecificTypeHolders) as $key) {
            $matches = \_PhpScoper26e51eeacccf\Nette\Utils\Strings::matchAll((string) $key, '#\\$[a-zA-Z_\\x7f-\\xff][a-zA-Z_0-9\\x7f-\\xff]*#');
            if ($matches === []) {
                continue;
            }
        ...
        return $this->scopeFactory->create($this->context, $this->isDeclareStrictTypes(), $this->constantTypes, $this->getFunction(), $this->getNamespace(), $variableTypes, $moreSpecificTypeHolders, $this->inClosureBindScopeClass, $this->anonymousFunctionReflection, $this->inFirstLevelStatement, $this->currentlyAssignedExpressions, $nativeTypes, $this->inFunctionCallsStack, $this->afterExtractCall, $this->parentScope);
    }
    ...
    public function invalidateExpression(\PhpParser\Node\Expr $expressionToInvalidate, bool $requireMoreCharacters = \false) : self
    {
        ...
        foreach (\array_keys($moreSpecificTypeHolders) as $exprString) {
            ...
            if (\_PhpScoper26e51eeacccf\Nette\Utils\Strings::startsWith($exprString, $exprStringToInvalidate)) {
                if ($exprString === $exprStringToInvalidate && $requireMoreCharacters) {
                    continue;
                }
                $nextLetter = \substr($exprString, \strlen($exprStringToInvalidate), 1);
                if (\_PhpScoper26e51eeacccf\Nette\Utils\Strings::match($nextLetter, '#[a-zA-Z_0-9\\x7f-\\xff]#') === null) {
                    unset($moreSpecificTypeHolders[$exprString]);
                    continue;
                }
            }
            $matches = \_PhpScoper26e51eeacccf\Nette\Utils\Strings::matchAll($exprString, '#\\$[a-zA-Z_\\x7f-\\xff][a-zA-Z_0-9\\x7f-\\xff]*#');
            if ($matches === []) {
                continue;
            }
            $matches = \array_column($matches, 0);
    ...
    public function filterBySpecifiedTypes(\PHPStan\Analyser\SpecifiedTypes $specifiedTypes) : self
    {
        ...
        foreach ($typeSpecifications as $typeSpecification) {
            ...
            if ($typeSpecification['sure']) {
                $scope = $scope->specifyExpressionType($expr, $specifiedTypes->shouldOverwrite() ? $type : \PHPStan\Type\TypeCombinator::intersect($type, $this->getType($expr)));
                if ($expr instanceof \PhpParser\Node\Expr\Variable && \is_string($expr->name)) {
    ...
    public function processAlwaysIterableForeachScopeWithoutPollute(self $finalScope) : self
    {
        ...
        return $this->scopeFactory->create($this->context, $this->isDeclareStrictTypes(), $this->constantTypes, $this->getFunction(), $this->getNamespace(), $variableTypeHolders, $moreSpecificTypes, $this->inClosureBindScopeClass, $this->anonymousFunctionReflection, $this->inFirstLevelStatement, [], $nativeTypes, [], $this->afterExtractCall, $this->parentScope);
    }
