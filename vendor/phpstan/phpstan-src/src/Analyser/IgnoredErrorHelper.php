            ...
            try {
                if (\is_array($ignoreError)) {
                    if (!isset($ignoreError['message'])) {
                        $errors[] = \sprintf('Ignored error %s is missing a message.', \_PhpScoper006a73f0e455\Nette\Utils\Json::encode($ignoreError));
                        continue;
                    }
                    if (!isset($ignoreError['path'])) {
                        if (!isset($ignoreError['paths'])) {
                            $errors[] = \sprintf('Ignored error %s is missing a path.', \_PhpScoper006a73f0e455\Nette\Utils\Json::encode($ignoreError));
                        }
                    ...
                    \_PhpScoper006a73f0e455\Nette\Utils\Strings::match('', $ignoreMessage);
