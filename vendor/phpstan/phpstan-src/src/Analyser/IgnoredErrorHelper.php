            ...
            try {
                if (\is_array($ignoreError)) {
                    if (!isset($ignoreError['message'])) {
                        $errors[] = \sprintf('Ignored error %s is missing a message.', \_PhpScoper88fe6e0ad041\Nette\Utils\Json::encode($ignoreError));
                        continue;
                    }
                    if (!isset($ignoreError['path'])) {
                        if (!isset($ignoreError['paths'])) {
                            $errors[] = \sprintf('Ignored error %s is missing a path.', \_PhpScoper88fe6e0ad041\Nette\Utils\Json::encode($ignoreError));
                        }
                    ...
                    \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::match('', $ignoreMessage);
