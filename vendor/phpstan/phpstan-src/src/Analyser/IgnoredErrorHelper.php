...
class IgnoredErrorHelper
{
    ...
    public function initialize() : \PHPStan\Analyser\IgnoredErrorHelperResult
    {
        ...
        foreach ($this->ignoreErrors as $i => $ignoreError) {
            try {
                if (\is_array($ignoreError)) {
                    if (!isset($ignoreError['message'])) {
                        $errors[] = \sprintf('Ignored error %s is missing a message.', \_PhpScopera143bcca66cb\Nette\Utils\Json::encode($ignoreError));
                        continue;
                    }
                    if (!isset($ignoreError['path'])) {
                        if (!isset($ignoreError['paths'])) {
                            $errors[] = \sprintf('Ignored error %s is missing a path.', \_PhpScopera143bcca66cb\Nette\Utils\Json::encode($ignoreError));
                        }
                    ...
                    \_PhpScopera143bcca66cb\Nette\Utils\Strings::match('', $ignoreMessage);
            ...
            } catch (\_PhpScopera143bcca66cb\Nette\Utils\RegexpException $e) {
                $errors[] = $e->getMessage();
            }
        }
