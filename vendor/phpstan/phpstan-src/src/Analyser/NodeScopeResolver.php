    ...
    private function processStmtNode(\PhpParser\Node\Stmt $stmt, \PHPStan\Analyser\MutatingScope $scope, callable $nodeCallback) : \PHPStan\Analyser\StatementResult
    {
        ...
        if ($stmt instanceof \PhpParser\Node\Stmt\Declare_) {
            ...
            if (isset($stmt->namespacedName)) {
                ...
                $betterReflectionClass = $nodeToReflection->__invoke($this->classReflector, $stmt, new \_PhpScoperbd5d0c5f7638\Roave\BetterReflection\SourceLocator\Located\LocatedSource(\PHPStan\File\FileReader::read($scope->getFile()), $scope->getFile()), $scope->getNamespace() !== null ? new \PhpParser\Node\Stmt\Namespace_(new \PhpParser\Node\Name($scope->getNamespace())) : null, null);
                if (!$betterReflectionClass instanceof \_PhpScoperbd5d0c5f7638\Roave\BetterReflection\Reflection\ReflectionClass) {
                    throw new \PHPStan\ShouldNotHappenException();
                }
                $classReflection = new \PHPStan\Reflection\ClassReflection($this->reflectionProvider, $this->fileTypeMapper, $this->phpVersion, $this->classReflectionExtensionRegistryProvider->getRegistry()->getPropertiesClassReflectionExtensions(), $this->classReflectionExtensionRegistryProvider->getRegistry()->getMethodsClassReflectionExtensions(), $betterReflectionClass->getName(), new \_PhpScoperbd5d0c5f7638\Roave\BetterReflection\Reflection\Adapter\ReflectionClass($betterReflectionClass), null, null, null, \sprintf('%s:%d', $scope->getFile(), $stmt->getStartLine()));
