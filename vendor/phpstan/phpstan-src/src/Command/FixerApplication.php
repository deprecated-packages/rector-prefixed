...
use _PhpScopera143bcca66cb\Composer\CaBundle\CaBundle;
...
use _PhpScopera143bcca66cb\React\Http\Browser;
use _PhpScopera143bcca66cb\React\Promise\CancellablePromiseInterface;
use _PhpScopera143bcca66cb\React\Promise\ExtendedPromiseInterface;
use _PhpScopera143bcca66cb\React\Promise\PromiseInterface;
use _PhpScopera143bcca66cb\React\Socket\ConnectionInterface;
use _PhpScopera143bcca66cb\React\Socket\Connector;
use _PhpScopera143bcca66cb\Symfony\Component\Console\Helper\ProgressBar;
...
use function _PhpScopera143bcca66cb\Clue\React\Block\await;
use function escapeshellarg;
use function file_exists;
use function _PhpScopera143bcca66cb\React\Promise\resolve;
class FixerApplication
{
     ...
     * @return int
     */
    public function run(?string $projectConfigFile, \PHPStan\Command\InceptionResult $inceptionResult, \_PhpScopera143bcca66cb\Symfony\Component\Console\Input\InputInterface $input, \_PhpScopera143bcca66cb\Symfony\Component\Console\Output\OutputInterface $output, array $fileSpecificErrors, array $notFileSpecificErrors, int $filesCount, string $mainScript) : int
    {
        $loop = new \_PhpScopera143bcca66cb\React\EventLoop\StreamSelectLoop();
        $server = new \_PhpScopera143bcca66cb\React\Socket\TcpServer('127.0.0.1:0', $loop);
        ...
        $server->on('connection', function (\_PhpScopera143bcca66cb\React\Socket\ConnectionInterface $connection) use($loop, $projectConfigFile, $input, $output, $fileSpecificErrors, $notFileSpecificErrors, $mainScript, $filesCount, $reanalyseProcessQueue, $inceptionResult) : void {
            $decoder = new \_PhpScopera143bcca66cb\Clue\React\NDJson\Decoder($connection, \true, 512, \defined('JSON_INVALID_UTF8_IGNORE') ? \JSON_INVALID_UTF8_IGNORE : 0, 128 * 1024 * 1024);
            $encoder = new \_PhpScopera143bcca66cb\Clue\React\NDJson\Encoder($connection, \defined('JSON_INVALID_UTF8_IGNORE') ? \JSON_INVALID_UTF8_IGNORE : 0);
            ...
            $decoder->on('data', function (array $data) use($loop, $encoder, $projectConfigFile, $input, $output, $mainScript, $reanalyseProcessQueue, $inceptionResult) : void {
                ...
                $this->reanalyseWithTmpFile($loop, $inceptionResult, $mainScript, $reanalyseProcessQueue, $projectConfigFile, $data['data']['tmpFile'], $data['data']['insteadOfFile'], $data['data']['fixerSuggestionId'], $input)->done(static function (string $output) use($encoder, $id) : void {
                    $encoder->write(['id' => $id, 'response' => \_PhpScopera143bcca66cb\Nette\Utils\Json::decode($output, \_PhpScopera143bcca66cb\Nette\Utils\Json::FORCE_ARRAY)]);
    ...
    private function getFixerProcess(\_PhpScopera143bcca66cb\Symfony\Component\Console\Output\OutputInterface $output, int $serverPort) : \_PhpScopera143bcca66cb\React\ChildProcess\Process
    {
        ...
        return new \_PhpScopera143bcca66cb\React\ChildProcess\Process(\sprintf('%s -d memory_limit=%s %s --port %d', \PHP_BINARY, \escapeshellarg(\ini_get('memory_limit')), \escapeshellarg($pharPath), $serverPort), null, $env, []);
    }
    private function downloadPhar(\_PhpScopera143bcca66cb\Symfony\Component\Console\Output\OutputInterface $output, string $pharPath, string $infoPath) : void
    {
        $currentVersion = null;
        if (\file_exists($pharPath) && \file_exists($infoPath)) {
            /** @var array{version: string, date: string} $currentInfo */
            $currentInfo = \_PhpScopera143bcca66cb\Nette\Utils\Json::decode(\PHPStan\File\FileReader::read($infoPath), \_PhpScopera143bcca66cb\Nette\Utils\Json::FORCE_ARRAY);
        ...
        $loop = new \_PhpScopera143bcca66cb\React\EventLoop\StreamSelectLoop();
        $client = new \_PhpScopera143bcca66cb\React\Http\Browser($loop, new \_PhpScopera143bcca66cb\React\Socket\Connector($loop, ['timeout' => 5, 'tls' => ['cafile' => \_PhpScopera143bcca66cb\Composer\CaBundle\CaBundle::getBundledCaBundlePath()], 'dns' => '1.1.1.1']));
        /** @var array{url: string, version: string} $latestInfo */
        $latestInfo = \_PhpScopera143bcca66cb\Nette\Utils\Json::decode((string) \_PhpScopera143bcca66cb\Clue\React\Block\await($client->get('https://fixer-download-api.phpstan.com/latest'), $loop, 5)->getBody(), \_PhpScopera143bcca66cb\Nette\Utils\Json::FORCE_ARRAY);
        ...
        $progressBar = new \_PhpScopera143bcca66cb\Symfony\Component\Console\Helper\ProgressBar($output);
        $client->requestStreaming('GET', $latestInfo['url'])->done(static function (\_PhpScopera143bcca66cb\Psr\Http\Message\ResponseInterface $response) use($progressBar, $pharPathResource) : void {
    ...
    {
        \PHPStan\File\FileWriter::write($infoPath, \_PhpScopera143bcca66cb\Nette\Utils\Json::encode(['version' => $version, 'date' => (new \DateTimeImmutable('', new \DateTimeZone('UTC')))->format(\DateTime::ATOM)]));
    }
    /**
     * @param LoopInterface $loop
     * @param callable(FileMonitorResult): void $hasChangesCallback
     */
    private function monitorFileChanges(\_PhpScopera143bcca66cb\React\EventLoop\LoopInterface $loop, callable $hasChangesCallback) : void
    {
    ...
    private function reanalyseWithTmpFile(\_PhpScopera143bcca66cb\React\EventLoop\LoopInterface $loop, \PHPStan\Command\InceptionResult $inceptionResult, string $mainScript, \PHPStan\Process\Runnable\RunnableQueue $runnableQueue, ?string $projectConfigFile, string $tmpFile, string $insteadOfFile, string $fixerSuggestionId, \_PhpScopera143bcca66cb\Symfony\Component\Console\Input\InputInterface $input) : \_PhpScopera143bcca66cb\React\Promise\PromiseInterface
    {
    ...
    private function reanalyseAfterFileChanges(\_PhpScopera143bcca66cb\React\EventLoop\LoopInterface $loop, \PHPStan\Command\InceptionResult $inceptionResult, string $mainScript, ?string $projectConfigFile, ?string $fixerSuggestionId, \_PhpScopera143bcca66cb\Symfony\Component\Console\Input\InputInterface $input) : \_PhpScopera143bcca66cb\React\Promise\PromiseInterface
    {
        ...
        if (\count($resultCache->getFilesToAnalyse()) === 0) {
            ...
            return \_PhpScopera143bcca66cb\React\Promise\resolve(['fileSpecificErrors' => $finalFileSpecificErrors, 'notFileSpecificErrors' => $finalNotFileSpecificErrors]);
        }
        ...
        return $this->processInProgress->then(static function (string $output) : array {
            return \_PhpScopera143bcca66cb\Nette\Utils\Json::decode($output, \_PhpScopera143bcca66cb\Nette\Utils\Json::FORCE_ARRAY);
        });
    }
    private function getPhpstanVersion() : string
    {
        try {
            return \_PhpScopera143bcca66cb\Jean85\PrettyVersions::getVersion('phpstan/phpstan')->getPrettyVersion();
