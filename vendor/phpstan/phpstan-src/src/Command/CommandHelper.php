...
use _PhpScoperbd5d0c5f7638\Nette\DI\Config\Adapters\PhpAdapter;
use _PhpScoperbd5d0c5f7638\Nette\DI\Helpers;
use _PhpScoperbd5d0c5f7638\Nette\Schema\Context as SchemaContext;
...
class CommandHelper
{
     ...
     */
    public static function begin(\_PhpScoperbd5d0c5f7638\Symfony\Component\Console\Input\InputInterface $input, \_PhpScoperbd5d0c5f7638\Symfony\Component\Console\Output\OutputInterface $output, array $paths, ?string $pathsFile, ?string $memoryLimit, ?string $autoloadFile, array $composerAutoloaderProjectPaths, ?string $projectConfigFile, ?string $generateBaselineFile, ?string $level, bool $allowXdebug, bool $manageMemoryLimitFile = \true, bool $debugEnabled = \false, ?string $singleReflectionFile = null) : \PHPStan\Command\InceptionResult
    {
        if (!$allowXdebug) {
            $xdebug = new \_PhpScoperbd5d0c5f7638\Composer\XdebugHandler\XdebugHandler('phpstan', '--ansi');
            $xdebug->check();
            unset($xdebug);
        }
        ...
        $errorOutput = (static function () use($input, $output) : Output {
            $symfonyErrorOutput = $output instanceof \_PhpScoperbd5d0c5f7638\Symfony\Component\Console\Output\ConsoleOutputInterface ? $output->getErrorOutput() : $output;
        ...
        if ($memoryLimit !== null) {
            if (\_PhpScoperbd5d0c5f7638\Nette\Utils\Strings::match($memoryLimit, '#^-?\\d+[kMG]?$#i') === null) {
        ...
        if ($projectConfigFile !== null) {
            ...
            } catch (\_PhpScoperbd5d0c5f7638\Nette\InvalidStateException|\_PhpScoperbd5d0c5f7638\Nette\FileNotFoundException $e) {
                $errorOutput->writeLineFormatted($e->getMessage());
                throw new \PHPStan\Command\InceptionNotSuccessfulException();
            }
            ...
            if (isset($projectConfig['parameters']['tmpDir'])) {
                $tmpDir = \_PhpScoperbd5d0c5f7638\Nette\DI\Helpers::expand($projectConfig['parameters']['tmpDir'], $defaultParameters);
            }
            ...
            if (\count($paths) === 0 && isset($projectConfig['parameters']['paths'])) {
                $analysedPathsFromConfig = \_PhpScoperbd5d0c5f7638\Nette\DI\Helpers::expand($projectConfig['parameters']['paths'], $defaultParameters);
        ...
        } catch (\_PhpScoperbd5d0c5f7638\Nette\DI\InvalidConfigurationException|\_PhpScoperbd5d0c5f7638\Nette\Utils\AssertionException $e) {
        ...
        $processor->onNewContext[] = static function (\_PhpScoperbd5d0c5f7638\Nette\Schema\Context $context) : void {
    ...
    private static function detectDuplicateIncludedFiles(\PHPStan\Command\Output $output, \PHPStan\File\FileHelper $fileHelper, array $configFiles, array $loaderParameters) : void
    {
        ...
        $phpAdapter = new \_PhpScoperbd5d0c5f7638\Nette\DI\Config\Adapters\PhpAdapter();
        $allConfigFiles = [];
     ...
     * @return string[]
     */
    private static function getConfigFiles(\PHPStan\File\FileHelper $fileHelper, \PHPStan\DependencyInjection\NeonAdapter $neonAdapter, \_PhpScoperbd5d0c5f7638\Nette\DI\Config\Adapters\PhpAdapter $phpAdapter, string $configFile, array $loaderParameters, ?string $generateBaselineFile) : array
    {
        ...
        if (\_PhpScoperbd5d0c5f7638\Nette\Utils\Strings::endsWith($configFile, '.php')) {
        ...
        if (isset($data['includes'])) {
            \_PhpScoperbd5d0c5f7638\Nette\Utils\Validators::assert($data['includes'], 'list', \sprintf("section 'includes' in file '%s'", $configFile));
            $includes = \_PhpScoperbd5d0c5f7638\Nette\DI\Helpers::expand($data['includes'], $loaderParameters);
    ...
    {
        return \_PhpScoperbd5d0c5f7638\Nette\Utils\Strings::match($includedFile, '#([a-z]+:)?[/\\\\]#Ai') !== null ? $includedFile : \dirname($mainFile) . '/' . $includedFile;
    }
}
