...
class ComposerJsonAndInstalledJsonSourceLocatorMaker
{
    ...
    public function create(string $projectInstallationPath) : ?\_PhpScopera143bcca66cb\Roave\BetterReflection\SourceLocator\Type\SourceLocator
    {
        ...
        try {
            ...
            $composer = \_PhpScopera143bcca66cb\Nette\Utils\Json::decode($composerJsonContents, \_PhpScopera143bcca66cb\Nette\Utils\Json::FORCE_ARRAY);
        } catch (\PHPStan\File\CouldNotReadFileException|\_PhpScopera143bcca66cb\Nette\Utils\JsonException $e) {
            return null;
        }
        try {
            $installedJsonContents = \PHPStan\File\FileReader::read($installedJsonPath);
            $installedJson = \_PhpScopera143bcca66cb\Nette\Utils\Json::decode($installedJsonContents, \_PhpScopera143bcca66cb\Nette\Utils\Json::FORCE_ARRAY);
        } catch (\PHPStan\File\CouldNotReadFileException|\_PhpScopera143bcca66cb\Nette\Utils\JsonException $e) {
            return null;
        }
        ...
        $locators = [];
        $locators[] = $this->optimizedPsrAutoloaderLocatorFactory->create(\_PhpScopera143bcca66cb\Roave\BetterReflection\SourceLocator\Type\Composer\Psr\Psr4Mapping::fromArrayMappings(\array_merge_recursive($this->prefixWithInstallationPath($this->packageToPsr4AutoloadNamespaces($composer), $projectInstallationPath), ...\array_map(function (array $package) use($projectInstallationPath, $installedJsonDirectoryPath) : array {
        ...
        $locators[] = $this->optimizedPsrAutoloaderLocatorFactory->create(\_PhpScopera143bcca66cb\Roave\BetterReflection\SourceLocator\Type\Composer\Psr\Psr0Mapping::fromArrayMappings(\array_merge_recursive($this->prefixWithInstallationPath($this->packageToPsr0AutoloadNamespaces($composer), $projectInstallationPath), ...\array_map(function (array $package) use($projectInstallationPath, $installedJsonDirectoryPath) : array {
        ...
        return new \_PhpScopera143bcca66cb\Roave\BetterReflection\SourceLocator\Type\AggregateSourceLocator($locators);
    }
    /**
     * @param mixed[] $package
     *
