...
class ComposerJsonAndInstalledJsonSourceLocatorMaker
{
    ...
    public function create(string $projectInstallationPath) : ?\_PhpScoper88fe6e0ad041\Roave\BetterReflection\SourceLocator\Type\SourceLocator
    {
        ...
        try {
            ...
            $composer = \_PhpScoper88fe6e0ad041\Nette\Utils\Json::decode($composerJsonContents, \_PhpScoper88fe6e0ad041\Nette\Utils\Json::FORCE_ARRAY);
        } catch (\PHPStan\File\CouldNotReadFileException|\_PhpScoper88fe6e0ad041\Nette\Utils\JsonException $e) {
            return null;
        }
        try {
            $installedJsonContents = \PHPStan\File\FileReader::read($installedJsonPath);
            $installedJson = \_PhpScoper88fe6e0ad041\Nette\Utils\Json::decode($installedJsonContents, \_PhpScoper88fe6e0ad041\Nette\Utils\Json::FORCE_ARRAY);
        } catch (\PHPStan\File\CouldNotReadFileException|\_PhpScoper88fe6e0ad041\Nette\Utils\JsonException $e) {
            return null;
        }
        ...
        $locators = [];
        $locators[] = $this->optimizedPsrAutoloaderLocatorFactory->create(\_PhpScoper88fe6e0ad041\Roave\BetterReflection\SourceLocator\Type\Composer\Psr\Psr4Mapping::fromArrayMappings(\array_merge_recursive($this->prefixWithInstallationPath($this->packageToPsr4AutoloadNamespaces($composer), $projectInstallationPath), ...\array_map(function (array $package) use($projectInstallationPath, $installedJsonDirectoryPath) : array {
        ...
        $locators[] = $this->optimizedPsrAutoloaderLocatorFactory->create(\_PhpScoper88fe6e0ad041\Roave\BetterReflection\SourceLocator\Type\Composer\Psr\Psr0Mapping::fromArrayMappings(\array_merge_recursive($this->prefixWithInstallationPath($this->packageToPsr0AutoloadNamespaces($composer), $projectInstallationPath), ...\array_map(function (array $package) use($projectInstallationPath, $installedJsonDirectoryPath) : array {
        ...
        return new \_PhpScoper88fe6e0ad041\Roave\BetterReflection\SourceLocator\Type\AggregateSourceLocator($locators);
    }
    /**
     * @param mixed[] $package
     *
