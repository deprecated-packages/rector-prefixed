...
class ComposerJsonAndInstalledJsonSourceLocatorMaker
{
    ...
    public function create(string $projectInstallationPath) : ?\_PhpScoperabd03f0baf05\Roave\BetterReflection\SourceLocator\Type\SourceLocator
    {
        ...
        try {
            ...
            $installedJson = \_PhpScoperabd03f0baf05\Nette\Utils\Json::decode($installedJsonContents, \_PhpScoperabd03f0baf05\Nette\Utils\Json::FORCE_ARRAY);
        } catch (\PHPStan\File\CouldNotReadFileException|\_PhpScoperabd03f0baf05\Nette\Utils\JsonException $e) {
            return null;
        }
        ...
        $locators = [];
        $locators[] = $this->optimizedPsrAutoloaderLocatorFactory->create(\_PhpScoperabd03f0baf05\Roave\BetterReflection\SourceLocator\Type\Composer\Psr\Psr4Mapping::fromArrayMappings(\array_merge_recursive($this->prefixWithInstallationPath($this->packageToPsr4AutoloadNamespaces($composer), $projectInstallationPath), ...\array_map(function (array $package) use($projectInstallationPath, $installedJsonDirectoryPath) : array {
        ...
        $locators[] = $this->optimizedPsrAutoloaderLocatorFactory->create(\_PhpScoperabd03f0baf05\Roave\BetterReflection\SourceLocator\Type\Composer\Psr\Psr0Mapping::fromArrayMappings(\array_merge_recursive($this->prefixWithInstallationPath($this->packageToPsr0AutoloadNamespaces($composer), $projectInstallationPath), ...\array_map(function (array $package) use($projectInstallationPath, $installedJsonDirectoryPath) : array {
        ...
        return new \_PhpScoperabd03f0baf05\Roave\BetterReflection\SourceLocator\Type\AggregateSourceLocator($locators);
    }
    /**
     * @param mixed[] $package
     *
