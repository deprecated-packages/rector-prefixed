...
class DecoderTest extends \_PhpScoper006a73f0e455\TestCase
{
    private $input;
    private $decoder;
    public function setUp()
    {
        $this->input = new \_PhpScoper006a73f0e455\React\Stream\ThroughStream();
        $this->decoder = new \_PhpScoper006a73f0e455\Clue\React\NDJson\Decoder($this->input);
    }
    ...
    public function testEmitDataBigIntOptionWillForwardAsString()
    {
        ...
        $this->decoder = new \_PhpScoper006a73f0e455\Clue\React\NDJson\Decoder($this->input, \false, 512, \JSON_BIGINT_AS_STRING);
    ...
    public function testEmitDataErrorWillForwardErrorAlsoWhenCreatedWithThrowOnError()
    {
        ...
        $this->input = new \_PhpScoper006a73f0e455\React\Stream\ThroughStream();
        $this->decoder = new \_PhpScoper006a73f0e455\Clue\React\NDJson\Decoder($this->input, \false, 512, \JSON_THROW_ON_ERROR);
        $this->decoder->on('data', $this->expectCallableNever());
        $this->decoder->on('error', $this->expectCallableOnce());
        $this->input->emit('data', array("invalid\n"));
    }
    ...
    {
        $this->decoder = new \_PhpScoper006a73f0e455\Clue\React\NDJson\Decoder($this->input, \false, 512, 0, 4);
        $this->decoder->on('data', $this->expectCallableOnceWith(null));
    ...
    {
        $this->decoder = new \_PhpScoper006a73f0e455\Clue\React\NDJson\Decoder($this->input, \false, 512, 0, 3);
        $this->decoder->on('data', $this->expectCallableNever());
        $this->decoder->on('error', $this->expectCallableOnce());
        $this->input->emit('data', array("null"));
    }
    ...
    {
        $dest = $this->getMockBuilder('_PhpScoper006a73f0e455\\React\\Stream\\WritableStreamInterface')->getMock();
        $ret = $this->decoder->pipe($dest);
        $this->assertSame($dest, $ret);
    }
    public function testForwardPauseToInput()
    {
        $this->input = $this->getMockBuilder('_PhpScoper006a73f0e455\\React\\Stream\\ReadableStreamInterface')->getMock();
...
\class_alias('_PhpScoper006a73f0e455\\DecoderTest', 'DecoderTest', \false);
