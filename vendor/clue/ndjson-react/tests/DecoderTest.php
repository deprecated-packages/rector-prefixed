...
class DecoderTest extends \_PhpScoper88fe6e0ad041\TestCase
{
    private $input;
    private $decoder;
    public function setUp()
    {
        $this->input = new \_PhpScoper88fe6e0ad041\React\Stream\ThroughStream();
        $this->decoder = new \_PhpScoper88fe6e0ad041\Clue\React\NDJson\Decoder($this->input);
    }
    ...
    public function testEmitDataBigIntOptionWillForwardAsString()
    {
        ...
        $this->decoder = new \_PhpScoper88fe6e0ad041\Clue\React\NDJson\Decoder($this->input, \false, 512, \JSON_BIGINT_AS_STRING);
    ...
    public function testEmitDataErrorWillForwardErrorAlsoWhenCreatedWithThrowOnError()
    {
        ...
        $this->input = new \_PhpScoper88fe6e0ad041\React\Stream\ThroughStream();
        $this->decoder = new \_PhpScoper88fe6e0ad041\Clue\React\NDJson\Decoder($this->input, \false, 512, \JSON_THROW_ON_ERROR);
        $this->decoder->on('data', $this->expectCallableNever());
        $this->decoder->on('error', $this->expectCallableOnce());
        $this->input->emit('data', array("invalid\n"));
    }
    ...
    {
        $this->decoder = new \_PhpScoper88fe6e0ad041\Clue\React\NDJson\Decoder($this->input, \false, 512, 0, 4);
        $this->decoder->on('data', $this->expectCallableOnceWith(null));
    ...
    {
        $this->decoder = new \_PhpScoper88fe6e0ad041\Clue\React\NDJson\Decoder($this->input, \false, 512, 0, 3);
        $this->decoder->on('data', $this->expectCallableNever());
        $this->decoder->on('error', $this->expectCallableOnce());
        $this->input->emit('data', array("null"));
    }
    ...
    {
        $dest = $this->getMockBuilder('_PhpScoper88fe6e0ad041\\React\\Stream\\WritableStreamInterface')->getMock();
        $ret = $this->decoder->pipe($dest);
        $this->assertSame($dest, $ret);
    }
    public function testForwardPauseToInput()
    {
        $this->input = $this->getMockBuilder('_PhpScoper88fe6e0ad041\\React\\Stream\\ReadableStreamInterface')->getMock();
...
\class_alias('_PhpScoper88fe6e0ad041\\DecoderTest', 'DecoderTest', \false);
