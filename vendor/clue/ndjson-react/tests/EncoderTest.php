...
class EncoderTest extends \_PhpScoperbd5d0c5f7638\TestCase
{
    private $output;
    private $encoder;
    public function setUp()
    {
        ...
        $loop = $this->getMockBuilder('_PhpScoperbd5d0c5f7638\\React\\EventLoop\\LoopInterface')->getMock();
        $this->output = new \_PhpScoperbd5d0c5f7638\React\Stream\WritableResourceStream($stream, $loop);
        $this->encoder = new \_PhpScoperbd5d0c5f7638\Clue\React\NDJson\Encoder($this->output);
    }
    /**
    ...
    public function testPrettyPrintDoesNotMakeSenseForNDJson()
    {
        ...
        $this->encoder = new \_PhpScoperbd5d0c5f7638\Clue\React\NDJson\Encoder($this->output, \JSON_PRETTY_PRINT);
    }
    public function testWriteString()
    {
        $this->output = $this->getMockBuilder('_PhpScoperbd5d0c5f7638\\React\\Stream\\WritableStreamInterface')->getMock();
    ...
    public function testWriteInfiniteWillEmitErrorAndCloseAlsoWhenCreatedWithThrowOnError()
    {
        ...
        $this->output = $this->getMockBuilder('_PhpScoperbd5d0c5f7638\\React\\Stream\\WritableStreamInterface')->getMock();
        $this->output->expects($this->once())->method('isWritable')->willReturn(\true);
        $this->encoder = new \_PhpScoperbd5d0c5f7638\Clue\React\NDJson\Encoder($this->output, \JSON_THROW_ON_ERROR);
        $this->output->expects($this->never())->method('write');
        $this->encoder->on('error', $this->expectCallableOnce());
        $this->encoder->on('close', $this->expectCallableOnce());
        $ret = $this->encoder->write(\INF);
...
\class_alias('_PhpScoperbd5d0c5f7638\\EncoderTest', 'EncoderTest', \false);
