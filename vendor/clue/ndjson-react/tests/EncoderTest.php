...
class EncoderTest extends \_PhpScoper88fe6e0ad041\TestCase
{
    private $output;
    private $encoder;
    public function setUp()
    {
        ...
        $loop = $this->getMockBuilder('_PhpScoper88fe6e0ad041\\React\\EventLoop\\LoopInterface')->getMock();
        $this->output = new \_PhpScoper88fe6e0ad041\React\Stream\WritableResourceStream($stream, $loop);
        $this->encoder = new \_PhpScoper88fe6e0ad041\Clue\React\NDJson\Encoder($this->output);
    }
    /**
    ...
    public function testPrettyPrintDoesNotMakeSenseForNDJson()
    {
        ...
        $this->encoder = new \_PhpScoper88fe6e0ad041\Clue\React\NDJson\Encoder($this->output, \JSON_PRETTY_PRINT);
    }
    public function testWriteString()
    {
        $this->output = $this->getMockBuilder('_PhpScoper88fe6e0ad041\\React\\Stream\\WritableStreamInterface')->getMock();
    ...
    public function testWriteInfiniteWillEmitErrorAndCloseAlsoWhenCreatedWithThrowOnError()
    {
        ...
        $this->output = $this->getMockBuilder('_PhpScoper88fe6e0ad041\\React\\Stream\\WritableStreamInterface')->getMock();
        $this->output->expects($this->once())->method('isWritable')->willReturn(\true);
        $this->encoder = new \_PhpScoper88fe6e0ad041\Clue\React\NDJson\Encoder($this->output, \JSON_THROW_ON_ERROR);
        $this->output->expects($this->never())->method('write');
        $this->encoder->on('error', $this->expectCallableOnce());
        $this->encoder->on('close', $this->expectCallableOnce());
        $ret = $this->encoder->write(\INF);
...
\class_alias('_PhpScoper88fe6e0ad041\\EncoderTest', 'EncoderTest', \false);
