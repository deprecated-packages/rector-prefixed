...
namespace RectorPrefix20210217\Symplify\PhpConfigPrinter\Yaml;
...
final class CheckerServiceParametersShifter
{
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\RectorPrefix20210217\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\RectorPrefix20210217\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \RectorPrefix20210217\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \RectorPrefix20210217\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \RectorPrefix20210217\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
    private function initializeServiceKeywords() : void
    {
        $reflectionClass = new \ReflectionClass(\RectorPrefix20210217\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class);
