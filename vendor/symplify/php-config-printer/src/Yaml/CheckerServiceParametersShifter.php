...
use _PhpScoperfce0de0de1ce\PhpCsFixer\Fixer\Comment\HeaderCommentFixer;
use ReflectionClass;
...
final class CheckerServiceParametersShifter
{
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScoperfce0de0de1ce\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScoperfce0de0de1ce\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \_PhpScoperfce0de0de1ce\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \_PhpScoperfce0de0de1ce\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function correctHeader(string $checker, array $serviceDefinition) : array
    {
        ...
        if ($checker !== \_PhpScoperfce0de0de1ce\PhpCsFixer\Fixer\Comment\HeaderCommentFixer::class) {
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScoperfce0de0de1ce\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
    private function initializeServiceKeywords() : void
    {
        $reflectionClass = new \ReflectionClass(\_PhpScoperfce0de0de1ce\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class);
