...
use _PhpScoper8b9c402c5f32\PhpCsFixer\Fixer\Comment\HeaderCommentFixer;
use ReflectionClass;
...
final class CheckerServiceParametersShifter
{
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScoper8b9c402c5f32\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScoper8b9c402c5f32\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \_PhpScoper8b9c402c5f32\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \_PhpScoper8b9c402c5f32\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function correctHeader(string $checker, array $serviceDefinition) : array
    {
        ...
        if ($checker !== \_PhpScoper8b9c402c5f32\PhpCsFixer\Fixer\Comment\HeaderCommentFixer::class) {
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScoper8b9c402c5f32\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
    private function initializeServiceKeywords() : void
    {
        $reflectionClass = new \ReflectionClass(\_PhpScoper8b9c402c5f32\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class);
