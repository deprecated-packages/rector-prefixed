...
namespace RectorPrefix20201229\Symplify\PhpConfigPrinter\Yaml;

use RectorPrefix20201229\Nette\Utils\Strings;
use RectorPrefix20201229\PhpCsFixer\Fixer\Comment\HeaderCommentFixer;
use ReflectionClass;
...
final class CheckerServiceParametersShifter
{
    ...
    {
        $this->stringFormatConverter = new \RectorPrefix20201229\Symplify\PackageBuilder\Strings\StringFormatConverter();
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\RectorPrefix20201229\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\RectorPrefix20201229\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \RectorPrefix20201229\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \RectorPrefix20201229\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function correctHeader(string $checker, array $serviceDefinition) : array
    {
        ...
        if ($checker !== \RectorPrefix20201229\PhpCsFixer\Fixer\Comment\HeaderCommentFixer::class) {
    ...
    private function escapeValue($value)
    {
        ...
        return \RectorPrefix20201229\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
    private function initializeServiceKeywords() : void
    {
        $reflectionClass = new \ReflectionClass(\RectorPrefix20201229\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class);
