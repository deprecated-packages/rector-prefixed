...
namespace RectorPrefix20210315\Symplify\PhpConfigPrinter\Yaml;
...
final class CheckerServiceParametersShifter
{
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\RectorPrefix20210315\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\RectorPrefix20210315\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        if (\RectorPrefix20210315\Nette\Utils\Strings::endsWith($checker, 'Fixer')) {
            return \true;
        }
        return \RectorPrefix20210315\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \RectorPrefix20210315\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
}
