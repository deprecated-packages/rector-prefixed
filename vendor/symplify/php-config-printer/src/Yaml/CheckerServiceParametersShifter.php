...
use _PhpScoper5b8c9e9ebd21\PhpCsFixer\Fixer\Comment\HeaderCommentFixer;
use ReflectionClass;
...
final class CheckerServiceParametersShifter
{
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        return \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::endsWith($checker, 'Fixer') || \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function correctHeader(string $checker, array $serviceDefinition) : array
    {
        ...
        if ($checker !== \_PhpScoper5b8c9e9ebd21\PhpCsFixer\Fixer\Comment\HeaderCommentFixer::class) {
    ...
    private function escapeValue($value)
    {
        ...
        return \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
    private function initializeServiceKeywords() : void
    {
        $reflectionClass = new \ReflectionClass(\_PhpScoper5b8c9e9ebd21\Symfony\Component\DependencyInjection\Loader\YamlFileLoader::class);
