...
use RectorPrefix20210215\Symplify\PhpConfigPrinter\Exception\NotImplementedYetException;
use RectorPrefix20210215\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory;
use RectorPrefix20210215\Symplify\PhpConfigPrinter\Sorter\YamlArgumentSorter;
 ...
 * imports: <---
 */
final class ImportCaseConverter implements \RectorPrefix20210215\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function __construct(\RectorPrefix20210215\Symplify\PhpConfigPrinter\Sorter\YamlArgumentSorter $yamlArgumentSorter, \RectorPrefix20210215\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory)
    {
    ...
    {
        return $rootKey === \RectorPrefix20210215\Symplify\PhpConfigPrinter\ValueObject\YamlKey::IMPORTS;
    }
    public function convertToMethodCall($key, $values) : \PhpParser\Node\Stmt\Expression
    {
        if (\is_array($values)) {
            $arguments = $this->yamlArgumentSorter->sortArgumentsByKeyIfExists($values, [\RectorPrefix20210215\Symplify\PhpConfigPrinter\ValueObject\YamlKey::RESOURCE => '', 'type' => null, \RectorPrefix20210215\Symplify\PhpConfigPrinter\ValueObject\YamlKey::IGNORE_ERRORS => \false]);
            return $this->createImportMethodCall($arguments);
        }
        throw new \RectorPrefix20210215\Symplify\PhpConfigPrinter\Exception\NotImplementedYetException();
    }
    /**
    ...
    private function shouldSkipDefaultValue(string $name, $value, array $arguments) : bool
    {
        ...
        if ($name === \RectorPrefix20210215\Symplify\PhpConfigPrinter\ValueObject\YamlKey::IGNORE_ERRORS && $value === \false) {
            return \true;
        }
        ...
        if (!isset($arguments[\RectorPrefix20210215\Symplify\PhpConfigPrinter\ValueObject\YamlKey::IGNORE_ERRORS])) {
            return \false;
        }
        return $arguments[\RectorPrefix20210215\Symplify\PhpConfigPrinter\ValueObject\YamlKey::IGNORE_ERRORS] === \false;
    }
    /**
     * @return mixed|string
     */
    private function replaceImportedFileSuffix($value)
    {
        ...
        return \RectorPrefix20210215\Nette\Utils\Strings::replace($value, self::INPUT_SUFFIX_REGEX, '.php');
    }
