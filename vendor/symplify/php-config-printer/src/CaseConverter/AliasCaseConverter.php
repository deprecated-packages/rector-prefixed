 ...
 *     Some: Other <---
 */
final class AliasCaseConverter implements \RectorPrefix20210122\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if (\RectorPrefix20210122\Nette\Utils\Strings::match($key, self::NAMED_ALIAS_REGEX)) {
            return \true;
        }
        if (!\is_string($values)) {
            return \false;
    ...
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \PhpParser\Node\Expr\MethodCall
    {
        ...
        \RectorPrefix20210122\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = '$' . \RectorPrefix20210122\Nette\Utils\Strings::after($key, '$');
