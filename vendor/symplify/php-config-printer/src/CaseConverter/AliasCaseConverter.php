...
use RectorPrefix20210311\Symplify\PackageBuilder\Reflection\ClassLikeExistenceChecker;
...
use RectorPrefix20210311\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory;
use RectorPrefix20210311\Symplify\PhpConfigPrinter\NodeFactory\Service\ServiceOptionNodeFactory;
...
final class AliasCaseConverter implements \RectorPrefix20210311\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function __construct(\RectorPrefix20210311\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory, \RectorPrefix20210311\Symplify\PhpConfigPrinter\NodeFactory\ArgsNodeFactory $argsNodeFactory, \RectorPrefix20210311\Symplify\PhpConfigPrinter\NodeFactory\Service\ServiceOptionNodeFactory $serviceOptionNodeFactory, \RectorPrefix20210311\Symplify\PackageBuilder\Reflection\ClassLikeExistenceChecker $classLikeExistenceChecker)
    {
    ...
    public function convertToMethodCall($key, $values) : \PhpParser\Node\Stmt\Expression
    {
        ...
        $servicesVariable = new \PhpParser\Node\Expr\Variable(\RectorPrefix20210311\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES);
        ...
        $fullClassName = \RectorPrefix20210311\Nette\Utils\Strings::before($key, ' $');
        ...
        if (\is_string($values) && $values[0] === '@') {
            ...
            $methodCall = new \PhpParser\Node\Expr\MethodCall($servicesVariable, \RectorPrefix20210311\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            return new \PhpParser\Node\Stmt\Expression($methodCall);
        }
        if (\is_array($values)) {
    ...
    {
        if ($rootKey !== \RectorPrefix20210311\Symplify\PhpConfigPrinter\ValueObject\YamlKey::SERVICES) {
            return \false;
        }
        if (isset($values[\RectorPrefix20210311\Symplify\PhpConfigPrinter\ValueObject\YamlKey::ALIAS])) {
            return \true;
        }
        if (\RectorPrefix20210311\Nette\Utils\Strings::match($key, self::NAMED_ALIAS_REGEX)) {
            return \true;
        }
        if (!\is_string($values)) {
            return \false;
    ...
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \PhpParser\Node\Expr\MethodCall
    {
        ...
        \RectorPrefix20210311\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = '$' . \RectorPrefix20210311\Nette\Utils\Strings::after($key, '$');
        ...
        return new \PhpParser\Node\Expr\MethodCall(new \PhpParser\Node\Expr\Variable(\RectorPrefix20210311\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \RectorPrefix20210311\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
    /**
     * @param mixed $values
     */
    private function createFromClassLike(string $key, $values, \PhpParser\Node\Expr\Variable $servicesVariable) : \PhpParser\Node\Stmt\Expression
    {
        ...
        $argValues[] = $values[\RectorPrefix20210311\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS] ?? $values;
    ...
    private function createFromAlias(string $className, string $key, \PhpParser\Node\Expr\Variable $servicesVariable) : \PhpParser\Node\Expr\MethodCall
    {
        ...
        return new \PhpParser\Node\Expr\MethodCall($servicesVariable, \RectorPrefix20210311\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
    /**
     * @param mixed[] $values
     */
    ...
    {
        if (isset($values[\RectorPrefix20210311\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS])) {
            $methodCall = $this->createFromAlias($values[\RectorPrefix20210311\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS], $key, $servicesVariable);
            unset($values[\RectorPrefix20210311\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS]);
        } else {
            throw new \RectorPrefix20210311\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
