...
use RectorPrefix20201229\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface;
 ...
 *     Some: Other <---
 */
final class AliasCaseConverter implements \RectorPrefix20201229\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function __construct(\RectorPrefix20201229\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory, \RectorPrefix20201229\Symplify\PhpConfigPrinter\NodeFactory\ArgsNodeFactory $argsNodeFactory, \RectorPrefix20201229\Symplify\PhpConfigPrinter\NodeFactory\Service\ServiceOptionNodeFactory $serviceOptionNodeFactory)
    {
    ...
    public function convertToMethodCall($key, $values) : \PhpParser\Node\Stmt\Expression
    {
        ...
        if (\class_exists($key) || \interface_exists($key)) {
            ...
            $argValues[] = $values[\RectorPrefix20201229\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS] ?? $values;
            $args = $this->argsNodeFactory->createFromValues($argValues, \true);
            $methodCall = new \PhpParser\Node\Expr\MethodCall($servicesVariable, \RectorPrefix20201229\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            return new \PhpParser\Node\Stmt\Expression($methodCall);
        }
        // handles: "SomeClass $someVariable: ..."
        $fullClassName = \RectorPrefix20201229\Nette\Utils\Strings::before($key, ' $');
        if ($fullClassName !== null) {
            $methodCall = $this->createAliasNode($key, $fullClassName, $values);
            return new \PhpParser\Node\Stmt\Expression($methodCall);
        }
        $methodCall = null;
        if (isset($values[\RectorPrefix20201229\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS])) {
            $className = $values[\RectorPrefix20201229\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS];
            ...
            $methodCall = new \PhpParser\Node\Expr\MethodCall($servicesVariable, \RectorPrefix20201229\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            unset($values[\RectorPrefix20201229\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS]);
        }
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if (isset($values[\RectorPrefix20201229\Symplify\PhpConfigPrinter\ValueObject\YamlKey::ALIAS])) {
            return \true;
        }
        if (\RectorPrefix20201229\Nette\Utils\Strings::match($key, self::NAMED_ALIAS_REGEX)) {
            return \true;
        }
    ...
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \PhpParser\Node\Expr\MethodCall
    {
        ...
        \RectorPrefix20201229\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = '$' . \RectorPrefix20201229\Nette\Utils\Strings::after($key, '$');
        ...
        return new \PhpParser\Node\Expr\MethodCall(new \PhpParser\Node\Expr\Variable(\RectorPrefix20201229\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \RectorPrefix20201229\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
}
