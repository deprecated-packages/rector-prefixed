...
final class AliasCaseConverter implements \Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    ...
    public function convertToMethodCall($key, $values) : \PhpParser\Node\Stmt\Expression
    {
        ...
        $fullClassName = \RectorPrefix20201226\Nette\Utils\Strings::before($key, ' $');
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if (\RectorPrefix20201226\Nette\Utils\Strings::match($key, self::NAMED_ALIAS_REGEX)) {
            return \true;
        }
    ...
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \PhpParser\Node\Expr\MethodCall
    {
        ...
        \RectorPrefix20201226\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = '$' . \RectorPrefix20201226\Nette\Utils\Strings::after($key, '$');
