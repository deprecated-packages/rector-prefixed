...
namespace RectorPrefix20210119\Symplify\PhpConfigPrinter\CaseConverter;
...
use RectorPrefix20210119\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface;
...
use RectorPrefix20210119\Symplify\PhpConfigPrinter\NodeFactory\Service\ServiceOptionNodeFactory;
use RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\MethodName;
use RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\VariableName;
use RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\YamlKey;
 ...
 *     Some: Other <---
 */
final class AliasCaseConverter implements \RectorPrefix20210119\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
    ...
    public function convertToMethodCall($key, $values) : \PhpParser\Node\Stmt\Expression
    {
        ...
        if (\class_exists($key) || \interface_exists($key)) {
            ...
            $argValues[] = $values[\RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS] ?? $values;
            $args = $this->argsNodeFactory->createFromValues($argValues, \true);
            $methodCall = new \PhpParser\Node\Expr\MethodCall($servicesVariable, \RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            return new \PhpParser\Node\Stmt\Expression($methodCall);
        }
        // handles: "SomeClass $someVariable: ..."
        $fullClassName = \RectorPrefix20210119\Nette\Utils\Strings::before($key, ' $');
        if ($fullClassName !== null) {
            $methodCall = $this->createAliasNode($key, $fullClassName, $values);
            return new \PhpParser\Node\Stmt\Expression($methodCall);
        }
        $methodCall = null;
        if (isset($values[\RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS])) {
            $className = $values[\RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS];
            ...
            $methodCall = new \PhpParser\Node\Expr\MethodCall($servicesVariable, \RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
            unset($values[\RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS]);
        }
    ...
    {
        if ($rootKey !== \RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\YamlKey::SERVICES) {
            return \false;
        }
        if (isset($values[\RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\YamlKey::ALIAS])) {
            return \true;
        }
        if (\RectorPrefix20210119\Nette\Utils\Strings::match($key, self::NAMED_ALIAS_REGEX)) {
            return \true;
        }
        if (!\is_string($values)) {
            return \false;
    ...
    private function createAliasNode(string $key, string $fullClassName, $serviceValues) : \PhpParser\Node\Expr\MethodCall
    {
        ...
        \RectorPrefix20210119\Nette\Utils\Strings::match($key, self::ARGUMENT_NAME_REGEX);
        $argumentName = '$' . \RectorPrefix20210119\Nette\Utils\Strings::after($key, '$');
        ...
        return new \PhpParser\Node\Expr\MethodCall(new \PhpParser\Node\Expr\Variable(\RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\MethodName::ALIAS, $args);
    }
}
