...
final class ConfiguredServiceCaseConverter implements \RectorPrefix20210318\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
     * @var ArgsNodeFactory
     */
    ...
    public function convertToMethodCall($key, $values) : \PhpParser\Node\Stmt\Expression
    {
        ...
        if (isset($values[\RectorPrefix20210318\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY])) {
            $valuesForArgs[] = $values[\RectorPrefix20210318\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY];
        }
        $args = $this->argsNodeFactory->createFromValues($valuesForArgs);
        $methodCall = new \PhpParser\Node\Expr\MethodCall(new \PhpParser\Node\Expr\Variable(\RectorPrefix20210318\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \RectorPrefix20210318\Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if ($key === \RectorPrefix20210318\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_DEFAULTS) {
            return \false;
        }
        if ($key === \RectorPrefix20210318\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_INSTANCEOF) {
            return \false;
        }
        if (isset($values[\RectorPrefix20210318\Symplify\PhpConfigPrinter\ValueObject\YamlKey::RESOURCE])) {
            return \false;
        }
    ...
    private function isAlias($values) : bool
    {
        ...
        return \RectorPrefix20210318\Nette\Utils\Strings::startsWith($values, '@');
    }
}
