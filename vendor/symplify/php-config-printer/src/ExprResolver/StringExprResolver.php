...
namespace RectorPrefix20210224\Symplify\PhpConfigPrinter\ExprResolver;
...
use RectorPrefix20210224\Symplify\PhpConfigPrinter\Configuration\SymfonyFunctionNameProvider;
use RectorPrefix20210224\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory;
use RectorPrefix20210224\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory;
use RectorPrefix20210224\Symplify\PhpConfigPrinter\ValueObject\FunctionName;
final class StringExprResolver
{
    ...
    public function resolve(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \PhpParser\Node\Expr
    {
        ...
        if (\RectorPrefix20210224\Nette\Utils\Strings::startsWith($value, '@=')) {
            $value = \ltrim($value, '@=');
            $expr = $this->resolve($value, $skipServiceReference, $skipClassesToConstantReference);
            $args = [new \PhpParser\Node\Arg($expr)];
            return new \PhpParser\Node\Expr\FuncCall(new \PhpParser\Node\Name\FullyQualified(\RectorPrefix20210224\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
        // is service reference
        if (\RectorPrefix20210224\Nette\Utils\Strings::startsWith($value, '@') && !$this->isFilePath($value)) {
    ...
    {
        return (bool) \RectorPrefix20210224\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
    /**
