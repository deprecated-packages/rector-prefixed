...
final class ServiceOptionNodeFactory
{
     ...
     */
    public function __construct(\RectorPrefix20210119\Symplify\PhpConfigPrinter\ServiceOptionAnalyzer\ServiceOptionAnalyzer $serviceOptionAnalyzer, array $serviceOptionKeyYamlToPhpFactories)
    {
    ...
    public function convertServiceOptionsToNodes(array $servicesValues, \PhpParser\Node\Expr\MethodCall $methodCall) : \PhpParser\Node\Expr\MethodCall
    {
        ...
        foreach ($servicesValues as $key => $value) {
            ...
            if (\RectorPrefix20210119\Nette\Utils\Strings::startsWith($key, 'decoration_') || $key === 'alias') {
                continue;
            }
    ...
    private function unNestArguments(array $servicesValues) : array
    {
        ...
        return [\RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\YamlServiceKey::ARGUMENTS => $servicesValues];
    }
}
