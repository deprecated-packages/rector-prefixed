...
namespace RectorPrefix20210210\Symplify\PhpConfigPrinter\NodeFactory\Service;

use PhpParser\Node\Arg;
use PhpParser\Node\Expr\MethodCall;
use RectorPrefix20210210\Symplify\PhpConfigPrinter\Contract\SymfonyVersionFeatureGuardInterface;
use RectorPrefix20210210\Symplify\PhpConfigPrinter\Converter\ServiceOptionsKeyYamlToPhpFactory\TagsServiceOptionKeyYamlToPhpFactory;
...
use RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\SymfonyVersionFeature;
...
final class AutoBindNodeFactory
{
    ...
    public function __construct(\RectorPrefix20210210\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory, \RectorPrefix20210210\Symplify\PhpConfigPrinter\NodeFactory\ArgsNodeFactory $argsNodeFactory, \RectorPrefix20210210\Symplify\PhpConfigPrinter\Contract\SymfonyVersionFeatureGuardInterface $symfonyVersionFeatureGuard, \RectorPrefix20210210\Symplify\PhpConfigPrinter\Converter\ServiceOptionsKeyYamlToPhpFactory\TagsServiceOptionKeyYamlToPhpFactory $tagsServiceOptionKeyYamlToPhpFactory)
    {
    ...
    public function createAutoBindCalls(array $yaml, \PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \PhpParser\Node\Expr\MethodCall
    {
        foreach ($yaml as $key => $value) {
            if ($key === \RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOWIRE) {
                $methodCall = $this->createAutowire($value, $methodCall, $type);
            }
            if ($key === \RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOCONFIGURE) {
                $methodCall = $this->createAutoconfigure($value, $methodCall, $type);
            }
            if ($key === \RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\YamlKey::PUBLIC) {
                $methodCall = $this->createPublicPrivate($value, $methodCall, $type);
            }
            if ($key === \RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\YamlKey::BIND) {
                $methodCall = $this->createBindMethodCall($methodCall, $yaml[\RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\YamlKey::BIND]);
            }
            if ($key === \RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\YamlKey::TAGS) {
    ...
    private function createBindMethodCall(\PhpParser\Node\Expr\MethodCall $methodCall, array $bindValues) : \PhpParser\Node\Expr\MethodCall
    {
        foreach ($bindValues as $key => $value) {
            ...
            $methodCall = new \PhpParser\Node\Expr\MethodCall($methodCall, \RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\YamlKey::BIND, $args);
        }
        return $methodCall;
    }
    ...
    {
        if ($value === \true) {
            return new \PhpParser\Node\Expr\MethodCall($methodCall, \RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOWIRE);
        }
        // skip default false
        ...
        return new \PhpParser\Node\Expr\MethodCall($methodCall, \RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOWIRE, $args);
    }
    ...
    {
        if ($value === \true) {
            return new \PhpParser\Node\Expr\MethodCall($methodCall, \RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOCONFIGURE);
        }
        // skip default false
        if ($type === self::TYPE_DEFAULTS) {
            return $methodCall;
        ...
        $args = [new \PhpParser\Node\Arg($this->commonNodeFactory->createFalse())];
        return new \PhpParser\Node\Expr\MethodCall($methodCall, \RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\YamlKey::AUTOCONFIGURE, $args);
    }
    private function createPublicPrivate($value, \PhpParser\Node\Expr\MethodCall $methodCall, string $type) : \PhpParser\Node\Expr\MethodCall
    {
        ...
        // default value
        if ($type === self::TYPE_DEFAULTS) {
            if ($this->symfonyVersionFeatureGuard->isAtLeastSymfonyVersion(\RectorPrefix20210210\Symplify\PhpConfigPrinter\ValueObject\SymfonyVersionFeature::PRIVATE_SERVICES_BY_DEFAULT)) {
                return $methodCall;
            }
