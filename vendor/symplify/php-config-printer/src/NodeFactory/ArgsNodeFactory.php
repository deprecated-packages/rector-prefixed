...
namespace RectorPrefix20210120\Symplify\PhpConfigPrinter\NodeFactory;
...
use RectorPrefix20210120\Symplify\PhpConfigPrinter\Exception\NotImplementedYetException;
use RectorPrefix20210120\Symplify\PhpConfigPrinter\ValueObject\FunctionName;
...
final class ArgsNodeFactory
{
    ...
    public function __construct(\RectorPrefix20210120\Symplify\PhpConfigPrinter\NodeFactory\CommonNodeFactory $commonNodeFactory, \RectorPrefix20210120\Symplify\PhpConfigPrinter\NodeFactory\ConstantNodeFactory $constantNodeFactory, \RectorPrefix20210120\Symplify\PhpConfigPrinter\Contract\SymfonyVersionFeatureGuardInterface $symfonyVersionFeatureGuard)
    {
    ...
    public function createFromValues($values, bool $skipServiceReference = \false, bool $skipClassesToConstantReference = \false) : array
    {
        ...
        throw new \RectorPrefix20210120\Symplify\PhpConfigPrinter\Exception\NotImplementedYetException();
    }
    ...
    private function createServiceReferenceFromTaggedValue(\RectorPrefix20210120\Symfony\Component\Yaml\Tag\TaggedValue $taggedValue) : \PhpParser\Node\Expr
    {
        ...
        if ($taggedValue->getTag() === self::TAG_RETURNS_CLONE) {
            ...
            $functionName = \RectorPrefix20210120\Symplify\PhpConfigPrinter\ValueObject\FunctionName::INLINE_SERVICE;
        } else {
    ...
    private function resolveStringExpr(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \PhpParser\Node\Expr
    {
        ...
        if (\RectorPrefix20210120\Nette\Utils\Strings::startsWith($value, '@=')) {
            $value = \ltrim($value, '@=');
            $args = $this->createFromValues($value);
            return new \PhpParser\Node\Expr\FuncCall(new \PhpParser\Node\Name\FullyQualified(\RectorPrefix20210120\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
        // is service reference
        if (\RectorPrefix20210120\Nette\Utils\Strings::startsWith($value, '@') && !$this->isFilePath($value)) {
    ...
    {
        if ($this->symfonyVersionFeatureGuard->isAtLeastSymfonyVersion(\RectorPrefix20210120\Symplify\PhpConfigPrinter\ValueObject\SymfonyVersionFeature::REF_OVER_SERVICE)) {
            return \RectorPrefix20210120\Symplify\PhpConfigPrinter\ValueObject\FunctionName::SERVICE;
        }
        return \RectorPrefix20210120\Symplify\PhpConfigPrinter\ValueObject\FunctionName::REF;
    }
    private function isFilePath(string $value) : bool
    {
        return (bool) \RectorPrefix20210120\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
    /**
