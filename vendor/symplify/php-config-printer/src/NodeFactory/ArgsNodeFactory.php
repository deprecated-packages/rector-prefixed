...
namespace RectorPrefix20210119\Symplify\PhpConfigPrinter\NodeFactory;
...
use RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\FunctionName;
...
final class ArgsNodeFactory
{
    ...
    private function createServiceReferenceFromTaggedValue(\RectorPrefix20210119\Symfony\Component\Yaml\Tag\TaggedValue $taggedValue) : \PhpParser\Node\Expr
    {
        ...
        if ($taggedValue->getTag() === self::TAG_RETURNS_CLONE) {
            ...
            $functionName = \RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\FunctionName::INLINE_SERVICE;
        } else {
    ...
    private function resolveStringExpr(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \PhpParser\Node\Expr
    {
        ...
        if (\RectorPrefix20210119\Nette\Utils\Strings::startsWith($value, '@=')) {
            $value = \ltrim($value, '@=');
            $args = $this->createFromValues($value);
            return new \PhpParser\Node\Expr\FuncCall(new \PhpParser\Node\Name\FullyQualified(\RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\FunctionName::EXPR), $args);
        }
        // is service reference
        if (\RectorPrefix20210119\Nette\Utils\Strings::startsWith($value, '@') && !$this->isFilePath($value)) {
    ...
    {
        if ($this->symfonyVersionFeatureGuard->isAtLeastSymfonyVersion(\RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\SymfonyVersionFeature::REF_OVER_SERVICE)) {
            return \RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\FunctionName::SERVICE;
        }
        return \RectorPrefix20210119\Symplify\PhpConfigPrinter\ValueObject\FunctionName::REF;
    }
    private function isFilePath(string $value) : bool
    {
        return (bool) \RectorPrefix20210119\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
    /**
