...
final class ArgsNodeFactory
{
    ...
    private function createServiceReferenceFromTaggedValue(\RectorPrefix2020DecSat\Symfony\Component\Yaml\Tag\TaggedValue $taggedValue) : \PhpParser\Node\Expr
    {
    ...
    private function resolveStringExpr(string $value, bool $skipServiceReference, bool $skipClassesToConstantReference) : \PhpParser\Node\Expr
    {
        ...
        if (\RectorPrefix2020DecSat\Nette\Utils\Strings::startsWith($value, '@=')) {
            $value = \ltrim($value, '@=');
        ...
        if (\RectorPrefix2020DecSat\Nette\Utils\Strings::startsWith($value, '@') && !$this->isFilePath($value)) {
    ...
    {
        return (bool) \RectorPrefix2020DecSat\Nette\Utils\Strings::match($value, self::TWIG_HTML_XML_SUFFIX_REGEX);
    }
