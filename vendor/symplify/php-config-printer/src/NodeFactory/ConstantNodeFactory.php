...
final class ConstantNodeFactory
{
    ...
    public function __construct(\_PhpScoper2a4e7ab1ecbc\Symplify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
     ...
     */
    public function createConstantIfValue(string $value) : ?\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr
    {
        if (\_PhpScoper2a4e7ab1ecbc\Nette\Utils\Strings::contains($value, '::')) {
            ...
            return new \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Name\FullyQualified($class), $constant);
        }
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\_PhpScoper2a4e7ab1ecbc\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
            return new \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Name($constantName));
        }
        return null;
    }
}
