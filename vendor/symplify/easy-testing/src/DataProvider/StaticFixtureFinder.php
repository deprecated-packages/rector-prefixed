...
namespace RectorPrefix20210316\Symplify\EasyTesting\DataProvider;
...
final class StaticFixtureFinder
{
    ...
    private static function yieldFileInfos(array $fileInfos) : \Iterator
    {
        foreach ($fileInfos as $fileInfo) {
            try {
                $smartFileInfo = new \RectorPrefix20210316\Symplify\SmartFileSystem\SmartFileInfo($fileInfo->getRealPath());
                (yield [$smartFileInfo]);
            } catch (\RectorPrefix20210316\Symplify\SmartFileSystem\Exception\FileNotFoundException $fileNotFoundException) {
            }
        }
    }
    /**
    ...
    private static function yieldFileInfosWithRelativePathname(array $fileInfos) : \Iterator
    {
        foreach ($fileInfos as $fileInfo) {
            ...
            } catch (\RectorPrefix20210316\Symplify\SmartFileSystem\Exception\FileNotFoundException $e) {
            }
        }
    }
    /**
    ...
    {
        $finder = \RectorPrefix20210316\Symfony\Component\Finder\Finder::create()->in($directory)->files()->name($suffix);
    ...
    {
        $iterator = \RectorPrefix20210316\Symfony\Component\Finder\Finder::create()->in($directory)->files()->notName($suffix)->getIterator();
        $relativeFilePaths = [];
        foreach ($iterator as $fileInfo) {
            $relativeFilePaths[] = \RectorPrefix20210316\Nette\Utils\Strings::substring($fileInfo->getRealPath(), \strlen(\getcwd()) + 1);
        }
        if ($relativeFilePaths === []) {
            return;
        }
        throw new \RectorPrefix20210316\Symplify\SymplifyKernel\Exception\ShouldNotHappenException(\sprintf('Files "%s" have invalid suffix, use "%s" suffix instead', \implode('", ', $relativeFilePaths), $suffix));
    }
}
