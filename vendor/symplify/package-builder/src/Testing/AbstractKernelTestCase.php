...
namespace RectorPrefix20210317\Symplify\PackageBuilder\Testing;
...
use RectorPrefix20210317\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException;
 ...
 */
abstract class AbstractKernelTestCase extends \RectorPrefix20210317\PHPUnit\Framework\TestCase
{
    /**
     * @var KernelInterface
     */
     ...
     */
    protected function bootKernelWithConfigs(string $kernelClass, array $configs) : \RectorPrefix20210317\Symfony\Component\HttpKernel\KernelInterface
    {
     ...
     * @param string[]|SmartFileInfo[] $configs
     */
    protected function bootKernelWithConfigsAndStaticCache(string $kernelClass, array $configs) : \RectorPrefix20210317\Symfony\Component\HttpKernel\KernelInterface
    {
        // unwrap file infos to real paths
        $configFilePaths = $this->resolveConfigFilePaths($configs);
        $configsHash = $this->resolveConfigsHash($configFilePaths);
    ...
    protected function getService(string $type) : object
    {
        if (self::$container === null) {
            throw new \RectorPrefix20210317\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, crewate container with booKernel(KernelClass::class)');
        }
    ...
    protected function bootKernel(string $kernelClass) : void
    {
        ...
        if (!$kernel instanceof \RectorPrefix20210317\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    protected function ensureKernelShutdown() : void
    {
        if (static::$kernel !== null) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \RectorPrefix20210317\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    protected function resolveConfigFilePaths(array $configs) : array
    {
        $configFilePaths = [];
        foreach ($configs as $config) {
            $configFilePaths[] = $config instanceof \RectorPrefix20210317\Symplify\SmartFileSystem\SmartFileInfo ? $config->getRealPath() : $config;
        }
        return $configFilePaths;
    }
    private function ensureIsConfigAwareKernel(\RectorPrefix20210317\Symfony\Component\HttpKernel\KernelInterface $kernel) : void
    {
        if ($kernel instanceof \RectorPrefix20210317\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface) {
            return;
        }
        throw new \RectorPrefix20210317\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException(\sprintf('"%s" is missing an "%s" interface', \get_class($kernel), \RectorPrefix20210317\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class));
    }
    private function bootAndReturnKernel(\RectorPrefix20210317\Symfony\Component\HttpKernel\KernelInterface $kernel) : \RectorPrefix20210317\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \RectorPrefix20210317\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \RectorPrefix20210317\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\RectorPrefix20210317\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\RectorPrefix20210317\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\RectorPrefix20210317\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
     ...
     */
    private function createBootedKernelFromConfigs(string $kernelClass, string $configsHash, array $configFilePaths) : \RectorPrefix20210317\Symfony\Component\HttpKernel\KernelInterface
    {
