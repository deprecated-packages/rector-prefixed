...
namespace RectorPrefix20210502\Symplify\PackageBuilder\Testing;
 ...
 */
abstract class AbstractKernelTestCase extends \RectorPrefix20210502\PHPUnit\Framework\TestCase
{
    /**
     * @var KernelInterface
     */
     ...
     */
    protected function bootKernelWithConfigs(string $kernelClass, array $configs) : \RectorPrefix20210502\Symfony\Component\HttpKernel\KernelInterface
    {
     ...
     * @param string[]|SmartFileInfo[] $configs
     */
    protected function bootKernelWithConfigsAndStaticCache(string $kernelClass, array $configs) : \RectorPrefix20210502\Symfony\Component\HttpKernel\KernelInterface
    {
        // unwrap file infos to real paths
        $configFilePaths = $this->resolveConfigFilePaths($configs);
        $configsHash = $this->resolveConfigsHash($configFilePaths);
    ...
    protected function getService(string $type) : object
    {
        if (self::$container === null) {
            throw new \RectorPrefix20210502\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, crewate container with booKernel(KernelClass::class)');
        }
    ...
    protected function bootKernel(string $kernelClass) : void
    {
        ...
        if (!$kernel instanceof \RectorPrefix20210502\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    protected function ensureKernelShutdown() : void
    {
        if (static::$kernel !== null) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \RectorPrefix20210502\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    private function ensureIsConfigAwareKernel(\RectorPrefix20210502\Symfony\Component\HttpKernel\KernelInterface $kernel) : void
    {
        if ($kernel instanceof \RectorPrefix20210502\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface) {
            return;
        }
        throw new \RectorPrefix20210502\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException(\sprintf('"%s" is missing an "%s" interface', \get_class($kernel), \RectorPrefix20210502\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class));
    }
    private function bootAndReturnKernel(\RectorPrefix20210502\Symfony\Component\HttpKernel\KernelInterface $kernel) : \RectorPrefix20210502\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \RectorPrefix20210502\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \RectorPrefix20210502\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\RectorPrefix20210502\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\RectorPrefix20210502\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\RectorPrefix20210502\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
     ...
     */
    private function createBootedKernelFromConfigs(string $kernelClass, string $configsHash, array $configFilePaths) : \RectorPrefix20210502\Symfony\Component\HttpKernel\KernelInterface
    {
