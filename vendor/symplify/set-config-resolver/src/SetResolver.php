...
final class SetResolver
{
    ...
    public function detectFromName(string $setName) : \RectorPrefix20210421\Symplify\SmartFileSystem\SmartFileInfo
    {
        $set = $this->setProvider->provideByName($setName);
        if (!$set instanceof \RectorPrefix20210421\Symplify\SetConfigResolver\ValueObject\Set) {
    ...
    private function reportSetNotFound(string $setName)
    {
        ...
        throw new \RectorPrefix20210421\Symplify\SetConfigResolver\Exception\SetNotFoundException($message, $setName, $this->setProvider->provideSetNames());
    }
}
