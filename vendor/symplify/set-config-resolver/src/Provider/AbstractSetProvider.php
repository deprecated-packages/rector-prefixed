...
namespace RectorPrefix20210502\Symplify\SetConfigResolver\Provider;
...
abstract class AbstractSetProvider implements \RectorPrefix20210502\Symplify\SetConfigResolver\Contract\SetProviderInterface
{
    /**
     * @return string[]
     */
    ...
    public function provideByName(string $desiredSetName) : ?\RectorPrefix20210502\Symplify\SetConfigResolver\ValueObject\Set
    {
        ...
        } catch (\RectorPrefix20210502\Symplify\SymplifyKernel\Exception\ShouldNotHappenException $shouldNotHappenException) {
        }
        $message = \sprintf('Set "%s" was not found', $desiredSetName);
        throw new \RectorPrefix20210502\Symplify\SetConfigResolver\Exception\SetNotFoundException($message, $desiredSetName, $this->provideSetNames());
    }
    private function resolveSetUniquePathId(string $setPath) : string
    {
        $setPath = \RectorPrefix20210502\Nette\Utils\Strings::after($setPath, \DIRECTORY_SEPARATOR, -2);
        if ($setPath === null) {
