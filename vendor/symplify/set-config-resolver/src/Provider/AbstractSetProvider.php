...
namespace RectorPrefix20210422\Symplify\SetConfigResolver\Provider;
...
abstract class AbstractSetProvider implements \RectorPrefix20210422\Symplify\SetConfigResolver\Contract\SetProviderInterface
{
    /**
     * @return string[]
     */
    ...
    public function provideByName(string $desiredSetName)
    {
        ...
        } catch (\RectorPrefix20210422\Symplify\SymplifyKernel\Exception\ShouldNotHappenException $shouldNotHappenException) {
        }
        $message = \sprintf('Set "%s" was not found', $desiredSetName);
        throw new \RectorPrefix20210422\Symplify\SetConfigResolver\Exception\SetNotFoundException($message, $desiredSetName, $this->provideSetNames());
    }
    private function resolveSetUniquePathId(string $setPath) : string
    {
        $setPath = \RectorPrefix20210422\Nette\Utils\Strings::after($setPath, \DIRECTORY_SEPARATOR, -2);
        if ($setPath === null) {
