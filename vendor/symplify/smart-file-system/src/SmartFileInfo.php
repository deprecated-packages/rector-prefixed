...
namespace _PhpScoper2a4e7ab1ecbc\Symplify\SmartFileSystem;
 ...
 */
final class SmartFileInfo extends \_PhpScoper2a4e7ab1ecbc\Symfony\Component\Finder\SplFileInfo
{
    /**
     * @var string
    ...
    public function __construct($filePath)
    {
        ...
        if (!\file_exists($filePath)) {
            throw new \_PhpScoper2a4e7ab1ecbc\Symplify\SmartFileSystem\Exception\FileNotFoundException(\sprintf('File path "%s" was not found while creating "%s" object.', $filePath, self::class));
        }
        // real path doesn't work in PHAR: https://www.php.net/manual/en/function.realpath.php
        if (\_PhpScoper2a4e7ab1ecbc\Nette\Utils\Strings::startsWith($filePath, 'phar://')) {
    ...
    {
        return \_PhpScoper2a4e7ab1ecbc\Nette\Utils\Strings::replace($this->getRealPath(), self::LAST_SUFFIX_REGEX, '');
    }
    ...
    public function getRelativeFilePathFromDirectory(string $directory) : string
    {
        if (!\file_exists($directory)) {
            throw new \_PhpScoper2a4e7ab1ecbc\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException(\sprintf('Directory "%s" was not found in %s.', $directory, self::class));
        }
    ...
    public function getRelativeFilePathFromCwdInTests() : string
    {
        ...
        if (\_PhpScoper2a4e7ab1ecbc\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            return $this->getRelativeFilePathFromDirectory(\_PhpScoper2a4e7ab1ecbc\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath());
        }
    ...
    {
        return \_PhpScoper2a4e7ab1ecbc\Nette\Utils\Strings::endsWith($this->getNormalizedRealPath(), $string);
    }
    ...
    {
        return \_PhpScoper2a4e7ab1ecbc\Nette\Utils\Strings::startsWith($this->getNormalizedRealPath(), $partialPath);
    }
