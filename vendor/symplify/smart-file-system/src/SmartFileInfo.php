...
namespace RectorPrefix20210317\Symplify\SmartFileSystem;
...
use RectorPrefix20210317\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException;
 ...
 */
final class SmartFileInfo extends \RectorPrefix20210317\Symfony\Component\Finder\SplFileInfo
{
    /**
     * @var string
    ...
    public function __construct(string $filePath)
    {
        ...
        if (!\file_exists($filePath)) {
            throw new \RectorPrefix20210317\Symplify\SmartFileSystem\Exception\FileNotFoundException(\sprintf('File path "%s" was not found while creating "%s" object.', $filePath, self::class));
        }
        // real path doesn't work in PHAR: https://www.php.net/manual/en/function.realpath.php
        if (\RectorPrefix20210317\Nette\Utils\Strings::startsWith($filePath, 'phar://')) {
    ...
    {
        return \RectorPrefix20210317\Nette\Utils\Strings::replace($this->getRealPath(), self::LAST_SUFFIX_REGEX, '');
    }
    ...
    public function getRelativeFilePathFromCwdInTests() : string
    {
        ...
        if (\RectorPrefix20210317\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            return $this->getRelativeFilePathFromDirectory(\RectorPrefix20210317\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath());
        }
