...
namespace RectorPrefix20210207\Symplify\SmartFileSystem;

use RectorPrefix20210207\Nette\Utils\Strings;
use RectorPrefix20210207\Symfony\Component\Finder\SplFileInfo;
use RectorPrefix20210207\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment;
use RectorPrefix20210207\Symplify\EasyTesting\StaticFixtureSplitter;
use RectorPrefix20210207\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException;
 ...
 */
final class SmartFileInfo extends \RectorPrefix20210207\Symfony\Component\Finder\SplFileInfo
{
    /**
     * @var string
    ...
    public function __construct($filePath)
    {
        ...
        if (!\file_exists($filePath)) {
            throw new \RectorPrefix20210207\Symplify\SmartFileSystem\Exception\FileNotFoundException(\sprintf('File path "%s" was not found while creating "%s" object.', $filePath, self::class));
        }
        // real path doesn't work in PHAR: https://www.php.net/manual/en/function.realpath.php
        if (\RectorPrefix20210207\Nette\Utils\Strings::startsWith($filePath, 'phar://')) {
    ...
    {
        return \RectorPrefix20210207\Nette\Utils\Strings::replace($this->getRealPath(), self::LAST_SUFFIX_REGEX, '');
    }
    ...
    public function getRelativeFilePathFromDirectory(string $directory) : string
    {
        if (!\file_exists($directory)) {
            throw new \RectorPrefix20210207\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException(\sprintf('Directory "%s" was not found in %s.', $directory, self::class));
        }
    ...
    public function getRelativeFilePathFromCwdInTests() : string
    {
        ...
        if (\RectorPrefix20210207\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            return $this->getRelativeFilePathFromDirectory(\RectorPrefix20210207\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath());
        }
    ...
    {
        return \RectorPrefix20210207\Nette\Utils\Strings::endsWith($this->getNormalizedRealPath(), $string);
    }
    ...
    {
        return \RectorPrefix20210207\Nette\Utils\Strings::startsWith($this->getNormalizedRealPath(), $partialPath);
    }
