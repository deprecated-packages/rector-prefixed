...
namespace RectorPrefix20210321\Symplify\Astral\Naming;
...
use RectorPrefix20210321\Symplify\Astral\Contract\NodeNameResolverInterface;
/**
...
final class SimpleNameResolver
{
    ...
    public function isName($node, string $desiredName) : bool
    {
        ...
        if (\RectorPrefix20210321\Nette\Utils\Strings::contains($desiredName, '*')) {
    ...
    public function resolveShortNameFromNode(\PhpParser\Node\Stmt\ClassLike $classLike) : ?string
    {
        ...
        if (\RectorPrefix20210321\Nette\Utils\Strings::match($className, self::ANONYMOUS_CLASS_REGEX)) {
            return null;
        }
    ...
    public function isNameMatch(\PhpParser\Node $node, string $desiredNameRegex) : bool
    {
        ...
        return (bool) \RectorPrefix20210321\Nette\Utils\Strings::match($name, $desiredNameRegex);
    }
    public function resolveShortName(string $className) : string
    {
        ...
        return (string) \RectorPrefix20210321\Nette\Utils\Strings::after($className, '\\', -1);
    }
}
