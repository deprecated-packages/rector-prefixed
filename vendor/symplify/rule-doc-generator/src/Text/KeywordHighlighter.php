...
namespace _PhpScopere8e811afab72\Symplify\RuleDocGenerator\Text;
...
final class KeywordHighlighter
{
    ...
    public function __construct(\_PhpScopere8e811afab72\Symplify\PackageBuilder\Reflection\ClassLikeExistenceChecker $classLikeExistenceChecker)
    {
        $this->classLikeExistenceChecker = $classLikeExistenceChecker;
    }
    public function highlight(string $content) : string
    {
        $words = \_PhpScopere8e811afab72\Nette\Utils\Strings::split($content, '# #');
    ...
    {
        if (\_PhpScopere8e811afab72\Nette\Utils\Strings::match($word, self::ANNOTATION_REGEX)) {
            return \true;
        }
        // already in code quotes
        if (\_PhpScopere8e811afab72\Nette\Utils\Strings::startsWith($word, '`') || \_PhpScopere8e811afab72\Nette\Utils\Strings::endsWith($word, '`')) {
            return \false;
        }
        // part of normal text
        ...
        if ((bool) \_PhpScopere8e811afab72\Nette\Utils\Strings::match($word, self::VARIABLE_CALL_OR_VARIABLE_REGEX)) {
            return \true;
        }
        return (bool) \_PhpScopere8e811afab72\Nette\Utils\Strings::match($word, self::STATIC_CALL_REGEX);
    }
    private function isFunctionOrClass(string $word) : bool
    {
        if (\_PhpScopere8e811afab72\Nette\Utils\Strings::match($word, self::METHOD_NAME_REGEX)) {
            return \true;
        }
        ...
        if ($this->classLikeExistenceChecker->doesClassLikeExist($word)) {
            // not a class
            if (!\_PhpScopere8e811afab72\Nette\Utils\Strings::contains($word, '\\')) {
