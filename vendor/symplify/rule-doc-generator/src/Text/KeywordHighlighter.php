...
final class KeywordHighlighter
{
    ...
    {
        $words = \_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::split($content, '# #');
    ...
    {
        if (\_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::match($word, self::ANNOTATION_REGEX)) {
            return \true;
        }
        // already in code quotes
        if (\_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::startsWith($word, '`') || \_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::endsWith($word, '`')) {
            return \false;
        }
        // part of normal text
        ...
        if ((bool) \_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::match($word, self::VARIABLE_CALL_OR_VARIABLE_REGEX)) {
            return \true;
        }
        return (bool) \_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::match($word, self::STATIC_CALL_REGEX);
    }
    private function isFunctionOrClass(string $word) : bool
    {
        if (\_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::match($word, self::METHOD_NAME_REGEX)) {
            return \true;
        }
        ...
        if ($this->classLikeExistenceChecker->doesClassLikeExist($word)) {
            // not a class
            if (!\_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::contains($word, '\\')) {
