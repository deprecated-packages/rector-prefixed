...
namespace _PhpScoperb75b35f52b74\Symplify\RuleDocGenerator\Text;
...
final class KeywordHighlighter
{
    ...
    public function __construct(\_PhpScoperb75b35f52b74\Symplify\PackageBuilder\Reflection\ClassLikeExistenceChecker $classLikeExistenceChecker)
    {
        $this->classLikeExistenceChecker = $classLikeExistenceChecker;
    }
    public function highlight(string $content) : string
    {
        $words = \_PhpScoperb75b35f52b74\Nette\Utils\Strings::split($content, '# #');
    ...
    {
        if (\_PhpScoperb75b35f52b74\Nette\Utils\Strings::match($word, self::ANNOTATION_REGEX)) {
            return \true;
        }
        // already in code quotes
        if (\_PhpScoperb75b35f52b74\Nette\Utils\Strings::startsWith($word, '`') || \_PhpScoperb75b35f52b74\Nette\Utils\Strings::endsWith($word, '`')) {
            return \false;
        }
        // part of normal text
        ...
        if ((bool) \_PhpScoperb75b35f52b74\Nette\Utils\Strings::match($word, self::VARIABLE_CALL_OR_VARIABLE_REGEX)) {
            return \true;
        }
        return (bool) \_PhpScoperb75b35f52b74\Nette\Utils\Strings::match($word, self::STATIC_CALL_REGEX);
    }
    private function isFunctionOrClass(string $word) : bool
    {
        if (\_PhpScoperb75b35f52b74\Nette\Utils\Strings::match($word, self::METHOD_NAME_REGEX)) {
            return \true;
        }
        ...
        if ($this->classLikeExistenceChecker->doesClassLikeExist($word)) {
            // not a class
            if (!\_PhpScoperb75b35f52b74\Nette\Utils\Strings::contains($word, '\\')) {
