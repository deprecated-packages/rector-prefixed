...
final class KeywordHighlighter
{
    ...
    {
        $words = \_PhpScoper17db12703726\Nette\Utils\Strings::split($content, '# #');
    ...
    {
        if (\_PhpScoper17db12703726\Nette\Utils\Strings::match($word, self::ANNOTATION_REGEX)) {
            return \true;
        }
        // already in code quotes
        if (\_PhpScoper17db12703726\Nette\Utils\Strings::startsWith($word, '`') || \_PhpScoper17db12703726\Nette\Utils\Strings::endsWith($word, '`')) {
            return \false;
        }
        // part of normal text
        ...
        if ((bool) \_PhpScoper17db12703726\Nette\Utils\Strings::match($word, self::VARIABLE_CALL_OR_VARIABLE_REGEX)) {
            return \true;
        }
        return (bool) \_PhpScoper17db12703726\Nette\Utils\Strings::match($word, self::STATIC_CALL_REGEX);
    }
    private function isFunctionOrClass(string $word) : bool
    {
        if (\_PhpScoper17db12703726\Nette\Utils\Strings::match($word, self::METHOD_NAME_REGEX)) {
            return \true;
        }
        ...
        if ($this->classLikeExistenceChecker->doesClassLikeExist($word)) {
            // not a class
            if (!\_PhpScoper17db12703726\Nette\Utils\Strings::contains($word, '\\')) {
