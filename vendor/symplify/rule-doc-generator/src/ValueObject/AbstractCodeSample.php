...
abstract class AbstractCodeSample implements \Symplify\RuleDocGenerator\Contract\CodeSampleInterface
{
    ...
    {
        if ($badCode === '') {
            throw new \RectorPrefix20210111\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('Bad sample good code cannot be empty');
        }
        if ($goodCode === $badCode) {
            $errorMessage = \sprintf('Good and bad code cannot be identical: "%s"', $goodCode);
            throw new \RectorPrefix20210111\Symplify\SymplifyKernel\Exception\ShouldNotHappenException($errorMessage);
        }
