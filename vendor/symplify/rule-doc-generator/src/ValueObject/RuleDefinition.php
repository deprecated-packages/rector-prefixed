...
use RectorPrefix20201227\Symplify\RuleDocGenerator\Exception\PoorDocumentationException;
...
final class RuleDefinition
{
    ...
    public function __construct(string $description, array $codeSamples)
    {
        ...
        if ($codeSamples === []) {
            throw new \RectorPrefix20201227\Symplify\RuleDocGenerator\Exception\PoorDocumentationException('Provide at least one code sample, so people can practically see what the rule does');
        }
    ...
    {
        return (string) \RectorPrefix20201227\Nette\Utils\Strings::after($this->ruleClass, '\\', -1);
    }
    /**
