...
namespace RectorPrefix20210218\Symplify\Skipper\FileSystem;
...
final class PathNormalizer
{
    ...
    {
        // ends with *
        if (\RectorPrefix20210218\Nette\Utils\Strings::match($path, self::ONLY_ENDS_WITH_ASTERISK_REGEX)) {
            return '*' . $path;
        }
        // starts with *
        if (\RectorPrefix20210218\Nette\Utils\Strings::match($path, self::ONLY_STARTS_WITH_ASTERISK_REGEX)) {
            return $path . '*';
        }
        if (\RectorPrefix20210218\Nette\Utils\Strings::contains($path, '..')) {
            $path = \realpath($path);
            if ($path === \false) {
                return '';
            }
