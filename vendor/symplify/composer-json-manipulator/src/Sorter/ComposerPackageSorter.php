...
namespace RectorPrefix20210105\Symplify\ComposerJsonManipulator\Sorter;
...
final class ComposerPackageSorter
{
    ...
    private function createNameWithPriority(string $requirementName) : string
    {
        if ($this->isPlatformPackage($requirementName)) {
            return (string) \RectorPrefix20210105\Nette\Utils\Strings::replace($requirementName, self::REQUIREMENT_TYPE_REGEX, function (array $match) : string {
                $name = $match['name'];
    ...
    {
        return (bool) \RectorPrefix20210105\Nette\Utils\Strings::match($name, self::PLATFORM_PACKAGE_REGEX);
    }
}
