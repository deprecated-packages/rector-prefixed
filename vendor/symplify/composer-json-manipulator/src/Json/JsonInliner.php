...
namespace RectorPrefix20201229\Symplify\ComposerJsonManipulator\Json;
...
final class JsonInliner
{
    ...
    {
        $inlineSections = $this->parameterProvider->provideArrayParameter(\RectorPrefix20201229\Symplify\ComposerJsonManipulator\ValueObject\Option::INLINE_SECTIONS);
        foreach ($inlineSections as $inlineSection) {
            ...
            $jsonContent = \RectorPrefix20201229\Nette\Utils\Strings::replace($jsonContent, $pattern, function (array $match) : string {
                $inlined = \RectorPrefix20201229\Nette\Utils\Strings::replace($match[2], self::SPACE_REGEX, ' ');
                $inlined = \trim($inlined);
                $inlined = '[' . $inlined . ']';
