...
final class JsonInliner
{
    ...
    {
        $inlineSections = $this->parameterProvider->provideArrayParameter(\_PhpScoper0a6b37af0871\Symplify\ComposerJsonManipulator\ValueObject\Option::INLINE_SECTIONS);
        foreach ($inlineSections as $inlineSection) {
            ...
            $jsonContent = \_PhpScoper0a6b37af0871\Nette\Utils\Strings::replace($jsonContent, $pattern, function (array $match) : string {
                $inlined = \_PhpScoper0a6b37af0871\Nette\Utils\Strings::replace($match[2], self::SPACE_REGEX, ' ');
                $inlined = \trim($inlined);
                $inlined = '[' . $inlined . ']';
