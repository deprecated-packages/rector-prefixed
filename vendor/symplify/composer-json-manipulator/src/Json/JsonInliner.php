...
namespace RectorPrefix20210203\Symplify\ComposerJsonManipulator\Json;
...
final class JsonInliner
{
    ...
    {
        $inlineSections = $this->parameterProvider->provideArrayParameter(\RectorPrefix20210203\Symplify\ComposerJsonManipulator\ValueObject\Option::INLINE_SECTIONS);
        foreach ($inlineSections as $inlineSection) {
            $pattern = '#("' . \preg_quote($inlineSection, '#') . '": )\\[(.*?)\\](,)#ms';
            $jsonContent = \RectorPrefix20210203\Nette\Utils\Strings::replace($jsonContent, $pattern, function (array $match) : string {
                $inlined = \RectorPrefix20210203\Nette\Utils\Strings::replace($match[2], self::SPACE_REGEX, ' ');
                $inlined = \trim($inlined);
                $inlined = '[' . $inlined . ']';
