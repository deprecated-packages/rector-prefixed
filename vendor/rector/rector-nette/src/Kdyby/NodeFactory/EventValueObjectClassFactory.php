...
use RectorPrefix20210321\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder;
use RectorPrefix20210321\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder;
use RectorPrefix20210321\Symplify\Astral\ValueObject\NodeBuilder\NamespaceBuilder;
/**
...
final class EventValueObjectClassFactory
{
    ...
    private function createEventClassBuilder(string $className) : \RectorPrefix20210321\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder
    {
        $shortClassName = $this->classNaming->getShortName($className);
        $classBuilder = new \RectorPrefix20210321\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder($shortClassName);
        $classBuilder->makeFinal();
        $classBuilder->extend(new \PhpParser\Node\Name\FullyQualified('RectorPrefix20210321\\Symfony\\Contracts\\EventDispatcher\\Event'));
        return $classBuilder;
    }
    /**
     * @param Arg[] $args
     */
    private function decorateWithConstructorIfHasArgs(\RectorPrefix20210321\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder $classBuilder, array $args) : void
    {
        if ($args === []) {
            return;
        }
    ...
    {
        $namespace = \RectorPrefix20210321\Nette\Utils\Strings::before($className, '\\', -1);
        $namespaceBuilder = new \RectorPrefix20210321\Symplify\Astral\ValueObject\NodeBuilder\NamespaceBuilder($namespace);
     ...
     */
    private function ensureVariablesAreUnique(array $variablesWithTypes, \RectorPrefix20210321\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder $classBuilder) : void
    {
        $usedVariableNames = [];
    ...
    {
        $methodBuilder = new \RectorPrefix20210321\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder(\Rector\Core\ValueObject\MethodName::CONSTRUCT);
