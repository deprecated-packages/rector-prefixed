...
final class ComposerNamespaceMatcher
{
    ...
    public function __construct(\RectorPrefix20210419\Symplify\SmartFileSystem\SmartFileSystem $smartFileSystem, \RectorPrefix20210419\Symplify\ComposerJsonManipulator\ComposerJsonFactory $composerJsonFactory)
    {
    ...
    public function matchNamespaceForLocation(string $path) : ?string
    {
        ...
        foreach ($autoload['psr-4'] ?? [] as $namespace => $directory) {
            ...
            foreach ($directory as $singleDirectory) {
                if (!\RectorPrefix20210419\Nette\Utils\Strings::startsWith($path, $singleDirectory)) {
                    continue;
                }
