...
final class ComposerNamespaceMatcher
{
    ...
    public function __construct(\RectorPrefix20210502\Symplify\SmartFileSystem\SmartFileSystem $smartFileSystem, \RectorPrefix20210502\Symplify\ComposerJsonManipulator\ComposerJsonFactory $composerJsonFactory)
    {
    ...
    public function matchNamespaceForLocation(string $path) : ?string
    {
        ...
        foreach ($autoload['psr-4'] ?? [] as $namespace => $directory) {
            ...
            foreach ($directory as $singleDirectory) {
                if (!\RectorPrefix20210502\Nette\Utils\Strings::startsWith($path, $singleDirectory)) {
                    continue;
                }
