...
final class ParseFileRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    private function isArgumentYamlFile(\PhpParser\Node\Expr\StaticCall $staticCall) : bool
    {
        ...
        // is yml/yaml file
        if (\RectorPrefix20210325\Nette\Utils\Strings::match($possibleFileNodeAsString, self::YAML_SUFFIX_IN_QUOTE_REGEX)) {
            return \true;
        }
        // is probably a file variable
        if (\RectorPrefix20210325\Nette\Utils\Strings::match($possibleFileNodeAsString, self::FILE_SUFFIX_REGEX)) {
            return \true;
        }
        ...
        return (bool) \RectorPrefix20210325\Nette\Utils\Strings::match($nodeType->getValue(), self::YAML_SUFFIX_REGEX);
    }
}
