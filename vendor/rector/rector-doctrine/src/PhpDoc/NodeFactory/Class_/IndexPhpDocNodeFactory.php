...
final class IndexPhpDocNodeFactory
{
    ...
    public function createIndexTagValueNodes(?array $indexes, string $annotationContent) : array
    {
        ...
        $indexContents = \RectorPrefix20210326\Nette\Utils\Strings::matchAll($annotationContent, self::INDEX_REGEX);
        $indexTagValueNodes = [];
        foreach ($indexes as $key => $index) {
            $currentContent = $indexContents[$key];
            $items = $this->annotationItemsResolver->resolve($index);
            $indexTagValueNodes[] = new \Rector\Doctrine\PhpDoc\Node\Class_\IndexTagValueNode($this->arrayPartPhpDocTagPrinter, $this->tagValueNodePrinter, $items, $currentContent['content'], $currentContent['tag']);
        }
