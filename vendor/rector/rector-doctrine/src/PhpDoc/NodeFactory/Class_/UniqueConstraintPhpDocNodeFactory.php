...
final class UniqueConstraintPhpDocNodeFactory
{
    ...
    public function createUniqueConstraintTagValueNodes(?array $uniqueConstraints, string $annotationContent) : array
    {
        ...
        $uniqueConstraintContents = \RectorPrefix20210326\Nette\Utils\Strings::matchAll($annotationContent, self::UNIQUE_CONSTRAINT_REGEX);
        ...
        foreach ($uniqueConstraints as $key => $uniqueConstraint) {
            ...
            $uniqueConstraintTagValueNodes[] = new \Rector\Doctrine\PhpDoc\Node\Class_\UniqueConstraintTagValueNode($this->arrayPartPhpDocTagPrinter, $this->tagValueNodePrinter, $items, $subAnnotationContent['content'], $subAnnotationContent['tag']);
        }
