...
CODE_SAMPLE
    ...
    private function renameDataProviderAnnotationsAndCollectRenamedMethods(\PhpParser\Node\Stmt\Class_ $class) : void
    {
        foreach ($class->getMethods() as $classMethod) {
            ...
            foreach ($dataProviderTagValueNodes as $dataProviderTagValueNode) {
                ...
                if (!\RectorPrefix20210413\Nette\Utils\Strings::startsWith($oldMethodName, 'test')) {
                    continue;
                }
                $newMethodName = $this->createNewMethodName($oldMethodName);
                $dataProviderTagValueNode->value->value = \RectorPrefix20210413\Nette\Utils\Strings::replace($oldMethodName, '#' . \preg_quote($oldMethodName, '#') . '#', $newMethodName);
                // invoke reprint
    ...
    {
        $newMethodName = \RectorPrefix20210413\Nette\Utils\Strings::substring($oldMethodName, \strlen('test'));
