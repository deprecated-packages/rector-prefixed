...
class PhpConfig
{
    ...
    {
        if ($data = $this->getDataAndReset()) {
            \RectorPrefix20210427\Composer\XdebugHandler\Process::setEnv('PHPRC', $data['tmpIni']);
            \RectorPrefix20210427\Composer\XdebugHandler\Process::setEnv('PHP_INI_SCAN_DIR', '');
        }
        return array();
    }
    /**
    ...
    {
        if ($data = \RectorPrefix20210427\Composer\XdebugHandler\XdebugHandler::getRestartSettings()) {
            \RectorPrefix20210427\Composer\XdebugHandler\Process::setEnv('PHPRC', $data['phprc']);
            \RectorPrefix20210427\Composer\XdebugHandler\Process::setEnv('PHP_INI_SCAN_DIR', $data['scanDir']);
        }
        return $data;
    }
}
