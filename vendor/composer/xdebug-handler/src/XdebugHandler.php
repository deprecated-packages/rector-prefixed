 ...
 */
namespace RectorPrefix20210408\Composer\XdebugHandler;
...
class XdebugHandler
{
    ...
    public function __construct($envPrefix, $colorOption = '')
    {
        ...
        $this->statusWriter = new \RectorPrefix20210408\Composer\XdebugHandler\Status($this->envAllowXdebug, (bool) $this->debug);
    }
    /**
    ...
    {
        $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::CHECK, $this->loaded);
        $envArgs = \explode('|', (string) \getenv($this->envAllowXdebug));
        if (empty($envArgs[0]) && $this->requiresRestart((bool) $this->loaded)) {
            // Restart required
            $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::RESTART);
        ...
        if (self::RESTART_ID === $envArgs[0] && \count($envArgs) === 5) {
            ...
            $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::RESTARTED);
            \RectorPrefix20210408\Composer\XdebugHandler\Process::setEnv($this->envAllowXdebug);
            self::$inRestart = \true;
            if (!$this->loaded) {
        ...
        $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::NORESTART);
    ...
    {
        $this->tryEnableSignals();
        $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::RESTARTING, $command);
        ...
        if (!isset($exitCode)) {
            ...
            $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::ERROR, 'Unable to restart process');
            $exitCode = -1;
        } else {
            $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::INFO, 'Restarted process exited ' . $exitCode);
        }
        if ($this->debug === '2') {
            $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::INFO, 'Temp ini saved: ' . $this->tmpIni);
        } else {
            @\unlink($this->tmpIni);
        }
        exit($exitCode);
    ...
    private function prepareRestart()
    {
        ...
        if ($error) {
            $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::ERROR, $error);
        }
        return empty($error);
    }
    /**
    ...
    private function getCommand()
    {
        ...
        if (\defined('STDOUT') && \RectorPrefix20210408\Composer\XdebugHandler\Process::supportsColor(\STDOUT)) {
            $args = \RectorPrefix20210408\Composer\XdebugHandler\Process::addColorOption($args, $this->colorOption);
        }
        $args = \array_merge($php, array($this->script), $args);
        $cmd = \RectorPrefix20210408\Composer\XdebugHandler\Process::escape(\array_shift($args), \true, \true);
        foreach ($args as $arg) {
            $cmd .= ' ' . \RectorPrefix20210408\Composer\XdebugHandler\Process::escape($arg);
        }
        return $cmd;
    }
    /**
    ...
    private function setEnvRestartSettings($envArgs)
    {
        ...
        \RectorPrefix20210408\Composer\XdebugHandler\Process::setEnv(self::RESTART_SETTINGS, \implode('|', $settings));
    }
    /**
    ...
    private function syncSettings(array $settings)
    {
        if (\false === \getenv($this->envOriginalInis)) {
            ...
            \RectorPrefix20210408\Composer\XdebugHandler\Process::setEnv($this->envOriginalInis, \implode(\PATH_SEPARATOR, $settings['inis']));
        }
        self::$skipped = $settings['skipped'];
        $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::INFO, 'Process called with existing restart settings');
    }
    /**
    ...
    private function tryEnableSignals()
    {
        if (\function_exists('pcntl_async_signals') && \function_exists('pcntl_signal')) {
            ...
            $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::INFO, $message);
        }
        if (!self::$inRestart && \function_exists('sapi_windows_set_ctrl_handler')) {
            ...
            $this->notify(\RectorPrefix20210408\Composer\XdebugHandler\Status::INFO, 'CTRL signals suppressed');
        }
    }
}
