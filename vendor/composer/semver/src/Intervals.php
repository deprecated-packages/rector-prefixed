...
class Intervals
{
     ...
     * @return bool
     */
    public static function isSubsetOf(\Composer\Semver\Constraint\ConstraintInterface $candidate, \Composer\Semver\Constraint\ConstraintInterface $constraint)
    {
        if ($constraint instanceof \Composer\Semver\Constraint\MatchAllConstraint) {
            return \true;
        }
        if ($candidate instanceof \Composer\Semver\Constraint\MatchNoneConstraint || $constraint instanceof \Composer\Semver\Constraint\MatchNoneConstraint) {
            return \false;
        }
        $intersectionIntervals = self::get(new \Composer\Semver\Constraint\MultiConstraint(array($candidate, $constraint), \true));
     ...
     * @return bool
     */
    public static function haveIntersections(\Composer\Semver\Constraint\ConstraintInterface $a, \Composer\Semver\Constraint\ConstraintInterface $b)
    {
        if ($a instanceof \Composer\Semver\Constraint\MatchAllConstraint || $b instanceof \Composer\Semver\Constraint\MatchAllConstraint) {
            return \true;
        }
        if ($a instanceof \Composer\Semver\Constraint\MatchNoneConstraint || $b instanceof \Composer\Semver\Constraint\MatchNoneConstraint) {
            return \false;
        }
        $intersectionIntervals = self::generateIntervals(new \Composer\Semver\Constraint\MultiConstraint(array($a, $b), \true), \true);
     ...
     */
    public static function compactConstraint(\Composer\Semver\Constraint\ConstraintInterface $constraint)
    {
        if (!$constraint instanceof \Composer\Semver\Constraint\MultiConstraint) {
            return $constraint;
        }
            ...
            for ($i = 0, $count = \count($intervals['numeric']); $i < $count; $i++) {
                ...
                if ($interval->getEnd()->getOperator() === '<' && $i + 1 < $count) {
                    ...
                    if ($interval->getEnd()->getVersion() === $nextInterval->getStart()->getVersion() && $nextInterval->getStart()->getOperator() === '>') {
                        ...
                        $unEqualConstraints[] = new \Composer\Semver\Constraint\Constraint('!=', $interval->getEnd()->getVersion());
                        continue;
                    }
                }
                if (\count($unEqualConstraints) > 0) {
                    ...
                    if (\count($unEqualConstraints) > 1) {
                        $constraints[] = new \Composer\Semver\Constraint\MultiConstraint($unEqualConstraints, \true);
                    } else {
                ...
                if ($interval->getStart()->getVersion() === $interval->getEnd()->getVersion() && $interval->getStart()->getOperator() === '>=' && $interval->getEnd()->getOperator() === '<=') {
                    $constraints[] = new \Composer\Semver\Constraint\Constraint('==', $interval->getStart()->getVersion());
                    continue;
                }
                if ((string) $interval->getStart() === (string) \Composer\Semver\Interval::fromZero()) {
                    ...
                    $constraints[] = new \Composer\Semver\Constraint\MultiConstraint(array($interval->getStart(), $interval->getEnd()), \true);
                }
            }
        }
        ...
        if (0 === \count($intervals['branches']['names'])) {
            ...
            foreach ($intervals['branches']['names'] as $branchName) {
                if ($intervals['branches']['exclude']) {
                    $devConstraints[] = new \Composer\Semver\Constraint\Constraint('!=', $branchName);
                } else {
                    $devConstraints[] = new \Composer\Semver\Constraint\Constraint('==', $branchName);
                }
            }
            ...
            if ($intervals['branches']['exclude']) {
                if (\count($constraints) > 1) {
                    return new \Composer\Semver\Constraint\MultiConstraint(\array_merge(array(new \Composer\Semver\Constraint\MultiConstraint($constraints, \false)), $devConstraints), \true);
                }
                if (\count($constraints) === 1 && (string) $constraints[0] === (string) \Composer\Semver\Interval::fromZero()) {
                    if (\count($devConstraints) > 1) {
                        return new \Composer\Semver\Constraint\MultiConstraint($devConstraints, \true);
                    }
                    return $devConstraints[0];
                }
                return new \Composer\Semver\Constraint\MultiConstraint(\array_merge($constraints, $devConstraints), \true);
            }
        ...
        if (\count($constraints) > 1) {
            return new \Composer\Semver\Constraint\MultiConstraint($constraints, \false);
        }
        if (\count($constraints) === 1) {
            return $constraints[0];
        }
        return new \Composer\Semver\Constraint\MatchNoneConstraint();
    }
    /**
     ...
     */
    public static function get(\Composer\Semver\Constraint\ConstraintInterface $constraint)
    {
     ...
     * @phpstan-return array{'numeric': Interval[], 'branches': array{'names': string[], 'exclude': bool}}
     */
    private static function generateIntervals(\Composer\Semver\Constraint\ConstraintInterface $constraint, $stopOnFirstValidInterval = \false)
    {
        if ($constraint instanceof \Composer\Semver\Constraint\MatchAllConstraint) {
        ...
        if ($constraint instanceof \Composer\Semver\Constraint\MatchNoneConstraint) {
        ...
        foreach ($borders as $border) {
            ...
            if (!$active && $activeIntervals >= $activationThreshold) {
                $start = new \Composer\Semver\Constraint\Constraint($border['operator'], $border['version']);
                $active = \true;
            }
            if ($active && $activeIntervals < $activationThreshold) {
                $active = \false;
                ...
                if (\version_compare($start->getVersion(), $border['version'], '=') && ($start->getOperator() === '>' && $border['operator'] === '<=' || $start->getOperator() === '>=' && $border['operator'] === '<')) {
                    ...
                    $intervals[$index] = new \Composer\Semver\Interval($start, new \Composer\Semver\Constraint\Constraint($border['operator'], $border['version']));
                    $index++;
     ...
     * @phpstan-return array{'numeric': Interval[], 'branches': array{'names': string[], 'exclude': bool}}}
     */
    private static function generateSingleConstraintIntervals(\Composer\Semver\Constraint\Constraint $constraint)
    {
        ...
        if ($op === '!=') {
            ...
            return array('numeric' => array(new \Composer\Semver\Interval(\Composer\Semver\Interval::fromZero(), new \Composer\Semver\Constraint\Constraint('<', $constraint->getVersion())), new \Composer\Semver\Interval(new \Composer\Semver\Constraint\Constraint('>', $constraint->getVersion()), \Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \Composer\Semver\Interval::anyDev());
        }
        // convert ==x to an interval of >=x - <=x
        return array('numeric' => array(new \Composer\Semver\Interval(new \Composer\Semver\Constraint\Constraint('>=', $constraint->getVersion()), new \Composer\Semver\Constraint\Constraint('<=', $constraint->getVersion()))), 'branches' => \Composer\Semver\Interval::noDev());
    }
}
