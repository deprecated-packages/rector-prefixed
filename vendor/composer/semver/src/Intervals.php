 ...
 */
namespace RectorPrefix20210413\Composer\Semver;

use RectorPrefix20210413\Composer\Semver\Constraint\Constraint;
use RectorPrefix20210413\Composer\Semver\Constraint\ConstraintInterface;
use RectorPrefix20210413\Composer\Semver\Constraint\MatchAllConstraint;
use RectorPrefix20210413\Composer\Semver\Constraint\MatchNoneConstraint;
use RectorPrefix20210413\Composer\Semver\Constraint\MultiConstraint;
/**
...
class Intervals
{
     ...
     * @return bool
     */
    public static function isSubsetOf(\RectorPrefix20210413\Composer\Semver\Constraint\ConstraintInterface $candidate, \RectorPrefix20210413\Composer\Semver\Constraint\ConstraintInterface $constraint)
    {
        if ($constraint instanceof \RectorPrefix20210413\Composer\Semver\Constraint\MatchAllConstraint) {
            return \true;
        }
        if ($candidate instanceof \RectorPrefix20210413\Composer\Semver\Constraint\MatchNoneConstraint || $constraint instanceof \RectorPrefix20210413\Composer\Semver\Constraint\MatchNoneConstraint) {
            return \false;
        }
        $intersectionIntervals = self::get(new \RectorPrefix20210413\Composer\Semver\Constraint\MultiConstraint(array($candidate, $constraint), \true));
     ...
     * @return bool
     */
    public static function haveIntersections(\RectorPrefix20210413\Composer\Semver\Constraint\ConstraintInterface $a, \RectorPrefix20210413\Composer\Semver\Constraint\ConstraintInterface $b)
    {
        if ($a instanceof \RectorPrefix20210413\Composer\Semver\Constraint\MatchAllConstraint || $b instanceof \RectorPrefix20210413\Composer\Semver\Constraint\MatchAllConstraint) {
            return \true;
        }
        if ($a instanceof \RectorPrefix20210413\Composer\Semver\Constraint\MatchNoneConstraint || $b instanceof \RectorPrefix20210413\Composer\Semver\Constraint\MatchNoneConstraint) {
            return \false;
        }
        $intersectionIntervals = self::generateIntervals(new \RectorPrefix20210413\Composer\Semver\Constraint\MultiConstraint(array($a, $b), \true), \true);
     ...
     */
    public static function compactConstraint(\RectorPrefix20210413\Composer\Semver\Constraint\ConstraintInterface $constraint)
    {
        if (!$constraint instanceof \RectorPrefix20210413\Composer\Semver\Constraint\MultiConstraint) {
            return $constraint;
        }
        $intervals = self::generateIntervals($constraint);
        $constraints = array();
        $hasNumericMatchAll = \false;
        if (\count($intervals['numeric']) === 1 && (string) $intervals['numeric'][0]->getStart() === (string) \RectorPrefix20210413\Composer\Semver\Interval::fromZero() && (string) $intervals['numeric'][0]->getEnd() === (string) \RectorPrefix20210413\Composer\Semver\Interval::untilPositiveInfinity()) {
            ...
            for ($i = 0, $count = \count($intervals['numeric']); $i < $count; $i++) {
                ...
                if ($interval->getEnd()->getOperator() === '<' && $i + 1 < $count) {
                    ...
                    if ($interval->getEnd()->getVersion() === $nextInterval->getStart()->getVersion() && $nextInterval->getStart()->getOperator() === '>') {
                        ...
                        if (\count($unEqualConstraints) === 0 && (string) $interval->getStart() !== (string) \RectorPrefix20210413\Composer\Semver\Interval::fromZero()) {
                            $unEqualConstraints[] = $interval->getStart();
                        }
                        $unEqualConstraints[] = new \RectorPrefix20210413\Composer\Semver\Constraint\Constraint('!=', $interval->getEnd()->getVersion());
                        continue;
                    }
                }
                if (\count($unEqualConstraints) > 0) {
                    // this is where the end of the following interval of a != constraint is added as explained above
                    if ((string) $interval->getEnd() !== (string) \RectorPrefix20210413\Composer\Semver\Interval::untilPositiveInfinity()) {
                        $unEqualConstraints[] = $interval->getEnd();
                    }
                    // count is 1 if entire constraint is just one != expression
                    if (\count($unEqualConstraints) > 1) {
                        $constraints[] = new \RectorPrefix20210413\Composer\Semver\Constraint\MultiConstraint($unEqualConstraints, \true);
                    } else {
                ...
                if ($interval->getStart()->getVersion() === $interval->getEnd()->getVersion() && $interval->getStart()->getOperator() === '>=' && $interval->getEnd()->getOperator() === '<=') {
                    $constraints[] = new \RectorPrefix20210413\Composer\Semver\Constraint\Constraint('==', $interval->getStart()->getVersion());
                    continue;
                }
                if ((string) $interval->getStart() === (string) \RectorPrefix20210413\Composer\Semver\Interval::fromZero()) {
                    $constraints[] = $interval->getEnd();
                } elseif ((string) $interval->getEnd() === (string) \RectorPrefix20210413\Composer\Semver\Interval::untilPositiveInfinity()) {
                    $constraints[] = $interval->getStart();
                } else {
                    $constraints[] = new \RectorPrefix20210413\Composer\Semver\Constraint\MultiConstraint(array($interval->getStart(), $interval->getEnd()), \true);
                }
            }
        }
        ...
        if (0 === \count($intervals['branches']['names'])) {
            if ($intervals['branches']['exclude']) {
                if ($hasNumericMatchAll) {
                    return new \RectorPrefix20210413\Composer\Semver\Constraint\MatchAllConstraint();
                }
            ...
            foreach ($intervals['branches']['names'] as $branchName) {
                if ($intervals['branches']['exclude']) {
                    $devConstraints[] = new \RectorPrefix20210413\Composer\Semver\Constraint\Constraint('!=', $branchName);
                } else {
                    $devConstraints[] = new \RectorPrefix20210413\Composer\Semver\Constraint\Constraint('==', $branchName);
                }
            }
            ...
            if ($intervals['branches']['exclude']) {
                if (\count($constraints) > 1) {
                    return new \RectorPrefix20210413\Composer\Semver\Constraint\MultiConstraint(\array_merge(array(new \RectorPrefix20210413\Composer\Semver\Constraint\MultiConstraint($constraints, \false)), $devConstraints), \true);
                }
                if (\count($constraints) === 1 && (string) $constraints[0] === (string) \RectorPrefix20210413\Composer\Semver\Interval::fromZero()) {
                    if (\count($devConstraints) > 1) {
                        return new \RectorPrefix20210413\Composer\Semver\Constraint\MultiConstraint($devConstraints, \true);
                    }
                    return $devConstraints[0];
                }
                return new \RectorPrefix20210413\Composer\Semver\Constraint\MultiConstraint(\array_merge($constraints, $devConstraints), \true);
            }
        ...
        if (\count($constraints) > 1) {
            return new \RectorPrefix20210413\Composer\Semver\Constraint\MultiConstraint($constraints, \false);
        }
        if (\count($constraints) === 1) {
            return $constraints[0];
        }
        return new \RectorPrefix20210413\Composer\Semver\Constraint\MatchNoneConstraint();
    }
    /**
     ...
     */
    public static function get(\RectorPrefix20210413\Composer\Semver\Constraint\ConstraintInterface $constraint)
    {
     ...
     * @phpstan-return array{'numeric': Interval[], 'branches': array{'names': string[], 'exclude': bool}}
     */
    private static function generateIntervals(\RectorPrefix20210413\Composer\Semver\Constraint\ConstraintInterface $constraint, $stopOnFirstValidInterval = \false)
    {
        if ($constraint instanceof \RectorPrefix20210413\Composer\Semver\Constraint\MatchAllConstraint) {
            return array('numeric' => array(new \RectorPrefix20210413\Composer\Semver\Interval(\RectorPrefix20210413\Composer\Semver\Interval::fromZero(), \RectorPrefix20210413\Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \RectorPrefix20210413\Composer\Semver\Interval::anyDev());
        }
        if ($constraint instanceof \RectorPrefix20210413\Composer\Semver\Constraint\MatchNoneConstraint) {
            return array('numeric' => array(), 'branches' => array('names' => array(), 'exclude' => \false));
        }
        if ($constraint instanceof \RectorPrefix20210413\Composer\Semver\Constraint\Constraint) {
        ...
        if ($constraint->isDisjunctive()) {
            $branches = \RectorPrefix20210413\Composer\Semver\Interval::noDev();
            ...
            $branches = \RectorPrefix20210413\Composer\Semver\Interval::anyDev();
            foreach ($constraintBranches as $b) {
                if ($b['exclude']) {
                    if ($branches['exclude']) {
        ...
        foreach ($borders as $border) {
            ...
            if (!$active && $activeIntervals >= $activationThreshold) {
                $start = new \RectorPrefix20210413\Composer\Semver\Constraint\Constraint($border['operator'], $border['version']);
                $active = \true;
            }
            if ($active && $activeIntervals < $activationThreshold) {
                $active = \false;
                ...
                if (\version_compare($start->getVersion(), $border['version'], '=') && ($start->getOperator() === '>' && $border['operator'] === '<=' || $start->getOperator() === '>=' && $border['operator'] === '<')) {
                    ...
                    $intervals[$index] = new \RectorPrefix20210413\Composer\Semver\Interval($start, new \RectorPrefix20210413\Composer\Semver\Constraint\Constraint($border['operator'], $border['version']));
                    $index++;
     ...
     * @phpstan-return array{'numeric': Interval[], 'branches': array{'names': string[], 'exclude': bool}}}
     */
    private static function generateSingleConstraintIntervals(\RectorPrefix20210413\Composer\Semver\Constraint\Constraint $constraint)
    {
        ...
        if (\strpos($constraint->getVersion(), 'dev-') === 0) {
            ...
            if ($op === '!=') {
                $intervals[] = new \RectorPrefix20210413\Composer\Semver\Interval(\RectorPrefix20210413\Composer\Semver\Interval::fromZero(), \RectorPrefix20210413\Composer\Semver\Interval::untilPositiveInfinity());
        ...
        if ($op[0] === '>') {
            // > & >=
            return array('numeric' => array(new \RectorPrefix20210413\Composer\Semver\Interval($constraint, \RectorPrefix20210413\Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \RectorPrefix20210413\Composer\Semver\Interval::noDev());
        }
        if ($op[0] === '<') {
            // < & <=
            return array('numeric' => array(new \RectorPrefix20210413\Composer\Semver\Interval(\RectorPrefix20210413\Composer\Semver\Interval::fromZero(), $constraint)), 'branches' => \RectorPrefix20210413\Composer\Semver\Interval::noDev());
        }
        if ($op === '!=') {
            // convert !=x to intervals of 0 - <x && >x - +inf + dev*
            return array('numeric' => array(new \RectorPrefix20210413\Composer\Semver\Interval(\RectorPrefix20210413\Composer\Semver\Interval::fromZero(), new \RectorPrefix20210413\Composer\Semver\Constraint\Constraint('<', $constraint->getVersion())), new \RectorPrefix20210413\Composer\Semver\Interval(new \RectorPrefix20210413\Composer\Semver\Constraint\Constraint('>', $constraint->getVersion()), \RectorPrefix20210413\Composer\Semver\Interval::untilPositiveInfinity())), 'branches' => \RectorPrefix20210413\Composer\Semver\Interval::anyDev());
        }
        // convert ==x to an interval of >=x - <=x
        return array('numeric' => array(new \RectorPrefix20210413\Composer\Semver\Interval(new \RectorPrefix20210413\Composer\Semver\Constraint\Constraint('>=', $constraint->getVersion()), new \RectorPrefix20210413\Composer\Semver\Constraint\Constraint('<=', $constraint->getVersion()))), 'branches' => \RectorPrefix20210413\Composer\Semver\Interval::noDev());
    }
}
