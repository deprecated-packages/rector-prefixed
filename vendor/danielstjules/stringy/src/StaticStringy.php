...
class StaticStringy
{
    ...
    public static function __callStatic($name, $arguments)
    {
        if (!static::$methodArgs) {
            $stringyClass = new \ReflectionClass('RectorPrefix20210425\\Stringy\\Stringy');
        ...
        $stringy = \RectorPrefix20210425\Stringy\Stringy::create($str, $encoding);
        $result = \call_user_func_array([$stringy, $name], $args);
        $cast = function ($val) {
            if (\is_object($val) && $val instanceof \RectorPrefix20210425\Stringy\Stringy) {
                return (string) $val;
            } else {
                return $val;
            }
