...
foreach ($grammarFileToName as $grammarFile => $name) {
    ...
    $grammarCode = \_PhpScoperbf340cb0be9d\resolveNodes($grammarCode);
    $grammarCode = \_PhpScoperbf340cb0be9d\resolveMacros($grammarCode);
    $grammarCode = \_PhpScoperbf340cb0be9d\resolveStackAccess($grammarCode);
    \file_put_contents($tmpGrammarFile, $grammarCode);
    $additionalArgs = $optionDebug ? '-t -v' : '';
    echo "Building {$name} parser.\n";
    $output = \_PhpScoperbf340cb0be9d\execCmd("{$kmyacc} {$additionalArgs} -m {$skeletonFile} -p {$name} {$tmpGrammarFile}");
    $resultCode = \file_get_contents($tmpResultFile);
    $resultCode = \_PhpScoperbf340cb0be9d\removeTrailingWhitespace($resultCode);
    \_PhpScoperbf340cb0be9d\ensureDirExists($resultDir);
    \file_put_contents("{$resultDir}/{$name}.php", $resultCode);
    \unlink($tmpResultFile);
    echo "Building token definition.\n";
    $output = \_PhpScoperbf340cb0be9d\execCmd("{$kmyacc} -m {$tokensTemplate} {$tmpGrammarFile}");
...
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \_PhpScoperbf340cb0be9d\resolveNodes($matches['params']);
        $params = \_PhpScoperbf340cb0be9d\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \_PhpScoperbf340cb0be9d\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \_PhpScoperbf340cb0be9d\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \_PhpScoperbf340cb0be9d\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \_PhpScoperbf340cb0be9d\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \_PhpScoperbf340cb0be9d\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \_PhpScoperbf340cb0be9d\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\_PhpScoperbf340cb0be9d\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
