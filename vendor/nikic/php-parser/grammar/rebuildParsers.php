...
foreach ($grammarFileToName as $grammarFile => $name) {
    ...
    $grammarCode = \RectorPrefix20210130\resolveNodes($grammarCode);
    $grammarCode = \RectorPrefix20210130\resolveMacros($grammarCode);
    $grammarCode = \RectorPrefix20210130\resolveStackAccess($grammarCode);
    \file_put_contents($tmpGrammarFile, $grammarCode);
    $additionalArgs = $optionDebug ? '-t -v' : '';
    echo "Building {$name} parser.\n";
    $output = \RectorPrefix20210130\execCmd("{$kmyacc} {$additionalArgs} -m {$skeletonFile} -p {$name} {$tmpGrammarFile}");
    $resultCode = \file_get_contents($tmpResultFile);
    $resultCode = \RectorPrefix20210130\removeTrailingWhitespace($resultCode);
    \RectorPrefix20210130\ensureDirExists($resultDir);
    \file_put_contents("{$resultDir}/{$name}.php", $resultCode);
    \unlink($tmpResultFile);
    echo "Building token definition.\n";
    $output = \RectorPrefix20210130\execCmd("{$kmyacc} -m {$tokensTemplate} {$tmpGrammarFile}");
...
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \RectorPrefix20210130\resolveNodes($matches['params']);
        $params = \RectorPrefix20210130\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \RectorPrefix20210130\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \RectorPrefix20210130\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \RectorPrefix20210130\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \RectorPrefix20210130\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \RectorPrefix20210130\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \RectorPrefix20210130\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\RectorPrefix20210130\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
