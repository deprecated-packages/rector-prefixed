...
foreach ($grammarFileToName as $grammarFile => $name) {
    ...
    $grammarCode = \RectorPrefix20210331\resolveNodes($grammarCode);
    $grammarCode = \RectorPrefix20210331\resolveMacros($grammarCode);
    $grammarCode = \RectorPrefix20210331\resolveStackAccess($grammarCode);
    \file_put_contents($tmpGrammarFile, $grammarCode);
    $additionalArgs = $optionDebug ? '-t -v' : '';
    echo "Building {$name} parser.\n";
    $output = \RectorPrefix20210331\execCmd("{$kmyacc} {$additionalArgs} -m {$skeletonFile} -p {$name} {$tmpGrammarFile}");
    $resultCode = \file_get_contents($tmpResultFile);
    $resultCode = \RectorPrefix20210331\removeTrailingWhitespace($resultCode);
    \RectorPrefix20210331\ensureDirExists($resultDir);
    \file_put_contents("{$resultDir}/{$name}.php", $resultCode);
    \unlink($tmpResultFile);
    echo "Building token definition.\n";
    $output = \RectorPrefix20210331\execCmd("{$kmyacc} -m {$tokensTemplate} {$tmpGrammarFile}");
...
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \RectorPrefix20210331\resolveNodes($matches['params']);
        $params = \RectorPrefix20210331\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \RectorPrefix20210331\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \RectorPrefix20210331\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \RectorPrefix20210331\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \RectorPrefix20210331\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \RectorPrefix20210331\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \RectorPrefix20210331\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\RectorPrefix20210331\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
