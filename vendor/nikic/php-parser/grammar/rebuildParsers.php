...
foreach ($grammarFileToName as $grammarFile => $name) {
    ...
    $grammarCode = \_PhpScoperfce0de0de1ce\resolveNodes($grammarCode);
    $grammarCode = \_PhpScoperfce0de0de1ce\resolveMacros($grammarCode);
    $grammarCode = \_PhpScoperfce0de0de1ce\resolveStackAccess($grammarCode);
    \file_put_contents($tmpGrammarFile, $grammarCode);
    $additionalArgs = $optionDebug ? '-t -v' : '';
    echo "Building {$name} parser.\n";
    $output = \_PhpScoperfce0de0de1ce\execCmd("{$kmyacc} {$additionalArgs} -m {$skeletonFile} -p {$name} {$tmpGrammarFile}");
    $resultCode = \file_get_contents($tmpResultFile);
    $resultCode = \_PhpScoperfce0de0de1ce\removeTrailingWhitespace($resultCode);
    \_PhpScoperfce0de0de1ce\ensureDirExists($resultDir);
    \file_put_contents("{$resultDir}/{$name}.php", $resultCode);
    \unlink($tmpResultFile);
    echo "Building token definition.\n";
    $output = \_PhpScoperfce0de0de1ce\execCmd("{$kmyacc} -m {$tokensTemplate} {$tmpGrammarFile}");
...
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \_PhpScoperfce0de0de1ce\resolveNodes($matches['params']);
        $params = \_PhpScoperfce0de0de1ce\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \_PhpScoperfce0de0de1ce\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \_PhpScoperfce0de0de1ce\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \_PhpScoperfce0de0de1ce\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \_PhpScoperfce0de0de1ce\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \_PhpScoperfce0de0de1ce\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \_PhpScoperfce0de0de1ce\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\_PhpScoperfce0de0de1ce\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
