...
final class DocParser
{
     ...
     */
    private static $classIdentifiers = [\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_IDENTIFIER, \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_TRUE, \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_FALSE, \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_NULL];
    /**
     * The lexer.
     *
     * @var DocLexer
    ...
    {
        $this->lexer = new \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer();
    }
    /**
     ...
     */
    private function syntaxError(string $expected, ?array $token = null) : \RectorPrefix20210220\Doctrine\Common\Annotations\AnnotationException
    {
        if ($token === null) {
        ...
        $message .= '.';
        return \RectorPrefix20210220\Doctrine\Common\Annotations\AnnotationException::syntaxError($message);
    }
    /**
    ...
    private function classExists(string $fqcn) : bool
    {
        ...
        return $this->classExists[$fqcn] = \RectorPrefix20210220\Doctrine\Common\Annotations\AnnotationRegistry::loadAnnotationClass($fqcn);
    }
    /**
    ...
    private function collectAnnotationMetadata(string $name) : void
    {
        ...
        if ($metadata['is_annotation']) {
            ...
            foreach (self::$metadataParser->parse($docComment, 'class @' . $name) as $annotation) {
                if ($annotation instanceof \RectorPrefix20210220\Doctrine\Common\Annotations\Annotation\Target) {
            ...
            if ($metadata['has_constructor'] === \false) {
                ...
                foreach ($class->getProperties(\ReflectionProperty::IS_PUBLIC) as $property) {
                    ...
                    $attribute = new \RectorPrefix20210220\Doctrine\Common\Annotations\Annotation\Attribute();
                    ...
                    self::$metadataParser->setTarget(\RectorPrefix20210220\Doctrine\Common\Annotations\Annotation\Target::TARGET_PROPERTY);
                    foreach (self::$metadataParser->parse($propertyComment, $context) as $annotation) {
                        if (!$annotation instanceof \RectorPrefix20210220\Doctrine\Common\Annotations\Annotation\Enum) {
                            continue;
                        }
     ...
     */
    private function collectAttributeTypeMetadata(array &$metadata, \RectorPrefix20210220\Doctrine\Common\Annotations\Annotation\Attribute $attribute) : void
    {
    ...
    private function Annotations() : array
    {
        ...
        while ($this->lexer->lookahead !== null) {
            if ($this->lexer->lookahead['type'] !== \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_AT) {
                $this->lexer->moveNext();
                continue;
            }
            ...
            if ($peek === null || $peek['type'] !== \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_NAMESPACE_SEPARATOR && !\in_array($peek['type'], self::$classIdentifiers, \true) || $peek['position'] !== $this->lexer->lookahead['position'] + 1) {
                $this->lexer->moveNext();
                continue;
            }
    ...
    {
        $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_AT);
        // check if we have an annotation
        $name = $this->Identifier();
        if ($this->lexer->isNextToken(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_MINUS) && $this->lexer->nextTokenIsAdjacent()) {
        ...
        if ($name[0] !== '\\') {
            ...
            if (!$found) {
                ...
                throw \RectorPrefix20210220\Doctrine\Common\Annotations\AnnotationException::semanticalError(\sprintf(<<<'EXCEPTION'
...
, $name, $this->context));
        ...
        if (!$this->classExists($name)) {
            throw \RectorPrefix20210220\Doctrine\Common\Annotations\AnnotationException::semanticalError(\sprintf('The annotation "@%s" in %s does not exist, or could not be auto-loaded.', $name, $this->context));
        }
...
, $name, $name, $originalName, $this->context));
        ...
        $target = $this->isNestedAnnotation ? \RectorPrefix20210220\Doctrine\Common\Annotations\Annotation\Target::TARGET_ANNOTATION : $this->target;
        // Next will be nested
...
, $originalName, $this->context, self::$annotationMetadata[$name]['targets_literal']));
        ...
        if (isset(self::$annotationMetadata[$name]['enum'])) {
            ...
            foreach (self::$annotationMetadata[$name]['enum'] as $property => $enum) {
                ...
                if (isset($values[$property]) && !\in_array($values[$property], $enum['value'])) {
                    throw \RectorPrefix20210220\Doctrine\Common\Annotations\AnnotationException::enumeratorError($property, $name, $this->context, $enum['literal'], $values[$property]);
                }
            }
        }
        // checks all declared attributes
        foreach (self::$annotationMetadata[$name]['attribute_types'] as $property => $type) {
            ...
            if (!isset($values[$property])) {
                if ($type['required']) {
                    throw \RectorPrefix20210220\Doctrine\Common\Annotations\AnnotationException::requiredError($property, $originalName, $this->context, 'a(n) ' . $type['value']);
                }
                continue;
            }
            if ($type['type'] === 'array') {
                ...
                if (isset($type['array_type'])) {
                    foreach ($values[$property] as $item) {
                        if (\gettype($item) !== $type['array_type'] && !$item instanceof $type['array_type']) {
                            throw \RectorPrefix20210220\Doctrine\Common\Annotations\AnnotationException::attributeTypeError($property, $originalName, $this->context, 'either a(n) ' . $type['array_type'] . ', or an array of ' . $type['array_type'] . 's', $item);
                        }
                    }
                }
        ...
        if (\is_subclass_of($name, \RectorPrefix20210220\Doctrine\Common\Annotations\NamedArgumentConstructorAnnotation::class)) {
            ...
            foreach ($values as $property => $value) {
                if (!isset(self::$annotationMetadata[$name]['constructor_args'][$property])) {
                    throw \RectorPrefix20210220\Doctrine\Common\Annotations\AnnotationException::creationError(\sprintf(<<<'EXCEPTION'
...
, $originalName, $this->context, $property, \implode(', ', self::$annotationMetadata[$name]['properties'])));
                ...
                if (!$property) {
                    throw \RectorPrefix20210220\Doctrine\Common\Annotations\AnnotationException::creationError(\sprintf('The annotation @%s declared on %s does not accept any values, but got %s.', $originalName, $this->context, \json_encode($values)));
                }
            }
    ...
    {
        $values = [];
        if (!$this->lexer->isNextToken(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS)) {
            return $values;
        }
        $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_OPEN_PARENTHESIS);
        if (!$this->lexer->isNextToken(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS)) {
            $values = $this->Values();
        }
        $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS);
        return $values;
    }
    /**
    ...
    {
        $values = [$this->Value()];
        while ($this->lexer->isNextToken(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_COMMA)) {
            $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_COMMA);
            if ($this->lexer->isNextToken(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_CLOSE_PARENTHESIS)) {
                break;
            }
            $token = $this->lexer->lookahead;
    ...
    private function Constant()
    {
        ...
        if (!\defined($identifier)) {
            throw \RectorPrefix20210220\Doctrine\Common\Annotations\AnnotationException::semanticalErrorConstants($identifier, $this->context);
        }
    ...
    private function Identifier() : string
    {
        ...
        while ($this->lexer->lookahead !== null && $this->lexer->lookahead['position'] === $this->lexer->token['position'] + \strlen($this->lexer->token['value']) && $this->lexer->isNextToken(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_NAMESPACE_SEPARATOR)) {
            $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_NAMESPACE_SEPARATOR);
    ...
    {
        $peek = $this->lexer->glimpse();
        if ($peek['type'] === \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_EQUALS) {
    ...
    {
        if ($this->lexer->isNextToken(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_OPEN_CURLY_BRACES)) {
            return $this->Arrayx();
        }
        if ($this->lexer->isNextToken(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_AT)) {
            return $this->Annotation();
        }
        if ($this->lexer->isNextToken(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_IDENTIFIER)) {
            return $this->Constant();
        }
        switch ($this->lexer->lookahead['type']) {
            case \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_STRING:
                $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_STRING);
                return $this->lexer->token['value'];
            case \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_INTEGER:
                $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_INTEGER);
                return (int) $this->lexer->token['value'];
            case \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_FLOAT:
                $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_FLOAT);
                return (float) $this->lexer->token['value'];
            case \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_TRUE:
                $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_TRUE);
                return \true;
            case \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_FALSE:
                $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_FALSE);
                return \false;
            case \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_NULL:
                $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_NULL);
                return null;
            default:
    ...
    {
        $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_IDENTIFIER);
        $fieldName = $this->lexer->token['value'];
        $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_EQUALS);
        $item = new \stdClass();
        $item->name = $fieldName;
    ...
    {
        $array = $values = [];
        $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_OPEN_CURLY_BRACES);
        // If the array is empty, stop parsing and return.
        if ($this->lexer->isNextToken(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_CLOSE_CURLY_BRACES)) {
            $this->match(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_CLOSE_CURLY_BRACES);
            return $array;
        }
    ...
    {
        $peek = $this->lexer->glimpse();
        if ($peek['type'] === \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_EQUALS || $peek['type'] === \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_COLON) {
            if ($this->lexer->isNextToken(\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_IDENTIFIER)) {
                $key = $this->Constant();
            } else {
                $this->matchAny([\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_INTEGER, \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_STRING]);
                $key = $this->lexer->token['value'];
            }
            $this->matchAny([\RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_EQUALS, \RectorPrefix20210220\Doctrine\Common\Annotations\DocLexer::T_COLON]);
