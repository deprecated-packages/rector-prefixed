 ...
 */
namespace RectorPrefix20210316\Symfony\Polyfill\Tests\Mbstring;

use RectorPrefix20210316\PHPUnit\Framework\TestCase;
use RectorPrefix20210316\Symfony\Polyfill\Mbstring\Mbstring as p;
/**
 * @author Nicolas Grekas <p@tchwork.com>
 *
 * @covers \Symfony\Polyfill\Mbstring\Mbstring::<!public>
 */
class MbstringTest extends \RectorPrefix20210316\PHPUnit\Framework\TestCase
{
    /**
    ...
    public function testDecodeNumericEntity()
    {
        ...
        $this->assertSame("�", \mb_decode_numericentity('&#351;', $convmap, 'ISO-8859-9'));
    }
    /**
     * @covers \Symfony\Polyfill\Mbstring\Mbstring::mb_decode_numericentity
     * @requires PHP < 8
    ...
    public function testEncodeNumericEntity()
    {
        ...
        $this->assertSame('&#128514; &#96;', \mb_encode_numericentity("\0 a", $convmap, 'UTF-8'));
        $convmap = [0x100, 0x10ffff, 0x0, 0x1fffff];
        $this->assertSame('&#351;', \mb_encode_numericentity("�", $convmap, 'ISO-8859-9'));
        $this->assertSame('&#351;', \mb_encode_numericentity("�", $convmap, 'ISO-8859-9', \false));
        $this->assertSame('&#x15F;', \mb_encode_numericentity("�", $convmap, 'ISO-8859-9', \true));
        // U+1F602 FACE WITH TEARS OF JOY is F0 9F 98 82 in UTF-8. ISO-8859-9 leaves 7F-9F undefined.
        $this->assertSame("abc &#287;���", \mb_encode_numericentity('abc 😂', $convmap, 'ISO-8859-9'));
    }
    /**
     * @covers \Symfony\Polyfill\Mbstring\Mbstring::mb_encode_numericentity
     * @requires PHP < 8
    ...
    public function testStrCase()
    {
        ...
        $this->assertSame('déjà σσς iiıi', \RectorPrefix20210316\Symfony\Polyfill\Mbstring\Mbstring::mb_strtolower('DÉJÀ Σσς İIıi'));
        $this->assertSame('DÉJÀ ΣΣΣ İIII', \mb_strtoupper('Déjà Σσς İIıi'));
        if (\PCRE_VERSION >= '8.10') {
            $this->assertSame('Déjà Σσσ Iı Ii İi', \RectorPrefix20210316\Symfony\Polyfill\Mbstring\Mbstring::mb_convert_case('DÉJÀ ΣΣΣ ıı iI İİ', \MB_CASE_TITLE));
        }
    ...
    public function testTitleCase()
    {
        for ($i = 1; $i < 127; ++$i) {
            ...
            $this->assertSame(\mb_convert_case('a' . \chr($i) . 'b', \MB_CASE_TITLE, 'UTF-8'), \RectorPrefix20210316\Symfony\Polyfill\Mbstring\Mbstring::mb_convert_case('a' . \chr($i) . 'b', \MB_CASE_TITLE, 'UTF-8'), 'Title case for char 0x' . \dechex($i));
        }
    }
    /**
    ...
    {
        $this->assertSame(2, \mb_strlen("\0�", 'ASCII'));
        $this->assertSame(2, \mb_strlen("\0�", 'CP850'));
    ...
    public function testSubstr()
    {
        ...
        $this->assertSame('', \mb_substr($c, -5, 0));
        $this->assertSame("�", \mb_substr("\0�", -1, 1, 'ASCII'));
        $this->assertSame("\0", \mb_substr("\0�", 0, 1, 'ASCII'));
        $this->assertSame("\0�", \mb_substr("\0�", 0, 2, 'ASCII'));
        $this->assertSame('', \mb_substr("\0�", 2, 1, 'ASCII'));
        $this->assertSame('', \mb_substr("\0�", 3, 1, 'ASCII'));
        $this->assertSame("�", \mb_substr("\0�", -1, 1, 'CP850'));
        $this->assertSame("\0", \mb_substr("\0�", 0, 1, 'CP850'));
        $this->assertSame("\0�", \mb_substr("\0�", 0, 2, 'CP850'));
        $this->assertSame('', \mb_substr("\0�", 2, 1, 'CP850'));
        $this->assertSame('', \mb_substr("\0�", 3, 1, 'CP850'));
    }
    /**
    ...
    public function testStrstr()
    {
        ...
        $this->assertSame('d', \mb_strstr('déjàdéjà', 'é', \true));
        $this->assertSame('D', \mb_stristr('DÉJÀDÉJÀ', 'é', \true));
        $this->assertSame('a', \mb_stristr('aςσb', 'ΣΣ', \true));
        $this->assertSame('déjàd', \mb_strrchr('déjàdéjà', 'é', \true));
        $this->assertSame('déjàd', \mb_strrchr('déjàdéjà', 'é', \true, 'ASCII'));
        $this->assertFalse(\mb_strrchr('déjàdéjà', 'X', \true, 'ASCII'));
        $this->assertSame('DÉJÀD', \mb_strrichr('DÉJÀDÉJÀ', 'é', \true));
    ...
    {
        $this->assertFalse(\RectorPrefix20210316\Symfony\Polyfill\Mbstring\Mbstring::mb_check_encoding());
        $this->assertTrue(\mb_check_encoding('aςσb', 'UTF8'));
        $this->assertTrue(\mb_check_encoding('abc', 'ASCII'));
        $this->assertTrue(\mb_check_encoding("�", 'Windows-1252'));
    }
    /**
    ...
    public function testDetectEncoding()
    {
        ...
        $this->assertSame('ISO-8859-1', \mb_detect_encoding("�", ['UTF-8', 'ASCII', 'ISO-8859-1'], \true));
        $this->assertFalse(\mb_detect_encoding("�", ['UTF-8', 'ASCII', 'Windows-1252'], \true));
    }
    /**
    ...
    public function testEncodingAliases()
    {
        ...
        $this->assertFalse(\RectorPrefix20210316\Symfony\Polyfill\Mbstring\Mbstring::mb_encoding_aliases('ASCII'));
    }
    /**
    ...
    {
        $this->assertSame(3, \mb_strwidth("\0実", 'UTF-8'));
    ...
    {
        $this->assertSame("𠮷", \mb_chr(0x20bb7));
        $this->assertSame("�", \mb_chr(0xe9, 'CP1252'));
    }
    /**
    ...
    {
        $this->assertSame(0x20bb7, \mb_ord("𠮷"));
        $this->assertSame(0xe9, \mb_ord("�", 'CP1252'));
    }
    public function testScrub()
    {
        $subst = \mb_substitute_character();
        \mb_substitute_character('none');
        $this->assertSame('ab', \mb_scrub("a�b"));
