<?php

/*
 * This file is part of the Symfony package.
 *
 * (c) Fabien Potencier <fabien@symfony.com>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */
namespace RectorPrefix20210316\Symfony\Polyfill\Tests\Intl\Icu;

use RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Collator;
use RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentValueNotImplementedException;
use RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodNotImplementedException;
use RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu;
/**
 * @group class-polyfill
 */
class CollatorTest extends \RectorPrefix20210316\Symfony\Polyfill\Tests\Intl\Icu\AbstractCollatorTest
{
    public function testConstructorWithUnsupportedLocale()
    {
        $this->expectException(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentValueNotImplementedException::class);
        $this->getCollator('pt_BR');
    }
    public function testCompare()
    {
        $this->expectException(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodNotImplementedException::class);
        $collator = $this->getCollator('en');
        $collator->compare('a', 'b');
    }
    public function testGetAttribute()
    {
        $this->expectException(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodNotImplementedException::class);
        $collator = $this->getCollator('en');
        $collator->getAttribute(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Collator::NUMERIC_COLLATION);
    }
    public function testGetErrorCode()
    {
        $collator = $this->getCollator('en');
        $this->assertEquals(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::U_ZERO_ERROR, $collator->getErrorCode());
    }
    public function testGetErrorMessage()
    {
        $collator = $this->getCollator('en');
        $this->assertEquals('U_ZERO_ERROR', $collator->getErrorMessage());
    }
    public function testGetLocale()
    {
        $collator = $this->getCollator('en');
        $this->assertEquals('en', $collator->getLocale());
    }
    public function testConstructWithoutLocale()
    {
        $collator = $this->getCollator(null);
        $this->assertInstanceOf(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Collator::class, $collator);
    }
    public function testGetSortKey()
    {
        $this->expectException(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodNotImplementedException::class);
        $collator = $this->getCollator('en');
        $collator->getSortKey('Hello');
    }
    public function testGetStrength()
    {
        $this->expectException(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodNotImplementedException::class);
        $collator = $this->getCollator('en');
        $collator->getStrength();
    }
    public function testSetAttribute()
    {
        $this->expectException(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodNotImplementedException::class);
        $collator = $this->getCollator('en');
        $collator->setAttribute(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Collator::NUMERIC_COLLATION, \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Collator::ON);
    }
    public function testSetStrength()
    {
        $this->expectException(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodNotImplementedException::class);
        $collator = $this->getCollator('en');
        $collator->setStrength(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Collator::PRIMARY);
    }
    public function testStaticCreate()
    {
        $collator = $this->getCollator('en');
        $collator = $collator::create('en');
        $this->assertInstanceOf(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Collator::class, $collator);
    }
    protected function getCollator(?string $locale) : \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Collator
    {
        return new class($locale) extends \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Collator
        {
        };
    }
}
