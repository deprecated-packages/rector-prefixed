 ...
 */
use RectorPrefix20210316\Symfony\Polyfill\Apcu as p;
if (\extension_loaded('Zend Data Cache')) {
    if (!\function_exists('apcu_add')) {
        function apcu_add($key, mixed $value, ?int $ttl = 0) : array|bool
        {
            return \RectorPrefix20210316\Symfony\Polyfill\Apcu\Apcu::apcu_add($key, $value, (int) $ttl);
        }
    }
    if (!\function_exists('apcu_delete')) {
        function apcu_delete($key) : array|bool
        {
            return \RectorPrefix20210316\Symfony\Polyfill\Apcu\Apcu::apcu_delete($key);
        }
    }
    if (!\function_exists('apcu_exists')) {
        function apcu_exists($key) : array|bool
        {
            return \RectorPrefix20210316\Symfony\Polyfill\Apcu\Apcu::apcu_exists($key);
        }
    }
    if (!\function_exists('apcu_fetch')) {
        function apcu_fetch($key, &$success = null) : mixed
        {
            return \RectorPrefix20210316\Symfony\Polyfill\Apcu\Apcu::apcu_fetch($key, $success);
        }
    }
    if (!\function_exists('apcu_store')) {
        function apcu_store($key, mixed $value, ?int $ttl = 0) : array|bool
        {
            return \RectorPrefix20210316\Symfony\Polyfill\Apcu\Apcu::apcu_store($key, $value, (int) $ttl);
        }
    }
} else {
    if (!\function_exists('apcu_add')) {
