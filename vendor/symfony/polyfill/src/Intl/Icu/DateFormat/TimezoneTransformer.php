 ...
 * @internal
 */
class TimezoneTransformer extends \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\DateFormat\Transformer
{
    /**
     * {@inheritdoc}
     *
    ...
    public function format(\DateTime $dateTime, int $length) : string
    {
        ...
        if (!\in_array($timeZone, ['Etc', 'UTC', 'GMT'])) {
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\NotImplementedException('Time zone different than GMT or UTC is not supported as a formatting output.');
        }
        if ('Etc' === $timeZone) {
        ...
        if ($length > 3) {
            return $dateTime->format('RectorPrefix20210316\\G\\M\\TP');
        }
    ...
    {
        return ['timezone' => self::getEtcTimeZoneId($matched)];
    }
    /**
    ...
    public static function getEtcTimeZoneId(string $formattedTimeZone) : string
    {
        if (\preg_match('/GMT(?P<signal>[+-])(?P<hours>\\d{2}):?(?P<minutes>\\d{2})/', $formattedTimeZone, $matches)) {
            ...
            if (0 < $minutes) {
                throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\NotImplementedException(\sprintf('It is not possible to use a GMT time zone with minutes offset different than zero (0). GMT time zone tried: "%s".', $formattedTimeZone));
            }
