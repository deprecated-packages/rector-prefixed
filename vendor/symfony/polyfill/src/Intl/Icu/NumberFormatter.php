 ...
 */
namespace RectorPrefix20210316\Symfony\Polyfill\Intl\Icu;
...
abstract class NumberFormatter
{
     ...
     * @var int
     */
    protected $errorCode = \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::U_ZERO_ERROR;
    /**
     ...
     */
    private $attributes = [self::FRACTION_DIGITS => 0, self::GROUPING_USED => 1, self::ROUNDING_MODE => self::ROUND_HALFEVEN];
    /**
     ...
     */
    private static $supportedStyles = ['CURRENCY' => self::CURRENCY, 'DECIMAL' => self::DECIMAL];
    /**
     * Supported attributes to the setAttribute() $attr argument.
     */
    private static $supportedAttributes = ['FRACTION_DIGITS' => self::FRACTION_DIGITS, 'GROUPING_USED' => self::GROUPING_USED, 'ROUNDING_MODE' => self::ROUNDING_MODE];
    /**
     ...
     */
    private static $roundingModes = ['ROUND_HALFEVEN' => self::ROUND_HALFEVEN, 'ROUND_HALFDOWN' => self::ROUND_HALFDOWN, 'ROUND_HALFUP' => self::ROUND_HALFUP, 'ROUND_CEILING' => self::ROUND_CEILING, 'ROUND_FLOOR' => self::ROUND_FLOOR, 'ROUND_DOWN' => self::ROUND_DOWN, 'ROUND_UP' => self::ROUND_UP];
    /**
     ...
     */
    private static $phpRoundingMap = [self::ROUND_HALFDOWN => \PHP_ROUND_HALF_DOWN, self::ROUND_HALFEVEN => \PHP_ROUND_HALF_EVEN, self::ROUND_HALFUP => \PHP_ROUND_HALF_UP];
    /**
     ...
     */
    private static $customRoundingList = [self::ROUND_CEILING => \true, self::ROUND_FLOOR => \true, self::ROUND_DOWN => \true, self::ROUND_UP => \true];
    /**
    ...
    private static $enSymbols = [self::DECIMAL => ['.', ',', ';', '%', '0', '#', '-', '+', '¤', '¤¤', '.', 'E', '‰', '*', '∞', 'NaN', '@', ','], self::CURRENCY => ['.', ',', ';', '%', '0', '#', '-', '+', '¤', '¤¤', '.', 'E', '‰', '*', '∞', 'NaN', '@', ',']];
    private static $enTextAttributes = [self::DECIMAL => ['', '', '-', '', ' ', 'XXX', ''], self::CURRENCY => ['¤', '', '-¤', '', ' ', 'XXX']];
    /**
    ...
    public function __construct(?string $locale = 'en', int $style = null, string $pattern = null)
    {
        if ('en' !== $locale && null !== $locale) {
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentValueNotImplementedException(__METHOD__, 'locale', $locale, 'Only the locale "en" is supported');
        }
        if (!\in_array($style, self::$supportedStyles)) {
            $message = \sprintf('The available styles are: %s.', \implode(', ', \array_keys(self::$supportedStyles)));
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentValueNotImplementedException(__METHOD__, 'style', $style, $message);
        }
        if (null !== $pattern) {
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentNotImplementedException(__METHOD__, 'pattern');
        }
        $this->style = $style;
    }
    /**
    ...
    public function formatCurrency(float $amount, string $currency)
    {
        ...
        if (null === ($symbol = \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Currencies::getSymbol($currency))) {
            return \false;
        }
        $fractionDigits = \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Currencies::getFractionDigits($currency);
        ...
        $ret = $symbol . (\mb_strlen($symbol, 'UTF-8') > 2 ? " " : '') . $amount;
    ...
    public function format($num, int $type = self::TYPE_DEFAULT)
    {
        ...
        if (self::TYPE_CURRENCY === $type) {
            ...
            \trigger_error(__METHOD__ . '(): Unsupported format type ' . $type, \E_USER_WARNING);
            return \false;
        }
        if (self::CURRENCY === $this->style) {
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\NotImplementedException(\sprintf('"%s()" method does not support the formatting of currencies (instance with CURRENCY style). "%s".', __METHOD__, \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\NotImplementedException::INTL_INSTALL_MESSAGE));
        }
        // Only the default type is supported.
        if (self::TYPE_DEFAULT !== $type) {
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentValueNotImplementedException(__METHOD__, 'type', $type, 'Only TYPE_DEFAULT is supported');
        }
     ...
     */
    public function getLocale(int $type = \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Locale::ACTUAL_LOCALE)
    {
        return 'en';
    }
    /**
    ...
    {
        throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodNotImplementedException(__METHOD__);
    }
    /**
    ...
    public function parse(string $string, int $type = self::TYPE_DOUBLE, &$offset = null)
    {
        ...
        if ($error) {
            \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::setError(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::U_PARSE_ERROR, 'Number parsing failed');
            $this->errorCode = \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::getErrorCode();
            $this->errorMessage = \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::getErrorMessage();
            return \false;
        }
    ...
    public function setAttribute(int $attribute, $value)
    {
        if (!\in_array($attribute, self::$supportedAttributes)) {
            $message = \sprintf('The available attributes are: %s', \implode(', ', \array_keys(self::$supportedAttributes)));
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentValueNotImplementedException(__METHOD__, 'attribute', $value, $message);
        }
        if (self::$supportedAttributes['ROUNDING_MODE'] === $attribute && $this->isInvalidRoundingMode($value)) {
            $message = \sprintf('The supported values for ROUNDING_MODE are: %s', \implode(', ', \array_keys(self::$roundingModes)));
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentValueNotImplementedException(__METHOD__, 'attribute', $value, $message);
        }
        ...
        $this->initializedAttributes[$attribute] = \true;
        return \true;
    }
    /**
    ...
    {
        \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::setError(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::U_ZERO_ERROR);
        $this->errorCode = \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::getErrorCode();
        $this->errorMessage = \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::getErrorMessage();
    }
    /**
    ...
    {
        $fractionDigits = \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Currencies::getFractionDigits($currency);
        $roundingIncrement = \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Currencies::getRoundingIncrement($currency);
    ...
    {
        $precision = $this->getUninitializedPrecision($value, $precision);
        return \number_format($value, $precision, '.', $this->getAttribute(self::GROUPING_USED) ? ',' : '');
    }
    /**
    ...
    {
        if (\in_array($value, self::$roundingModes, \true)) {
            return \false;
        }
        return \true;
    }
