...
use RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\DateFormat\FullTransformer;
...
abstract class IntlDateFormatter
{
     ...
     */
    private $defaultDateFormats = [self::NONE => '', self::FULL => 'EEEE, MMMM d, y', self::LONG => 'MMMM d, y', self::MEDIUM => 'MMM d, y', self::SHORT => 'M/d/yy'];
    /**
     * Patterns used to format the time when no pattern is provided.
     */
    private $defaultTimeFormats = [self::FULL => 'h:mm:ss a zzzz', self::LONG => 'h:mm:ss a z', self::MEDIUM => 'h:mm:ss a', self::SHORT => 'h:mm a'];
    private $dateType;
    private $timeType;
    /**
     * @var string
    ...
    public function __construct(?string $locale, ?int $dateType, ?int $timeType, $timezone = null, $calendar = null, ?string $pattern = '')
    {
        ...
        if (self::GREGORIAN !== $calendar && null !== $calendar) {
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentValueNotImplementedException(__METHOD__, 'calendar', $calendar, 'Only the GREGORIAN calendar is supported');
        }
    ...
    public function format($datetime)
    {
        ...
        if (\is_array($datetime)) {
            ...
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentValueNotImplementedException(__METHOD__, 'timestamp', $datetime, $message);
        }
        ...
        if (null !== $argumentError) {
            \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::setError(\RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Icu::U_ILLEGAL_ARGUMENT_ERROR, $argumentError);
        ...
        $transformer = new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\DateFormat\FullTransformer($this->getPattern(), $this->getTimeZoneId());
    ...
    public function parse(string $string, &$offset = null)
    {
        ...
        if (null !== $offset) {
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentNotImplementedException(__METHOD__, 'offset');
        }
    ...
    {
        if ($lenient) {
            throw new \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\Exception\MethodArgumentValueNotImplementedException(__METHOD__, 'lenient', $lenient, 'Only the strict parser is supported');
        }
        return \true;
    }
    /**
    ...
    public function setTimeZone($timezone)
    {
        ...
        if (null === $timezone) {
            ...
            $this->uninitializedTimeZoneId = \true;
        }
        ...
        if ('GMT' !== $timezone && 0 === \strpos($timezone, 'GMT')) {
            try {
                $timezone = \RectorPrefix20210316\Symfony\Polyfill\Intl\Icu\DateFormat\TimezoneTransformer::getEtcTimeZoneId($timezone);
