...
trait ReferenceSetArgumentTrait
{
    ...
    public function setValues(array $values)
    {
        foreach ($values as $k => $v) {
            if (null !== $v && !$v instanceof \RectorPrefix20210421\Symfony\Component\DependencyInjection\Reference) {
                throw new \RectorPrefix20210421\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('A "%s" must hold only Reference instances, "%s" given.', __CLASS__, \get_debug_type($v)));
            }
        }
        $this->values = $values;
    }
