...
use _PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils;
 ...
 */
class XmlFileLoader extends \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Loader\FileLoader
{
    ...
    private function parseImports(\DOMDocument $xml, string $file)
    {
        ...
        foreach ($imports as $import) {
            ...
            $this->import($import->getAttribute('resource'), \_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::phpize($import->getAttribute('type')) ?: null, \_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::phpize($import->getAttribute('ignore-errors')) ?: \false, $file);
        }
    }
    private function parseDefinitions(\DOMDocument $xml, string $file, \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Definition $defaults)
    {
        $xpath = new \DOMXPath($xml);
        $xpath->registerNamespace('container', self::NS);
        ...
        foreach ($instanceof as $service) {
            $this->setDefinition((string) $service->getAttribute('id'), $this->parseDefinition($service, $file, new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Definition()));
        }
            ...
            if ('stack' === $service->tagName) {
                ...
                foreach ($this->getChildren($service, 'service') as $k => $frame) {
                    ...
                    if ($alias = $frame->getAttribute('alias')) {
                        ...
                        $stack[$k] = new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Reference($alias);
                    } else {
                ...
                if ('prototype' === $service->tagName) {
                    ...
                    if ($service->hasAttribute('exclude')) {
                        if (\count($excludes) > 0) {
                            throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('You cannot use both the attribute "exclude" and <exclude> tags at the same time.');
                        }
    ...
    private function getServiceDefaults(\DOMDocument $xml, string $file) : \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Definition
    {
        $xpath = new \DOMXPath($xml);
        $xpath->registerNamespace('container', self::NS);
        if (null === ($defaultsNode = $xpath->query('//container:services/container:defaults')->item(0))) {
            return new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Definition();
        }
        $defaultsNode->setAttribute('id', '<defaults>');
        return $this->parseDefinition($defaultsNode, $file, new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Definition());
    }
    /**
     * Parses an individual Definition.
     */
    private function parseDefinition(\DOMElement $service, string $file, \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Definition $defaults) : ?\_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Definition
    {
        if ($alias = $service->getAttribute('alias')) {
            $this->validateAlias($service, $file);
            $this->container->setAlias((string) $service->getAttribute('id'), $alias = new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Alias($alias));
            if ($publicAttr = $service->getAttribute('public')) {
                $alias->setPublic(\_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::phpize($publicAttr));
        ...
        if ($this->isLoadingInstanceof) {
            ...
            $definition = new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\ChildDefinition($parent);
        } else {
        ...
        foreach (['class', 'public', 'shared', 'synthetic', 'abstract'] as $key) {
            if ($value = $service->getAttribute($key)) {
                $method = 'set' . $key;
                $definition->{$method}($value = \_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::phpize($value));
            }
        }
        if ($value = $service->getAttribute('lazy')) {
            $definition->setLazy((bool) ($value = \_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::phpize($value)));
            if (\is_string($value)) {
                $definition->addTag('proxy', ['interface' => $value]);
            }
        }
        if ($value = $service->getAttribute('autowire')) {
            $definition->setAutowired(\_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::phpize($value));
        }
        if ($value = $service->getAttribute('autoconfigure')) {
            $definition->setAutoconfigured(\_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::phpize($value));
        }
        ...
        $definition->setArguments($this->getArgumentsAsPhp($service, 'argument', $file, $definition instanceof \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\ChildDefinition));
        ...
        if ($factories = $this->getChildren($service, 'factory')) {
            ...
            if ($function = $factory->getAttribute('function')) {
                ...
                if ($childService = $factory->getAttribute('service')) {
                    $class = new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Reference($childService, \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE);
                } else {
        ...
        foreach ($this->getChildren($service, 'call') as $call) {
            $definition->addMethodCall($call->getAttribute('method'), $this->getArgumentsAsPhp($call, 'argument', $file), \_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::phpize($call->getAttribute('returns-clone')));
        }
        ...
        foreach ($tags as $tag) {
            ...
            foreach ($tag->attributes as $name => $node) {
                ...
                if (\false !== \strpos($name, '-') && \false === \strpos($name, '_') && !\array_key_exists($normalizedName = \str_replace('-', '_', $name), $parameters)) {
                    $parameters[$normalizedName] = \_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::phpize($node->nodeValue);
                }
                // keep not normalized key
                $parameters[$name] = \_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::phpize($node->nodeValue);
            }
            if ('' === $tagName && '' === ($tagName = $tag->getAttribute('name'))) {
                throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The tag name for service "%s" in "%s" must be a non-empty string.', (string) $service->getAttribute('id'), $file));
            }
        ...
        if ($decorates = $service->getAttribute('decorates')) {
            ...
            if ('exception' === $decorationOnInvalid) {
                ...
                throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid value "%s" for attribute "decoration-on-invalid" on service "%s". Did you mean "exception", "ignore" or "null" in "%s"?', $decorationOnInvalid, (string) $service->getAttribute('id'), $file));
            }
    ...
    {
        try {
            $dom = \_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::loadFile($file, [$this, 'validateSchema']);
        } catch (\InvalidArgumentException $e) {
            throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Unable to parse file "%s": ', $file) . $e->getMessage(), $e->getCode(), $e);
        }
    ...
    private function processAnonymousServices(\DOMDocument $xml, string $file)
    {
        $definitions = [];
        $count = 0;
        $suffix = '~' . \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\ContainerBuilder::hash($file);
        $xpath = new \DOMXPath($xml);
        $xpath->registerNamespace('container', self::NS);
        ...
        if (\false !== ($nodes = $xpath->query('//container:services/container:service[not(@id)]'))) {
            foreach ($nodes as $node) {
                throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Top-level services must have "id" attribute, none found in "%s" at line %d.', $file, $node->getLineNo()));
            }
        }
        // resolve definitions
        \uksort($definitions, 'strnatcmp');
        foreach (\array_reverse($definitions) as $id => [$domElement, $file]) {
            if (null !== ($definition = $this->parseDefinition($domElement, $file, new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Definition()))) {
            ...
            switch ($arg->getAttribute('type')) {
                case 'service':
                    if ('' === $arg->getAttribute('id')) {
                        throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Tag "<%s>" with type="service" has no or empty "id" attribute in "%s".', $name, $file));
                    }
                    $arguments[$key] = new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Reference($arg->getAttribute('id'), $invalidBehavior);
                    break;
                case 'expression':
                    ...
                    $arguments[$key] = new \_PhpScoper0a2ac50786fa\Symfony\Component\ExpressionLanguage\Expression($arg->nodeValue);
                    break;
                case 'collection':
                ...
                case 'iterator':
                    ...
                    try {
                        $arguments[$key] = new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Argument\IteratorArgument($arg);
                    } catch (\_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException $e) {
                        throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Tag "<%s>" with type="iterator" only accepts collections of type="service" references in "%s".', $name, $file));
                    }
                    break;
                case 'service_locator':
                    $arg = $this->getArgumentsAsPhp($arg, $name, $file);
                    try {
                        $arguments[$key] = new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument($arg);
                    } catch (\_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException $e) {
                        throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Tag "<%s>" with type="service_locator" only accepts maps of type="service" references in "%s".', $name, $file));
                    }
                    break;
                case 'tagged':
                ...
                case 'tagged_locator':
                    ...
                    if (!$arg->getAttribute('tag')) {
                        throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Tag "<%s>" with type="%s" has no or empty "tag" attribute in "%s".', $name, $type, $file));
                    }
                    $arguments[$key] = new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument($arg->getAttribute('tag'), $arg->getAttribute('index-by') ?: null, $arg->getAttribute('default-index-method') ?: null, $forLocator, $arg->getAttribute('default-priority-method') ?: null);
                    if ($forLocator) {
                        $arguments[$key] = new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument($arguments[$key]);
                    }
                    break;
                case 'binary':
                    if (\false === ($value = \base64_decode($arg->nodeValue))) {
                        throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Tag "<%s>" with type="binary" is not a valid base64 encoded string.', $name));
                    }
                    $arguments[$key] = $value;
                    break;
                case 'abstract':
                    $arguments[$key] = new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Argument\AbstractArgument($arg->nodeValue);
                    break;
                case 'string':
                ...
                case 'constant':
                    ...
                    break;
                default:
                    $arguments[$key] = \_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::phpize($arg->nodeValue);
            }
        }
        return $arguments;
    }
        ...
        if ($element = $dom->documentElement->getAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'schemaLocation')) {
            ...
            for ($i = 0, $nb = \count($items); $i < $nb; $i += 2) {
                ...
                if (($extension = $this->container->getExtension($items[$i])) && \false !== $extension->getXsdValidationBasePath()) {
                    ...
                    if (!\is_file($path)) {
                        throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Extension "%s" references a non-existent XSD file "%s".', \get_debug_type($extension), $path));
                    }
...
<xsd:schema xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <xsd:complexType name="testType"/>
    ...
    private function validateAlias(\DOMElement $alias, string $file)
    {
        foreach ($alias->attributes as $name => $node) {
            if (!\in_array($name, ['alias', 'id', 'public'])) {
                throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid attribute "%s" defined for alias "%s" in "%s".', $name, $alias->getAttribute('id'), $file));
            }
        }
        foreach ($alias->childNodes as $child) {
            ...
            if (!\in_array($child->localName, ['deprecated'], \true)) {
                throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid child element "%s" defined for alias "%s" in "%s".', $child->localName, $alias->getAttribute('id'), $file));
            }
        }
    }
    /**
    ...
    private function validateExtensions(\DOMDocument $dom, string $file)
    {
        foreach ($dom->documentElement->childNodes as $node) {
            ...
            if (!$this->container->hasExtension($node->namespaceURI)) {
                ...
                throw new \_PhpScoper0a2ac50786fa\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('There is no extension able to load the configuration for "%s" (in "%s"). Looked for namespace "%s", found "%s".', $node->tagName, $file, $node->namespaceURI, $extensionNamespaces ? \implode('", "', $extensionNamespaces) : 'none'));
            }
        }
    }
    /**
    ...
    {
        return \_PhpScoper0a2ac50786fa\Symfony\Component\Config\Util\XmlUtils::convertDomElementToArray($element);
    }
}
