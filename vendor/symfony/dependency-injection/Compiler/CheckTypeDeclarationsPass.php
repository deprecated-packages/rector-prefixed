...
use _PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException;
...
use _PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\ServiceLocator;
 ...
 */
final class CheckTypeDeclarationsPass extends \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, $isRoot = \false)
    {
        ...
        if (!$value instanceof \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Definition || $value->hasErrors()) {
            return parent::processValue($value, $isRoot);
        }
        if (!$this->autoload && !\class_exists($class = $value->getClass(), \false) && !\interface_exists($class, \false)) {
            return parent::processValue($value, $isRoot);
        }
        if (\_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\ServiceLocator::class === $value->getClass()) {
            return parent::processValue($value, $isRoot);
        }
     ...
     */
    private function checkTypeDeclarations(\_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Definition $checkedDefinition, \ReflectionFunctionAbstract $reflectionFunction, array $values) : void
    {
        $numberOfRequiredParameters = $reflectionFunction->getNumberOfRequiredParameters();
        if (\count($values) < $numberOfRequiredParameters) {
            throw new \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid definition for service "%s": "%s::%s()" requires %d arguments, %d passed.', $this->currentId, $reflectionFunction->class, $reflectionFunction->name, $numberOfRequiredParameters, \count($values)));
        }
        $reflectionParameters = $reflectionFunction->getParameters();
        $checksCount = \min($reflectionFunction->getNumberOfParameters(), \count($values));
        $envPlaceholderUniquePrefix = $this->container->getParameterBag() instanceof \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag ? $this->container->getParameterBag()->getEnvPlaceholderUniquePrefix() : null;
     ...
     */
    private function checkType(\_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Definition $checkedDefinition, $value, \ReflectionParameter $parameter, ?string $envPlaceholderUniquePrefix, string $type = null) : void
    {
        if (null === $type) {
            ...
            if ($type instanceof \ReflectionUnionType) {
                foreach ($type->getTypes() as $type) {
                    try {
                        ...
                        return;
                    } catch (\_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException $e) {
                    }
                }
                throw new \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, $e->getCode(), $parameter);
            }
            $type = $type->getName();
        }
        if ($value instanceof \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Reference) {
            if (!$this->container->has($value = (string) $value)) {
                return;
            }
            if ('service_container' === $value && \is_a($type, \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Container::class, \true)) {
                return;
            }
        ...
        if (null === $class) {
            if ($value instanceof \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                $class = \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Argument\RewindableGenerator::class;
                ...
                $class = \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\ServiceLocator::class;
        ...
        if ('callable' === $type && \is_array($value) && isset($value[0]) && ($value[0] instanceof \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Reference || $value[0] instanceof \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Definition || \is_string($value[0]))) {
            return;
        }
        ...
        if (!$parameter->getType()->isBuiltin() || !$checkFunction($value)) {
            throw new \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, \is_object($value) ? $class : \get_debug_type($value), $parameter);
        }
    }
    private function getExpressionLanguage() : \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\ExpressionLanguage
    {
        if (null === $this->expressionLanguage) {
            $this->expressionLanguage = new \_PhpScoperbd5d0c5f7638\Symfony\Component\DependencyInjection\ExpressionLanguage(null, $this->container->getExpressionLanguageProviders());
        }
