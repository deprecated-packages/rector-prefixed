...
use RectorPrefix20210203\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException;
 ...
 */
final class CheckTypeDeclarationsPass extends \RectorPrefix20210203\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, $isRoot = \false)
    {
        ...
        if (!$value instanceof \RectorPrefix20210203\Symfony\Component\DependencyInjection\Definition || $value->hasErrors() || $value->isDeprecated()) {
            return parent::processValue($value, $isRoot);
        }
        if (!$this->autoload && !\class_exists($class = $value->getClass(), \false) && !\interface_exists($class, \false)) {
            return parent::processValue($value, $isRoot);
        }
        if (\RectorPrefix20210203\Symfony\Component\DependencyInjection\ServiceLocator::class === $value->getClass()) {
            return parent::processValue($value, $isRoot);
        }
     ...
     */
    private function checkTypeDeclarations(\RectorPrefix20210203\Symfony\Component\DependencyInjection\Definition $checkedDefinition, \ReflectionFunctionAbstract $reflectionFunction, array $values) : void
    {
        $numberOfRequiredParameters = $reflectionFunction->getNumberOfRequiredParameters();
        if (\count($values) < $numberOfRequiredParameters) {
            throw new \RectorPrefix20210203\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid definition for service "%s": "%s::%s()" requires %d arguments, %d passed.', $this->currentId, $reflectionFunction->class, $reflectionFunction->name, $numberOfRequiredParameters, \count($values)));
        }
        $reflectionParameters = $reflectionFunction->getParameters();
        $checksCount = \min($reflectionFunction->getNumberOfParameters(), \count($values));
        $envPlaceholderUniquePrefix = $this->container->getParameterBag() instanceof \RectorPrefix20210203\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag ? $this->container->getParameterBag()->getEnvPlaceholderUniquePrefix() : null;
     ...
     */
    private function checkType(\RectorPrefix20210203\Symfony\Component\DependencyInjection\Definition $checkedDefinition, $value, \ReflectionParameter $parameter, ?string $envPlaceholderUniquePrefix, \ReflectionType $reflectionType = null) : void
    {
        ...
        if ($reflectionType instanceof \ReflectionUnionType) {
            foreach ($reflectionType->getTypes() as $t) {
                try {
                    ...
                    return;
                } catch (\RectorPrefix20210203\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException $e) {
                }
            }
            throw new \RectorPrefix20210203\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, $e->getCode(), $parameter);
        }
        if (!$reflectionType instanceof \ReflectionNamedType) {
            return;
        }
        $type = $reflectionType->getName();
        if ($value instanceof \RectorPrefix20210203\Symfony\Component\DependencyInjection\Reference) {
            if (!$this->container->has($value = (string) $value)) {
                return;
            }
            if ('service_container' === $value && \is_a($type, \RectorPrefix20210203\Symfony\Component\DependencyInjection\Container::class, \true)) {
                return;
            }
        ...
        if (null === $class) {
            if ($value instanceof \RectorPrefix20210203\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                $class = \RectorPrefix20210203\Symfony\Component\DependencyInjection\Argument\RewindableGenerator::class;
                ...
                $class = \RectorPrefix20210203\Symfony\Component\DependencyInjection\ServiceLocator::class;
        ...
        if ('callable' === $type && \is_array($value) && isset($value[0]) && ($value[0] instanceof \RectorPrefix20210203\Symfony\Component\DependencyInjection\Reference || $value[0] instanceof \RectorPrefix20210203\Symfony\Component\DependencyInjection\Definition || \is_string($value[0]))) {
            return;
        }
        ...
        throw new \RectorPrefix20210203\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, \is_object($value) ? $class : \get_debug_type($value), $parameter);
    }
    private function getExpressionLanguage() : \RectorPrefix20210203\Symfony\Component\DependencyInjection\ExpressionLanguage
    {
        if (null === $this->expressionLanguage) {
            $this->expressionLanguage = new \RectorPrefix20210203\Symfony\Component\DependencyInjection\ExpressionLanguage(null, $this->container->getExpressionLanguageProviders());
        }
