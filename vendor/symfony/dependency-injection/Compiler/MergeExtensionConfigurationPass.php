...
use RectorPrefix20210317\Symfony\Component\Config\Definition\BaseNode;
...
use RectorPrefix20210317\Symfony\Component\DependencyInjection\Extension\ConfigurationExtensionInterface;
...
use RectorPrefix20210317\Symfony\Component\DependencyInjection\Extension\PrependExtensionInterface;
 ...
 */
class MergeExtensionConfigurationPass implements \RectorPrefix20210317\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    /**
     * {@inheritdoc}
    ...
    public function process($container)
    {
        ...
        $configAvailable = \class_exists(\RectorPrefix20210317\Symfony\Component\Config\Definition\BaseNode::class);
        foreach ($container->getExtensions() as $extension) {
            if ($extension instanceof \RectorPrefix20210317\Symfony\Component\DependencyInjection\Extension\PrependExtensionInterface) {
        ...
        foreach ($container->getExtensions() as $name => $extension) {
            ...
            if ($resolvingBag instanceof \RectorPrefix20210317\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag && $extension instanceof \RectorPrefix20210317\Symfony\Component\DependencyInjection\Extension\Extension) {
                // create a dedicated bag so that we can track env vars per-extension
                $resolvingBag = new \RectorPrefix20210317\Symfony\Component\DependencyInjection\Compiler\MergeExtensionConfigurationParameterBag($resolvingBag);
                if ($configAvailable) {
                    \RectorPrefix20210317\Symfony\Component\Config\Definition\BaseNode::setPlaceholderUniquePrefix($resolvingBag->getEnvPlaceholderUniquePrefix());
                }
            }
            $config = $resolvingBag->resolveValue($config);
            try {
                $tmpContainer = new \RectorPrefix20210317\Symfony\Component\DependencyInjection\Compiler\MergeExtensionConfigurationContainerBuilder($extension, $resolvingBag);
                $tmpContainer->setResourceTracking($container->isTrackingResources());
                $tmpContainer->addObjectResource($extension);
                if ($extension instanceof \RectorPrefix20210317\Symfony\Component\DependencyInjection\Extension\ConfigurationExtensionInterface && null !== ($configuration = $extension->getConfiguration($config, $tmpContainer))) {
                ...
                if ($resolvingBag instanceof \RectorPrefix20210317\Symfony\Component\DependencyInjection\Compiler\MergeExtensionConfigurationParameterBag) {
                    $container->getParameterBag()->mergeEnvPlaceholders($resolvingBag);
                }
                if ($configAvailable) {
                    \RectorPrefix20210317\Symfony\Component\Config\Definition\BaseNode::resetPlaceholders();
                }
                throw $e;
            }
            if ($resolvingBag instanceof \RectorPrefix20210317\Symfony\Component\DependencyInjection\Compiler\MergeExtensionConfigurationParameterBag) {
        ...
        foreach ($bag->getEnvPlaceholders() as $env => $placeholders) {
            ...
            foreach ($placeholders as $placeholder) {
                if (\false !== \stripos($value, $placeholder)) {
                    throw new \RectorPrefix20210317\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Using a cast in "env(%s)" is incompatible with resolution at compile time in "%s". The logic in the extension should be moved to a compiler pass, or an env parameter with no cast should be used instead.', $env, $this->extensionClass));
                }
            }
        }
        return parent::resolveEnvPlaceholders($value, $format, $usedEnvs);
