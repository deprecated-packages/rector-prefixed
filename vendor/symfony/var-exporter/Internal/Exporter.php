 ...
 */
namespace RectorPrefix20210120\Symfony\Component\VarExporter\Internal;

use RectorPrefix20210120\Symfony\Component\VarExporter\Exception\NotInstantiableTypeException;
/**
...
class Exporter
{
    ...
    public static function prepare($values, $objectsPool, &$refsPool, &$objectsCount, &$valuesAreStatic) : array
    {
        ...
        foreach ($values as $k => $value) {
            if (\is_resource($value)) {
                throw new \RectorPrefix20210120\Symfony\Component\VarExporter\Exception\NotInstantiableTypeException(\get_resource_type($value) . ' resource');
            }
            $refs[$k] = $objectsPool;
            if ($isRef = !($valueIsStatic = $values[$k] !== $objectsPool)) {
                ...
                if ($value instanceof \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Reference && 0 > $value->id) {
                    $valuesAreStatic = \false;
                    ++$value->count;
                    continue;
                }
                $refsPool[] = [&$refs[$k], $value, &$value];
                $refs[$k] = $values[$k] = new \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Reference(-\count($refsPool), $value);
            }
            if (\is_array($value)) {
                if ($value) {
            ...
            if (isset($objectsPool[$value])) {
                ++$objectsCount;
                $value = new \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Reference($objectsPool[$value][0]);
                goto handle_value;
            }
            $class = \get_class($value);
            $reflector = \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::$reflectors[$class] ?? \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::getClassReflector($class);
            if ($reflector->hasMethod('__serialize')) {
                if (!$reflector->getMethod('__serialize')->isPublic()) {
                    throw new \Error(\sprintf('Call to %s method "%s::__serialize()".', $reflector->getMethod('__serialize')->isProtected() ? 'protected' : 'private', $class));
                }
                if (!\is_array($properties = $value->__serialize())) {
                    throw new \RectorPrefix20210120\Typerror($class . '::__serialize() must return an array');
                }
                goto prepare_value;
            }
            $properties = [];
            ...
            $proto = \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::$prototypes[$class];
            if (($value instanceof \ArrayIterator || $value instanceof \ArrayObject) && null !== $proto) {
                ...
                \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::getClassReflector($class, \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::$instantiableWithoutConstructor[$class], \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::$cloneable[$class]);
            } elseif ($value instanceof \SplObjectStorage && \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::$cloneable[$class] && null !== $proto) {
                ...
                $value = new \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Reference($id);
                goto handle_value;
            }
    ...
    public static function export($value, string $indent = '')
    {
        ...
        if ($value instanceof \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Reference) {
            if (0 <= $value->id) {
                return '$o[' . $value->id . ']';
            }
            if (!$value->count) {
        ...
        if ($value instanceof \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Values) {
            $code = $subIndent . "\$r = [],\n";
        ...
        if ($value instanceof \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry) {
            return self::exportRegistry($value, $indent, $subIndent);
        }
        if ($value instanceof \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Hydrator) {
    ...
    private static function exportRegistry(\RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry $value, string $indent, string $subIndent) : string
    {
        $code = '';
        $serializables = [];
        $seen = [];
        $prototypesAccess = 0;
        $factoriesAccess = 0;
        $r = '\\' . \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::class;
        $j = -1;
        foreach ($value as $k => $class) {
            ...
            if (!\RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::$instantiableWithoutConstructor[$class]) {
            ...
            $c = '[' . self::export($class) . ']';
            if ($seen[$class] ?? \false) {
                if (\RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::$cloneable[$class]) {
                    ++$prototypesAccess;
                    $code .= 'clone $p' . $c;
                } else {
                    ++$factoriesAccess;
    ...
    private static function exportHydrator(\RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Hydrator $value, string $indent, string $subIndent) : string
    {
        $code = '';
    ...
    private static function getArrayObjectProperties($value, array &$arrayValue, $proto) : array
    {
        ...
        $reflector = \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::$reflectors[$reflector] ?? \RectorPrefix20210120\Symfony\Component\VarExporter\Internal\Registry::getClassReflector($reflector);
