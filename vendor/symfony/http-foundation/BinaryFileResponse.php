...
use RectorPrefix2020DecSat\Symfony\Component\HttpFoundation\File\File;
/**
 ...
 */
class BinaryFileResponse extends \RectorPrefix2020DecSat\Symfony\Component\HttpFoundation\Response
{
    ...
    {
        if (!$file instanceof \RectorPrefix2020DecSat\Symfony\Component\HttpFoundation\File\File) {
            if ($file instanceof \SplFileInfo) {
                $file = new \RectorPrefix2020DecSat\Symfony\Component\HttpFoundation\File\File($file->getPathname());
            } else {
                $file = new \RectorPrefix2020DecSat\Symfony\Component\HttpFoundation\File\File((string) $file);
            }
        }
        if (!$file->isReadable()) {
            throw new \RectorPrefix2020DecSat\Symfony\Component\HttpFoundation\File\Exception\FileException('File must be readable.');
        }
        $this->file = $file;
        if ($autoEtag) {
            $this->setAutoEtag();
     ...
     * {@inheritdoc}
     */
    public function prepare(\RectorPrefix2020DecSat\Symfony\Component\HttpFoundation\Request $request)
    {
        ...
        if (self::$trustXSendfileTypeHeader && $request->headers->has('X-Sendfile-Type')) {
            ...
            if ('x-accel-redirect' === \strtolower($type)) {
                ...
                $parts = \RectorPrefix2020DecSat\Symfony\Component\HttpFoundation\HeaderUtils::split($request->headers->get('X-Accel-Mapping', ''), ',=');
