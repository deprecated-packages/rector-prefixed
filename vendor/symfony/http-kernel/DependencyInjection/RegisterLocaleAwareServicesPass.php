 ...
 */
class RegisterLocaleAwareServicesPass implements \RectorPrefix20210203\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    ...
    public function process(\RectorPrefix20210203\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->localeAwareTag) as $id => $tags) {
            $services[] = new \RectorPrefix20210203\Symfony\Component\DependencyInjection\Reference($id);
        }
        if (!$services) {
            $container->removeDefinition($this->listenerServiceId);
            return;
        }
        $container->getDefinition($this->listenerServiceId)->setArgument(0, new \RectorPrefix20210203\Symfony\Component\DependencyInjection\Argument\IteratorArgument($services));
    }
}
