 ...
 */
namespace RectorPrefix20210424\Symfony\Component\String;

use RectorPrefix20210424\Symfony\Component\String\Exception\ExceptionInterface;
use RectorPrefix20210424\Symfony\Component\String\Exception\InvalidArgumentException;
use RectorPrefix20210424\Symfony\Component\String\Exception\RuntimeException;
/**
 ...
 */
class ByteString extends \RectorPrefix20210424\Symfony\Component\String\AbstractString
{
    ...
    {
        if ($length <= 0) {
            throw new \RectorPrefix20210424\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('A strictly positive length is expected, "%d" given.', $length));
        }
        $alphabet = $alphabet ?? self::ALPHABET_ALPHANUMERIC;
        $alphabetSize = \strlen($alphabet);
        $bits = (int) \ceil(\log($alphabetSize, 2.0));
        if ($bits <= 0 || $bits > 56) {
            throw new \RectorPrefix20210424\Symfony\Component\String\Exception\InvalidArgumentException('The length of the alphabet must in the [2^1, 2^56] range.');
        }
        $ret = '';
        while ($length > 0) {
    ...
    {
        if (1 > $length) {
            throw new \RectorPrefix20210424\Symfony\Component\String\Exception\InvalidArgumentException('The chunk length must be greater than zero.');
        }
        if ('' === $this->string) {
            return [];
        }
    ...
    public function match(string $regexp, int $flags = 0, int $offset = 0) : array
    {
        ...
        \set_error_handler(static function ($t, $m) {
            throw new \RectorPrefix20210424\Symfony\Component\String\Exception\InvalidArgumentException($m);
        });
        try {
            if (\false === $match($regexp, $this->string, $matches, $flags, $offset)) {
                ...
                foreach (\get_defined_constants(\true)['pcre'] as $k => $v) {
                    if ($lastError === $v && '_ERROR' === \substr($k, -6)) {
                        throw new \RectorPrefix20210424\Symfony\Component\String\Exception\RuntimeException('Matching failed with ' . $k . '.');
                    }
                }
                throw new \RectorPrefix20210424\Symfony\Component\String\Exception\RuntimeException('Matching failed with unknown error code.');
            }
    ...
    public function snake()
    {
        ...
        $str->string = \strtolower(\preg_replace(['/([A-Z]+)([A-Z][a-z])/', '/([a-z\\d])([A-Z])/'], 'RectorPrefix20210424\\1_\\2', $str->string));
        return $str;
    }
    ...
    public function split(string $delimiter, int $limit = null, int $flags = null) : array
    {
        if (1 > ($limit = $limit ?? \PHP_INT_MAX)) {
            throw new \RectorPrefix20210424\Symfony\Component\String\Exception\InvalidArgumentException('Split limit must be a positive integer.');
        }
        if ('' === $delimiter) {
            throw new \RectorPrefix20210424\Symfony\Component\String\Exception\InvalidArgumentException('Split delimiter is empty.');
        }
        if (null !== $flags) {
     ...
     */
    public function toUnicodeString($fromEncoding = null) : \RectorPrefix20210424\Symfony\Component\String\UnicodeString
    {
        return new \RectorPrefix20210424\Symfony\Component\String\UnicodeString($this->toCodePointString($fromEncoding)->string);
    }
    /**
     * @param string|null $fromEncoding
     */
    public function toCodePointString($fromEncoding = null) : \RectorPrefix20210424\Symfony\Component\String\CodePointString
    {
        $u = new \RectorPrefix20210424\Symfony\Component\String\CodePointString();
        ...
        try {
            ...
            } catch (\RectorPrefix20210424\Symfony\Component\String\Exception\InvalidArgumentException $e) {
        ...
        if (!$validEncoding) {
            throw new \RectorPrefix20210424\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('Invalid "%s" string.', $fromEncoding ?? 'Windows-1252'));
        }
    ...
    public function width(bool $ignoreAnsiDecoration = \true) : int
    {
        ...
        return (new \RectorPrefix20210424\Symfony\Component\String\CodePointString($string))->width($ignoreAnsiDecoration);
    }
}
