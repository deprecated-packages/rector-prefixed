...
use RectorPrefix20210207\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use RectorPrefix20210207\Doctrine\Persistence\Proxy;
use RectorPrefix20210207\Mockery\MockInterface;
use RectorPrefix20210207\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use RectorPrefix20210207\PHPUnit\Framework\MockObject\MockObject;
use RectorPrefix20210207\Prophecy\Prophecy\ProphecySubjectInterface;
use RectorPrefix20210207\ProxyManager\Proxy\ProxyInterface;
/**
...
class DebugClassLoader
{
    ...
    public static function enable() : void
    {
        ...
        \class_exists(\RectorPrefix20210207\Symfony\Component\ErrorHandler\ErrorHandler::class);
        \class_exists(\RectorPrefix20210207\Psr\Log\LogLevel::class);
    ...
    public static function checkClasses() : bool
    {
        ...
        foreach ($offsets as $getSymbols => $i) {
            ...
            for (; $i < \count($symbols); ++$i) {
                if (!\is_subclass_of($symbols[$i], \RectorPrefix20210207\PHPUnit\Framework\MockObject\MockObject::class) && !\is_subclass_of($symbols[$i], \RectorPrefix20210207\Prophecy\Prophecy\ProphecySubjectInterface::class) && !\is_subclass_of($symbols[$i], \RectorPrefix20210207\Doctrine\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \RectorPrefix20210207\ProxyManager\Proxy\ProxyInterface::class) && !\is_subclass_of($symbols[$i], \RectorPrefix20210207\Doctrine\Common\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \RectorPrefix20210207\Mockery\MockInterface::class)) {
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class) : array
    {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($finalOrInternal || $method->isConstructor() || \false === \strpos($doc, '@param') || \RectorPrefix20210207\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation::class === $class) {
                continue;
            }
