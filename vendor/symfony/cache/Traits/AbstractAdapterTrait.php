...
use _PhpScoperb75b35f52b74\Psr\Log\LoggerAwareTrait;
...
trait AbstractAdapterTrait
{
    ...
    public function hasItem($key)
    {
        ...
        try {
            ...
            \_PhpScoperb75b35f52b74\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to check if key "{key}" is cached: ' . $e->getMessage(), ['key' => $key, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
            return \false;
        }
    }
    /**
    ...
    public function clear(string $prefix = '')
    {
        ...
        try {
            ...
            \_PhpScoperb75b35f52b74\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to clear the cache: ' . $e->getMessage(), ['exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
            return \false;
        }
    }
    /**
    ...
    public function getItem($key)
    {
        ...
        try {
            ...
            \_PhpScoperb75b35f52b74\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to fetch key "{key}": ' . $e->getMessage(), ['key' => $key, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
        }
    ...
    public function getItems(array $keys = [])
    {
        ...
        try {
            ...
            \_PhpScoperb75b35f52b74\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to fetch items: ' . $e->getMessage(), ['keys' => $keys, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
            $items = [];
        }
    ...
    private function generateItems(iterable $items, array &$keys) : iterable
    {
        ...
        try {
            foreach ($items as $id => $value) {
                if (!isset($keys[$id])) {
                    throw new \_PhpScoperb75b35f52b74\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Could not match value id "%s" to keys "%s".', $id, \implode('", "', $keys)));
                }
                $key = $keys[$id];
                unset($keys[$id]);
            ...
            \_PhpScoperb75b35f52b74\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to fetch items: ' . $e->getMessage(), ['keys' => \array_values($keys), 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
        }
        foreach ($keys as $key) {
