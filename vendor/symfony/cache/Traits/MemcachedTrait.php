...
trait MemcachedTrait
{
    ...
    private function init(\Memcached $client, string $namespace, int $defaultLifetime, ?\RectorPrefix20210317\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller)
    {
        ...
        if ('Memcached' === \get_class($client)) {
            ...
            if (\Memcached::SERIALIZER_PHP !== $opt && \Memcached::SERIALIZER_IGBINARY !== $opt) {
                throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\CacheException('MemcachedAdapter: "serializer" option must be "php" or "igbinary".');
            }
    ...
    public static function createConnection($servers, array $options = [])
    {
        if (\is_string($servers)) {
            ...
            throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('MemcachedAdapter::createClient() expects array or string as first argument, "%s" given.', \gettype($servers)));
        }
        if (!static::isSupported()) {
            throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\CacheException('Memcached >= 2.2.0 is required.');
        }
        ...
        try {
            ...
            foreach ($servers as $i => $dsn) {
                ...
                if (0 !== \strpos($dsn, 'memcached:')) {
                    throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Memcached DSN: "%s" does not start with "memcached:".', $dsn));
                }
                ...
                if (\false === ($params = \parse_url($params))) {
                    throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Memcached DSN: "%s".', $dsn));
                }
                $query = $hosts = [];
    ...
    private function checkResultCode($result)
    {
        ...
        throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\CacheException('MemcachedAdapter client error: ' . \strtolower($this->client->getResultMessage()));
    }
    private function getClient() : \Memcached
    {
        ...
        if ('' !== ($prefix = (string) $this->lazyClient->getOption(\Memcached::OPT_PREFIX_KEY))) {
            throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\CacheException(\sprintf('MemcachedAdapter: "prefix_key" option must be empty when using proxified connections, "%s" given.', $prefix));
        }
