...
use RectorPrefix20210317\Symfony\Component\Cache\Traits\FilesystemCommonTrait;
 ...
 */
class PhpFilesAdapter extends \RectorPrefix20210317\Symfony\Component\Cache\Adapter\AbstractAdapter implements \RectorPrefix20210317\Symfony\Component\Cache\PruneableInterface
{
    ...
    protected function doFetch($ids)
    {
        ...
        foreach ($ids as $id) {
            if (null === ($value = $this->values[$id] ?? null)) {
                ...
                $values[$id] = $value;
            } elseif (!$value instanceof \RectorPrefix20210317\Symfony\Component\Cache\Adapter\LazyValue) {
                $values[$id] = $value();
            ...
            foreach ($missingIds as $k => $id) {
                try {
                    ...
                    if (isset(self::$valuesCache[$file])) {
                        ...
                        $this->values[$id] = new \RectorPrefix20210317\Symfony\Component\Cache\Adapter\LazyValue($file);
                    }
                    if ($now >= $expiresAt) {
    ...
    protected function doHave($id)
    {
        ...
        try {
            ...
            if (isset(self::$valuesCache[$file])) {
                ...
                $value = new \RectorPrefix20210317\Symfony\Component\Cache\Adapter\LazyValue($file);
            }
        } catch (\ErrorException $e) {
            return \false;
        } finally {
    ...
    protected function doSave($values, $lifetime)
    {
        ...
        foreach ($values as $key => $value) {
            ...
            if (null === $value) {
                ...
                try {
                    $value = \RectorPrefix20210317\Symfony\Component\VarExporter\VarExporter::export($value, $isStaticValue);
                } catch (\Exception $e) {
        ...
        if (!$ok && !\is_writable($this->directory)) {
            throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\CacheException(\sprintf('Cache directory is not writable (%s).', $this->directory));
        }
        return $ok;
    }
    /**
