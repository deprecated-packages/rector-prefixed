...
use RectorPrefix20210314\Psr\Cache\InvalidArgumentException;
 ...
 */
class TagAwareAdapter implements \RectorPrefix20210314\Symfony\Component\Cache\Adapter\TagAwareAdapterInterface, \RectorPrefix20210314\Symfony\Contracts\Cache\TagAwareCacheInterface, \RectorPrefix20210314\Symfony\Component\Cache\PruneableInterface, \RectorPrefix20210314\Symfony\Component\Cache\ResettableInterface
{
    ...
    public function __construct(\RectorPrefix20210314\Symfony\Component\Cache\Adapter\AdapterInterface $itemsPool, \RectorPrefix20210314\Symfony\Component\Cache\Adapter\AdapterInterface $tagsPool = null, float $knownTagVersionsTtl = 0.15)
    {
        $this->pool = $itemsPool;
        $this->tags = $tagsPool ?: $itemsPool;
        $this->knownTagVersionsTtl = $knownTagVersionsTtl;
        $this->createCacheItem = \Closure::bind(static function ($key, $value, \RectorPrefix20210314\Symfony\Component\Cache\CacheItem $protoItem) {
        ...
        $this->setCacheItemTags = \Closure::bind(static function (\RectorPrefix20210314\Symfony\Component\Cache\CacheItem $item, $key, array &$itemTags) {
            $item->isTaggable = \true;
            if (!$item->isHit) {
                return $item;
            }
            if (isset($itemTags[$key])) {
                foreach ($itemTags[$key] as $tag => $version) {
                    $item->metadata[\RectorPrefix20210314\Symfony\Component\Cache\CacheItem::METADATA_TAGS][$tag] = $tag;
                }
                unset($itemTags[$key]);
            } else {
                $item->value = null;
        ...
        $this->getTagsByKey = \Closure::bind(static function ($deferred) {
            ...
            foreach ($deferred as $key => $item) {
                $tagsByKey[$key] = $item->newMetadata[\RectorPrefix20210314\Symfony\Component\Cache\CacheItem::METADATA_TAGS] ?? [];
                $item->metadata = $item->newMetadata;
            }
            return $tagsByKey;
        }, null, \RectorPrefix20210314\Symfony\Component\Cache\CacheItem::class);
        $this->invalidateTags = \Closure::bind(static function (\RectorPrefix20210314\Symfony\Component\Cache\Adapter\AdapterInterface $tagsAdapter, array $tags) {
            foreach ($tags as $v) {
                $v->expiry = 0;
    ...
    public function invalidateTags(array $tags)
    {
        ...
        $invalidatedTags = [];
        foreach ($tags as $tag) {
            \RectorPrefix20210314\Symfony\Component\Cache\CacheItem::validateKey($tag);
    ...
    public function getItems(array $keys = [])
    {
        ...
        } catch (\RectorPrefix20210314\Psr\Cache\InvalidArgumentException $e) {
