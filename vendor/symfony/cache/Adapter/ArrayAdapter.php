 ...
 */
class ArrayAdapter implements \RectorPrefix20210122\Symfony\Component\Cache\Adapter\AdapterInterface, \RectorPrefix20210122\Symfony\Contracts\Cache\CacheInterface, \RectorPrefix20210122\Psr\Log\LoggerAwareInterface, \RectorPrefix20210122\Symfony\Component\Cache\ResettableInterface
{
    ...
    {
        if (0 > $maxLifetime) {
            throw new \RectorPrefix20210122\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Argument $maxLifetime must be positive, %F passed.', $maxLifetime));
        }
        if (0 > $maxItems) {
            throw new \RectorPrefix20210122\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Argument $maxItems must be a positive integer, %d passed.', $maxItems));
        }
    ...
    private function unfreeze(string $key, bool &$isHit)
    {
        ...
        if (\is_string($value) && isset($value[2]) && ':' === $value[1]) {
            try {
                ...
                \RectorPrefix20210122\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to unserialize key "{key}": ' . $e->getMessage(), ['key' => $key, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
                $value = \false;
            }
            if (\false === $value) {
                $value = null;
