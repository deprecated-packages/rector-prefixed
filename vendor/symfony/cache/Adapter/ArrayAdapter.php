 ...
 */
class ArrayAdapter implements \RectorPrefix20210317\Symfony\Component\Cache\Adapter\AdapterInterface, \RectorPrefix20210317\Symfony\Contracts\Cache\CacheInterface, \RectorPrefix20210317\Psr\Log\LoggerAwareInterface, \RectorPrefix20210317\Symfony\Component\Cache\ResettableInterface
{
    ...
    public function __construct($defaultLifetime = 0, $storeSerialized = \true, $maxLifetime = 0, $maxItems = 0)
    {
        ...
        if (0 > $maxItems) {
            throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Argument $maxItems must be a positive integer, %d passed.', $maxItems));
        }
    ...
    private function unfreeze($key, &$isHit)
    {
        ...
        if (\is_string($value) && isset($value[2]) && ':' === $value[1]) {
            try {
                ...
                \RectorPrefix20210317\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to unserialize key "{key}": ' . $e->getMessage(), ['key' => $key, 'exception' => $e, 'cache-adapter' => \get_debug_type($this)]);
                $value = \false;
            }
            if (\false === $value) {
                $value = null;
