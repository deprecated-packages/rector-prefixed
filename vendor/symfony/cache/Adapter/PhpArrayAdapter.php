...
use RectorPrefix20210317\Symfony\Component\VarExporter\VarExporter;
 ...
 */
class PhpArrayAdapter implements \RectorPrefix20210317\Symfony\Component\Cache\Adapter\AdapterInterface, \RectorPrefix20210317\Symfony\Contracts\Cache\CacheInterface, \RectorPrefix20210317\Symfony\Component\Cache\PruneableInterface, \RectorPrefix20210317\Symfony\Component\Cache\ResettableInterface
{
    use ContractsTrait;
    use ProxyTrait;
    private $file;
     ...
     */
    public function __construct(string $file, \RectorPrefix20210317\Symfony\Component\Cache\Adapter\AdapterInterface $fallbackPool)
    {
        $this->file = $file;
    ...
    public function warmUp($values)
    {
        if (\file_exists($this->file)) {
            if (!\is_file($this->file)) {
                throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Cache path exists and is not a file: "%s".', $this->file));
            }
            if (!\is_writable($this->file)) {
                throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Cache file is not writable: "%s".', $this->file));
            }
        } else {
            $directory = \dirname($this->file);
            if (!\is_dir($directory) && !@\mkdir($directory, 0777, \true)) {
                throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Cache directory does not exist and cannot be created: "%s".', $directory));
            }
            if (!\is_writable($directory)) {
                throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Cache directory is not writable: "%s".', $directory));
            }
        }
        $preload = [];
        $dumpedValues = '';
...
EOF;
        foreach ($values as $key => $value) {
            \RectorPrefix20210317\Symfony\Component\Cache\CacheItem::validateKey(\is_int($key) ? (string) $key : $key);
            $isStaticValue = \true;
            if (null === $value) {
                $value = "'N;'";
            } elseif (\is_object($value) || \is_array($value)) {
                try {
                    $value = \RectorPrefix20210317\Symfony\Component\VarExporter\VarExporter::export($value, $isStaticValue, $preload);
                } catch (\Exception $e) {
                    throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Cache key "%s" has non-serializable "%s" value.', $key, \get_debug_type($value)), 0, $e);
                }
                ...
                throw new \RectorPrefix20210317\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Cache key "%s" has non-serializable "%s" value.', $key, \get_debug_type($value)));
            } else {
                $value = \var_export($value, \true);
            }
            if (!$isStaticValue) {
