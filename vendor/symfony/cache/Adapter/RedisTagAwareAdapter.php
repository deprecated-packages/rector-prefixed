...
use RectorPrefix20210211\Predis\Connection\Aggregate\PredisCluster;
...
use RectorPrefix20210211\Symfony\Component\Cache\Exception\LogicException;
use RectorPrefix20210211\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use RectorPrefix20210211\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use RectorPrefix20210211\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use RectorPrefix20210211\Symfony\Component\Cache\Traits\RedisTrait;
/**
 ...
 */
class RedisTagAwareAdapter extends \RectorPrefix20210211\Symfony\Component\Cache\Adapter\AbstractTagAwareAdapter
{
    use RedisTrait;
    /**
     ...
     */
    public function __construct($redisClient, string $namespace = '', int $defaultLifetime = 0, \RectorPrefix20210211\Symfony\Component\Cache\Marshaller\MarshallerInterface $marshaller = null)
    {
        if ($redisClient instanceof \RectorPrefix20210211\Predis\ClientInterface && $redisClient->getConnection() instanceof \RectorPrefix20210211\Predis\Connection\Aggregate\ClusterInterface && !$redisClient->getConnection() instanceof \RectorPrefix20210211\Predis\Connection\Aggregate\PredisCluster) {
            throw new \RectorPrefix20210211\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Unsupported Predis cluster connection: only "%s" is, "%s" given.', \RectorPrefix20210211\Predis\Connection\Aggregate\PredisCluster::class, \get_debug_type($redisClient->getConnection())));
        }
        if (\defined('Redis::OPT_COMPRESSION') && ($redisClient instanceof \Redis || $redisClient instanceof \RedisArray || $redisClient instanceof \RedisCluster)) {
            ...
            foreach (\is_array($compression) ? $compression : [$compression] as $c) {
                if (\Redis::COMPRESSION_NONE !== $c) {
                    throw new \RectorPrefix20210211\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('phpredis compression must be disabled when using "%s", use "%s" instead.', static::class, \RectorPrefix20210211\Symfony\Component\Cache\Marshaller\DeflateMarshaller::class));
                }
            }
        }
        $this->init($redisClient, $namespace, $defaultLifetime, new \RectorPrefix20210211\Symfony\Component\Cache\Marshaller\TagAwareMarshaller($marshaller));
    }
    /**
     * {@inheritdoc}
     */
    protected function doSave(array $values, int $lifetime, array $addTagData = [], array $delTagData = []) : array
    {
        ...
        if ('noeviction' !== $eviction && 0 !== \strpos($eviction, 'volatile-')) {
            throw new \RectorPrefix20210211\Symfony\Component\Cache\Exception\LogicException(\sprintf('Redis maxmemory-policy setting "%s" is *not* supported by RedisTagAwareAdapter, use "noeviction" or  "volatile-*" eviction policies.', $eviction));
        }
        // serialize values
...
EOLUA;
    ...
    {
        if (!$this->redis instanceof \RectorPrefix20210211\Predis\ClientInterface || !$this->redis->getConnection() instanceof \RectorPrefix20210211\Predis\Connection\Aggregate\PredisCluster) {
