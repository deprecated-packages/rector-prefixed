...
use _PhpScoperabd03f0baf05\Nette\Neon\Neon;
...
return ['files-whitelist' => $whitelistedStubsProvider->provide(), 'whitelist' => \Rector\Compiler\PhpScoper\StaticEasyPrefixer::getExcludedNamespacesAndClasses(), 'patchers' => [
    ...
    function (string $filePath, string $prefix, string $content) : string {
        if (!\_PhpScoperabd03f0baf05\Nette\Utils\Strings::endsWith($filePath, 'vendor/nette/di/src/DI/Compiler.php')) {
            return $content;
        }
        return \str_replace('_PhpScoperabd03f0baf05\\|Nette\\DI\\Statement', \sprintf('_PhpScoperabd03f0baf05\\|\\%s\\Nette\\DI\\Statement', $prefix), $content);
    },
    function (string $filePath, string $prefix, string $content) : string {
        if (!\_PhpScoperabd03f0baf05\Nette\Utils\Strings::endsWith($filePath, 'vendor/nette/di/src/DI/Config/DefinitionSchema.php')) {
            return $content;
        }
        $content = \str_replace(\sprintf('_PhpScoperabd03f0baf05\\\'%s\\callable', $prefix), "'callable", $content);
        return \str_replace('_PhpScoperabd03f0baf05\\|Nette\\DI\\Definitions\\Statement', \sprintf('_PhpScoperabd03f0baf05\\|%s\\Nette\\DI\\Definitions\\Statement', $prefix), $content);
    },
    function (string $filePath, string $prefix, string $content) : string {
        if (!\_PhpScoperabd03f0baf05\Nette\Utils\Strings::endsWith($filePath, 'vendor/nette/di/src/DI/Extensions/ExtensionsExtension.php')) {
            return $content;
        }
        $content = \str_replace(\sprintf('_PhpScoperabd03f0baf05\\\'%s\\string', $prefix), "'string", $content);
        return \str_replace('_PhpScoperabd03f0baf05\\|Nette\\DI\\Definitions\\Statement', \sprintf('_PhpScoperabd03f0baf05\\|%s\\Nette\\DI\\Definitions\\Statement', $prefix), $content);
    },
    function (string $filePath, string $prefix, string $content) : string {
        if (!\_PhpScoperabd03f0baf05\Nette\Utils\Strings::endsWith($filePath, 'vendor/phpstan/phpstan-src/src/Testing/TestCase.php')) {
            return $content;
        }
        return \str_replace(\sprintf('_PhpScoperabd03f0baf05\\%s\\PHPUnit\\Framework\\TestCase', $prefix), '_PhpScoperabd03f0baf05\\PHPUnit\\Framework\\TestCase', $content);
    },
    function (string $filePath, string $prefix, string $content) : string {
        if (!\_PhpScoperabd03f0baf05\Nette\Utils\Strings::endsWith($filePath, 'vendor/phpstan/phpstan-src/src/Testing/LevelsTestCase.php')) {
            return $content;
        }
        return \str_replace([\sprintf('_PhpScoperabd03f0baf05\\%s\\PHPUnit\\Framework\\AssertionFailedError', $prefix), \sprintf('_PhpScoperabd03f0baf05\\%s\\PHPUnit\\Framework\\TestCase', $prefix)], ['\\PHPUnit\\Framework\\AssertionFailedError', '\\PHPUnit\\Framework\\TestCase'], $content);
    },
    ...
    function (string $filePath, string $prefix, string $content) : string {
        foreach (\Rector\Compiler\PhpScoper\StaticEasyPrefixer::EXCLUDED_CLASSES as $excludedClass) {
            ...
            $content = \_PhpScoperabd03f0baf05\Nette\Utils\Strings::replace($content, $prefixedClassPattern, $excludedClass);
        }
        return $content;
    },
    ...
    function (string $filePath, string $prefix, string $content) : string {
        // only *.neon files
        if (!\_PhpScoperabd03f0baf05\Nette\Utils\Strings::endsWith($filePath, '.neon')) {
            return $content;
        }
        if ($content === '') {
            return $content;
        }
        $neon = \_PhpScoperabd03f0baf05\Nette\Neon\Neon::decode($content);
        $updatedNeon = $neon;
        ...
        $updatedContent = \_PhpScoperabd03f0baf05\Nette\Neon\Neon::encode($updatedNeon, \_PhpScoperabd03f0baf05\Nette\Neon\Neon::BLOCK);
        // default indent is tab, we have spaces
        return \_PhpScoperabd03f0baf05\Nette\Utils\Strings::replace($updatedContent, '#\\t#', '    ');
    },
    ...
    function (string $filePath, string $prefix, string $content) : string {
        if (!\_PhpScoperabd03f0baf05\Nette\Utils\Strings::match($filePath, '#phpstan-src\\/(config|src|rules|packages)\\/#')) {
            return $content;
        }
    ...
    function (string $filePath, string $prefix, string $content) : string {
        ...
        return \str_replace(\sprintf('_PhpScoperabd03f0baf05\\%s\\ReflectionUnionType', $prefix), 'ReflectionUnionType', $content);
    },
    ...
    function (string $filePath, string $prefix, string $content) : string {
        ...
        return \str_replace(\sprintf('_PhpScoperabd03f0baf05\\%s\\Composer\\Autoload\\ClassLoader', $prefix), '_PhpScoperabd03f0baf05\\Composer\\Autoload\\ClassLoader', $content);
    },
]];
