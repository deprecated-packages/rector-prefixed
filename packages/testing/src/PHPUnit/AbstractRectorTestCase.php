...
use _PhpScoper0a6b37af0871\PHPUnit\Framework\ExpectationFailedException;
use _PhpScoper0a6b37af0871\Rector\Core\Application\FileProcessor;
...
use _PhpScoper0a6b37af0871\Rector\Naming\Tests\Rector\Class_\RenamePropertyToMatchTypeRector\Source\ContainerInterface;
...
use _PhpScoper0a6b37af0871\Rector\Testing\Finder\RectorsFinder;
use _PhpScoper0a6b37af0871\Rector\Testing\Guard\FixtureGuard;
use _PhpScoper0a6b37af0871\Rector\Testing\PhpConfigPrinter\PhpConfigPrinterFactory;
use _PhpScoper0a6b37af0871\Rector\Testing\PHPUnit\Behavior\MovingFilesTrait;
use _PhpScoper0a6b37af0871\Rector\Testing\PHPUnit\Behavior\RunnableTestTrait;
...
use _PhpScoper0a6b37af0871\Symplify\EasyTesting\DataProvider\StaticFixtureUpdater;
...
abstract class AbstractRectorTestCase extends \_PhpScoper0a6b37af0871\Symplify\PackageBuilder\Testing\AbstractKernelTestCase
{
    ...
    {
        $this->runnableRectorFactory = new \_PhpScoper0a6b37af0871\Rector\Testing\PHPUnit\RunnableRectorFactory();
        $this->smartFileSystem = new \_PhpScoper0a6b37af0871\Symplify\SmartFileSystem\SmartFileSystem();
        $this->fixtureGuard = new \_PhpScoper0a6b37af0871\Rector\Testing\Guard\FixtureGuard();
        if ($this->provideConfigFileInfo() !== null) {
            $configFileInfos = $this->resolveConfigs($this->provideConfigFileInfo());
            $this->bootKernelWithConfigInfos(\_PhpScoper0a6b37af0871\Rector\Core\HttpKernel\RectorKernel::class, $configFileInfos);
            $enabledRectorsProvider = static::$container->get(\_PhpScoper0a6b37af0871\Rector\Testing\Application\EnabledRectorsProvider::class);
            ...
            if (\defined('RECTOR_REPOSITORY')) {
                ...
                $this->bootKernelWithConfigs(\_PhpScoper0a6b37af0871\Rector\Core\HttpKernel\RectorKernel::class, $configs);
            }
            $enabledRectorsProvider = $this->getService(\_PhpScoper0a6b37af0871\Rector\Testing\Application\EnabledRectorsProvider::class);
            $enabledRectorsProvider->reset();
            $this->configureEnabledRectors($enabledRectorsProvider);
        }
        // load stubs
        $stubLoader = static::$container->get(\_PhpScoper0a6b37af0871\Rector\Core\Stubs\StubLoader::class);
        $stubLoader->loadStubs();
        // disable any output
        $symfonyStyle = static::$container->get(\_PhpScoper0a6b37af0871\Symfony\Component\Console\Style\SymfonyStyle::class);
        $symfonyStyle->setVerbosity(\_PhpScoper0a6b37af0871\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        $this->fileProcessor = static::$container->get(\_PhpScoper0a6b37af0871\Rector\Core\Application\FileProcessor::class);
        $this->nonPhpFileProcessor = static::$container->get(\_PhpScoper0a6b37af0871\Rector\Core\NonPhpFile\NonPhpFileProcessor::class);
        $this->parameterProvider = static::$container->get(\_PhpScoper0a6b37af0871\Symplify\PackageBuilder\Parameter\ParameterProvider::class);
        $this->removedAndAddedFilesCollector = $this->getService(\_PhpScoper0a6b37af0871\Rector\Core\Application\FileSystem\RemovedAndAddedFilesCollector::class);
        $this->removedAndAddedFilesCollector->reset();
        // needed for PHPStan, because the analyzed file is just create in /temp
        $this->nodeScopeResolver = static::$container->get(\_PhpScoper0a6b37af0871\PHPStan\Analyser\NodeScopeResolver::class);
        $this->configurePhpVersionFeatures();
        // so the files are removed and added
        $configuration = static::$container->get(\_PhpScoper0a6b37af0871\Rector\Core\Configuration\Configuration::class);
    ...
    protected function tearDown() : void
    {
        ...
        if ($this->getPhpVersion() !== self::PHP_VERSION_UNDEFINED) {
            $this->setParameter(\_PhpScoper0a6b37af0871\Rector\Core\Configuration\Option::PHP_VERSION_FEATURES, \_PhpScoper0a6b37af0871\Rector\Core\ValueObject\PhpVersion::PHP_10);
        }
    }
    ...
    {
        $parameterProvider = $this->getService(\_PhpScoper0a6b37af0871\Symplify\PackageBuilder\Parameter\ParameterProvider::class);
        if ($name !== \_PhpScoper0a6b37af0871\Rector\Core\Configuration\Option::PHP_VERSION_FEATURES) {
     ...
     */
    protected function bootKernelWithConfigInfos(string $class, array $configFileInfos) : \_PhpScoper0a6b37af0871\Symfony\Component\HttpKernel\KernelInterface
    {
        $configFiles = [];
    ...
    protected function doTestFileInfoWithoutAutoload(\_PhpScoper0a6b37af0871\Symplify\SmartFileSystem\SmartFileInfo $fileInfo) : void
    {
     ...
     */
    protected function doTestFileInfo(\_PhpScoper0a6b37af0871\Symplify\SmartFileSystem\SmartFileInfo $fixtureFileInfo, array $extraFiles = []) : void
    {
        $this->fixtureGuard->ensureFileInfoHasDifferentBeforeAndAfterContent($fixtureFileInfo);
        $inputFileInfoAndExpectedFileInfo = \_PhpScoper0a6b37af0871\Symplify\EasyTesting\StaticFixtureSplitter::splitFileInfoToLocalInputAndExpectedFileInfos($fixtureFileInfo, $this->autoloadTestFixture);
        ...
        if (!\_PhpScoper0a6b37af0871\Nette\Utils\Strings::contains($inputFileInfo->getContents(), \_PhpScoper0a6b37af0871\Rector\Testing\Contract\RunnableInterface::class)) {
            return;
        }
    ...
    {
        return \_PhpScoper0a6b37af0871\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath();
    }
    protected function doTestExtraFile(string $expectedExtraFileName, string $expectedExtraContentFilePath) : void
    {
        $temporaryPath = \_PhpScoper0a6b37af0871\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath();
     ...
     */
    private function resolveConfigs(\_PhpScoper0a6b37af0871\Symplify\SmartFileSystem\SmartFileInfo $configFileInfo) : array
    {
    ...
    private function configureEnabledRectors(\_PhpScoper0a6b37af0871\Rector\Testing\Application\EnabledRectorsProvider $enabledRectorsProvider) : void
    {
    ...
    private function configurePhpVersionFeatures() : void
    {
        ...
        $this->setParameter(\_PhpScoper0a6b37af0871\Rector\Core\Configuration\Option::PHP_VERSION_FEATURES, $this->getPhpVersion());
    }
    ...
    {
        if (\is_a($rectorClass, \_PhpScoper0a6b37af0871\Rector\Core\Contract\Rector\PhpRectorInterface::class, \true)) {
            return;
        }
        throw new \_PhpScoper0a6b37af0871\Rector\Core\Exception\ShouldNotHappenException(\sprintf('Class "%s" in "%s()" method must be type of "%s"', $rectorClass, $methodName, \_PhpScoper0a6b37af0871\Rector\Core\Contract\Rector\PhpRectorInterface::class));
    }
    /**
     * @param InputFilePathWithExpectedFile[] $extraFiles
     */
    private function doTestFileMatchesExpectedContent(\_PhpScoper0a6b37af0871\Symplify\SmartFileSystem\SmartFileInfo $originalFileInfo, \_PhpScoper0a6b37af0871\Symplify\SmartFileSystem\SmartFileInfo $expectedFileInfo, \_PhpScoper0a6b37af0871\Symplify\SmartFileSystem\SmartFileInfo $fixtureFileInfo, array $extraFiles = []) : void
    {
        $this->setParameter(\_PhpScoper0a6b37af0871\Rector\Core\Configuration\Option::SOURCE, [$originalFileInfo->getRealPath()]);
        if (!\_PhpScoper0a6b37af0871\Nette\Utils\Strings::endsWith($originalFileInfo->getFilename(), '.blade.php') && \in_array($originalFileInfo->getSuffix(), ['php', 'phpt'], \true)) {
            if ($extraFiles === []) {
            ...
            $removedAndAddedFilesProcessor = $this->getService(\_PhpScoper0a6b37af0871\Rector\Core\Application\FileSystem\RemovedAndAddedFilesProcessor::class);
            $removedAndAddedFilesProcessor->run();
        } elseif (\_PhpScoper0a6b37af0871\Nette\Utils\Strings::match($originalFileInfo->getFilename(), \_PhpScoper0a6b37af0871\Rector\Core\ValueObject\StaticNonPhpFileSuffixes::getSuffixRegexPattern())) {
        ...
        } catch (\_PhpScoper0a6b37af0871\PHPUnit\Framework\ExpectationFailedException $expectationFailedException) {
            \_PhpScoper0a6b37af0871\Symplify\EasyTesting\DataProvider\StaticFixtureUpdater::updateFixtureContent($originalFileInfo, $changedContent, $fixtureFileInfo);
    ...
    {
        $rectorsFinder = new \_PhpScoper0a6b37af0871\Rector\Testing\Finder\RectorsFinder();
        ...
        $this->bootKernelWithConfigs(\_PhpScoper0a6b37af0871\Rector\Core\HttpKernel\RectorKernel::class, [$filePath]);
    }
    /**
    ...
    {
        $phpConfigPrinterFactory = new \_PhpScoper0a6b37af0871\Rector\Testing\PhpConfigPrinter\PhpConfigPrinterFactory();
    ...
    {
        return \_PhpScoper0a6b37af0871\Nette\Utils\Strings::replace($string, '#\\r\\n|\\r|\\n#', "\n");
    }
}
