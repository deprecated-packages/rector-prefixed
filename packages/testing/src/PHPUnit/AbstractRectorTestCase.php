...
use RectorPrefix20210308\PHPUnit\Framework\ExpectationFailedException;
use RectorPrefix20210308\Psr\Container\ContainerInterface;
...
use RectorPrefix20210308\Symplify\EasyTesting\DataProvider\StaticFixtureUpdater;
use RectorPrefix20210308\Symplify\EasyTesting\StaticFixtureSplitter;
...
use RectorPrefix20210308\Symplify\SmartFileSystem\SmartFileSystem;
abstract class AbstractRectorTestCase extends \RectorPrefix20210308\Symplify\PackageBuilder\Testing\AbstractKernelTestCase
{
    ...
    protected function setUp() : void
    {
        ...
        if ($this->provideConfigFilePath() !== '') {
            $configFileInfo = new \RectorPrefix20210308\Symplify\SmartFileSystem\SmartFileInfo($this->provideConfigFilePath());
        ...
        $this->parameterProvider = $this->getService(\RectorPrefix20210308\Symplify\PackageBuilder\Parameter\ParameterProvider::class);
    ...
    {
        return \RectorPrefix20210308\Symplify\EasyTesting\DataProvider\StaticFixtureFinder::yieldDirectory($directory, $suffix);
    }
    /**
     * @param SmartFileInfo[] $extraFileInfos
     */
    protected function doTestFileInfo(\RectorPrefix20210308\Symplify\SmartFileSystem\SmartFileInfo $fixtureFileInfo, array $extraFileInfos = []) : void
    {
        self::$fixtureGuard->ensureFileInfoHasDifferentBeforeAndAfterContent($fixtureFileInfo);
        $inputFileInfoAndExpectedFileInfo = \RectorPrefix20210308\Symplify\EasyTesting\StaticFixtureSplitter::splitFileInfoToLocalInputAndExpectedFileInfos($fixtureFileInfo, \false);
    ...
    protected function doTestExtraFile(string $expectedExtraFileName, string $expectedExtraContentFilePath) : void
    {
        ...
        foreach ($addedFilesWithContents as $addedFileWithContent) {
            if (!\RectorPrefix20210308\Nette\Utils\Strings::endsWith($addedFileWithContent->getFilePath(), $expectedExtraFileName)) {
                continue;
            }
        ...
        foreach ($addedFilesWithNodes as $addedFileWithNode) {
            if (!\RectorPrefix20210308\Nette\Utils\Strings::endsWith($addedFileWithNode->getFilePath(), $expectedExtraFileName)) {
                continue;
            }
        ...
        foreach ($movedFilesWithContent as $movedFileWithContent) {
            if (!\RectorPrefix20210308\Nette\Utils\Strings::endsWith($movedFileWithContent->getNewPathname(), $expectedExtraFileName)) {
                continue;
            }
    ...
    protected function assertOriginalAndFixedFileResultEquals(\RectorPrefix20210308\Symplify\SmartFileSystem\SmartFileInfo $originalFileInfo, \RectorPrefix20210308\Symplify\SmartFileSystem\SmartFileInfo $expectedFileInfo) : void
    {
     ...
     * @param SmartFileInfo[] $extraFileInfos
     */
    private function doTestFileMatchesExpectedContent(\RectorPrefix20210308\Symplify\SmartFileSystem\SmartFileInfo $originalFileInfo, \RectorPrefix20210308\Symplify\SmartFileSystem\SmartFileInfo $expectedFileInfo, \RectorPrefix20210308\Symplify\SmartFileSystem\SmartFileInfo $fixtureFileInfo, array $extraFileInfos = []) : void
    {
        $this->parameterProvider->changeParameter(\Rector\Core\Configuration\Option::SOURCE, [$originalFileInfo->getRealPath()]);
        if (!\RectorPrefix20210308\Nette\Utils\Strings::endsWith($originalFileInfo->getFilename(), '.blade.php') && \in_array($originalFileInfo->getSuffix(), ['php', 'phpt'], \true)) {
            if ($extraFileInfos === []) {
        ...
        } elseif (\RectorPrefix20210308\Nette\Utils\Strings::match($originalFileInfo->getFilename(), \Rector\Core\ValueObject\StaticNonPhpFileSuffixes::getSuffixRegexPattern())) {
        ...
        } catch (\RectorPrefix20210308\PHPUnit\Framework\ExpectationFailedException $expectationFailedException) {
            \RectorPrefix20210308\Symplify\EasyTesting\DataProvider\StaticFixtureUpdater::updateFixtureContent($originalFileInfo, $changedContent, $fixtureFileInfo);
    ...
    {
        return \RectorPrefix20210308\Nette\Utils\Strings::replace($string, '#\\r\\n|\\r|\\n#', "\n");
    }
    /**
    ...
    private function initializeDependencies() : void
    {
        ...
        self::$smartFileSystem = new \RectorPrefix20210308\Symplify\SmartFileSystem\SmartFileSystem();
    ...
    private function processRunnableTest(\RectorPrefix20210308\Symplify\SmartFileSystem\SmartFileInfo $inputFileInfo, \RectorPrefix20210308\Symplify\SmartFileSystem\SmartFileInfo $expectedFileInfo) : void
    {
        // runnable?
        if (!\file_exists($inputFileInfo->getPathname())) {
            return;
        }
        if (!\RectorPrefix20210308\Nette\Utils\Strings::contains($inputFileInfo->getContents(), \Rector\Testing\Contract\RunnableInterface::class)) {
            return;
        }
