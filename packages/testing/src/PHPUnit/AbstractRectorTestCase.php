...
abstract class AbstractRectorTestCase extends \Symplify\PackageBuilder\Testing\AbstractKernelTestCase
{
     ...
     */
    protected function bootKernelWithConfigInfos(string $class, array $configFileInfos) : \_PhpScopera143bcca66cb\Symfony\Component\HttpKernel\KernelInterface
    {
        $configFiles = [];
    ...
    protected function doTestFileInfo(\Symplify\SmartFileSystem\SmartFileInfo $fixtureFileInfo, array $extraFiles = []) : void
    {
        ...
        if (!\_PhpScopera143bcca66cb\Nette\Utils\Strings::contains($inputFileInfo->getContents(), \Rector\Testing\Contract\RunnableInterface::class)) {
            return;
        }
    ...
    private function doTestFileMatchesExpectedContent(\Symplify\SmartFileSystem\SmartFileInfo $originalFileInfo, \Symplify\SmartFileSystem\SmartFileInfo $expectedFileInfo, \Symplify\SmartFileSystem\SmartFileInfo $fixtureFileInfo, array $extraFiles = []) : void
    {
        ...
        if (!\_PhpScopera143bcca66cb\Nette\Utils\Strings::endsWith($originalFileInfo->getFilename(), '.blade.php') && \in_array($originalFileInfo->getSuffix(), ['php', 'phpt'], \true)) {
            if ($extraFiles === []) {
        ...
        } elseif (\_PhpScopera143bcca66cb\Nette\Utils\Strings::match($originalFileInfo->getFilename(), \Rector\Core\ValueObject\StaticNonPhpFileSuffixes::getSuffixRegexPattern())) {
        ...
        } catch (\_PhpScopera143bcca66cb\PHPUnit\Framework\ExpectationFailedException $expectationFailedException) {
