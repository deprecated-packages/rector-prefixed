...
use RectorPrefix20201227\Symfony\Component\HttpKernel\KernelInterface;
use RectorPrefix20201227\Symplify\EasyTesting\DataProvider\StaticFixtureFinder;
use RectorPrefix20201227\Symplify\EasyTesting\DataProvider\StaticFixtureUpdater;
use RectorPrefix20201227\Symplify\EasyTesting\StaticFixtureSplitter;
...
abstract class AbstractRectorTestCase extends \RectorPrefix20201227\Symplify\PackageBuilder\Testing\AbstractKernelTestCase
{
    ...
    protected function setUp() : void
    {
        ...
        $this->smartFileSystem = new \RectorPrefix20201227\Symplify\SmartFileSystem\SmartFileSystem();
        ...
        // disable any output
        $symfonyStyle = static::$container->get(\RectorPrefix20201227\Symfony\Component\Console\Style\SymfonyStyle::class);
        $symfonyStyle->setVerbosity(\RectorPrefix20201227\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        ...
        $this->parameterProvider = static::$container->get(\RectorPrefix20201227\Symplify\PackageBuilder\Parameter\ParameterProvider::class);
    ...
    protected function provideConfigFileInfo() : ?\RectorPrefix20201227\Symplify\SmartFileSystem\SmartFileInfo
    {
        // can be implemented
        return null;
    }
    ...
    {
        return \RectorPrefix20201227\Symplify\EasyTesting\DataProvider\StaticFixtureFinder::yieldDirectory($directory, $suffix);
    }
    /**
     * @param mixed $value
     */
    ...
    {
        $parameterProvider = $this->getService(\RectorPrefix20201227\Symplify\PackageBuilder\Parameter\ParameterProvider::class);
     ...
     */
    protected function bootKernelWithConfigInfos(string $class, array $configFileInfos) : \RectorPrefix20201227\Symfony\Component\HttpKernel\KernelInterface
    {
        $configFiles = [];
    ...
    protected function doTestFileInfoWithoutAutoload(\RectorPrefix20201227\Symplify\SmartFileSystem\SmartFileInfo $fileInfo) : void
    {
     ...
     */
    protected function doTestFileInfo(\RectorPrefix20201227\Symplify\SmartFileSystem\SmartFileInfo $fixtureFileInfo, array $extraFiles = []) : void
    {
        $this->fixtureGuard->ensureFileInfoHasDifferentBeforeAndAfterContent($fixtureFileInfo);
        $inputFileInfoAndExpectedFileInfo = \RectorPrefix20201227\Symplify\EasyTesting\StaticFixtureSplitter::splitFileInfoToLocalInputAndExpectedFileInfos($fixtureFileInfo, $this->autoloadTestFixture);
        ...
        if (!\RectorPrefix20201227\Nette\Utils\Strings::contains($inputFileInfo->getContents(), \Rector\Testing\Contract\RunnableInterface::class)) {
            return;
        }
    ...
    {
        return \RectorPrefix20201227\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath();
    }
    protected function doTestExtraFile(string $expectedExtraFileName, string $expectedExtraContentFilePath) : void
    {
        $temporaryPath = \RectorPrefix20201227\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath();
     ...
     */
    private function resolveConfigs(\RectorPrefix20201227\Symplify\SmartFileSystem\SmartFileInfo $configFileInfo) : array
    {
     ...
     * @param InputFilePathWithExpectedFile[] $extraFiles
     */
    private function doTestFileMatchesExpectedContent(\RectorPrefix20201227\Symplify\SmartFileSystem\SmartFileInfo $originalFileInfo, \RectorPrefix20201227\Symplify\SmartFileSystem\SmartFileInfo $expectedFileInfo, \RectorPrefix20201227\Symplify\SmartFileSystem\SmartFileInfo $fixtureFileInfo, array $extraFiles = []) : void
    {
        $this->setParameter(\Rector\Core\Configuration\Option::SOURCE, [$originalFileInfo->getRealPath()]);
        if (!\RectorPrefix20201227\Nette\Utils\Strings::endsWith($originalFileInfo->getFilename(), '.blade.php') && \in_array($originalFileInfo->getSuffix(), ['php', 'phpt'], \true)) {
            if ($extraFiles === []) {
        ...
        } elseif (\RectorPrefix20201227\Nette\Utils\Strings::match($originalFileInfo->getFilename(), \Rector\Core\ValueObject\StaticNonPhpFileSuffixes::getSuffixRegexPattern())) {
        ...
        } catch (\RectorPrefix20201227\PHPUnit\Framework\ExpectationFailedException $expectationFailedException) {
            \RectorPrefix20201227\Symplify\EasyTesting\DataProvider\StaticFixtureUpdater::updateFixtureContent($originalFileInfo, $changedContent, $fixtureFileInfo);
    ...
    {
        return \RectorPrefix20201227\Nette\Utils\Strings::replace($string, '#\\r\\n|\\r|\\n#', "\n");
    }
}
