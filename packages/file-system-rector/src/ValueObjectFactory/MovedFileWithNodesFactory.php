...
namespace _PhpScoper0a6b37af0871\Rector\FileSystemRector\ValueObjectFactory;
...
use _PhpScoper0a6b37af0871\Rector\Autodiscovery\Configuration\CategoryNamespaceProvider;
...
final class MovedFileWithNodesFactory
{
    ...
    public function __construct(\_PhpScoper0a6b37af0871\Rector\Core\PhpParser\Node\BetterNodeFinder $betterNodeFinder, \_PhpScoper0a6b37af0871\Rector\Autodiscovery\Configuration\CategoryNamespaceProvider $categoryNamespaceProvider, \_PhpScoper0a6b37af0871\Rector\PSR4\FileRelocationResolver $fileRelocationResolver)
    {
     ...
     * @param Node[] $nodes
     */
    public function createWithDesiredGroup(\_PhpScoper0a6b37af0871\Symplify\SmartFileSystem\SmartFileInfo $oldFileInfo, array $nodes, string $desiredGroupName) : ?\_PhpScoper0a6b37af0871\Rector\FileSystemRector\ValueObject\MovedFileWithNodes
    {
        /** @var Namespace_|null $currentNamespace */
        $currentNamespace = $this->betterNodeFinder->findFirstInstanceOf($nodes, \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Namespace_::class);
        ...
        if (\_PhpScoper0a6b37af0871\Nette\Utils\Strings::endsWith($currentNamespaceName, '\\' . $desiredGroupName)) {
            return null;
        }
        ...
        if (\_PhpScoper0a6b37af0871\Nette\Utils\Strings::match($oldClassName, '#\\b' . $desiredGroupName . '\\b#')) {
            return null;
        }
        // 1. rename namespace
        ...
        $classLike = $this->betterNodeFinder->findFirstInstanceOf($nodes, \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\ClassLike::class);
        if ($classLike === null) {
            return null;
        }
        // clone to prevent deep override
        $classLike = clone $classLike;
        $classLike->namespacedName = new \_PhpScoper0a6b37af0871\PhpParser\Node\Name\FullyQualified($newClassName);
        return new \_PhpScoper0a6b37af0871\Rector\FileSystemRector\ValueObject\MovedFileWithNodes($nodes, $newFileDestination, $oldFileInfo, $oldClassName, $newClassName);
    }
    private function createNewNamespaceName(string $desiredGroupName, \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Namespace_ $currentNamespace) : string
    {
    ...
    private function createNewClassName(\_PhpScoper0a6b37af0871\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, string $newNamespaceName) : string
    {
    ...
    private function renameNamespace(array $nodes, string $newNamespaceName) : void
    {
        foreach ($nodes as $node) {
            ...
            $node = clone $node;
            $node->name = new \_PhpScoper0a6b37af0871\PhpParser\Node\Name($newNamespaceName);
            break;
        }
    }
}
