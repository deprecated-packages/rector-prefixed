...
final class MovedFileWithNodesFactory
{
     ...
     * @param Node[] $nodes
     */
    public function createWithDesiredGroup(\Symplify\SmartFileSystem\SmartFileInfo $oldFileInfo, array $nodes, string $desiredGroupName) : ?\Rector\FileSystemRector\ValueObject\MovedFileWithNodes
    {
        ...
        if (\_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::endsWith($currentNamespaceName, '\\' . $desiredGroupName)) {
            return null;
        }
        ...
        if (\_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::match($oldClassName, '#\\b' . $desiredGroupName . '\\b#')) {
            return null;
        }
        // 1. rename namespace
