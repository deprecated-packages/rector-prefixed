...
final class NodeRepository
{
    ...
    {
        if (\_PhpScoperbd5d0c5f7638\Nette\Utils\Strings::contains($methodName, '\\')) {
    ...
    {
        $calls = \_PhpScoperbd5d0c5f7638\Nette\Utils\Arrays::flatten($this->callsByTypeAndMethod);
        return \array_filter($calls, function (\PhpParser\Node $node) : bool {
    ...
    public function findClassesBySuffix(string $suffix) : array
    {
        ...
        foreach ($this->parsedNodeCollector->getClasses() as $className => $classNode) {
            if (!\_PhpScoperbd5d0c5f7638\Nette\Utils\Strings::endsWith($className, $suffix)) {
                continue;
            }
