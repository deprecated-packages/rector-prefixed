...
final class NodeRepository
{
    ...
    {
        if (\_PhpScoper5edc98a7cce2\Nette\Utils\Strings::contains($methodName, '\\')) {
    ...
    {
        $calls = \_PhpScoper5edc98a7cce2\Nette\Utils\Arrays::flatten($this->callsByTypeAndMethod);
        return \array_filter($calls, function (\PhpParser\Node $node) : bool {
    ...
    public function findClassesBySuffix(string $suffix) : array
    {
        ...
        foreach ($this->parsedNodeCollector->getClasses() as $className => $classNode) {
            if (!\_PhpScoper5edc98a7cce2\Nette\Utils\Strings::endsWith($className, $suffix)) {
                continue;
            }
