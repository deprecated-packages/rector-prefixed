...
final class NodeRepository
{
    ...
    {
        if (\_PhpScoper8b9c402c5f32\Nette\Utils\Strings::contains($methodName, '\\')) {
    ...
    {
        $calls = \_PhpScoper8b9c402c5f32\Nette\Utils\Arrays::flatten($this->callsByTypeAndMethod);
        return \array_filter($calls, function (\PhpParser\Node $node) : bool {
    ...
    public function findClassesBySuffix(string $suffix) : array
    {
        ...
        foreach ($this->parsedNodeCollector->getClasses() as $className => $classNode) {
            if (!\_PhpScoper8b9c402c5f32\Nette\Utils\Strings::endsWith($className, $suffix)) {
                continue;
            }
