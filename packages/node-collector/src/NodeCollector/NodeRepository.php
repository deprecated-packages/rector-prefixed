...
final class NodeRepository
{
    ...
    {
        if (\_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::contains($methodName, '\\')) {
    ...
    {
        $calls = \_PhpScoperf18a0c41e2d2\Nette\Utils\Arrays::flatten($this->callsByTypeAndMethod);
        return \array_filter($calls, function (\PhpParser\Node $node) : bool {
    ...
    public function findClassesBySuffix(string $suffix) : array
    {
        ...
        foreach ($this->parsedNodeCollector->getClasses() as $className => $classNode) {
            if (!\_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::endsWith($className, $suffix)) {
                continue;
            }
