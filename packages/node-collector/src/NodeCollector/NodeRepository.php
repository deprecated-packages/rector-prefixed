...
final class NodeRepository
{
    ...
    {
        if (\_PhpScoperbf340cb0be9d\Nette\Utils\Strings::contains($methodName, '\\')) {
    ...
    {
        $calls = \_PhpScoperbf340cb0be9d\Nette\Utils\Arrays::flatten($this->callsByTypeAndMethod);
        return \array_filter($calls, function (\PhpParser\Node $node) : bool {
    ...
    public function findClassesBySuffix(string $suffix) : array
    {
        ...
        foreach ($this->parsedNodeCollector->getClasses() as $className => $classNode) {
            if (!\_PhpScoperbf340cb0be9d\Nette\Utils\Strings::endsWith($className, $suffix)) {
                continue;
            }
