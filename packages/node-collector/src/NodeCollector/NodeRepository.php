...
final class NodeRepository
{
    ...
    {
        if (\_PhpScoperfce0de0de1ce\Nette\Utils\Strings::contains($methodName, '\\')) {
    ...
    {
        $calls = \_PhpScoperfce0de0de1ce\Nette\Utils\Arrays::flatten($this->callsByTypeAndMethod);
        return \array_filter($calls, function (\PhpParser\Node $node) : bool {
    ...
    public function findClassesBySuffix(string $suffix) : array
    {
        ...
        foreach ($this->parsedNodeCollector->getClasses() as $className => $classNode) {
            if (!\_PhpScoperfce0de0de1ce\Nette\Utils\Strings::endsWith($className, $suffix)) {
                continue;
            }
