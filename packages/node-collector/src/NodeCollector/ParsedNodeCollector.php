...
final class ParsedNodeCollector
{
    ...
    public function findByShortName(string $shortName) : ?\PhpParser\Node\Stmt\Class_
    {
        foreach ($this->classes as $className => $classNode) {
            if (\RectorPrefix20210130\Nette\Utils\Strings::endsWith($className, '\\' . $shortName)) {
                return $classNode;
            }
        }
        return null;
    ...
    {
        if (\RectorPrefix20210130\Nette\Utils\Strings::contains($constantName, '\\')) {
    ...
    private function isClassAnonymous(\PhpParser\Node\Stmt\Class_ $class) : bool
    {
        ...
        // PHPStan polution
        return \RectorPrefix20210130\Nette\Utils\Strings::startsWith($className, 'AnonymousClass');
    }
}
