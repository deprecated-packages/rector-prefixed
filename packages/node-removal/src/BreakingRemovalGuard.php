...
{
    public function ensureNodeCanBeRemove(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : void
    {
        ...
        throw new \_PhpScoper0a6b37af0871\Rector\Core\Exception\ShouldNotHappenException(\sprintf('Node "%s" on line %d is child of "%s", so it cannot be removed as it would break PHP code. Change or remove the parent node instead.', \get_class($node), $node->getLine(), \get_class($parentNode)));
    }
    public function isLegalNodeRemoval(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : bool
    {
        ...
        if ($parent instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\BooleanNot) {
    ...
    private function isIfCondition(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : bool
    {
        $parentNode = $node->getAttribute(\_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\Node\AttributeKey::PARENT_NODE);
    ...
    private function isWhileCondition(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : bool
    {
        $parentNode = $node->getAttribute(\_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\Node\AttributeKey::PARENT_NODE);
