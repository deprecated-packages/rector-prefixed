...
namespace _PhpScopere8e811afab72\Rector\NodeNameResolver;
...
use _PhpScopere8e811afab72\Rector\NodeNameResolver\Regex\RegexPatternDetector;
...
final class NodeNameResolver
{
    ...
    public function isName(\_PhpScopere8e811afab72\PhpParser\Node $node, string $name) : bool
    {
        ...
        if ($this->regexPatternDetector->isRegexPattern($name)) {
            return (bool) \_PhpScopere8e811afab72\Nette\Utils\Strings::match($resolvedName, $name);
        }
        // is probably fnmatch
    ...
    public function getName(\_PhpScopere8e811afab72\PhpParser\Node $node) : ?string
    {
        if ($node instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\MethodCall || $node instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\StaticCall) {
            if ($node->name instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\MethodCall || $node->name instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\StaticCall || $node->name instanceof \_PhpScopere8e811afab72\PhpParser\Node\Identifier) {
                return null;
            }
    ...
    public function isLocalPropertyFetchNamed(\_PhpScopere8e811afab72\PhpParser\Node $node, string $name) : bool
    {
    ...
    public function isLocalStaticPropertyFetchNamed(\_PhpScopere8e811afab72\PhpParser\Node $node, string $name) : bool
    {
     ...
     * @param string[] $names
     */
    public function isFuncCallNames(\_PhpScopere8e811afab72\PhpParser\Node $node, array $names) : bool
    {
     ...
     */
    private function reportInvalidNodeForName(\_PhpScopere8e811afab72\PhpParser\Node $node) : void
    {
        ...
        if ($rectorBacktrace) {
            ...
            if (\file_exists($rectorBacktrace[self::FILE])) {
                $fileInfo = new \_PhpScopere8e811afab72\Symplify\SmartFileSystem\SmartFileInfo($rectorBacktrace[self::FILE]);
    ...
    private function matchRectorBacktraceCall(array $backtrace) : ?array
    {
        foreach ($backtrace as $singleTrace) {
            ...
            if (!\is_a($singleTrace['object'], \_PhpScopere8e811afab72\Rector\Core\Contract\Rector\RectorInterface::class)) {
                continue;
            }
            return $singleTrace;
        }
