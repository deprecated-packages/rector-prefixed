...
final class NodeNameResolver
{
    ...
    public function isName(\PhpParser\Node $node, string $name) : bool
    {
        ...
        if ($this->regexPatternDetector->isRegexPattern($name)) {
            return (bool) \RectorPrefix20210105\Nette\Utils\Strings::match($resolvedName, $name);
        }
        // is probably fnmatch
        if (\RectorPrefix20210105\Nette\Utils\Strings::contains($name, '*')) {
    ...
    private function reportInvalidNodeForName(\PhpParser\Node $node) : void
    {
        ...
        if ($rectorBacktrace) {
            ...
            if (\file_exists($rectorBacktrace[self::FILE])) {
                $fileInfo = new \RectorPrefix20210105\Symplify\SmartFileSystem\SmartFileInfo($rectorBacktrace[self::FILE]);
