...
final class NodeNameResolver
{
    ...
    public function endsWith(string $currentName, string $expectedName) : bool
    {
        ...
        return (bool) \RectorPrefix20210219\Nette\Utils\Strings::match($currentName, $suffixNamePattern);
    }
    ...
    private function reportInvalidNodeForName(\PhpParser\Node $node) : void
    {
        ...
        if ($rectorBacktrace) {
            ...
            if (\file_exists($rectorBacktrace[self::FILE])) {
                $fileInfo = new \RectorPrefix20210219\Symplify\SmartFileSystem\SmartFileInfo($rectorBacktrace[self::FILE]);
    ...
    private function isSingleName(\PhpParser\Node $node, string $name) : bool
    {
        ...
        if ($this->regexPatternDetector->isRegexPattern($name)) {
            return (bool) \RectorPrefix20210219\Nette\Utils\Strings::match($resolvedName, $name);
        }
        // is probably fnmatch
