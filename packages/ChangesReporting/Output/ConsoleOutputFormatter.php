...
final class ConsoleOutputFormatter implements \Rector\ChangesReporting\Contract\Output\OutputFormatterInterface
{
    ...
    public function __construct(\Rector\Core\Configuration\Configuration $configuration, \RectorPrefix20210505\Symfony\Component\Console\Style\SymfonyStyle $symfonyStyle, \Rector\ChangesReporting\Annotation\RectorsChangelogResolver $rectorsChangelogResolver)
    {
    ...
    private function normalizePathsToRelativeWithLine(string $errorMessage) : string
    {
        ...
        $errorMessage = \RectorPrefix20210505\Nette\Utils\Strings::replace($errorMessage, $regex, '');
        return \RectorPrefix20210505\Nette\Utils\Strings::replace($errorMessage, self::ON_LINE_REGEX, ':');
    }
    ...
    private function createRectorChangelogLines(\Rector\Core\ValueObject\Reporting\FileDiff $fileDiff) : array
    {
        ...
        foreach ($rectorsChangelogs as $rectorClass => $changelog) {
            $rectorShortClass = (string) \RectorPrefix20210505\Nette\Utils\Strings::after($rectorClass, '\\', -1);
