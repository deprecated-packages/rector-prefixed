...
final class DoctrineAnnotationDecorator
{
    ...
    private function transformGenericTagValueNodesToDoctrineAnnotationTagValueNodes(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode $phpDocNode, \PhpParser\Node $currentPhpNode) : void
    {
        foreach ($phpDocNode->children as $key => $phpDocChildNode) {
            ...
            if (!\RectorPrefix20210502\Nette\Utils\Strings::contains($fullyQualifiedAnnotationClass, '\\')) {
                continue;
            }
            $genericTagValueNode = $phpDocChildNode->value;
    ...
    private function isClosedContent(string $composedContent) : bool
    {
        ...
        do {
            if ($composedTokenIterator->isCurrentTokenTypes([\PHPStan\PhpDocParser\Lexer\Lexer::TOKEN_OPEN_CURLY_BRACKET, \PHPStan\PhpDocParser\Lexer\Lexer::TOKEN_OPEN_PARENTHESES]) || \RectorPrefix20210502\Nette\Utils\Strings::contains($composedTokenIterator->currentTokenValue(), '(')) {
                ++$openBracketCount;
            }
            if ($composedTokenIterator->isCurrentTokenTypes([\PHPStan\PhpDocParser\Lexer\Lexer::TOKEN_CLOSE_CURLY_BRACKET, \PHPStan\PhpDocParser\Lexer\Lexer::TOKEN_CLOSE_PARENTHESES]) || \RectorPrefix20210502\Nette\Utils\Strings::contains($composedTokenIterator->currentTokenValue(), ')')) {
                ++$closeBracketCount;
            }
