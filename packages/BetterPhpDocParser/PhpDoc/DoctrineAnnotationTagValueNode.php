...
final class DoctrineAnnotationTagValueNode extends \Rector\BetterPhpDocParser\ValueObject\PhpDocNode\AbstractTagValueNode implements \PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagValueNode
{
    /**
     * @var string
    ...
    )
    {
        ...
        $this->hasChanged = \true;
        parent::__construct($values, $docContent);
    }
    ...
    {
        // is quoted?
        $isQuoted = (bool) \RectorPrefix20210404\Nette\Utils\Strings::match($this->items[0], self::UNQUOTED_VALUE_REGEX);
        if ($isQuoted) {
            $value = '"' . $value . '"';
        }
        $this->values[0] = $value;
    ...
    {
        // is quoted?
        $isQuoted = (bool) \RectorPrefix20210404\Nette\Utils\Strings::match($this->values[$key], self::UNQUOTED_VALUE_REGEX);
        if ($isQuoted) {
            $value = '"' . $value . '"';
        }
    ...
    private function removeQuotes($value)
    {
        ...
        $matches = \RectorPrefix20210404\Nette\Utils\Strings::match($value, self::UNQUOTED_VALUE_REGEX);
        if ($matches === null) {
            return $value;
        }
