...
abstract class AbstractValuesAwareNode implements \PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagValueNode
{
    ...
    public function changeValue(string $key, $value) : void
    {
        ...
        if (isset($this->values[$key])) {
            $isQuoted = (bool) \RectorPrefix20210413\Nette\Utils\Strings::match($this->values[$key], self::UNQUOTED_VALUE_REGEX);
            if ($isQuoted) {
                $value = '"' . $value . '"';
            }
        }
    ...
    {
        // is quoted?
        $isQuoted = (bool) \RectorPrefix20210413\Nette\Utils\Strings::match($this->values[0], self::UNQUOTED_VALUE_REGEX);
        if ($isQuoted) {
            $value = '"' . $value . '"';
        }
        $this->values[0] = $value;
    ...
    protected function removeQuotes($value)
    {
        ...
        $matches = \RectorPrefix20210413\Nette\Utils\Strings::match($value, self::UNQUOTED_VALUE_REGEX);
        if ($matches === null) {
            return $value;
        }
