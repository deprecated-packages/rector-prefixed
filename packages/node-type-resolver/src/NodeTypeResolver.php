...
final class NodeTypeResolver
{
    ...
    public function isObjectType(\PhpParser\Node $node, $requiredType) : bool
    {
        ...
        if (\is_string($requiredType) && \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::contains($requiredType, '*')) {
    ...
    public function getStaticType(\PhpParser\Node $node) : \PHPStan\Type\Type
    {
        ...
        if ($node instanceof \PhpParser\Node\Param || $node instanceof \PhpParser\Node\Scalar) {
            return $this->resolve($node);
        }
        /** @var Scope|null $nodeScope */
        $nodeScope = $node->getAttribute(\Rector\NodeTypeResolver\Node\AttributeKey::SCOPE);
        if (!$node instanceof \PhpParser\Node\Expr || $nodeScope === null) {
    ...
    private function resolveFirstType(\PhpParser\Node $node) : \PHPStan\Type\Type
    {
        ...
        /** @var Scope|null $nodeScope */
        $nodeScope = $node->getAttribute(\Rector\NodeTypeResolver\Node\AttributeKey::SCOPE);
        if ($nodeScope === null || !$node instanceof \PhpParser\Node\Expr) {
            return new \PHPStan\Type\MixedType();
        }
    ...
    {
        return $node instanceof \PhpParser\Node\Expr && $this->arrayTypeAnalyzer->isArrayType($node);
    }
    ...
    private function resolveAnonymousClassType(\PhpParser\Node\Expr\New_ $new) : \PHPStan\Type\ObjectWithoutClassType
    {
        ...
        if ($class->extends !== null) {
            ...
            $types[] = new \Rector\PHPStan\Type\FullyQualifiedObjectType($parentClass);
        }
        ...
        if (\count($types) > 1) {
            $unionType = \Rector\PHPStan\TypeFactoryStaticHelper::createUnionObjectType($types);
