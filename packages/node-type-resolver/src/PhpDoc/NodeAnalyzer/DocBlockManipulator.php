...
final class DocBlockManipulator
{
    ...
    {
        $docContent = \RectorPrefix20210103\Nette\Utils\Strings::replace($docContent, self::NEWLINE_MIDDLE_DOC_REGEX, ' ');
        return \RectorPrefix20210103\Nette\Utils\Strings::replace($docContent, self::NEWLINE_CLOSING_DOC_REGEX, ' */');
    }
    /**
    ...
    private function completeSimpleCommentsToPhpDoc(\PhpParser\Node $node, string $phpDoc) : string
    {
        ...
        foreach ($node->getComments() as $comment) {
            ...
            if (!\RectorPrefix20210103\Nette\Utils\Strings::startsWith($comment->getText(), '//')) {
                continue;
            }
    ...
    {
        return \RectorPrefix20210103\Nette\Utils\Strings::replace($content, self::SPACE_OR_ASTERISK_REGEX, '');
    }
}
