...
final class DocBlockManipulator
{
    ...
    public function __construct(\_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\PhpDoc\NodeAnalyzer\DocBlockClassRenamer $docBlockClassRenamer, \_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Printer\PhpDocInfoPrinter $phpDocInfoPrinter)
    {
        $this->phpDocInfoPrinter = $phpDocInfoPrinter;
        $this->docBlockClassRenamer = $docBlockClassRenamer;
    }
    public function changeType(\_PhpScoper0a6b37af0871\PhpParser\Node $node, \_PhpScoper0a6b37af0871\PHPStan\Type\Type $oldType, \_PhpScoper0a6b37af0871\PHPStan\Type\Type $newType) : void
    {
    ...
    public function replaceAnnotationInNode(\_PhpScoper0a6b37af0871\PhpParser\Node $node, \_PhpScoper0a6b37af0871\Rector\Renaming\ValueObject\RenameAnnotation $renameAnnotation) : void
    {
        /** @var PhpDocInfo $phpDocInfo */
        $phpDocInfo = $node->getAttribute(\_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\Node\AttributeKey::PHP_DOC_INFO);
        $this->replaceTagByAnother($phpDocInfo->getPhpDocNode(), $renameAnnotation->getOldAnnotation(), $renameAnnotation->getNewAnnotation());
    }
    public function replaceTagByAnother(\_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode $phpDocNode, string $oldTag, string $newTag) : void
    {
        $oldTag = \_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Annotation\StaticAnnotationNaming::normalizeName($oldTag);
        $newTag = \_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Annotation\StaticAnnotationNaming::normalizeName($newTag);
     ...
     */
    public function hasNodeTypeTags(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : bool
    {
        ...
        return $phpDocInfo->hasByType(\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Contract\PhpDocNode\TypeAwareTagValueNodeInterface::class);
    }
    public function updateNodeWithPhpDocInfo(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : void
    {
        // nothing to change
        /** @var PhpDocInfo|null $phpDocInfo */
        $phpDocInfo = $node->getAttribute(\_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\Node\AttributeKey::PHP_DOC_INFO);
        ...
        if ($phpDoc === '') {
            if ($phpDocInfo->getOriginalPhpDocNode()->children !== []) {
                ...
                $node->setAttribute(\_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\Node\AttributeKey::COMMENTS, null);
            }
            return;
        }
        ...
        $node->setDocComment(new \_PhpScoper0a6b37af0871\PhpParser\Comment\Doc($phpDoc));
    }
    public function getDoctrineFqnTargetEntity(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : ?string
    {
        /** @var PhpDocInfo $phpDocInfo */
        $phpDocInfo = $node->getAttribute(\_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\Node\AttributeKey::PHP_DOC_INFO);
        $doctrineRelationTagValueNode = $phpDocInfo->getByType(\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Contract\Doctrine\DoctrineRelationTagValueNodeInterface::class);
        if ($doctrineRelationTagValueNode === null) {
            return null;
        }
        return $doctrineRelationTagValueNode->getFullyQualifiedTargetEntity();
    }
    private function printPhpDocInfoToString(\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\PhpDocInfo\PhpDocInfo $phpDocInfo) : string
    {
    ...
    private function haveDocCommentOrCommentsChanged(\_PhpScoper0a6b37af0871\PhpParser\Node $node, string $phpDoc) : bool
    {
    ...
    {
        $docContent = \_PhpScoper0a6b37af0871\Nette\Utils\Strings::replace($docContent, self::NEWLINE_MIDDLE_DOC_REGEX, ' ');
        return \_PhpScoper0a6b37af0871\Nette\Utils\Strings::replace($docContent, self::NEWLINE_CLOSING_DOC_REGEX, ' */');
    }
    /**
     * add // comments to phpdoc (only has /**
     */
    private function completeSimpleCommentsToPhpDoc(\_PhpScoper0a6b37af0871\PhpParser\Node $node, string $phpDoc) : string
    {
        $startComments = '';
        foreach ($node->getComments() as $comment) {
            // skip non-simple comments
            if (!\_PhpScoper0a6b37af0871\Nette\Utils\Strings::startsWith($comment->getText(), '//')) {
                continue;
            }
    ...
    {
        return \_PhpScoper0a6b37af0871\Nette\Utils\Strings::replace($content, self::SPACE_OR_ASTERISK_REGEX, '');
    }
}
