...
use RectorPrefix20210326\PHPUnit\Framework\ExpectationFailedException;
use RectorPrefix20210326\Psr\Container\ContainerInterface;
...
use RectorPrefix20210326\Symplify\EasyTesting\DataProvider\StaticFixtureFinder;
use RectorPrefix20210326\Symplify\EasyTesting\DataProvider\StaticFixtureUpdater;
use RectorPrefix20210326\Symplify\EasyTesting\StaticFixtureSplitter;
use RectorPrefix20210326\Symplify\PackageBuilder\Parameter\ParameterProvider;
use RectorPrefix20210326\Symplify\PackageBuilder\Testing\AbstractKernelTestCase;
use RectorPrefix20210326\Symplify\SmartFileSystem\SmartFileInfo;
abstract class AbstractRectorTestCase extends \RectorPrefix20210326\Symplify\PackageBuilder\Testing\AbstractKernelTestCase implements \Rector\Testing\Contract\RectorTestInterface
{
    ...
    protected function setUp() : void
    {
        ...
        $configFileInfo = new \RectorPrefix20210326\Symplify\SmartFileSystem\SmartFileInfo($this->provideConfigFilePath());
        ...
        $this->parameterProvider = $this->getService(\RectorPrefix20210326\Symplify\PackageBuilder\Parameter\ParameterProvider::class);
    ...
    {
        return \RectorPrefix20210326\Symplify\EasyTesting\DataProvider\StaticFixtureFinder::yieldDirectoryExclusively($directory, $suffix);
    }
    /**
     * @param SmartFileInfo[] $extraFileInfos
     */
    protected function doTestFileInfo(\RectorPrefix20210326\Symplify\SmartFileSystem\SmartFileInfo $fixtureFileInfo, array $extraFileInfos = []) : void
    {
        $inputFileInfoAndExpectedFileInfo = \RectorPrefix20210326\Symplify\EasyTesting\StaticFixtureSplitter::splitFileInfoToLocalInputAndExpectedFileInfos($fixtureFileInfo, \false);
    ...
    protected function doTestExtraFile(string $expectedExtraFileName, string $expectedExtraContentFilePath) : void
    {
        ...
        foreach ($addedFilesWithContents as $addedFileWithContent) {
            if (!\RectorPrefix20210326\Nette\Utils\Strings::endsWith($addedFileWithContent->getFilePath(), $expectedExtraFileName)) {
                continue;
            }
        ...
        foreach ($addedFilesWithNodes as $addedFileWithNode) {
            if (!\RectorPrefix20210326\Nette\Utils\Strings::endsWith($addedFileWithNode->getFilePath(), $expectedExtraFileName)) {
                continue;
            }
        ...
        foreach ($movedFilesWithContent as $movedFileWithContent) {
            if (!\RectorPrefix20210326\Nette\Utils\Strings::endsWith($movedFileWithContent->getNewPathname(), $expectedExtraFileName)) {
                continue;
            }
     ...
     * @param SmartFileInfo[] $extraFileInfos
     */
    private function doTestFileMatchesExpectedContent(\RectorPrefix20210326\Symplify\SmartFileSystem\SmartFileInfo $originalFileInfo, \RectorPrefix20210326\Symplify\SmartFileSystem\SmartFileInfo $expectedFileInfo, \RectorPrefix20210326\Symplify\SmartFileSystem\SmartFileInfo $fixtureFileInfo, array $extraFileInfos = []) : void
    {
        $this->parameterProvider->changeParameter(\Rector\Core\Configuration\Option::SOURCE, [$originalFileInfo->getRealPath()]);
        if (!\RectorPrefix20210326\Nette\Utils\Strings::endsWith($originalFileInfo->getFilename(), '.blade.php') && \in_array($originalFileInfo->getSuffix(), ['php', 'phpt'], \true)) {
            if ($extraFileInfos === []) {
        ...
        } elseif (\RectorPrefix20210326\Nette\Utils\Strings::match($originalFileInfo->getFilename(), \Rector\Core\ValueObject\StaticNonPhpFileSuffixes::getSuffixRegexPattern())) {
        ...
        } catch (\RectorPrefix20210326\PHPUnit\Framework\ExpectationFailedException $expectationFailedException) {
            \RectorPrefix20210326\Symplify\EasyTesting\DataProvider\StaticFixtureUpdater::updateFixtureContent($originalFileInfo, $changedContent, $fixtureFileInfo);
    ...
    {
        return \RectorPrefix20210326\Nette\Utils\Strings::replace($string, '#\\r\\n|\\r|\\n#', "\n");
    }
    /**
