...
use RectorPrefix20210319\PHPUnit\Framework\ExpectationFailedException;
...
use RectorPrefix20210319\Symplify\EasyTesting\StaticFixtureSplitter;
...
abstract class AbstractRectorTestCase extends \RectorPrefix20210319\Symplify\PackageBuilder\Testing\AbstractKernelTestCase implements \Rector\Testing\Contract\RectorTestInterface
{
    ...
    {
        return \RectorPrefix20210319\Symplify\EasyTesting\DataProvider\StaticFixtureFinder::yieldDirectoryExclusively($directory, $suffix);
    }
    /**
    ...
    protected function doTestExtraFile(string $expectedExtraFileName, string $expectedExtraContentFilePath) : void
    {
        ...
        foreach ($addedFilesWithContents as $addedFileWithContent) {
            if (!\RectorPrefix20210319\Nette\Utils\Strings::endsWith($addedFileWithContent->getFilePath(), $expectedExtraFileName)) {
                continue;
            }
        ...
        foreach ($addedFilesWithNodes as $addedFileWithNode) {
            if (!\RectorPrefix20210319\Nette\Utils\Strings::endsWith($addedFileWithNode->getFilePath(), $expectedExtraFileName)) {
                continue;
            }
        ...
        foreach ($movedFilesWithContent as $movedFileWithContent) {
            if (!\RectorPrefix20210319\Nette\Utils\Strings::endsWith($movedFileWithContent->getNewPathname(), $expectedExtraFileName)) {
                continue;
            }
     ...
     * @param SmartFileInfo[] $extraFileInfos
     */
    private function doTestFileMatchesExpectedContent(\RectorPrefix20210319\Symplify\SmartFileSystem\SmartFileInfo $originalFileInfo, \RectorPrefix20210319\Symplify\SmartFileSystem\SmartFileInfo $expectedFileInfo, \RectorPrefix20210319\Symplify\SmartFileSystem\SmartFileInfo $fixtureFileInfo, array $extraFileInfos = []) : void
    {
        $this->parameterProvider->changeParameter(\Rector\Core\Configuration\Option::SOURCE, [$originalFileInfo->getRealPath()]);
        if (!\RectorPrefix20210319\Nette\Utils\Strings::endsWith($originalFileInfo->getFilename(), '.blade.php') && \in_array($originalFileInfo->getSuffix(), ['php', 'phpt'], \true)) {
            if ($extraFileInfos === []) {
        ...
        } elseif (\RectorPrefix20210319\Nette\Utils\Strings::match($originalFileInfo->getFilename(), \Rector\Core\ValueObject\StaticNonPhpFileSuffixes::getSuffixRegexPattern())) {
        ...
        } catch (\RectorPrefix20210319\PHPUnit\Framework\ExpectationFailedException $expectationFailedException) {
            \RectorPrefix20210319\Symplify\EasyTesting\DataProvider\StaticFixtureUpdater::updateFixtureContent($originalFileInfo, $changedContent, $fixtureFileInfo);
    ...
    {
        return \RectorPrefix20210319\Nette\Utils\Strings::replace($string, '#\\r\\n|\\r|\\n#', "\n");
    }
    /**
