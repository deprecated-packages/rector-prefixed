...
use _PhpScoper0a6b37af0871\Rector\Core\Exception\Node\MissingParentNodeException;
...
final class ReadWritePropertyAnalyzer
{
    ...
    public function __construct(\_PhpScoper0a6b37af0871\Rector\SOLID\Guard\VariableToConstantGuard $variableToConstantGuard, \_PhpScoper0a6b37af0871\Rector\Core\PhpParser\Node\Manipulator\AssignManipulator $assignManipulator, \_PhpScoper0a6b37af0871\Rector\ReadWrite\NodeAnalyzer\ReadExprAnalyzer $readExprAnalyzer)
    {
    ...
    {
        \_PhpScoper0a6b37af0871\Webmozart\Assert\Assert::isAnyOf($node, [\_PhpScoper0a6b37af0871\PhpParser\Node\Expr\PropertyFetch::class, \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\StaticPropertyFetch::class]);
        $parent = $node->getAttribute(\_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\Node\AttributeKey::PARENT_NODE);
        if ($parent === null) {
            throw new \_PhpScoper0a6b37af0871\Rector\Core\Exception\Node\MissingParentNodeException();
        }
        ...
        if ($parent instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\ArrayDimFetch && $parent->dim === $node) {
            return $this->isArrayDimFetchRead($parent);
        }
        return !$this->assignManipulator->isLeftPartOfAssign($node);
    }
    private function unwrapPostPreIncDec(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : \_PhpScoper0a6b37af0871\PhpParser\Node
    {
        if ($node instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\PreInc || $node instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\PreDec || $node instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\PostInc || $node instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\PostDec) {
            $node = $node->getAttribute(\_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\Node\AttributeKey::PARENT_NODE);
            if ($node === null) {
                throw new \_PhpScoper0a6b37af0871\Rector\Core\Exception\Node\MissingParentNodeException();
            }
        }
    ...
    private function isArrayDimFetchRead(\_PhpScoper0a6b37af0871\PhpParser\Node\Expr\ArrayDimFetch $arrayDimFetch) : bool
    {
