...
abstract class AbstractTagValueNode implements \Rector\BetterPhpDocParser\Contract\PhpDocNode\AttributeAwareNodeInterface, \PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagValueNode
{
    ...
    {
        $tagValueNodeConfigurationFactory = new \Rector\BetterPhpDocParser\ValueObjectFactory\TagValueNodeConfigurationFactory(new \RectorPrefix20210202\Symplify\PackageBuilder\Php\TypeChecker());
        // prevent override
    ...
    {
        // @todo decouple
        if ($this->tagValueNodeConfiguration->getOriginalContent() !== null && \RectorPrefix20210202\Nette\Utils\Strings::endsWith($this->tagValueNodeConfiguration->getOriginalContent(), '()')) {
            return \true;
        }
