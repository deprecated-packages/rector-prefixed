...
final class AnnotationContentResolver
{
    ...
    public function __construct(\RectorPrefix20201227\Symplify\PackageBuilder\Reflection\PrivatesAccessor $privatesAccessor, \Rector\BetterPhpDocParser\PhpDocInfo\TokenIteratorFactory $tokenIteratorFactory)
    {
    ...
    public function resolveFromTokenIterator(\PHPStan\PhpDocParser\Parser\TokenIterator $tokenIterator) : string
    {
        ...
        while (\true) {
            ...
            if (\RectorPrefix20201227\Nette\Utils\Strings::contains($tokenIterator->currentTokenValue(), '*')) {
                $tokenValueWithoutAsterisk = \RectorPrefix20201227\Nette\Utils\Strings::replace($tokenIterator->currentTokenValue(), '#\\*#', '');
