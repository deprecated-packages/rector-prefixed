...
final class AnnotationContentResolver
{
    ...
    public function resolveFromTokenIterator(\PHPStan\PhpDocParser\Parser\TokenIterator $tokenIterator) : string
    {
        ...
        while (\true) {
            ...
            if (\_PhpScoper5edc98a7cce2\Nette\Utils\Strings::contains($tokenIterator->currentTokenValue(), '*')) {
                $tokenValueWithoutAsterisk = \_PhpScoper5edc98a7cce2\Nette\Utils\Strings::replace($tokenIterator->currentTokenValue(), '#\\*#', '');
    ...
    {
        return \_PhpScoper5edc98a7cce2\Nette\Utils\Strings::replace($annotationContent, self::MULTILINE_COMENT_ASTERISK_REGEX, '$1$3');
    }
