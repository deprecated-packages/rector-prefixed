...
final class AnnotationContentResolver
{
    ...
    public function __construct(\RectorPrefix20210202\Symplify\PackageBuilder\Reflection\PrivatesAccessor $privatesAccessor, \Rector\BetterPhpDocParser\PhpDocInfo\TokenIteratorFactory $tokenIteratorFactory)
    {
    ...
    public function resolveFromTokenIterator(\PHPStan\PhpDocParser\Parser\TokenIterator $tokenIterator) : string
    {
        ...
        while (\true) {
            ...
            if (\RectorPrefix20210202\Nette\Utils\Strings::contains($tokenIterator->currentTokenValue(), '*')) {
                $tokenValueWithoutAsterisk = \RectorPrefix20210202\Nette\Utils\Strings::replace($tokenIterator->currentTokenValue(), '#\\*#', '');
    ...
    {
        return \RectorPrefix20210202\Nette\Utils\Strings::replace($annotationContent, self::MULTILINE_COMENT_ASTERISK_REGEX, '$1$3');
    }
