...
final class ClassAnnotationMatcher
{
    ...
    private function isUseMatchingName(string $tag, \PhpParser\Node\Stmt\UseUse $useUse) : bool
    {
        ...
        return (bool) \RectorPrefix20201227\Nette\Utils\Strings::match($tag, '#' . $shortNamePattern . '(\\\\[\\w]+)?#i');
    }
    private function resolveName(string $tag, \PhpParser\Node\Stmt\UseUse $useUse) : string
    {
        ...
        $unaliasedShortClass = \RectorPrefix20201227\Nette\Utils\Strings::substring($tag, \RectorPrefix20201227\Nette\Utils\Strings::length($useUse->alias->toString()));
        if (\RectorPrefix20201227\Nette\Utils\Strings::startsWith($unaliasedShortClass, '\\')) {
