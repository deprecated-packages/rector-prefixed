...
final class ClassAnnotationMatcher
{
    ...
    private function isUseMatchingName(string $tag, \PhpParser\Node\Stmt\UseUse $useUse) : bool
    {
        ...
        return (bool) \RectorPrefix2020DecSat\Nette\Utils\Strings::match($tag, '#' . $shortNamePattern . '(\\\\[\\w]+)?#i');
    }
    private function resolveName(string $tag, \PhpParser\Node\Stmt\UseUse $useUse) : string
    {
        ...
        $unaliasedShortClass = \RectorPrefix2020DecSat\Nette\Utils\Strings::substring($tag, \RectorPrefix2020DecSat\Nette\Utils\Strings::length($useUse->alias->toString()));
        if (\RectorPrefix2020DecSat\Nette\Utils\Strings::startsWith($unaliasedShortClass, '\\')) {
