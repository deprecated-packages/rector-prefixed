...
final class ClassAnnotationMatcher
{
    ...
    public function resolveTagFullyQualifiedName(string $tag, \_PhpScopere8e811afab72\PhpParser\Node $node) : string
    {
     ...
     * @param Use_[] $uses
     */
    private function resolveFullyQualifiedClass(array $uses, \_PhpScopere8e811afab72\PhpParser\Node $node, string $tag) : string
    {
        if ($uses === []) {
    ...
    private function isUseMatchingName(string $tag, \_PhpScopere8e811afab72\PhpParser\Node\Stmt\UseUse $useUse) : bool
    {
        $shortName = $useUse->alias !== null ? $useUse->alias->name : $useUse->name->getLast();
        $shortNamePattern = \preg_quote($shortName, '#');
        return (bool) \_PhpScopere8e811afab72\Nette\Utils\Strings::match($tag, '#' . $shortNamePattern . '(\\\\[\\w]+)?#i');
    }
    private function resolveName(string $tag, \_PhpScopere8e811afab72\PhpParser\Node\Stmt\UseUse $useUse) : string
    {
        if ($useUse->alias === null) {
            return $useUse->name->toString();
        }
        $unaliasedShortClass = \_PhpScopere8e811afab72\Nette\Utils\Strings::substring($tag, \_PhpScopere8e811afab72\Nette\Utils\Strings::length($useUse->alias->toString()));
        if (\_PhpScopere8e811afab72\Nette\Utils\Strings::startsWith($unaliasedShortClass, '\\')) {
