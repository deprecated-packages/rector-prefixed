...
abstract class AbstractPhpDocNodeFactory
{
    ...
    {
        $match = \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::match($annotationContent, self::OPENING_SPACE_REGEX);
        $openingSpace = $match['opening_space'] ?? '';
        $match = \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::match($annotationContent, self::CLOSING_SPACE_REGEX);
    ...
    {
        return \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::replace($targetEntity, self::CLASS_CONST_REGEX, '');
    }
}
