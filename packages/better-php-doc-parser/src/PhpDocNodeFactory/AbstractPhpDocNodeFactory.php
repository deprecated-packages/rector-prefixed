...
use _PhpScoperb75b35f52b74\Rector\BetterPhpDocParser\AnnotationReader\NodeAnnotationReader;
...
abstract class AbstractPhpDocNodeFactory
{
     ...
     * @required
     */
    public function autowireAbstractPhpDocNodeFactory(\_PhpScoperb75b35f52b74\Rector\BetterPhpDocParser\AnnotationReader\NodeAnnotationReader $nodeAnnotationReader, \_PhpScoperb75b35f52b74\Rector\BetterPhpDocParser\PhpDocParser\AnnotationContentResolver $annotationContentResolver, \_PhpScoperb75b35f52b74\Rector\BetterPhpDocParser\Annotation\AnnotationItemsResolver $annotationItemsResolver, \_PhpScoperb75b35f52b74\Rector\TypeDeclaration\PHPStan\Type\ObjectTypeSpecifier $objectTypeSpecifier) : void
    {
    ...
    protected function resolveContentFromTokenIterator(\_PhpScoperb75b35f52b74\PHPStan\PhpDocParser\Parser\TokenIterator $tokenIterator) : string
    {
        return $this->annotationContentResolver->resolveFromTokenIterator($tokenIterator);
    }
    protected function resolveFqnTargetEntity(string $targetEntity, \_PhpScoperb75b35f52b74\PhpParser\Node $node) : string
    {
        $targetEntity = $this->getCleanedUpTargetEntity($targetEntity);
        if (\class_exists($targetEntity)) {
            return $targetEntity;
        }
        $namespacedTargetEntity = $node->getAttribute(\_PhpScoperb75b35f52b74\Rector\NodeTypeResolver\Node\AttributeKey::NAMESPACE_NAME) . '\\' . $targetEntity;
        if (\class_exists($namespacedTargetEntity)) {
            return $namespacedTargetEntity;
        }
        $resolvedType = $this->objectTypeSpecifier->narrowToFullyQualifiedOrAliasedObjectType($node, new \_PhpScoperb75b35f52b74\PHPStan\Type\ObjectType($targetEntity));
        if ($resolvedType instanceof \_PhpScoperb75b35f52b74\Rector\PHPStan\Type\ShortenedObjectType) {
     ...
     */
    protected function matchCurlyBracketAroundSpaces(string $annotationContent) : \_PhpScoperb75b35f52b74\Rector\BetterPhpDocParser\ValueObject\AroundSpaces
    {
        $match = \_PhpScoperb75b35f52b74\Nette\Utils\Strings::match($annotationContent, self::OPENING_SPACE_REGEX);
        $openingSpace = $match['opening_space'] ?? '';
        $match = \_PhpScoperb75b35f52b74\Nette\Utils\Strings::match($annotationContent, self::CLOSING_SPACE_REGEX);
        $closingSpace = $match['closing_space'] ?? '';
        return new \_PhpScoperb75b35f52b74\Rector\BetterPhpDocParser\ValueObject\AroundSpaces($openingSpace, $closingSpace);
    }
    private function getCleanedUpTargetEntity(string $targetEntity) : string
    {
        return \_PhpScoperb75b35f52b74\Nette\Utils\Strings::replace($targetEntity, self::CLASS_CONST_REGEX, '');
    }
}
