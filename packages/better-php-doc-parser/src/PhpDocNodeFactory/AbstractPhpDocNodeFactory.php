...
abstract class AbstractPhpDocNodeFactory
{
    ...
    {
        $match = \RectorPrefix20210115\Nette\Utils\Strings::match($annotationContent, self::OPENING_SPACE_REGEX);
        $openingSpace = $match['opening_space'] ?? '';
        $match = \RectorPrefix20210115\Nette\Utils\Strings::match($annotationContent, self::CLOSING_SPACE_REGEX);
    ...
    {
        return \RectorPrefix20210115\Nette\Utils\Strings::replace($targetEntity, self::CLASS_CONST_REGEX, '');
    }
}
