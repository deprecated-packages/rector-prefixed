...
abstract class AbstractPhpDocNodeFactory
{
    ...
    {
        $match = \_PhpScoperbd5d0c5f7638\Nette\Utils\Strings::match($annotationContent, self::OPENING_SPACE_REGEX);
        $openingSpace = $match['opening_space'] ?? '';
        $match = \_PhpScoperbd5d0c5f7638\Nette\Utils\Strings::match($annotationContent, self::CLOSING_SPACE_REGEX);
    ...
    {
        return \_PhpScoperbd5d0c5f7638\Nette\Utils\Strings::replace($targetEntity, self::CLASS_CONST_REGEX, '');
    }
}
