...
final class ArrayItemStaticHelper
{
    ...
    public static function resolveAnnotationItemsOrder(string $content, ?string $silentKey = null) : array
    {
        ...
        $itemsOrder = [];
        $matches = \_PhpScopera143bcca66cb\Nette\Utils\Strings::matchAll($content, self::ITEM_EQUALS_REGEX);
    ...
    {
        if (!\_PhpScopera143bcca66cb\Nette\Utils\Strings::match($content, self::NON_EMPTY_SILENT_KEY_REGEX)) {
            return \false;
        }
        if ($silentKey === null) {
            return \false;
