...
final class ArrayItemStaticHelper
{
    ...
    public static function resolveAnnotationItemsOrder(string $content, ?string $silentKey = null) : array
    {
        ...
        $itemsOrder = [];
        $matches = \RectorPrefix20210219\Nette\Utils\Strings::matchAll($content, self::ITEM_EQUALS_REGEX);
    ...
    {
        if (!\RectorPrefix20210219\Nette\Utils\Strings::match($content, self::NON_EMPTY_SILENT_KEY_REGEX)) {
            return \false;
        }
        if ($silentKey === null) {
            return \false;
