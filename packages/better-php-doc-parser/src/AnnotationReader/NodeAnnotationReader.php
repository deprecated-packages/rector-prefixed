...
use _PhpScoperbd5d0c5f7638\Doctrine\Common\Annotations\AnnotationException;
...
final class NodeAnnotationReader
{
    ...
    public function __construct(\Rector\DoctrineAnnotationGenerated\PhpDocNode\ConstantReferenceIdentifierRestorer $constantReferenceIdentifierRestorer, \Rector\NodeNameResolver\NodeNameResolver $nodeNameResolver, \_PhpScoperbd5d0c5f7638\Doctrine\Common\Annotations\Reader $reader)
    {
        $this->reader = $reader;
    ...
    public function readClassAnnotation(\PhpParser\Node\Stmt\Class_ $class, string $annotationClassName) : ?object
    {
        ...
        } catch (\_PhpScoperbd5d0c5f7638\Doctrine\Common\Annotations\AnnotationException $annotationException) {
            // unable to load
            return null;
        }
    }
    ...
    private function createPropertyReflectionFromPropertyNode(\PhpParser\Node\Stmt\Property $property) : ?\ReflectionProperty
    {
        ...
        if ($className === null || !\Rector\NodeTypeResolver\ClassExistenceStaticHelper::doesClassLikeExist($className)) {
            // probably fresh node
            return null;
        }
        try {
