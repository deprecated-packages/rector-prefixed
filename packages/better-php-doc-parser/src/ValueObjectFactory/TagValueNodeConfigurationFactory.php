...
final class TagValueNodeConfigurationFactory
{
     ...
     */
    private $typeChecker;
    public function __construct(\RectorPrefix20210307\Symplify\PackageBuilder\Php\TypeChecker $typeChecker)
    {
    ...
    public function createFromOriginalContent(?string $originalContent, \PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagValueNode $phpDocTagValueNode) : \Rector\BetterPhpDocParser\ValueObject\TagValueNodeConfiguration
    {
        ...
        $hasNewlineAfterOpening = (bool) \RectorPrefix20210307\Nette\Utils\Strings::match($originalContent, self::NEWLINE_AFTER_OPENING_REGEX);
        $hasNewlineBeforeClosing = (bool) \RectorPrefix20210307\Nette\Utils\Strings::match($originalContent, self::NEWLINE_BEFORE_CLOSING_REGEX);
        $hasOpeningBracket = (bool) \RectorPrefix20210307\Nette\Utils\Strings::match($originalContent, self::OPENING_BRACKET_REGEX);
        $hasClosingBracket = (bool) \RectorPrefix20210307\Nette\Utils\Strings::match($originalContent, self::CLOSING_BRACKET_REGEX);
        $keysByQuotedStatus = [];
        ...
        $isSilentKeyExplicit = \RectorPrefix20210307\Nette\Utils\Strings::contains($originalContent, \sprintf('%s=', $silentKey));
    ...
    private function isKeyQuoted(string $originalContent, string $key, ?string $silentKey) : bool
    {
        ...
        if ((bool) \RectorPrefix20210307\Nette\Utils\Strings::match($originalContent, $quotedKeyPattern)) {
            return \true;
        }
        // @see https://regex101.com/r/VgvK8C/5/
        $quotedArrayPattern = \sprintf('#%s=\\{"(.*)"\\}|\\{"(.*)"\\}#', $escapedKey);
        return (bool) \RectorPrefix20210307\Nette\Utils\Strings::match($originalContent, $quotedArrayPattern);
    }
    ...
    {
        $hasArrayColonSeparator = (bool) \RectorPrefix20210307\Nette\Utils\Strings::match($originalContent, self::ARRAY_COLON_SEPARATOR_REGEX);
