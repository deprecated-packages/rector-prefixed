...
final class SpacePatternFactory
{
    ...
    public function createSpacePattern(\_PhpScoper2a4e7ab1ecbc\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagNode $phpDocTagNode) : string
    {
        $spacePattern = \preg_quote($phpDocTagNode->name, '#') . '(?<space>\\s+)';
        // we have to match exact @param space, in case of multiple @param s
        if ($phpDocTagNode->value instanceof \_PhpScoper2a4e7ab1ecbc\Rector\AttributeAwarePhpDoc\Ast\PhpDoc\AttributeAwareParamTagValueNode) {
            return $this->createSpacePatternForParamTagValueNode($phpDocTagNode->value, $spacePattern);
        }
        if ($phpDocTagNode->value instanceof \_PhpScoper2a4e7ab1ecbc\Rector\AttributeAwarePhpDoc\Ast\PhpDoc\AttributeAwareGenericTagValueNode) {
    ...
    private function createSpacePatternForParamTagValueNode(\_PhpScoper2a4e7ab1ecbc\Rector\AttributeAwarePhpDoc\Ast\PhpDoc\AttributeAwareParamTagValueNode $attributeAwareParamTagValueNode, string $spacePattern) : string
    {
