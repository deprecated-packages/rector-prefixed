...
final class ArrayPartPhpDocTagPrinter
{
    ...
    {
        $content = \RectorPrefix20210216\Nette\Utils\Json::encode($item);
        // separate by space only items separated by comma, not in "quotes"
        $content = \RectorPrefix20210216\Nette\Utils\Strings::replace($content, '#,#', ', ');
        // @see https://regex101.com/r/C2fDQp/2
        $content = \RectorPrefix20210216\Nette\Utils\Strings::replace($content, '#("[^",]+)(\\s+)?,(\\s+)?([^"]+")#', '$1,$4');
        // change brackets from content to annotations
        $content = \RectorPrefix20210216\Nette\Utils\Strings::replace($content, '#^\\[(.*?)\\]$#', '{$1}');
    ...
    {
        return \RectorPrefix20210216\Nette\Utils\Strings::replace($content, '#(\\"|\\w)\\:(\\"|\\w)#', '$1' . $tagValueNodeConfiguration->getArrayEqualSign() . '$2');
    }
    ...
    private function quoteKeys(array $item, string $key, string $json, string $originalContent) : string
    {
        foreach (\array_keys($item) as $itemKey) {
            ...
            $isKeyQuoted = (bool) \RectorPrefix20210216\Nette\Utils\Strings::match($originalContent, $quotedKeyPattern);
            if (!$isKeyQuoted) {
                continue;
            }
            $json = \RectorPrefix20210216\Nette\Utils\Strings::replace($json, '#([^\\"])' . $itemKey . '([^\\"])#', '$1"' . $itemKey . '"$2');
        }
        return $json;
    }
}
