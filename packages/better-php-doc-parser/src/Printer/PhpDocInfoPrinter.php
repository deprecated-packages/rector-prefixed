...
final class PhpDocInfoPrinter
{
    ...
    public function printFormatPreserving(\Rector\BetterPhpDocParser\PhpDocInfo\PhpDocInfo $phpDocInfo) : string
    {
        ...
        return \RectorPrefix20210303\Nette\Utils\Strings::replace($phpDocString, self::CALLABLE_REGEX, 'callable(');
    }
    private function printPhpDocNode(\Rector\AttributeAwarePhpDoc\Ast\PhpDoc\AttributeAwarePhpDocNode $attributeAwarePhpDocNode) : string
    {
        ...
        // fix missing start
        if (!\RectorPrefix20210303\Nette\Utils\Strings::match($output, self::DOCBLOCK_START_REGEX) && $output) {
            $output = '/**' . $output;
        }
        // fix missing end
        if (\RectorPrefix20210303\Nette\Utils\Strings::match($output, self::OPENING_DOCBLOCK_REGEX) && $output && !\RectorPrefix20210303\Nette\Utils\Strings::match($output, self::CLOSING_DOCBLOCK_REGEX)) {
            $output .= ' */';
        }
        return $output;
    }
    ...
    {
        return \RectorPrefix20210303\Nette\Utils\Strings::replace($phpDocString, self::SPACE_AFTER_ASTERISK_REGEX, '$1*');
    }
    ...
    private function addTokensFromTo(string $output, int $from, int $to, bool $shouldSkipEmptyLinesAbove = \false) : string
    {
        ...
        if ($shouldSkipEmptyLinesAbove && \RectorPrefix20210303\Nette\Utils\Strings::contains($this->tokens[$from][0], \PHP_EOL) && \RectorPrefix20210303\Nette\Utils\Strings::contains($this->tokens[$from + 1][0], \PHP_EOL)) {
            ++$from;
        }
    ...
    private function printPhpDocTagNode(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagNode $phpDocTagNode, \Rector\BetterPhpDocParser\ValueObject\StartAndEnd $startAndEnd, string $output) : string
    {
        ...
        if ($this->hasDescription($phpDocTagNode)) {
            ...
            $pattern = \RectorPrefix20210303\Nette\Utils\Strings::replace($quotedDescription, '#[\\s]+#', '\\s+');
            $nodeOutput = \RectorPrefix20210303\Nette\Utils\Strings::replace($nodeOutput, '#' . $pattern . '#', function () use($phpDocTagNode) {
                // warning: classic string replace() breaks double "\\" slashes to "\"
                return $phpDocTagNode->value->description;
            });
            if (\substr_count($nodeOutput, "\n") !== 0) {
                $nodeOutput = \RectorPrefix20210303\Nette\Utils\Strings::replace($nodeOutput, "#\n#", self::NEWLINE_ASTERISK);
            }
        }
    ...
    private function resolveTagSpaceSeparator(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagNode $phpDocTagNode) : string
    {
        ...
        $matches = \RectorPrefix20210303\Nette\Utils\Strings::match($originalContent, $spacePattern);
