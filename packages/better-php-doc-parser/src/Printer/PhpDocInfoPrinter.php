...
final class PhpDocInfoPrinter
{
    ...
    public function __construct(\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Printer\EmptyPhpDocDetector $emptyPhpDocDetector, \_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Printer\MultilineSpaceFormatPreserver $multilineSpaceFormatPreserver, \_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Printer\OriginalSpacingRestorer $originalSpacingRestorer, \_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Printer\SpacePatternFactory $spacePatternFactory)
    {
     ...
     */
    public function printFormatPreserving(\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\PhpDocInfo\PhpDocInfo $phpDocInfo) : string
    {
        ...
        return \_PhpScoper0a6b37af0871\Nette\Utils\Strings::replace($phpDocString, self::CALLABLE_REGEX, 'callable(');
    }
    private function printPhpDocNode(\_PhpScoper0a6b37af0871\Rector\AttributeAwarePhpDoc\Ast\PhpDoc\AttributeAwarePhpDocNode $attributeAwarePhpDocNode) : string
    {
        ...
        // fix missing start
        if (!\_PhpScoper0a6b37af0871\Nette\Utils\Strings::match($output, self::DOCBLOCK_START_REGEX) && $output) {
            $output = '/**' . $output;
        }
        // fix missing end
        if (\_PhpScoper0a6b37af0871\Nette\Utils\Strings::match($output, self::OPENING_DOCBLOCK_REGEX) && $output && !\_PhpScoper0a6b37af0871\Nette\Utils\Strings::match($output, self::CLOSING_DOCBLOCK_REGEX)) {
            $output .= ' */';
        }
        return $output;
    }
    ...
    {
        return \_PhpScoper0a6b37af0871\Nette\Utils\Strings::replace($phpDocString, self::SPACE_AFTER_ASTERISK_REGEX, '$1*');
    }
    private function printNode(\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Contract\PhpDocNode\AttributeAwareNodeInterface $attributeAwareNode, ?\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\ValueObject\StartAndEnd $startAndEnd = null, int $key = 0, int $nodeCount = 0) : string
    {
        $output = '';
        /** @var StartAndEnd|null $startAndEnd */
        $startAndEnd = $attributeAwareNode->getAttribute(\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Attributes\Attribute\Attribute::START_END) ?: $startAndEnd;
        ...
        if ($attributeAwareNode instanceof \_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagNode) {
            if ($startAndEnd !== null) {
        ...
        if (!$attributeAwareNode instanceof \_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTextNode && !$attributeAwareNode instanceof \_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\GenericTagValueNode && $startAndEnd) {
    ...
    {
        $lastTokenPosition = $this->attributeAwarePhpDocNode->getAttribute(\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Attributes\Attribute\Attribute::LAST_TOKEN_POSITION) ?: $this->currentTokenPosition;
    ...
    private function addTokensFromTo(string $output, int $from, int $to, bool $shouldSkipEmptyLinesAbove = \false) : string
    {
        ...
        if (isset($this->tokens[$from - 1]) && $this->tokens[$from - 1][1] === \_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Lexer\Lexer::TOKEN_HORIZONTAL_WS) {
            --$from;
        }
        // skip extra empty lines above if this is the last one
        if ($shouldSkipEmptyLinesAbove && \_PhpScoper0a6b37af0871\Nette\Utils\Strings::contains($this->tokens[$from][0], \PHP_EOL) && \_PhpScoper0a6b37af0871\Nette\Utils\Strings::contains($this->tokens[$from + 1][0], \PHP_EOL)) {
            ++$from;
        }
     ...
     */
    private function printPhpDocTagNode(\_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagNode $phpDocTagNode, \_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\ValueObject\StartAndEnd $startAndEnd, string $output) : string
    {
        $output .= $phpDocTagNode->name;
        $phpDocTagNodeValue = $phpDocTagNode->value;
        if (!$phpDocTagNodeValue instanceof \_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Contract\PhpDocNode\AttributeAwareNodeInterface) {
        ...
        if ($this->hasDescription($phpDocTagNode)) {
            ...
            $pattern = \_PhpScoper0a6b37af0871\Nette\Utils\Strings::replace($quotedDescription, '#[\\s]+#', '\\s+');
            $nodeOutput = \_PhpScoper0a6b37af0871\Nette\Utils\Strings::replace($nodeOutput, '#' . $pattern . '#', function () use($phpDocTagNode) {
                // warning: classic string replace() breaks double "\\" slashes to "\"
                return $phpDocTagNode->value->description;
            });
            if (\substr_count($nodeOutput, "\n") !== 0) {
                $nodeOutput = \_PhpScoper0a6b37af0871\Nette\Utils\Strings::replace($nodeOutput, "#\n#", self::NEWLINE_ASTERISK);
            }
        }
        return $output . $nodeOutput;
    }
    private function printAttributeWithAsterisk(\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Contract\PhpDocNode\AttributeAwareNodeInterface $attributeAwareNode) : string
    {
    ...
    private function getRemovedNodesPositions() : array
    {
        ...
        foreach ($removedNodes as $removedNode) {
            ...
            $removedPhpDocNodeInfo = $removedNode->getAttribute(\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Attributes\Attribute\Attribute::START_END);
            // change start position to start of the line, so the whole line is removed
            $seekPosition = $removedPhpDocNodeInfo->getStart();
            while ($this->tokens[$seekPosition][1] !== \_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Lexer\Lexer::TOKEN_HORIZONTAL_WS) {
                --$seekPosition;
            }
            $this->removedNodePositions[] = new \_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\ValueObject\StartAndEnd($seekPosition - 1, $removedPhpDocNodeInfo->getEnd());
        }
        return $this->removedNodePositions;
    }
    /**
     ...
     */
    private function resolveTagSpaceSeparator(\_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagNode $phpDocTagNode) : string
    {
        $originalContent = $this->phpDocInfo->getOriginalContent();
        $spacePattern = $this->spacePatternFactory->createSpacePattern($phpDocTagNode);
        $matches = \_PhpScoper0a6b37af0871\Nette\Utils\Strings::match($originalContent, $spacePattern);
    ...
    private function hasDescription(\_PhpScoper0a6b37af0871\Rector\AttributeAwarePhpDoc\Ast\PhpDoc\AttributeAwarePhpDocTagNode $attributeAwarePhpDocTagNode) : bool
    {
        $hasDescriptionWithOriginalSpaces = $attributeAwarePhpDocTagNode->getAttribute(\_PhpScoper0a6b37af0871\Rector\BetterPhpDocParser\Attributes\Attribute\Attribute::HAS_DESCRIPTION_WITH_ORIGINAL_SPACES);
    ...
    private function isCommonTag(\_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagNode $phpDocTagNode) : bool
    {
        if ($phpDocTagNode->value instanceof \_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\ParamTagValueNode) {
            return \true;
        }
        if ($phpDocTagNode->value instanceof \_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\VarTagValueNode) {
            return \true;
        }
        if ($phpDocTagNode->value instanceof \_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\ReturnTagValueNode) {
            return \true;
        }
        return $phpDocTagNode->value instanceof \_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\ThrowsTagValueNode;
    }
}
