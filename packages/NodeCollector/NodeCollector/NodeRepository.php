...
final class NodeRepository
{
    ...
    {
        if (\RectorPrefix20210507\Nette\Utils\Strings::contains($methodName, '\\')) {
    ...
    {
        $calls = \RectorPrefix20210507\Nette\Utils\Arrays::flatten($this->callsByTypeAndMethod);
        return \array_filter($calls, function (\PhpParser\Node $node) : bool {
    ...
    public function findClassesBySuffix(string $suffix) : array
    {
        ...
        foreach ($this->parsedNodeCollector->getClasses() as $className => $classNode) {
            if (!\RectorPrefix20210507\Nette\Utils\Strings::endsWith($className, $suffix)) {
                continue;
            }
