...
final class RectorRecipe
{
     ...
     */
    public function __construct(string $name, array $nodeTypes, string $description, string $codeBefore, string $codeAfter)
    {
    ...
    {
        if (!\_PhpScoper006a73f0e455\Nette\Utils\Strings::endsWith($name, 'Rector')) {
    ...
    {
        $this->category = (string) \_PhpScoper006a73f0e455\Nette\Utils\Strings::after($nodeTypes[0], '\\', -1);
    }
    private function normalizeCode(string $code) : string
    {
        if (\_PhpScoper006a73f0e455\Nette\Utils\Strings::startsWith($code, '<?php')) {
            $code = \ltrim($code, '<?php');
        }
        return \trim($code);
    }
