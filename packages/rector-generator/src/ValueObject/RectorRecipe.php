...
final class RectorRecipe
{
    ...
    {
        if (!\RectorPrefix20210212\Nette\Utils\Strings::endsWith($name, 'Rector')) {
    ...
    {
        $this->category = (string) \RectorPrefix20210212\Nette\Utils\Strings::after($nodeTypes[0], '\\', -1);
    }
    private function normalizeCode(string $code) : string
    {
        if (\RectorPrefix20210212\Nette\Utils\Strings::startsWith($code, '<?php')) {
            $code = \ltrim($code, '<?php');
        }
        return \trim($code);
    }
