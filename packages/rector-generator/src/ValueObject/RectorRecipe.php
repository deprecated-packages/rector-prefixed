...
namespace _PhpScoperb75b35f52b74\Rector\RectorGenerator\ValueObject;
...
final class RectorRecipe
{
    ...
    public function __construct(string $package, string $name, array $nodeTypes, string $description, string $codeBefore, string $codeAfter)
    {
        ...
        if ($codeBefore === $codeAfter) {
            throw new \_PhpScoperb75b35f52b74\Rector\RectorGenerator\Exception\ConfigurationException('Code before and after are identical. They have to be different');
        }
    ...
    public function getPackageDirectory() : string
    {
        ...
        return \_PhpScoperb75b35f52b74\Rector\Core\Util\StaticRectorStrings::camelCaseToDashes($this->getPackage());
    }
    /**
     * @api
     */
    ...
    {
        if (!\_PhpScoperb75b35f52b74\Nette\Utils\Strings::endsWith($name, 'Rector')) {
            $message = \sprintf('Rector name "%s" must end with "Rector"', $name);
            throw new \_PhpScoperb75b35f52b74\Rector\RectorGenerator\Exception\ConfigurationException($message);
        }
        $this->name = $name;
    }
    /**
    ...
    private function setNodeTypes(array $nodeTypes) : void
    {
        foreach ($nodeTypes as $nodeType) {
            if (\is_a($nodeType, \_PhpScoperb75b35f52b74\PhpParser\Node::class, \true)) {
                continue;
            }
            $message = \sprintf('Node type "%s" does not exist, implement "%s" interface or is not imported in "rector-recipe.php"', $nodeType, \_PhpScoperb75b35f52b74\PhpParser\Node::class);
            throw new \_PhpScoperb75b35f52b74\Rector\Core\Exception\ShouldNotHappenException($message);
        }
        if (\count($nodeTypes) < 1) {
            $message = \sprintf('"$nodeTypes" argument requires at least one item, e.g. "%s"', \_PhpScoperb75b35f52b74\PhpParser\Node\Expr\FuncCall::class);
            throw new \_PhpScoperb75b35f52b74\Rector\RectorGenerator\Exception\ConfigurationException($message);
        }
    ...
    {
        $this->category = (string) \_PhpScoperb75b35f52b74\Nette\Utils\Strings::after($nodeTypes[0], '\\', -1);
    }
    private function normalizeCode(string $code) : string
    {
        if (\_PhpScoperb75b35f52b74\Nette\Utils\Strings::startsWith($code, '<?php')) {
            $code = \ltrim($code, '<?php');
        }
        return \trim($code);
    }
