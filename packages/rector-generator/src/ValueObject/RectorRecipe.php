...
final class RectorRecipe
{
    ...
    {
        if (!\_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::endsWith($name, 'Rector')) {
    ...
    {
        $this->category = (string) \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::after($nodeTypes[0], '\\', -1);
    }
    private function normalizeCode(string $code) : string
    {
        if (\_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::startsWith($code, '<?php')) {
            $code = \ltrim($code, '<?php');
        }
        return \trim($code);
    }
