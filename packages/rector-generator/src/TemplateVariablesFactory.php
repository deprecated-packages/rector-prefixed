...
use _PhpScoper0a6b37af0871\Rector\RectorGenerator\NodeFactory\ConfigurationNodeFactory;
...
final class TemplateVariablesFactory
{
    ...
    public function __construct(\_PhpScoper0a6b37af0871\Rector\Core\PhpParser\Printer\BetterStandardPrinter $betterStandardPrinter, \_PhpScoper0a6b37af0871\Rector\RectorGenerator\NodeFactory\ConfigurationNodeFactory $configurationNodeFactory, \_PhpScoper0a6b37af0871\Rector\Core\PhpParser\Node\NodeFactory $nodeFactory, \_PhpScoper0a6b37af0871\Rector\RectorGenerator\TemplateFactory $templateFactory)
    {
     ...
     */
    public function createFromRectorRecipe(\_PhpScoper0a6b37af0871\Rector\RectorGenerator\ValueObject\RectorRecipe $rectorRecipe) : array
    {
        ...
        $rectorClass = $this->templateFactory->create(\_PhpScoper0a6b37af0871\Rector\RectorGenerator\Config\ConfigFilesystem::RECTOR_FQN_NAME_PATTERN, $data);
    ...
    {
        if (\_PhpScoper0a6b37af0871\Nette\Utils\Strings::contains($code, \PHP_EOL)) {
            // multi lines
    ...
    private function createRuleConfiguration(string $rectorClass, array $configuration) : string
    {
        ...
        foreach ($configuration as $constantName => $variableConfiguration) {
            if ($rectorClass === self::SELF) {
                $class = new \_PhpScoper0a6b37af0871\PhpParser\Node\Name(self::SELF);
            } else {
                $class = new \_PhpScoper0a6b37af0871\PhpParser\Node\Name\FullyQualified($rectorClass);
            }
            $classConstFetch = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\ClassConstFetch($class, $constantName);
            if (\is_array($variableConfiguration)) {
                $variableConfiguration = $this->nodeFactory->createArray($variableConfiguration);
            } else {
                $variableConfiguration = \_PhpScoper0a6b37af0871\PhpParser\BuilderHelpers::normalizeValue($variableConfiguration);
            }
            $arrayItems[] = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\ArrayItem($variableConfiguration, $classConstFetch);
        }
    ...
    private function createNodeTypePhp(\_PhpScoper0a6b37af0871\Rector\RectorGenerator\ValueObject\RectorRecipe $rectorRecipe) : string
    {
