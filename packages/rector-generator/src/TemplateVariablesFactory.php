...
use _PhpScoperb75b35f52b74\Rector\RectorGenerator\NodeFactory\ConfigurationNodeFactory;
...
final class TemplateVariablesFactory
{
     ...
     */
    public function createFromRectorRecipe(\_PhpScoperb75b35f52b74\Rector\RectorGenerator\ValueObject\RectorRecipe $rectorRecipe) : array
    {
        ...
        $rectorClass = $this->templateFactory->create(\_PhpScoperb75b35f52b74\Rector\RectorGenerator\Config\ConfigFilesystem::RECTOR_FQN_NAME_PATTERN, $data);
    ...
    {
        if (\_PhpScoperb75b35f52b74\Nette\Utils\Strings::contains($code, \PHP_EOL)) {
            // multi lines
    ...
    private function createRuleConfiguration(string $rectorClass, array $configuration) : string
    {
        ...
        foreach ($configuration as $constantName => $variableConfiguration) {
            if ($rectorClass === self::SELF) {
                $class = new \_PhpScoperb75b35f52b74\PhpParser\Node\Name(self::SELF);
            } else {
                $class = new \_PhpScoperb75b35f52b74\PhpParser\Node\Name\FullyQualified($rectorClass);
            }
            $classConstFetch = new \_PhpScoperb75b35f52b74\PhpParser\Node\Expr\ClassConstFetch($class, $constantName);
            if (\is_array($variableConfiguration)) {
                $variableConfiguration = $this->nodeFactory->createArray($variableConfiguration);
            } else {
                $variableConfiguration = \_PhpScoperb75b35f52b74\PhpParser\BuilderHelpers::normalizeValue($variableConfiguration);
            }
            $arrayItems[] = new \_PhpScoperb75b35f52b74\PhpParser\Node\Expr\ArrayItem($variableConfiguration, $classConstFetch);
        }
    ...
    private function createNodeTypePhp(\_PhpScoperb75b35f52b74\Rector\RectorGenerator\ValueObject\RectorRecipe $rectorRecipe) : string
    {
