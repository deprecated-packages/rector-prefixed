...
final class TemplateFileSystem
{
    ...
    public function resolveDestination(\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, array $templateVariables, \Rector\RectorGenerator\ValueObject\RectorRecipe $rectorRecipe, string $targetDirectory) : string
    {
        ...
        if (!$rectorRecipe->isRectorRepository()) {
            ...
            $destination = \_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::replace($destination, self::PACKAGE_RULES_PATH_REGEX, 'utils/rector');
        }
        // remove _Configured|_Extra prefix
        $destination = $this->applyVariables($destination, $templateVariables);
        $destination = \_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::replace($destination, self::CONFIGURED_OR_EXTRA_REGEX, '');
        // remove ".inc" protection from PHPUnit if not a test case
        if ($this->isNonFixtureFileWithIncSuffix($destination)) {
            $destination = \_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::before($destination, '.inc');
        }
        // special hack for tests, to PHPUnit doesn't load the generated file as test case
        /** @var string $destination */
        if (\_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::endsWith($destination, 'Test.php') && \Rector\Testing\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            $destination .= '.inc';
        }
    ...
    {
        if (\_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::match($filePath, self::FIXTURE_SHORT_REGEX)) {
            return \false;
        }
        return \_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::endsWith($filePath, '.inc');
    }
}
