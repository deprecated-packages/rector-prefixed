...
final class JsonStringFormatter
{
    ...
    public function inlineSections(string $jsonContent, array $sections) : string
    {
        foreach ($sections as $section) {
            ...
            $jsonContent = \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::replace($jsonContent, $pattern, function (array $match) : string {
                $inlined = \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::replace($match[2], '#\\s+#', ' ');
                $inlined = \trim($inlined);
                $inlined = '[' . $inlined . ']';
    ...
    {
        return \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::replace($jsonContent, self::REPLACE_REGEX, function (array $match) : string {
            $inlined = \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::replace($match['content'], '#\\s+#', ' ');
            $inlined = \trim($inlined);
            $inlined = \_PhpScoper5b8c9e9ebd21\Nette\Utils\Strings::replace($inlined, '#},#', "},\n       ");
