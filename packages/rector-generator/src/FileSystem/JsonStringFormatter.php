...
final class JsonStringFormatter
{
    ...
    public function inlineSections(string $jsonContent, array $sections) : string
    {
        foreach ($sections as $section) {
            ...
            $jsonContent = \RectorPrefix20210109\Nette\Utils\Strings::replace($jsonContent, $pattern, function (array $match) : string {
                $inlined = \RectorPrefix20210109\Nette\Utils\Strings::replace($match[2], '#\\s+#', ' ');
                $inlined = \trim($inlined);
                $inlined = '[' . $inlined . ']';
    ...
    {
        return \RectorPrefix20210109\Nette\Utils\Strings::replace($jsonContent, self::REPLACE_REGEX, function (array $match) : string {
            $inlined = \RectorPrefix20210109\Nette\Utils\Strings::replace($match['content'], '#\\s+#', ' ');
            $inlined = \trim($inlined);
            $inlined = \RectorPrefix20210109\Nette\Utils\Strings::replace($inlined, '#},#', "},\n       ");
