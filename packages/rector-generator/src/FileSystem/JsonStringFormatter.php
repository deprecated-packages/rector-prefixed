...
final class JsonStringFormatter
{
    ...
    public function inlineSections(string $jsonContent, array $sections) : string
    {
        foreach ($sections as $section) {
            ...
            $jsonContent = \_PhpScoper26e51eeacccf\Nette\Utils\Strings::replace($jsonContent, $pattern, function (array $match) : string {
                $inlined = \_PhpScoper26e51eeacccf\Nette\Utils\Strings::replace($match[2], '#\\s+#', ' ');
                $inlined = \trim($inlined);
                $inlined = '[' . $inlined . ']';
    ...
    {
        return \_PhpScoper26e51eeacccf\Nette\Utils\Strings::replace($jsonContent, self::REPLACE_REGEX, function (array $match) : string {
            $inlined = \_PhpScoper26e51eeacccf\Nette\Utils\Strings::replace($match['content'], '#\\s+#', ' ');
            $inlined = \trim($inlined);
            $inlined = \_PhpScoper26e51eeacccf\Nette\Utils\Strings::replace($inlined, '#},#', "},\n       ");
