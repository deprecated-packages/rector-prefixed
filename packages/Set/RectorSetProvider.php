...
use ReflectionClass;
use RectorPrefix20210410\Stringy\Stringy;
use RectorPrefix20210410\Symplify\SetConfigResolver\Exception\SetNotFoundException;
use RectorPrefix20210410\Symplify\SetConfigResolver\Provider\AbstractSetProvider;
use RectorPrefix20210410\Symplify\SetConfigResolver\ValueObject\Set;
use RectorPrefix20210410\Symplify\SmartFileSystem\SmartFileInfo;
final class RectorSetProvider extends \RectorPrefix20210410\Symplify\SetConfigResolver\Provider\AbstractSetProvider
{
    /**
     * @var string
    ...
    public function provideByName(string $desiredSetName) : ?\RectorPrefix20210410\Symplify\SetConfigResolver\ValueObject\Set
    {
        $foundSet = parent::provideByName($desiredSetName);
        if ($foundSet instanceof \RectorPrefix20210410\Symplify\SetConfigResolver\ValueObject\Set) {
            return $foundSet;
        }
        ...
        foreach ($this->sets as $set) {
            ...
            $desiredSetFileInfo = new \RectorPrefix20210410\Symplify\SmartFileSystem\SmartFileInfo($desiredSetName);
        ...
        throw new \RectorPrefix20210410\Symplify\SetConfigResolver\Exception\SetNotFoundException($message, $desiredSetName, $this->provideSetNames());
    }
    private function hydrateSetsFromConstants(\ReflectionClass $setListReflectionClass) : void
    {
        foreach ($setListReflectionClass->getConstants() as $name => $setPath) {
            ...
            $stringy = new \RectorPrefix20210410\Stringy\Stringy($name);
            $setName = (string) $stringy->dasherize();
            // remove `-` before numbers
            $setName = \RectorPrefix20210410\Nette\Utils\Strings::replace($setName, self::DASH_NUMBER_REGEX, '$1');
            $this->sets[] = new \RectorPrefix20210410\Symplify\SetConfigResolver\ValueObject\Set($setName, new \RectorPrefix20210410\Symplify\SmartFileSystem\SmartFileInfo($setPath));
        }
    }
}
