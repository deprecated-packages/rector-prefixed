...
final class CheckstyleOutputFormatter implements \Rector\ChangesReporting\Contract\Output\OutputFormatterInterface
{
    ...
    private function writeFileErrors(\Rector\Core\ValueObject\Reporting\FileDiff $fileDiff) : void
    {
        ...
        foreach ($fileDiff->getRectorChanges() as $rectorWithFileAndLineChange) {
            ...
            $error = \sprintf('  <error line="%d" column="1" severity="error" message="%s" />', $this->escape((string) $rectorWithFileAndLineChange->getLine()), $message);
            $this->symfonyStyle->writeln($error);
        }
        $this->symfonyStyle->writeln('</file>');
    }
    private function writeNonFileErrors(\Rector\ChangesReporting\Application\ErrorAndDiffCollector $errorAndDiffCollector) : void
    {
