...
final class ConsoleOutputFormatter implements \Rector\ChangesReporting\Contract\Output\OutputFormatterInterface
{
    ...
    public function __construct(\Rector\Core\PhpParser\Printer\BetterStandardPrinter $betterStandardPrinter, \Rector\Core\Configuration\Configuration $configuration, \RectorPrefix20210103\Symfony\Component\Console\Style\SymfonyStyle $symfonyStyle)
    {
    ...
    {
        $errorMessage = \RectorPrefix20210103\Nette\Utils\Strings::replace($errorMessage, '#' . \preg_quote(\getcwd(), '#') . '/#', '');
        return $errorMessage = \RectorPrefix20210103\Nette\Utils\Strings::replace($errorMessage, self::ON_LINE_REGEX, ':');
    }
    private function reportRemovedNodes(\Rector\ChangesReporting\Application\ErrorAndDiffCollector $errorAndDiffCollector) : void
    {
        ...
        if ($this->symfonyStyle->isVeryVerbose()) {
            ...
            foreach ($errorAndDiffCollector->getRemovedNodes() as $removedNode) {
                ...
                $printedNode = '-' . \RectorPrefix20210103\Nette\Utils\Strings::replace($printedNode, '#\\n#', "\n-");
