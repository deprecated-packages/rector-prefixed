...
final class MarkdownNodeInfosPrinter
{
    ...
    private function printCodeExample(\Rector\Utils\NodeDocumentationGenerator\ValueObject\NodeInfo $nodeInfo) : string
    {
        $contentLines = [];
    ...
    private function printPublicProperties(\Rector\Utils\NodeDocumentationGenerator\ValueObject\NodeInfo $nodeInfo) : string
    {
    ...
    private function printNodeCodeSample(\Rector\Utils\NodeDocumentationGenerator\ValueObject\NodeCodeSample $nodeCodeSample) : string
    {
        $contentLines = [$this->printPhpSnippet($nodeCodeSample->getPhpCode()), '↓', $this->printPhpSnippet($nodeCodeSample->getPrintedContent())];
        return $this->implodeLinesWithSpace($contentLines);
    }
    /**
     * @param string[] $contentLines
