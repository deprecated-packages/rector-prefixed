...
use RectorPrefix20210301\Webmozart\Assert\Assert;
/**
...
, [self::GROUP_NAMES_BY_SUFFIX => ['Repository']])]);
    ...
    public function configure(array $configuration) : void
    {
        ...
        \RectorPrefix20210301\Webmozart\Assert\Assert::allString($groupNamesBySuffix);
     ...
     */
    private function processGroupNamesBySuffix(\RectorPrefix20210301\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, \Rector\Core\PhpParser\Node\CustomNode\FileNode $fileNode, array $groupNamesBySuffix) : void
    {
        foreach ($groupNamesBySuffix as $groupName) {
            // has class suffix
            $suffixPattern = '\\w+' . $groupName . '(Test)?\\.php$';
            if (!\RectorPrefix20210301\Nette\Utils\Strings::match($smartFileInfo->getRealPath(), '#' . $suffixPattern . '#')) {
                continue;
            }
            ...
            if (\RectorPrefix20210301\Nette\Utils\Strings::match($smartFileInfo->getPath(), '#' . $groupName . '$#')) {
                continue;
            }
    ...
    private function isLocatedInExpectedLocation(string $groupName, string $suffixPattern, \RectorPrefix20210301\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo) : bool
    {
        $expectedLocationFilePattern = $this->expectedFileLocationResolver->resolve($groupName, $suffixPattern);
        return (bool) \RectorPrefix20210301\Nette\Utils\Strings::match($smartFileInfo->getRealPath(), $expectedLocationFilePattern);
    }
    private function moveFileToGroupName(\RectorPrefix20210301\Symplify\SmartFileSystem\SmartFileInfo $fileInfo, \Rector\Core\PhpParser\Node\CustomNode\FileNode $fileNode, string $desiredGroupName) : void
    {
