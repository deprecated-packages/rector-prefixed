...
use _PhpScoper0a6b37af0871\PhpParser\Node\Expr\Clone_;
...
use _PhpScoper0a6b37af0871\Rector\PhpSpecToPHPUnit\MatchersManipulator;
 ...
 */
final class PhpSpecPromisesToPHPUnitAssertRector extends \_PhpScoper0a6b37af0871\Rector\PhpSpecToPHPUnit\Rector\AbstractPhpSpecToPHPUnitRector
{
    /**
    ...
    public function __construct(\_PhpScoper0a6b37af0871\Rector\PhpSpecToPHPUnit\MatchersManipulator $matchersManipulator, \_PhpScoper0a6b37af0871\Rector\PhpSpecToPHPUnit\Naming\PhpSpecRenaming $phpSpecRenaming)
    {
    ...
    public function refactor(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : ?\_PhpScoper0a6b37af0871\PhpParser\Node
    {
        ...
        if ($this->isName($node->name, 'clone')) {
            return new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Clone_($this->testedObjectPropertyFetch);
        }
    ...
    private function processDuring(\_PhpScoper0a6b37af0871\PhpParser\Node\Expr\MethodCall $methodCall) : \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\MethodCall
    {
        if (!isset($methodCall->args[0])) {
            throw new \_PhpScoper0a6b37af0871\Rector\Core\Exception\ShouldNotHappenException();
        }
        $name = $this->getValue($methodCall->args[0]->value);
        $thisObjectPropertyMethodCall = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\MethodCall($this->testedObjectPropertyFetch, $name);
        if (isset($methodCall->args[1]) && $methodCall->args[1]->value instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Array_) {
            /** @var Array_ $array */
            $array = $methodCall->args[1]->value;
            if (isset($array->items[0])) {
                $thisObjectPropertyMethodCall->args[] = new \_PhpScoper0a6b37af0871\PhpParser\Node\Arg($array->items[0]->value);
            }
        }
        /** @var MethodCall $parentMethodCall */
        $parentMethodCall = $methodCall->var;
        $parentMethodCall->name = new \_PhpScoper0a6b37af0871\PhpParser\Node\Identifier('expectException');
        // add $this->object->someCall($withArgs)
        $this->addNodeAfterNode($thisObjectPropertyMethodCall, $methodCall);
        return $parentMethodCall;
    }
    private function processDuringInstantiation(\_PhpScoper0a6b37af0871\PhpParser\Node\Expr\MethodCall $methodCall) : \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\MethodCall
    {
        /** @var MethodCall $parentMethodCall */
        $parentMethodCall = $methodCall->var;
        $parentMethodCall->name = new \_PhpScoper0a6b37af0871\PhpParser\Node\Identifier('expectException');
    ...
    private function prepareMethodCall(\_PhpScoper0a6b37af0871\PhpParser\Node\Expr\MethodCall $methodCall) : void
    {
        if ($this->isPrepared) {
            return;
        }
    ...
    private function processBeConstructed(\_PhpScoper0a6b37af0871\PhpParser\Node\Expr\MethodCall $methodCall) : ?\_PhpScoper0a6b37af0871\PhpParser\Node
    {
        if ($this->isName($methodCall->name, 'beConstructedWith')) {
            $new = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\New_(new \_PhpScoper0a6b37af0871\PhpParser\Node\Name\FullyQualified($this->testedClass));
            $new->args = $methodCall->args;
            return new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Assign($this->testedObjectPropertyFetch, $new);
        }
        if ($this->isName($methodCall->name, 'beConstructedThrough')) {
            ...
            return new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Assign($this->testedObjectPropertyFetch, $staticCall);
        }
        return null;
    }
    /**
     ...
     */
    private function processMatchersKeys(\_PhpScoper0a6b37af0871\PhpParser\Node\Expr\MethodCall $methodCall) : void
    {
        foreach ($this->matchersKeys as $matcherKey) {
            ...
            $arrayDimFetch = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\ArrayDimFetch($thisGetMatchers, new \_PhpScoper0a6b37af0871\PhpParser\Node\Scalar\String_($matcherKey));
            $matcherCallableVariable = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Variable('matcherCallable');
            $assign = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Assign($matcherCallableVariable, $arrayDimFetch);
            // 2. call it on result
            $funcCall = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\FuncCall($matcherCallableVariable);
            $funcCall->args = $methodCall->args;
            $methodCall->name = $methodCall->var->name;
            $methodCall->var = $this->testedObjectPropertyFetch;
            $methodCall->args = [];
            $funcCall->args[] = new \_PhpScoper0a6b37af0871\PhpParser\Node\Arg($methodCall);
            $this->addNodesAfterNode([$assign, $funcCall], $methodCall);
            $this->removeNode($methodCall);
            return;
        }
    }
    private function createAssertMethod(string $name, \_PhpScoper0a6b37af0871\PhpParser\Node\Expr $value, ?\_PhpScoper0a6b37af0871\PhpParser\Node\Expr $expected) : \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\MethodCall
    {
        ...
        if (!$this->isBoolAssert && $expected) {
            $assetMethodCall->args[] = new \_PhpScoper0a6b37af0871\PhpParser\Node\Arg($this->thisToTestedObjectPropertyFetch($expected));
        }
        $assetMethodCall->args[] = new \_PhpScoper0a6b37af0871\PhpParser\Node\Arg($this->thisToTestedObjectPropertyFetch($value));
    ...
    private function createTestedObjectPropertyFetch(\_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Class_ $class) : \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\PropertyFetch
    {
        $propertyName = $this->phpSpecRenaming->resolveObjectPropertyName($class);
        return new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\PropertyFetch(new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Variable(self::THIS), $propertyName);
    }
    private function moveConstructorArguments(\_PhpScoper0a6b37af0871\PhpParser\Node\Expr\MethodCall $methodCall, \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\StaticCall $staticCall) : void
    {
        if (!isset($methodCall->args[1])) {
            return;
        }
        if (!$methodCall->args[1]->value instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Array_) {
            return;
        }
        /** @var Array_ $array */
        $array = $methodCall->args[1]->value;
        foreach ($array->items as $arrayItem) {
            ...
            $staticCall->args[] = new \_PhpScoper0a6b37af0871\PhpParser\Node\Arg($arrayItem->value);
        }
    }
    private function resolveBoolMethodName(string $name, \_PhpScoper0a6b37af0871\PhpParser\Node\Expr $expr) : string
    {
        if (!$this->isBool($expr)) {
            return $name;
        }
    ...
    private function thisToTestedObjectPropertyFetch(\_PhpScoper0a6b37af0871\PhpParser\Node\Expr $expr) : \_PhpScoper0a6b37af0871\PhpParser\Node\Expr
    {
