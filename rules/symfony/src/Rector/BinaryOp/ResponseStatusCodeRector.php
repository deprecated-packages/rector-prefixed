...
namespace _PhpScoperb75b35f52b74\Rector\Symfony\Rector\BinaryOp;
 ...
 */
final class ResponseStatusCodeRector extends \_PhpScoperb75b35f52b74\Rector\Core\Rector\AbstractRector
{
    /**
     * @var string
     */
    ...
    {
        return new \_PhpScoperb75b35f52b74\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Turns status code numbers to constants', [new \_PhpScoperb75b35f52b74\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    {
        return [\_PhpScoperb75b35f52b74\PhpParser\Node\Expr\BinaryOp::class, \_PhpScoperb75b35f52b74\PhpParser\Node\Expr\MethodCall::class];
    }
    /**
    ...
    private function processMethodCall(\_PhpScoperb75b35f52b74\PhpParser\Node\Expr\MethodCall $methodCall) : ?\_PhpScoperb75b35f52b74\PhpParser\Node\Expr\MethodCall
    {
        ...
        if (!$statusCode instanceof \_PhpScoperb75b35f52b74\PhpParser\Node\Scalar\LNumber) {
            return null;
        }
        ...
        $methodCall->args[0] = new \_PhpScoperb75b35f52b74\PhpParser\Node\Arg($classConstFetch);
        return $methodCall;
    }
    private function processBinaryOp(\_PhpScoperb75b35f52b74\PhpParser\Node\Expr\BinaryOp $binaryOp) : ?\_PhpScoperb75b35f52b74\PhpParser\Node\Expr\BinaryOp
    {
        if (!$this->isGetStatusMethod($binaryOp->left) && !$this->isGetStatusMethod($binaryOp->right)) {
            return null;
        }
        if ($binaryOp->right instanceof \_PhpScoperb75b35f52b74\PhpParser\Node\Scalar\LNumber && $this->isGetStatusMethod($binaryOp->left)) {
            $binaryOp->right = $this->convertNumberToConstant($binaryOp->right);
            return $binaryOp;
        }
        if ($binaryOp->left instanceof \_PhpScoperb75b35f52b74\PhpParser\Node\Scalar\LNumber && $this->isGetStatusMethod($binaryOp->right)) {
            $binaryOp->left = $this->convertNumberToConstant($binaryOp->left);
            return $binaryOp;
        }
        return null;
    }
    private function isGetStatusMethod(\_PhpScoperb75b35f52b74\PhpParser\Node $node) : bool
    {
        if (!$node instanceof \_PhpScoperb75b35f52b74\PhpParser\Node\Expr\MethodCall) {
            return \false;
        }
     ...
     */
    private function convertNumberToConstant(\_PhpScoperb75b35f52b74\PhpParser\Node\Scalar\LNumber $lNumber)
    {
