...
final class ChangeFileLoaderInExtensionAndKernelRector extends \Rector\Core\Rector\AbstractRector implements \Rector\Core\Contract\Rector\ConfigurableRectorInterface
{
     ...
     */
    private const FILE_LOADERS_BY_TYPE = ['xml' => '_PhpScoper5edc98a7cce2\\Symfony\\Component\\DependencyInjection\\Loader\\XmlFileLoader', 'yaml' => '_PhpScoper5edc98a7cce2\\Symfony\\Component\\DependencyInjection\\Loader\\YamlFileLoader', 'php' => '_PhpScoper5edc98a7cce2\\Symfony\\Component\\DependencyInjection\\Loader\\PhpFileLoader'];
    /**
     * @var string
     */
    private $from;
...
, [self::FROM => 'xml', self::TO => 'yaml'])]);
    ...
    {
        if ($this->isObjectType($class, '_PhpScoper5edc98a7cce2\\Symfony\\Component\\HttpKernel\\DependencyInjection\\Extension')) {
            return \true;
        }
        return $this->isObjectType($class, '_PhpScoper5edc98a7cce2\\Symfony\\Component\\HttpKernel\\Kernel');
    }
    ...
    private function refactorLoadMethodCall(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if (!$this->isObjectType($node->var, '_PhpScoper5edc98a7cce2\\Symfony\\Component\\Config\\Loader\\LoaderInterface')) {
            return null;
        }
    ...
    private function replaceSuffix(\PhpParser\Node\Expr\MethodCall $methodCall, string $from, string $to) : void
    {
        ...
        $this->traverseNodesWithCallable([$fileArgument], function (\PhpParser\Node $node) use($from, $to) : ?Node {
            ...
            $node->value = \_PhpScoper5edc98a7cce2\Nette\Utils\Strings::replace($node->value, '#\\.' . $from . '$#', '.' . $to);
            return $node;
        });
    }
}
