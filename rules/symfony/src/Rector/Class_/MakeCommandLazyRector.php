...
final class MakeCommandLazyRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Make Symfony commands lazy', [new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node, 'RectorPrefix20201228\\Symfony\\Component\\Console\\Command\\Command')) {
            return null;
        }
    ...
    private function resolveCommandNameFromConstructor(\PhpParser\Node\Stmt\Class_ $class) : ?\PhpParser\Node
    {
        ...
        $this->traverseNodesWithCallable($class->stmts, function (\PhpParser\Node $node) use(&$commandName) {
            ...
            if (!$this->isObjectType($node->class, 'RectorPrefix20201228\\Symfony\\Component\\Console\\Command\\Command')) {
                return null;
            }
    ...
    private function resolveCommandNameFromSetName(\PhpParser\Node\Stmt\Class_ $class) : ?\PhpParser\Node
    {
        ...
        $this->traverseNodesWithCallable($class->stmts, function (\PhpParser\Node $node) use(&$commandName) {
            ...
            if (!$this->isObjectType($node->var, 'RectorPrefix20201228\\Symfony\\Component\\Console\\Command\\Command')) {
                return null;
            }
