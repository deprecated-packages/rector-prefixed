...
namespace _PhpScopere8e811afab72\Rector\Symfony;

use _PhpScopere8e811afab72\Nette\Utils\Json;
use _PhpScopere8e811afab72\Nette\Utils\Strings;
use _PhpScopere8e811afab72\Rector\Symfony\Exception\XmlContainerNotExistsException;
use _PhpScopere8e811afab72\Rector\Symfony\ValueObject\ServiceDefinition;
use _PhpScopere8e811afab72\Rector\Symfony\ValueObject\ServiceMap\ServiceMap;
...
final class ServiceMapProvider
{
    ...
    public function __construct(\_PhpScopere8e811afab72\Symplify\PackageBuilder\Parameter\ParameterProvider $parameterProvider, \_PhpScopere8e811afab72\Symplify\SmartFileSystem\SmartFileSystem $smartFileSystem)
    {
        $this->parameterProvider = $parameterProvider;
        $this->smartFileSystem = $smartFileSystem;
    }
    public function provide() : \_PhpScopere8e811afab72\Rector\Symfony\ValueObject\ServiceMap\ServiceMap
    {
        $symfonyContainerXmlPath = $this->getSymfonyContainerXmlPath();
        if ($symfonyContainerXmlPath === '') {
            return new \_PhpScopere8e811afab72\Rector\Symfony\ValueObject\ServiceMap\ServiceMap([]);
        }
    ...
    private function createServiceMapFromXml(string $fileContents) : \_PhpScopere8e811afab72\Rector\Symfony\ValueObject\ServiceMap\ServiceMap
    {
        // "@" intentionally
        $xml = @\simplexml_load_string($fileContents);
        if (!$xml) {
            throw new \_PhpScopere8e811afab72\Rector\Symfony\Exception\XmlContainerNotExistsException(\sprintf('Container "%s" cannot be parsed', $this->getSymfonyContainerXmlPath()));
        }
        ...
        return new \_PhpScopere8e811afab72\Rector\Symfony\ValueObject\ServiceMap\ServiceMap($services);
    }
    /**
     * @return mixed[]
     */
    ...
    {
        $data = \_PhpScopere8e811afab72\Nette\Utils\Json::decode(\_PhpScopere8e811afab72\Nette\Utils\Json::encode((array) $simpleXMLElement), \_PhpScopere8e811afab72\Nette\Utils\Json::FORCE_ARRAY);
     ...
     * @param mixed[] $tags
     */
    private function createServiceFromXmlAndTagsData(\SimpleXMLElement $attrs, array $tags) : \_PhpScopere8e811afab72\Rector\Symfony\ValueObject\ServiceDefinition
    {
        $tags = $this->createTagsFromData($tags);
        return new \_PhpScopere8e811afab72\Rector\Symfony\ValueObject\ServiceDefinition(\strpos((string) $attrs->id, '.') === 0 ? \_PhpScopere8e811afab72\Nette\Utils\Strings::substring((string) $attrs->id, 1) : (string) $attrs->id, \property_exists($attrs, 'class') && $attrs->class !== null ? (string) $attrs->class : null, !(\property_exists($attrs, 'public') && $attrs->public !== null) || (string) $attrs->public !== 'false', \property_exists($attrs, 'synthetic') && $attrs->synthetic !== null && (string) $attrs->synthetic === 'true', \property_exists($attrs, 'alias') && $attrs->alias !== null ? (string) $attrs->alias : null, $tags);
    }
    /**
    ...
    private function createAliasServiceDefinitions(array $aliases, array $services) : array
    {
        foreach ($aliases as $service) {
            ...
            $id = $service->getId();
            $services[$id] = new \_PhpScopere8e811afab72\Rector\Symfony\ValueObject\ServiceDefinition($id, $services[$alias]->getClass(), $service->isPublic(), $service->isSynthetic(), $alias, []);
        }
        return $services;
    }
    /**
    ...
    private function createTagsFromData(array $tagsData) : array
    {
        ...
        foreach ($tagsData as $key => $tag) {
            ...
            if ($name === 'kernel.event_listener') {
                $tagValueObjects[$key] = new \_PhpScopere8e811afab72\Rector\Symfony\ValueObject\Tag\EventListenerTag($data['event'] ?? '', $data['method'] ?? '', (int) ($data['priority'] ?? 0));
            } else {
                unset($data['name']);
                $tagValueObjects[$key] = new \_PhpScopere8e811afab72\Rector\Symfony\ValueObject\Tag($name, $data ?? []);
            }
        }
