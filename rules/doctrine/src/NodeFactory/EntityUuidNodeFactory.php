...
final class EntityUuidNodeFactory
{
    ...
    public function createUuidPropertyDefaultValueAssign(string $uuidVariableName) : \PhpParser\Node\Stmt\Expression
    {
        ...
        $uuid4StaticCall = new \PhpParser\Node\Expr\StaticCall(new \PhpParser\Node\Name\FullyQualified(\_PhpScoper006a73f0e455\Ramsey\Uuid\Uuid::class), 'uuid4');
    ...
    private function clearVarAndOrmAnnotations(\PhpParser\Node $node) : void
    {
        ...
        $clearedDocCommentText = \_PhpScoper006a73f0e455\Nette\Utils\Strings::replace($docComment->getText(), self::ORM_VAR_DOC_LINE_REGEX);
    ...
    private function replaceIntSerializerTypeWithString(\PhpParser\Node $node) : void
    {
        ...
        $stringTypeText = \_PhpScoper006a73f0e455\Nette\Utils\Strings::replace($docComment->getText(), self::SERIALIZER_SHORT_ANNOTATION_REGEX, '$1string$3');
