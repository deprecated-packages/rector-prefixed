 ...
 */
final class ChangeSetIdToUuidValueRector extends \_PhpScoper2a4e7ab1ecbc\Rector\Core\Rector\AbstractRector
{
    /**
    ...
    {
        return new \_PhpScoper2a4e7ab1ecbc\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Change set id to uuid values', [new \_PhpScoper2a4e7ab1ecbc\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    public function refactor(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) : ?\_PhpScoper2a4e7ab1ecbc\PhpParser\Node
    {
        ...
        if ($argumentValue instanceof \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\ClassConstFetch) {
            ...
            if ($constantValueStaticType instanceof \_PhpScoper2a4e7ab1ecbc\PHPStan\Type\StringType) {
                return null;
            }
            // update constant value
            $classConst->consts[0]->value = $this->createUuidStringNode();
            $node->args[0]->value = $this->createStaticCall(\_PhpScoper2a4e7ab1ecbc\Ramsey\Uuid\Uuid::class, 'fromString', [$argumentValue]);
            return $node;
        }
        // C. set uuid from string with generated string
        $value = $this->createStaticCall(\_PhpScoper2a4e7ab1ecbc\Ramsey\Uuid\Uuid::class, 'fromString', [$this->createUuidStringNode()]);
    ...
    private function getSetUuidMethodCallOnSameVariable(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\MethodCall $methodCall) : ?\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\MethodCall
    {
        ...
        $methodCall = $this->betterNodeFinder->findFirst($parentNode, function (\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) use($variableName, $variableType) : bool {
    ...
    private function createUuidStringNode() : \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Scalar\String_
    {
        $uuidValue = \_PhpScoper2a4e7ab1ecbc\Ramsey\Uuid\Uuid::uuid4();
        $uuidValueString = $uuidValue->toString();
        return new \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Scalar\String_($uuidValueString);
    }
    private function isUuidType(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr $expr) : bool
    {
        $argumentStaticType = $this->getStaticType($expr);
        // UUID is already set
        if (!$argumentStaticType instanceof \_PhpScoper2a4e7ab1ecbc\PHPStan\Type\ObjectType) {
            return \false;
        }
        return $argumentStaticType->getClassName() === \_PhpScoper2a4e7ab1ecbc\Ramsey\Uuid\Uuid::class;
    }
}
