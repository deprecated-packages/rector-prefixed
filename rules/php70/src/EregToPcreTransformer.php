...
final class EregToPcreTransformer
{
    ...
    private const CHARACTER_CLASS_MAP = [
        ...
        ':punct:' => '[:punct:]',
        // should include VT
        ':space:' => '_PhpScoper267b3276efc2\\013\\s',
        ':upper:' => '[:upper:]',
        ':xdigit:' => '[:xdigit:]',
    ];
    /**
    ...
    {
        if (!\_PhpScoper267b3276efc2\Nette\Utils\Strings::contains($ereg, $this->pcreDelimiter)) {
    ...
    private function _ere2pcre(string $content, int $i) : array
    {
        ...
        while ($i < $l) {
            ...
            if ($char === '(') {
                ...
                if ($i + 1 < $l && \_PhpScoper267b3276efc2\Nette\Utils\Strings::contains('0123456789', $content[$i + 1])) {
                    $r[$rr] .= '\\{';
                } else {
                    throw new \Rector\Php70\Exception\InvalidEregException('unescaped metacharacter "' . $char . '"');
                }
    ...
    {
        do {
            if ($s[$i] === '[' && $i + 1 < $l && \_PhpScoper267b3276efc2\Nette\Utils\Strings::contains('.=:', $s[$i + 1])) {
                /** @var string $cls */
    ...
    private function _ere2pcre_escape(string $content) : string
    {
        ...
        if (\_PhpScoper267b3276efc2\Nette\Utils\Strings::contains('\\^$.[]|()?*+{}-/', $content)) {
            return '\\' . $content;
        }
        return $content;
    }
    ...
    private function processCurlyBracket(string $s, int $i, array &$r, int $rr) : int
    {
        ...
        $start = $i + 1;
        $length = $ii - ($i + 1);
        $bound = \_PhpScoper267b3276efc2\Nette\Utils\Strings::substring($s, $start, $length);
        $matches = \_PhpScoper267b3276efc2\Nette\Utils\Strings::match($bound, self::BOUND_REGEX);
        if (!$matches) {
    ...
    private function processCharacterClass(string $content, int $i, string $cls) : array
    {
        ...
        $start = $i + 1;
        $length = $ii - ($i + 1);
        $ccls = \_PhpScoper267b3276efc2\Nette\Utils\Strings::substring($content, $start, $length);
