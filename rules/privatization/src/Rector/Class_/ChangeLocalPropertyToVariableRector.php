...
namespace _PhpScoper2a4e7ab1ecbc\Rector\Privatization\Rector\Class_;
 ...
 */
final class ChangeLocalPropertyToVariableRector extends \_PhpScoper2a4e7ab1ecbc\Rector\Core\Rector\AbstractRector
{
    /**
     * @var string[]
     */
    private const SCOPE_CHANGING_NODE_TYPES = [\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\Do_::class, \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\While_::class, \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\If_::class, \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\Else_::class];
    /**
    ...
    public function __construct(\_PhpScoper2a4e7ab1ecbc\Rector\Core\PhpParser\Node\Manipulator\ClassManipulator $classManipulator, \_PhpScoper2a4e7ab1ecbc\Rector\Core\PhpParser\Node\Manipulator\PropertyFetchManipulator $propertyFetchManipulator)
    {
    ...
    {
        return new \_PhpScoper2a4e7ab1ecbc\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Change local property used in single method to local variable', [new \_PhpScoper2a4e7ab1ecbc\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
class SomeClass
{
    private $count;
    public function run()
...
CODE_SAMPLE
     ...
     * @return string[][]
     */
    private function collectPropertyFetchByMethods(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\Class_ $class, array $privatePropertyNames) : array
    {
        $propertyUsageByMethods = [];
        foreach ($privatePropertyNames as $privatePropertyName) {
            foreach ($class->getMethods() as $classMethod) {
                $hasProperty = (bool) $this->betterNodeFinder->findFirst($classMethod, function (\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) use($privatePropertyName) : bool {
     ...
     */
    private function replacePropertyFetchesByLocalProperty(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\Class_ $class, array $propertyUsageByMethods) : void
    {
        foreach ($propertyUsageByMethods as $propertyName => $methodNames) {
            ...
            $this->traverseNodesWithCallable((array) $classMethod->getStmts(), function (\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) use($propertyName) : ?Variable {
                if (!$node instanceof \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\PropertyFetch) {
                    return null;
                }
                ...
                return new \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\Variable($propertyName);
            });
        }
    }
    /**
     ...
     */
    private function isPropertyChangingInMultipleMethodCalls(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\ClassMethod $classMethod, string $privatePropertyName) : bool
    {
        $isPropertyChanging = \false;
        $isPropertyReadInIf = \false;
        $isIfFollowedByAssign = \false;
        $this->traverseNodesWithCallable((array) $classMethod->getStmts(), function (\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) use(&$isPropertyChanging, $privatePropertyName, &$isPropertyReadInIf, &$isIfFollowedByAssign) : ?int {
            if ($isPropertyReadInIf) {
                ...
                if ($parentNode instanceof \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\Assign && $parentNode->var === $node) {
    ...
    private function refactorIf(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\If_ $if, string $privatePropertyName) : ?bool
    {
        $this->traverseNodesWithCallable($if->cond, function (\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) use($privatePropertyName, &$isPropertyReadInIf) : ?int {
            if (!$this->propertyFetchManipulator->isLocalPropertyOfNames($node, [$privatePropertyName])) {
                return null;
            }
    ...
    private function isPropertyChanging(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node, self $this__, string $privatePropertyName) : bool
    {
        $isPropertyChanging = \false;
        // here cannot be any property assign
        $this__->traverseNodesWithCallable($node, function (\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) use(&$isPropertyChanging, $privatePropertyName) : ?int {
