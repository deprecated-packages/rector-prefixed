...
final class RemoveUselessJustForSakeInterfaceRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        foreach ($node->implements as $key => $implement) {
            ...
            if (!\RectorPrefix20210102\Nette\Utils\Strings::match($implementedInterfaceName, $this->interfacePattern)) {
                continue;
            }
            // is interface in /vendor? probably useful
            $classFileLocation = $this->resolveClassFileLocation($implementedInterfaceName);
            if (\RectorPrefix20210102\Nette\Utils\Strings::contains($classFileLocation, 'vendor')) {
                continue;
            }
...
CODE_SAMPLE
    ...
    private function removeInterfaceFile(string $interfaceName, string $classFileLocation) : void
    {
        if (\Rector\Testing\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            ...
            $smartFileInfo = new \RectorPrefix20210102\Symplify\SmartFileSystem\SmartFileInfo($classFileLocation);
