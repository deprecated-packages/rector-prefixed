...
final class TemplateGuesser
{
    ...
    private function resolve(string $namespace, string $class, string $method) : string
    {
        ...
        $action = \RectorPrefix20210129\Nette\Utils\Strings::replace($method, self::ACTION_MATCH_REGEX, '');
        $fullPath = '';
        if ($bundle !== '') {
            $fullPath .= $bundle . '/';
        }
    ...
    private function resolveBundle(string $class, string $namespace) : string
    {
        ...
        $bundle = \RectorPrefix20210129\Nette\Utils\Strings::match($namespace, self::BUNDLE_NAME_MATCHING_REGEX)['bundle'] ?? '';
        $bundle = \RectorPrefix20210129\Nette\Utils\Strings::replace($bundle, self::BUNDLE_SUFFIX_REGEX, '');
        return $bundle !== '' ? '@' . $bundle : '';
    }
    private function resolveController(string $class) : string
    {
        $match = \RectorPrefix20210129\Nette\Utils\Strings::match($class, self::CONTROLLER_NAME_MATCH_REGEX);
        if (!$match) {
            return '';
        }
        $controller = \RectorPrefix20210129\Nette\Utils\Strings::replace($match['class_name_without_suffix'], self::SMALL_LETTER_BIG_LETTER_REGEX, '1_\\2');
