...
use _PhpScoper2a4e7ab1ecbc\Rector\Symfony3\NodeFactory\BuilderFormNodeFactory;
use _PhpScoper2a4e7ab1ecbc\Rector\Symfony3\NodeFactory\ConfigureOptionsNodeFactory;
use ReflectionClass;
 ...
 */
final class FormTypeInstanceToClassConstRector extends \_PhpScoper2a4e7ab1ecbc\Rector\Symfony3\Rector\MethodCall\AbstractFormAddRector
{
    /**
     * @var string[]
     */
    private const CONTROLLER_TYPES = ['_PhpScoper2a4e7ab1ecbc\\Symfony\\Bundle\\FrameworkBundle\\Controller\\Controller', '_PhpScoper2a4e7ab1ecbc\\Symfony\\Bundle\\FrameworkBundle\\Controller\\AbstractController'];
    /**
    ...
    public function __construct(\_PhpScoper2a4e7ab1ecbc\Rector\Symfony3\NodeFactory\BuilderFormNodeFactory $builderFormNodeFactory, \_PhpScoper2a4e7ab1ecbc\Rector\Symfony3\NodeFactory\ConfigureOptionsNodeFactory $configureOptionsNodeFactory)
    {
    ...
    {
        return new \_PhpScoper2a4e7ab1ecbc\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Changes createForm(new FormType), add(new FormType) to ones with "FormType::class"', [new \_PhpScoper2a4e7ab1ecbc\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    private function processNewInstance(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\MethodCall $methodCall, int $position, int $optionsPosition) : ?\_PhpScoper2a4e7ab1ecbc\PhpParser\Node
    {
        if (!isset($methodCall->args[$position])) {
            return null;
        }
        $argValue = $methodCall->args[$position]->value;
        if (!$argValue instanceof \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\New_) {
            return null;
        }
        // we can only process direct name
        if (!$argValue->class instanceof \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Name) {
            return null;
        }
        if ($argValue->args !== []) {
    ...
    private function refactorCollectionOptions(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\MethodCall $methodCall) : void
    {
        ...
        foreach ($optionsArray->items as $arrayItem) {
            ...
            if (!$newClass instanceof \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Name) {
                continue;
            }
     ...
     * @param Arg[] $argNodes
     */
    private function moveArgumentsToOptions(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\MethodCall $methodCall, int $position, int $optionsPosition, string $className, array $argNodes) : ?\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\MethodCall
    {
        ...
        if ($position + 1 !== $optionsPosition && !isset($methodCall->args[$position + 1])) {
            $methodCall->args[$position + 1] = new \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Arg($this->createNull());
        }
        ...
        if (!isset($methodCall->args[$optionsPosition])) {
            ...
            foreach ($namesToArgs as $name => $arg) {
                $array->items[] = new \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\ArrayItem($arg->value, new \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Scalar\String_($name));
            }
            $methodCall->args[$optionsPosition] = new \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Arg($array);
        }
        $formTypeClass = $this->nodeRepository->findClass($className);
        if ($formTypeClass === null) {
            return null;
        }
        $constructorClassMethod = $formTypeClass->getMethod(\_PhpScoper2a4e7ab1ecbc\Rector\Core\ValueObject\MethodName::CONSTRUCT);
    ...
    private function addBuildFormMethod(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\Class_ $class, \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\ClassMethod $classMethod) : void
    {
     ...
     */
    private function addConfigureOptionsMethod(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\Class_ $class, array $namesToArgs) : void
    {
