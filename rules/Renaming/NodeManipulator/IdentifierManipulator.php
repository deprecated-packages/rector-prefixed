...
final class IdentifierManipulator
{
    ...
    {
        \RectorPrefix20210314\Webmozart\Assert\Assert::isAnyOf($node, [\PhpParser\Node\Expr\ClassConstFetch::class, \PhpParser\Node\Expr\MethodCall::class, \PhpParser\Node\Expr\PropertyFetch::class, \PhpParser\Node\Expr\StaticCall::class, \PhpParser\Node\Stmt\ClassMethod::class]);
    ...
    public function removeSuffix(\PhpParser\Node $node, string $suffixToRemove) : void
    {
        ...
        $newName = \RectorPrefix20210314\Nette\Utils\Strings::replace($name, \sprintf('#%s$#', $suffixToRemove), '');
