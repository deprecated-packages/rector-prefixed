...
CODE_SAMPLE
    ...
    {
        if (!$this->classManipulator->hasInterface($node, 'RectorPrefix2020DecSat\\Gedmo\\Translatable\\Translatable')) {
            return null;
        }
        $this->classManipulator->removeInterface($node, 'RectorPrefix2020DecSat\\Gedmo\\Translatable\\Translatable');
        // 1. replace trait
        $this->classInsertManipulator->addAsFirstTrait($node, 'RectorPrefix2020DecSat\\Knp\\DoctrineBehaviors\\Model\\Translatable\\TranslatableTrait');
        // 2. add interface
        $node->implements[] = new \PhpParser\Node\Name\FullyQualified('RectorPrefix2020DecSat\\Knp\\DoctrineBehaviors\\Contract\\Entity\\TranslatableInterface');
    ...
    private function dumpEntityTranslation(\PhpParser\Node\Stmt\Class_ $class, array $translatedPropertyToPhpDocInfos) : void
    {
        ...
        $class->implements[] = new \PhpParser\Node\Name\FullyQualified('RectorPrefix2020DecSat\\Knp\\DoctrineBehaviors\\Contract\\Entity\\TranslationInterface');
        $this->classInsertManipulator->addAsFirstTrait($class, 'RectorPrefix2020DecSat\\Knp\\DoctrineBehaviors\\Model\\Translatable\\TranslationTrait');
