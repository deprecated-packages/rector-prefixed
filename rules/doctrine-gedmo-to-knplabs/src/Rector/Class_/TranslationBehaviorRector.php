 ...
 */
final class TranslationBehaviorRector extends \_PhpScopere8e811afab72\Rector\Core\Rector\AbstractRector
{
    /**
    ...
    {
        return new \_PhpScopere8e811afab72\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Change Translation from gedmo/doctrine-extensions to knplabs/doctrine-behaviors', [new \_PhpScopere8e811afab72\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    {
        if (!$this->classManipulator->hasInterface($node, '_PhpScopere8e811afab72\\Gedmo\\Translatable\\Translatable')) {
            return null;
        }
        $this->classManipulator->removeInterface($node, '_PhpScopere8e811afab72\\Gedmo\\Translatable\\Translatable');
        // 1. replace trait
        $this->classInsertManipulator->addAsFirstTrait($node, '_PhpScopere8e811afab72\\Knp\\DoctrineBehaviors\\Model\\Translatable\\TranslatableTrait');
        // 2. add interface
        $node->implements[] = new \_PhpScopere8e811afab72\PhpParser\Node\Name\FullyQualified('_PhpScopere8e811afab72\\Knp\\DoctrineBehaviors\\Contract\\Entity\\TranslatableInterface');
     ...
     * @return PhpDocInfo[]
     */
    private function collectAndRemoveTranslatableProperties(\_PhpScopere8e811afab72\PhpParser\Node\Stmt\Class_ $class) : array
    {
        ...
        foreach ($class->getProperties() as $property) {
            ...
            $hasTypeLocaleTagValueNode = $propertyPhpDocInfo->hasByType(\_PhpScopere8e811afab72\Rector\BetterPhpDocParser\ValueObject\PhpDocNode\Gedmo\LocaleTagValueNode::class);
            if ($hasTypeLocaleTagValueNode) {
                $this->removeNode($property);
                continue;
            }
            $hasTypeTranslatableTagValueNode = $propertyPhpDocInfo->hasByType(\_PhpScopere8e811afab72\Rector\BetterPhpDocParser\ValueObject\PhpDocNode\Gedmo\TranslatableTagValueNode::class);
            if (!$hasTypeTranslatableTagValueNode) {
                continue;
            }
            $propertyPhpDocInfo->removeByType(\_PhpScopere8e811afab72\Rector\BetterPhpDocParser\ValueObject\PhpDocNode\Gedmo\TranslatableTagValueNode::class);
     ...
     */
    private function removeSetAndGetMethods(\_PhpScopere8e811afab72\PhpParser\Node\Stmt\Class_ $class, array $removedPropertyNames) : void
    {
     ...
     */
    private function dumpEntityTranslation(\_PhpScopere8e811afab72\PhpParser\Node\Stmt\Class_ $class, array $translatedPropertyToPhpDocInfos) : void
    {
        /** @var SmartFileInfo|null $fileInfo */
        $fileInfo = $class->getAttribute(\_PhpScopere8e811afab72\Rector\NodeTypeResolver\Node\AttributeKey::FILE_INFO);
        if ($fileInfo === null) {
        ...
        $namespace = $class->getAttribute(\_PhpScopere8e811afab72\Rector\NodeTypeResolver\Node\AttributeKey::PARENT_NODE);
        if (!$namespace instanceof \_PhpScopere8e811afab72\PhpParser\Node\Stmt\Namespace_) {
            throw new \_PhpScopere8e811afab72\Rector\Core\Exception\ShouldNotHappenException();
        }
        $namespace = new \_PhpScopere8e811afab72\PhpParser\Node\Stmt\Namespace_($namespace->name);
        $class = new \_PhpScopere8e811afab72\PhpParser\Node\Stmt\Class_($classShortName);
        $class->implements[] = new \_PhpScopere8e811afab72\PhpParser\Node\Name\FullyQualified('_PhpScopere8e811afab72\\Knp\\DoctrineBehaviors\\Contract\\Entity\\TranslationInterface');
        $this->classInsertManipulator->addAsFirstTrait($class, '_PhpScopere8e811afab72\\Knp\\DoctrineBehaviors\\Model\\Translatable\\TranslationTrait');
        $phpDocInfo = $this->phpDocInfoFactory->createFromNodeOrEmpty($class);
        $phpDocInfo->addTagValueNodeWithShortName(new \_PhpScopere8e811afab72\Rector\BetterPhpDocParser\ValueObject\PhpDocNode\Doctrine\Class_\EntityTagValueNode([]));
        foreach ($translatedPropertyToPhpDocInfos as $translatedPropertyName => $translatedPhpDocInfo) {
            ...
            $property->setAttribute(\_PhpScopere8e811afab72\Rector\NodeTypeResolver\Node\AttributeKey::PHP_DOC_INFO, $translatedPhpDocInfo);
