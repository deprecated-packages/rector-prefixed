...
final class TranslationBehaviorRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Change Translation from gedmo/doctrine-extensions to knplabs/doctrine-behaviors', [new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    {
        if (!$this->classManipulator->hasInterface($node, 'RectorPrefix20201228\\Gedmo\\Translatable\\Translatable')) {
            return null;
        }
        $this->classManipulator->removeInterface($node, 'RectorPrefix20201228\\Gedmo\\Translatable\\Translatable');
        // 1. replace trait
        $this->classInsertManipulator->addAsFirstTrait($node, 'RectorPrefix20201228\\Knp\\DoctrineBehaviors\\Model\\Translatable\\TranslatableTrait');
        // 2. add interface
        $node->implements[] = new \PhpParser\Node\Name\FullyQualified('RectorPrefix20201228\\Knp\\DoctrineBehaviors\\Contract\\Entity\\TranslatableInterface');
    ...
    private function dumpEntityTranslation(\PhpParser\Node\Stmt\Class_ $class, array $translatedPropertyToPhpDocInfos) : void
    {
        ...
        $class->implements[] = new \PhpParser\Node\Name\FullyQualified('RectorPrefix20201228\\Knp\\DoctrineBehaviors\\Contract\\Entity\\TranslationInterface');
        $this->classInsertManipulator->addAsFirstTrait($class, 'RectorPrefix20201228\\Knp\\DoctrineBehaviors\\Model\\Translatable\\TranslationTrait');
