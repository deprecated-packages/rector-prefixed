 ...
 */
final class GetClassOnNullRector extends \_PhpScoper0a6b37af0871\Rector\Core\Rector\AbstractRector
{
    public function getRuleDefinition() : \_PhpScoper0a6b37af0871\Symplify\RuleDocGenerator\ValueObject\RuleDefinition
    {
        return new \_PhpScoper0a6b37af0871\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Null is no more allowed in get_class()', [new \_PhpScoper0a6b37af0871\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    public function refactor(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : ?\_PhpScoper0a6b37af0871\PhpParser\Node
    {
        ...
        if ($nodeScope instanceof \_PhpScoper0a6b37af0871\PHPStan\Analyser\Scope && !$nodeScope->isInClass()) {
            return null;
        }
        ...
        if (!$this->isNullableType($valueNode) && !$this->isStaticType($valueNode, \_PhpScoper0a6b37af0871\PHPStan\Type\NullType::class)) {
            return null;
        }
        $notIdentical = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\BinaryOp\NotIdentical($valueNode, $this->createNull());
        $funcCall = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\FuncCall($node->name, $node->args);
        $selfClassConstFetch = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\ClassConstFetch(new \_PhpScoper0a6b37af0871\PhpParser\Node\Name('self'), new \_PhpScoper0a6b37af0871\PhpParser\Node\Identifier('class'));
        $ternary = new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Ternary($notIdentical, $funcCall, $selfClassConstFetch);
        $funcCall->setAttribute(\_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\Node\AttributeKey::PARENT_NODE, $ternary);
        return $ternary;
    }
     ...
     */
    private function isIdenticalToNotNull(\_PhpScoper0a6b37af0871\PhpParser\Node\Expr\FuncCall $funcCall, \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Ternary $ternary) : bool
    {
        if (!$ternary->cond instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\BinaryOp\Identical) {
            return \false;
        }
     ...
     * E.g. "$value !== null ? get_class($value)"
     */
    private function isNotIdenticalToNull(\_PhpScoper0a6b37af0871\PhpParser\Node\Expr\FuncCall $funcCall, \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Ternary $ternary) : bool
    {
        if (!$ternary->cond instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\BinaryOp\NotIdentical) {
            return \false;
        }
        if ($this->areNodesEqual($ternary->cond->left, $funcCall->args[0]->value) && $this->isNull($ternary->cond->right)) {
            return \true;
