...
final class PSR4NamespaceMatcher implements \Rector\PSR4\Contract\PSR4AutoloadNamespaceMatcherInterface
{
    ...
    public function getExpectedNamespace(\Rector\Core\ValueObject\Application\File $file, \PhpParser\Node $node) : ?string
    {
        ...
        foreach ($psr4Autoloads as $namespace => $path) {
            ...
            foreach ($paths as $path) {
                $path = \rtrim($path, '/');
                if (!\RectorPrefix20210429\Nette\Utils\Strings::startsWith($smartFileInfo->getRelativeDirectoryPath(), $path)) {
                    continue;
                }
     ...
     */
    private function resolveExtraNamespace(\RectorPrefix20210429\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, string $path) : string
    {
        $extraNamespace = \RectorPrefix20210429\Nette\Utils\Strings::substring($smartFileInfo->getRelativeDirectoryPath(), \RectorPrefix20210429\Nette\Utils\Strings::length($path) + 1);
        $extraNamespace = \RectorPrefix20210429\Nette\Utils\Strings::replace($extraNamespace, '#/#', '\\');
