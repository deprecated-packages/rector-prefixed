...
CODE_SAMPLE
    ...
    private function resolvePathFromClassAndMethodNodes(\PhpParser\Node\Stmt\Class_ $class, \PhpParser\Node\Stmt\ClassMethod $classMethod) : string
    {
        ...
        $presenterPart = \RectorPrefix20210406\Nette\Utils\Strings::after($presenterName, '\\', -1);
        $presenterPart = \RectorPrefix20210406\Nette\Utils\Strings::substring($presenterPart, 0, -\RectorPrefix20210406\Nette\Utils\Strings::length('Presenter'));
        $stringy = new \RectorPrefix20210406\Stringy\Stringy($presenterPart);
        $presenterPart = (string) $stringy->dasherize();
        $match = (array) \RectorPrefix20210406\Nette\Utils\Strings::match($this->getName($classMethod), self::ACTION_RENDER_NAME_MATCHING_REGEX);
