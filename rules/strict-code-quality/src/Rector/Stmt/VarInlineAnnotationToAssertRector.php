...
namespace _PhpScoper0a2ac50786fa\Rector\StrictCodeQuality\Rector\Stmt;
 ...
 */
final class VarInlineAnnotationToAssertRector extends \_PhpScoper0a2ac50786fa\Rector\Core\Rector\AbstractRector
{
    /**
     * @var string
     */
    ...
    {
        return new \_PhpScoper0a2ac50786fa\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Turn @var inline checks above code to assert() of the type', [new \_PhpScoper0a2ac50786fa\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
class SomeClass
{
    public function run()
    {
...
CODE_SAMPLE
    ...
    private function getVarDocVariableName(\_PhpScoper0a2ac50786fa\Rector\BetterPhpDocParser\PhpDocInfo\PhpDocInfo $phpDocInfo) : ?string
    {
    ...
    private function findVariableByName(\_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt $stmt, string $docVariableName) : ?\_PhpScoper0a2ac50786fa\PhpParser\Node
    {
        return $this->betterNodeFinder->findFirst($stmt, function (\_PhpScoper0a2ac50786fa\PhpParser\Node $stmt) use($docVariableName) : bool {
            return $this->isVariableName($stmt, $docVariableName);
        });
    }
    private function isVariableJustCreated(\_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt $stmt, string $docVariableName) : bool
    {
        if (!$stmt instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt\Expression) {
            return \false;
        }
        if (!$stmt->expr instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Assign) {
            return \false;
        }
        $assign = $stmt->expr;
    ...
    private function refactorFreshlyCreatedNode(\_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt $stmt, \_PhpScoper0a2ac50786fa\Rector\BetterPhpDocParser\PhpDocInfo\PhpDocInfo $phpDocInfo, \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Variable $variable) : ?\_PhpScoper0a2ac50786fa\PhpParser\Node
    {
        $stmt->setAttribute(\_PhpScoper0a2ac50786fa\Rector\NodeTypeResolver\Node\AttributeKey::COMMENTS, null);
    ...
    private function refactorAlreadyCreatedNode(\_PhpScoper0a2ac50786fa\PhpParser\Node\Stmt $stmt, \_PhpScoper0a2ac50786fa\Rector\BetterPhpDocParser\PhpDocInfo\PhpDocInfo $phpDocInfo, \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Variable $variable) : ?\_PhpScoper0a2ac50786fa\PhpParser\Node
    {
    ...
    private function createFuncCallBasedOnType(\_PhpScoper0a2ac50786fa\PHPStan\Type\Type $type, \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Variable $variable) : ?\_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\FuncCall
    {
        if ($type instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\ObjectType) {
            $instanceOf = new \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Instanceof_($variable, new \_PhpScoper0a2ac50786fa\PhpParser\Node\Name\FullyQualified($type->getClassName()));
            return $this->createFuncCall(self::ASSERT, [$instanceOf]);
        }
        if ($type instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\IntegerType) {
            $isInt = $this->createFuncCall('is_int', [$variable]);
            return $this->createFuncCall(self::ASSERT, [$isInt]);
        }
        if ($type instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\FloatType) {
