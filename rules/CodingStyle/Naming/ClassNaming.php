...
final class ClassNaming
{
    ...
    public function getShortName($name) : string
    {
        ...
        $name = \trim($name, '\\');
        return \RectorPrefix20210402\Nette\Utils\Strings::after($name, '\\', -1) ?: $name;
    }
    public function getNamespace(string $fullyQualifiedName) : ?string
    {
        $fullyQualifiedName = \trim($fullyQualifiedName, '\\');
        return \RectorPrefix20210402\Nette\Utils\Strings::before($fullyQualifiedName, '\\', -1) ?: null;
    }
    public function getNameFromFileInfo(\RectorPrefix20210402\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo) : string
    {
        $basenameWithoutSuffix = $smartFileInfo->getBasenameWithoutSuffix();
        // remove PHPUnit fixture file prefix
        if (\Rector\Testing\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            $basenameWithoutSuffix = \RectorPrefix20210402\Nette\Utils\Strings::replace($basenameWithoutSuffix, self::INPUT_HASH_NAMING_REGEX, '');
        }
        $stringy = new \RectorPrefix20210402\Stringy\Stringy($basenameWithoutSuffix);
    ...
    public function createMethodNameFromFunction(\PhpParser\Node\Stmt\Function_ $function) : string
    {
        ...
        $stringy = new \RectorPrefix20210402\Stringy\Stringy($functionName);
        return (string) $stringy->camelize();
    }
    public function replaceSuffix(string $content, string $oldSuffix, string $newSuffix) : string
    {
        if (!\RectorPrefix20210402\Nette\Utils\Strings::endsWith($content, $oldSuffix)) {
            return $content . $newSuffix;
        }
        $contentWithoutOldSuffix = \RectorPrefix20210402\Nette\Utils\Strings::substring($content, 0, -\RectorPrefix20210402\Nette\Utils\Strings::length($oldSuffix));
