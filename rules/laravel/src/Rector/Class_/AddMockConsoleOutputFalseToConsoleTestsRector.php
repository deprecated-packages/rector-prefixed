...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node, '_PhpScoper17db12703726\\Illuminate\\Foundation\\Testing\\TestCase')) {
            return null;
        }
    ...
    private function isTestingConsoleOutput(\PhpParser\Node\Stmt\Class_ $class) : bool
    {
        return (bool) $this->betterNodeFinder->findFirst($class->stmts, function (\PhpParser\Node $node) : bool {
            return $this->isStaticCallNamed($node, '_PhpScoper17db12703726\\Illuminate\\Support\\Facades\\Artisan', 'output');
        });
    }
