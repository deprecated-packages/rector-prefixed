...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node, '_PhpScopera143bcca66cb\\Illuminate\\Foundation\\Testing\\TestCase')) {
            return null;
        }
    ...
    private function isTestingConsoleOutput(\PhpParser\Node\Stmt\Class_ $class) : bool
    {
        return (bool) $this->betterNodeFinder->findFirst((array) $class->stmts, function (\PhpParser\Node $node) : bool {
            return $this->isStaticCallNamed($node, '_PhpScopera143bcca66cb\\Illuminate\\Support\\Facades\\Artisan', 'output');
        });
    }
