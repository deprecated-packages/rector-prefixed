...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node, '_PhpScoper88fe6e0ad041\\Illuminate\\Foundation\\Testing\\TestCase')) {
            return null;
        }
    ...
    private function isTestingConsoleOutput(\PhpParser\Node\Stmt\Class_ $class) : bool
    {
        return (bool) $this->betterNodeFinder->findFirst((array) $class->stmts, function (\PhpParser\Node $node) : bool {
            return $this->isStaticCallNamed($node, '_PhpScoper88fe6e0ad041\\Illuminate\\Support\\Facades\\Artisan', 'output');
        });
    }
