...
final class SensitiveHereNowDocRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Changes heredoc/nowdoc that contains closing word to safe wrapper name', [new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
$value = <<<A
    A
A
CODE_SAMPLE
...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if (!\RectorPrefix20201228\Nette\Utils\Strings::contains($node->value, $docLabel)) {
            return null;
        }
    ...
    private function uniquateDocLabel(string $value, string $docLabel) : string
    {
        ...
        $i = 0;
        while (\RectorPrefix20201228\Nette\Utils\Strings::contains($value, $docLabel)) {
