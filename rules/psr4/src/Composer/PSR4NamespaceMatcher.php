...
final class PSR4NamespaceMatcher implements \Rector\PSR4\Contract\PSR4AutoloadNamespaceMatcherInterface
{
    ...
    public function getExpectedNamespace(\PhpParser\Node $node) : ?string
    {
        ...
        foreach ($psr4Autoloads as $namespace => $path) {
            ...
            foreach ($paths as $singlePath) {
                ...
                if (!\RectorPrefix20210206\Nette\Utils\Strings::startsWith($smartFileInfo->getRelativeDirectoryPath(), $singlePath)) {
                    continue;
                }
     ...
     */
    private function resolveExtraNamespace(\RectorPrefix20210206\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, string $path) : string
    {
        $extraNamespace = \RectorPrefix20210206\Nette\Utils\Strings::substring($smartFileInfo->getRelativeDirectoryPath(), \RectorPrefix20210206\Nette\Utils\Strings::length($path) + 1);
        $extraNamespace = \RectorPrefix20210206\Nette\Utils\Strings::replace($extraNamespace, '#/#', '\\');
