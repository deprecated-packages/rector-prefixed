...
final class PSR4NamespaceMatcher implements \Rector\PSR4\Contract\PSR4AutoloadNamespaceMatcherInterface
{
    ...
    public function getExpectedNamespace(\PhpParser\Node $node) : ?string
    {
        ...
        foreach ($psr4Autoloads as $namespace => $path) {
            ...
            foreach ($paths as $singlePath) {
                ...
                if (!\RectorPrefix20201231\Nette\Utils\Strings::startsWith($smartFileInfo->getRelativeDirectoryPath(), $singlePath)) {
                    continue;
                }
     ...
     */
    private function resolveExtraNamespace(\RectorPrefix20201231\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, string $path) : string
    {
        $extraNamespace = \RectorPrefix20201231\Nette\Utils\Strings::substring($smartFileInfo->getRelativeDirectoryPath(), \RectorPrefix20201231\Nette\Utils\Strings::length($path) + 1);
        $extraNamespace = \RectorPrefix20201231\Nette\Utils\Strings::replace($extraNamespace, '#/#', '\\');
