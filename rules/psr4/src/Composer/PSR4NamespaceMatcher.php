...
final class PSR4NamespaceMatcher implements \Rector\PSR4\Contract\PSR4AutoloadNamespaceMatcherInterface
{
    ...
    public function getExpectedNamespace(\PhpParser\Node $node) : ?string
    {
        ...
        foreach ($psr4Autoloads as $namespace => $path) {
            ...
            foreach ($paths as $singlePath) {
                ...
                if (!\_PhpScoper267b3276efc2\Nette\Utils\Strings::startsWith($smartFileInfo->getRelativeDirectoryPath(), $singlePath)) {
                    continue;
                }
    ...
    {
        $extraNamespace = \_PhpScoper267b3276efc2\Nette\Utils\Strings::substring($smartFileInfo->getRelativeDirectoryPath(), \_PhpScoper267b3276efc2\Nette\Utils\Strings::length($path) + 1);
        $extraNamespace = \_PhpScoper267b3276efc2\Nette\Utils\Strings::replace($extraNamespace, '#/#', '\\');
