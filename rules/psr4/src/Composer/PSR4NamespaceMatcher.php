...
final class PSR4NamespaceMatcher implements \Rector\PSR4\Contract\PSR4AutoloadNamespaceMatcherInterface
{
    ...
    public function getExpectedNamespace(\PhpParser\Node $node) : ?string
    {
        ...
        foreach ($psr4Autoloads as $namespace => $path) {
            ...
            foreach ($paths as $singlePath) {
                ...
                if (!\_PhpScoper567b66d83109\Nette\Utils\Strings::startsWith($smartFileInfo->getRelativeDirectoryPath(), $singlePath)) {
                    continue;
                }
    ...
    {
        $extraNamespace = \_PhpScoper567b66d83109\Nette\Utils\Strings::substring($smartFileInfo->getRelativeDirectoryPath(), \_PhpScoper567b66d83109\Nette\Utils\Strings::length($path) + 1);
        $extraNamespace = \_PhpScoper567b66d83109\Nette\Utils\Strings::replace($extraNamespace, '#/#', '\\');
