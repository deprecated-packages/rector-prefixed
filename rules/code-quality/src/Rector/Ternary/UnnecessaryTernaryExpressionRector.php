 ...
 */
final class UnnecessaryTernaryExpressionRector extends \_PhpScoper0a2ac50786fa\Rector\Core\Rector\AbstractRector
{
    /**
    ...
    {
        return new \_PhpScoper0a2ac50786fa\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Remove unnecessary ternary expressions.', [new \_PhpScoper0a2ac50786fa\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample('$foo === $bar ? true : false;', '$foo === $bar;')]);
    }
    /**
     * @return string[]
     */
    ...
    public function refactor(\_PhpScoper0a2ac50786fa\PhpParser\Node $node) : ?\_PhpScoper0a2ac50786fa\PhpParser\Node
    {
        ...
        if (!$ternaryExpression->if instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr) {
            return null;
        }
        ...
        if (!$condition instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\BinaryOp) {
    ...
    private function processNonBinaryCondition(\_PhpScoper0a2ac50786fa\PhpParser\Node\Expr $ifExpression, \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr $elseExpression, \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr $condition) : ?\_PhpScoper0a2ac50786fa\PhpParser\Node
    {
        if ($this->isTrue($ifExpression) && $this->isFalse($elseExpression)) {
            if ($this->isStaticType($condition, \_PhpScoper0a2ac50786fa\PHPStan\Type\BooleanType::class)) {
                return $condition;
            }
            return new \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Cast\Bool_($condition);
        }
        if ($this->isFalse($ifExpression) && $this->isTrue($elseExpression)) {
            if ($this->isStaticType($condition, \_PhpScoper0a2ac50786fa\PHPStan\Type\BooleanType::class)) {
                return new \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\BooleanNot($condition);
            }
            return new \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\BooleanNot(new \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Cast\Bool_($condition));
        }
        return null;
    }
}
