...
CODE_SAMPLE
    ...
    private function determineDelimiter(\PhpParser\Node\Expr\FuncCall $funcCall) : ?string
    {
        ...
        $possibleLeftDelimiter = \RectorPrefix20210124\Nette\Utils\Strings::substring($leftMostConcatNode->value, 0, 1);
        if (!$rightMostConcatNode instanceof \PhpParser\Node\Scalar\String_) {
            return null;
        }
        $possibleRightDelimiter = \RectorPrefix20210124\Nette\Utils\Strings::substring(\rtrim($rightMostConcatNode->value, self::ALL_MODIFIERS), -1, 1);
