...
namespace _PhpScoper0a6b37af0871\Rector\CodeQuality\Rector\Catch_;
 ...
 */
final class ThrowWithPreviousExceptionRector extends \_PhpScoper0a6b37af0871\Rector\Core\Rector\AbstractRector
{
    /**
     * @var int
     */
    ...
    {
        return new \_PhpScoper0a6b37af0871\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('When throwing into a catch block, checks that the previous exception is passed to the new throw clause', [new \_PhpScoper0a6b37af0871\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
class SomeClass
{
    public function run()
    {
...
CODE_SAMPLE
    ...
    public function refactor(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : ?\_PhpScoper0a6b37af0871\PhpParser\Node
    {
        ...
        $this->traverseNodesWithCallable($node->stmts, function (\_PhpScoper0a6b37af0871\PhpParser\Node $node) use($caughtThrowableVariable) : ?int {
    ...
    private function refactorThrow(\_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Throw_ $throw, \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\Variable $catchedThrowableVariable) : ?int
    {
        if (!$throw->expr instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\New_) {
            return null;
        }
        if (!$throw->expr->class instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Name) {
            return null;
        }
        ...
        if (!isset($throw->expr->args[1])) {
            // get previous code
            $throw->expr->args[1] = new \_PhpScoper0a6b37af0871\PhpParser\Node\Arg(new \_PhpScoper0a6b37af0871\PhpParser\Node\Expr\MethodCall($catchedThrowableVariable, 'getCode'));
        }
        $throw->expr->args[$exceptionArgumentPosition] = new \_PhpScoper0a6b37af0871\PhpParser\Node\Arg($catchedThrowableVariable);
        // nothing more to add
        return \_PhpScoper0a6b37af0871\PhpParser\NodeTraverser::STOP_TRAVERSAL;
    }
    private function resolveExceptionArgumentPosition(\_PhpScoper0a6b37af0871\PhpParser\Node\Name $exceptionName) : ?int
    {
        $fullyQualifiedName = $this->getName($exceptionName);
        // is native exception?
        if (!\_PhpScoper0a6b37af0871\Nette\Utils\Strings::contains($fullyQualifiedName, '\\')) {
        ...
        if (!$reflectionClass->hasMethod(\_PhpScoper0a6b37af0871\Rector\Core\ValueObject\MethodName::CONSTRUCT)) {
            return self::DEFAULT_EXCEPTION_ARGUMENT_POSITION;
        }
