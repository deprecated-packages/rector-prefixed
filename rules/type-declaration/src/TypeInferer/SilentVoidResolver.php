...
final class SilentVoidResolver
{
     ...
     */
    public function hasSilentVoid(\_PhpScopere8e811afab72\PhpParser\Node\FunctionLike $functionLike) : bool
    {
        ...
        foreach ((array) $functionLike->stmts as $stmt) {
            ...
            if ($stmt instanceof \_PhpScopere8e811afab72\PhpParser\Node\Stmt\Switch_ && $this->isSwitchWithAlwaysReturn($stmt)) {
                return \false;
            }
            // is part of try/catch
            if ($stmt instanceof \_PhpScopere8e811afab72\PhpParser\Node\Stmt\TryCatch && $this->isTryCatchAlwaysReturn($stmt)) {
                return \false;
            }
            if ($stmt instanceof \_PhpScopere8e811afab72\PhpParser\Node\Stmt\Throw_) {
                return \false;
            }
        }
        return \true;
    ...
    private function isSwitchWithAlwaysReturn(\_PhpScopere8e811afab72\PhpParser\Node\Stmt\Switch_ $switch) : bool
    {
        $casesWithReturn = 0;
        foreach ($switch->cases as $case) {
            foreach ($case->stmts as $caseStmt) {
                if ($caseStmt instanceof \_PhpScopere8e811afab72\PhpParser\Node\Stmt\Return_) {
                    ++$casesWithReturn;
                    break;
                }
            }
    ...
    private function isTryCatchAlwaysReturn(\_PhpScopere8e811afab72\PhpParser\Node\Stmt\TryCatch $tryCatch) : bool
    {
