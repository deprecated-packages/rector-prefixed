...
namespace _PhpScoper0a6b37af0871\Rector\CodingStyle\Application;
...
use _PhpScoper0a6b37af0871\Rector\CodingStyle\ClassNameImport\UsedImportsResolver;
...
final class UseImportsAdder
{
    ...
    public function __construct(\_PhpScoper0a6b37af0871\Rector\CodingStyle\ClassNameImport\UsedImportsResolver $usedImportsResolver)
    {
    ...
    public function addImportsToStmts(array $stmts, array $useImportTypes, array $functionUseImportTypes) : array
    {
        ...
        foreach ($stmts as $key => $stmt) {
            if ($stmt instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Declare_) {
                if (isset($stmts[$key + 1]) && $stmts[$key + 1] instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Use_) {
                    $nodesToAdd = $newUses;
                } else {
                    // add extra space, if there are no new use imports to be added
                    $nodesToAdd = \array_merge([new \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Nop()], $newUses);
                }
     ...
     * @param FullyQualifiedObjectType[] $functionUseImportTypes
     */
    public function addImportsToNamespace(\_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Namespace_ $namespace, array $useImportTypes, array $functionUseImportTypes) : void
    {
    ...
    private function getNamespaceName(\_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Namespace_ $namespace) : ?string
    {
    ...
    private function isCurrentNamespace(string $namespaceName, \_PhpScoper0a6b37af0871\PHPStan\Type\ObjectType $objectType) : bool
    {
        $afterCurrentNamespace = \_PhpScoper0a6b37af0871\Nette\Utils\Strings::after($objectType->getClassName(), $namespaceName . '\\');
        if (!$afterCurrentNamespace) {
            return \false;
        }
        return !\_PhpScoper0a6b37af0871\Nette\Utils\Strings::contains($afterCurrentNamespace, '\\');
    }
}
