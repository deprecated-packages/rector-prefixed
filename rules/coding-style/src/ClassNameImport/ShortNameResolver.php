...
final class ShortNameResolver
{
    ...
    public function __construct(\_PhpScoper0a6b37af0871\Rector\Core\PhpParser\NodeTraverser\CallableNodeTraverser $callableNodeTraverser, \_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\FileSystem\CurrentFileInfoProvider $currentFileInfoProvider)
    {
     ...
     * @return string[]
     */
    public function resolveShortClassLikeNamesForNode(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : array
    {
        ...
        $shortClassLikeNames = [];
        $this->callableNodeTraverser->traverseNodesWithCallable($namespace, function (\_PhpScoper0a6b37af0871\PhpParser\Node $node) use(&$shortClassLikeNames) {
            // ...
    ...
    private function getNodeRealPath(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : ?string
    {
    ...
    {
        $shortNames = [];
        $this->callableNodeTraverser->traverseNodesWithCallable($stmts, function (\_PhpScoper0a6b37af0871\PhpParser\Node $node) use(&$shortNames) : void {
            // class name is used!
            if ($node instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\ClassLike && $node->name instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Identifier) {
            ...
            $originalName = $node->getAttribute(\_PhpScoper0a6b37af0871\Rector\NodeTypeResolver\Node\AttributeKey::ORIGINAL_NAME);
            if (!$originalName instanceof \_PhpScoper0a6b37af0871\PhpParser\Node\Name) {
                return;
            }
            // already short
            if (\_PhpScoper0a6b37af0871\Nette\Utils\Strings::contains($originalName->toString(), '\\')) {
                return;
            }
    ...
    {
        $shortNames = [];
        $this->callableNodeTraverser->traverseNodesWithCallable($stmts, function (\_PhpScoper0a6b37af0871\PhpParser\Node $node) use(&$shortNames) {
    ...
    private function resolveShortTagNameFromPhpDocChildNode(\_PhpScoper0a6b37af0871\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocChildNode $phpDocChildNode) : ?string
    {
        ...
        if (\_PhpScoper0a6b37af0871\Nette\Utils\Strings::match($tagName, self::BIG_LETTER_START_REGEX)) {
            return $tagName;
        }
        ...
        if (\_PhpScoper0a6b37af0871\Nette\Utils\Strings::contains($typeNode->name, '\\')) {
            return null;
        }
