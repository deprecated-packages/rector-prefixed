...
final class ShortNameResolver
{
     ...
     */
    private $classNaming;
    public function __construct(\RectorPrefix20210210\Symplify\Astral\NodeTraverser\SimpleCallableNodeTraverser $simpleCallableNodeTraverser, \Rector\NodeTypeResolver\FileSystem\CurrentFileInfoProvider $currentFileInfoProvider, \Rector\BetterPhpDocParser\PhpDocInfo\PhpDocInfoFactory $phpDocInfoFactory, \Rector\CodingStyle\Naming\ClassNaming $classNaming)
    {
    ...
    private function resolveForStmts(array $stmts) : array
    {
        ...
        $this->simpleCallableNodeTraverser->traverseNodesWithCallable($stmts, function (\PhpParser\Node $node) use(&$shortNames) : void {
            ...
            // already short
            if (\RectorPrefix20210210\Nette\Utils\Strings::contains($originalName->toString(), '\\')) {
                return;
            }
    ...
    private function resolveShortTagNameFromPhpDocChildNode(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocChildNode $phpDocChildNode) : ?string
    {
        ...
        if (\RectorPrefix20210210\Nette\Utils\Strings::match($tagName, self::BIG_LETTER_START_REGEX)) {
            return $tagName;
        }
        ...
        if (\RectorPrefix20210210\Nette\Utils\Strings::contains($typeNode->name, '\\')) {
            return null;
        }
