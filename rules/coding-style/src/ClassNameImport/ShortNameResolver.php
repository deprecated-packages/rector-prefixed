...
use RectorPrefix20210128\Symplify\Astral\NodeTraverser\SimpleCallableNodeTraverser;
...
final class ShortNameResolver
{
    ...
    public function __construct(\RectorPrefix20210128\Symplify\Astral\NodeTraverser\SimpleCallableNodeTraverser $simpleCallableNodeTraverser, \Rector\NodeTypeResolver\FileSystem\CurrentFileInfoProvider $currentFileInfoProvider, \Rector\BetterPhpDocParser\PhpDocInfo\PhpDocInfoFactory $phpDocInfoFactory)
    {
    ...
    private function resolveForStmts(array $stmts) : array
    {
        ...
        $this->simpleCallableNodeTraverser->traverseNodesWithCallable($stmts, function (\PhpParser\Node $node) use(&$shortNames) : void {
            ...
            // already short
            if (\RectorPrefix20210128\Nette\Utils\Strings::contains($originalName->toString(), '\\')) {
                return;
            }
    ...
    private function resolveShortTagNameFromPhpDocChildNode(\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocChildNode $phpDocChildNode) : ?string
    {
        ...
        if (\RectorPrefix20210128\Nette\Utils\Strings::match($tagName, self::BIG_LETTER_START_REGEX)) {
            return $tagName;
        }
        ...
        if (\RectorPrefix20210128\Nette\Utils\Strings::contains($typeNode->name, '\\')) {
            return null;
        }
