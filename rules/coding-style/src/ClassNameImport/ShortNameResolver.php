...
final class ShortNameResolver
{
    ...
    public function __construct(\_PhpScoper2a4e7ab1ecbc\Rector\Core\PhpParser\NodeTraverser\CallableNodeTraverser $callableNodeTraverser, \_PhpScoper2a4e7ab1ecbc\Rector\NodeTypeResolver\FileSystem\CurrentFileInfoProvider $currentFileInfoProvider)
    {
     ...
     * @return string[]
     */
    public function resolveShortClassLikeNamesForNode(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) : array
    {
        ...
        $shortClassLikeNames = [];
        $this->callableNodeTraverser->traverseNodesWithCallable($namespace, function (\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) use(&$shortClassLikeNames) {
            // ...
    ...
    private function getNodeRealPath(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) : ?string
    {
    ...
    {
        $shortNames = [];
        $this->callableNodeTraverser->traverseNodesWithCallable($stmts, function (\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) use(&$shortNames) : void {
            // class name is used!
            if ($node instanceof \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\ClassLike && $node->name instanceof \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Identifier) {
            ...
            $originalName = $node->getAttribute(\_PhpScoper2a4e7ab1ecbc\Rector\NodeTypeResolver\Node\AttributeKey::ORIGINAL_NAME);
            if (!$originalName instanceof \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Name) {
                return;
            }
            // already short
            if (\_PhpScoper2a4e7ab1ecbc\Nette\Utils\Strings::contains($originalName->toString(), '\\')) {
                return;
            }
    ...
    {
        $shortNames = [];
        $this->callableNodeTraverser->traverseNodesWithCallable($stmts, function (\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) use(&$shortNames) {
    ...
    private function resolveShortTagNameFromPhpDocChildNode(\_PhpScoper2a4e7ab1ecbc\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocChildNode $phpDocChildNode) : ?string
    {
        ...
        if (\_PhpScoper2a4e7ab1ecbc\Nette\Utils\Strings::match($tagName, self::BIG_LETTER_START_REGEX)) {
            return $tagName;
        }
        ...
        if (\_PhpScoper2a4e7ab1ecbc\Nette\Utils\Strings::contains($typeNode->name, '\\')) {
            return null;
        }
