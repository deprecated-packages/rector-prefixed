...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if (\_PhpScopera143bcca66cb\Nette\Utils\Strings::match($docCommentText, $returnRegex)) {
            $docCommentText = \_PhpScopera143bcca66cb\Nette\Utils\Strings::replace($docCommentText, $returnRegex, '');
        }
        return $this->processUpdateDocblock($node, $text, $docCommentText);
    }
    ...
    private function processCleanDocCommentParam(array $params, string $key, \PHPStan\Type\Type $type, string $docCommentText) : string
    {
        foreach ($params as $param) {
            ...
            if (\_PhpScopera143bcca66cb\Nette\Utils\Strings::match($docCommentText, $paramRegex)) {
                $docCommentText = \_PhpScopera143bcca66cb\Nette\Utils\Strings::replace($docCommentText, $paramRegex, '');
            }
        }
        return $docCommentText;
    }
