...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if (\_PhpScoper006a73f0e455\Nette\Utils\Strings::match($docCommentText, $returnRegex)) {
            $docCommentText = \_PhpScoper006a73f0e455\Nette\Utils\Strings::replace($docCommentText, $returnRegex, '');
        }
        return $this->processUpdateDocblock($node, $text, $docCommentText);
    }
    ...
    private function processCleanDocCommentParam(array $params, string $key, \PHPStan\Type\Type $type, string $docCommentText) : string
    {
        foreach ($params as $param) {
            ...
            if (\_PhpScoper006a73f0e455\Nette\Utils\Strings::match($docCommentText, $paramRegex)) {
                $docCommentText = \_PhpScoper006a73f0e455\Nette\Utils\Strings::replace($docCommentText, $paramRegex, '');
            }
        }
        return $docCommentText;
    }
