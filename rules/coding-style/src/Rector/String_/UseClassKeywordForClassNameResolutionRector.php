...
final class UseClassKeywordForClassNameResolutionRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Use `class` keyword for class name resolution in string instead of hardcoded string reference', [new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    {
        /** @var mixed[] $matches */
        $matches = \RectorPrefix20201228\Nette\Utils\Strings::matchAll($string->value, self::CLASS_BEFORE_STATIC_ACCESS_REGEX, \PREG_PATTERN_ORDER);
    ...
    public function getParts(\PhpParser\Node\Scalar\String_ $string, array $classNames) : array
    {
        ...
        $parts = \RectorPrefix20201228\Nette\Utils\Strings::split($string->value, '#(' . \implode('|', $classNames) . ')#');
