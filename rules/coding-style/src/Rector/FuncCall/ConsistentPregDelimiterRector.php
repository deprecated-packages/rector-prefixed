...
final class ConsistentPregDelimiterRector extends \Rector\Core\Rector\AbstractRector implements \Rector\Core\Contract\Rector\ConfigurableRectorInterface
{
     ...
     */
    private const STATIC_METHODS_WITH_REGEX_PATTERN = ['_PhpScoperf18a0c41e2d2\\Nette\\Utils\\Strings' => ['match' => 1, 'matchAll' => 1, 'replace' => 1, 'split' => 1]];
    /**
     * @var string
     */
    private $delimiter = '#';
...
, [self::DELIMITER => '#'])]);
    ...
    private function refactorArgument(\PhpParser\Node\Arg $arg) : void
    {
        ...
        $string = $arg->value;
        $value = $string->value;
        $string->value = \_PhpScoperf18a0c41e2d2\Nette\Utils\Strings::replace($value, self::INNER_REGEX, function (array $match) : string {
