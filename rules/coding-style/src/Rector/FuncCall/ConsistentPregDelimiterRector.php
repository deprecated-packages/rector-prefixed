...
final class ConsistentPregDelimiterRector extends \Rector\Core\Rector\AbstractRector implements \Rector\Core\Contract\Rector\ConfigurableRectorInterface
{
     ...
     */
    private const STATIC_METHODS_WITH_REGEX_PATTERN = ['RectorPrefix2020DecSat\\Nette\\Utils\\Strings' => ['match' => 1, 'matchAll' => 1, 'replace' => 1, 'split' => 1]];
    /**
     * @var string
     */
    private $delimiter = '#';
...
, [self::DELIMITER => '#'])]);
    ...
    private function refactorArgument(\PhpParser\Node\Arg $arg) : void
    {
        ...
        $string = $arg->value;
        $value = $string->value;
        $string->value = \RectorPrefix2020DecSat\Nette\Utils\Strings::replace($value, self::INNER_REGEX, function (array $match) : string {
