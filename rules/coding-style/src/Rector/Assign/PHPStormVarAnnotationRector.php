...
final class PHPStormVarAnnotationRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Change various @var annotation formats to one PHPStorm understands', [new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
$config = 5;
...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if (!\RectorPrefix20201228\Nette\Utils\Strings::contains($docContent, '@var')) {
            return null;
        }
        ...
        if (!\RectorPrefix20201228\Nette\Utils\Strings::match($docContent, $varPattern)) {
            return null;
        }
        // switch docs
    ...
    private function createDocComment(\PhpParser\Node $node) : \PhpParser\Comment\Doc
    {
        ...
        if (\RectorPrefix20201228\Nette\Utils\Strings::startsWith($docContent, '/* ')) {
            $docContent = \RectorPrefix20201228\Nette\Utils\Strings::replace($docContent, self::SINGLE_ASTERISK_COMMENT_START_REGEX, '/** ');
        }
        // $value is first, instead of type is first
        if (\RectorPrefix20201228\Nette\Utils\Strings::match($docContent, self::VAR_ANNOTATION_REGEX)) {
            $docContent = \RectorPrefix20201228\Nette\Utils\Strings::replace($docContent, self::VARIABLE_NAME_AND_TYPE_MATCH_REGEX, '$3$2$1');
        }
