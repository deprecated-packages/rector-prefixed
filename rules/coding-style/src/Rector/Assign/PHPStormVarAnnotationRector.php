...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if (!\RectorPrefix20210216\Nette\Utils\Strings::contains($docContent, '@var')) {
            return null;
        }
        ...
        if (!\RectorPrefix20210216\Nette\Utils\Strings::match($docContent, $varPattern)) {
            return null;
        }
        // switch docs
    ...
    private function createDocComment(\PhpParser\Node $node) : \PhpParser\Comment\Doc
    {
        ...
        if (\RectorPrefix20210216\Nette\Utils\Strings::startsWith($docContent, '/* ')) {
            $docContent = \RectorPrefix20210216\Nette\Utils\Strings::replace($docContent, self::SINGLE_ASTERISK_COMMENT_START_REGEX, '/** ');
        }
        // $value is first, instead of type is first
        if (\RectorPrefix20210216\Nette\Utils\Strings::match($docContent, self::VAR_ANNOTATION_REGEX)) {
            $docContent = \RectorPrefix20210216\Nette\Utils\Strings::replace($docContent, self::VARIABLE_NAME_AND_TYPE_MATCH_REGEX, '$3$2$1');
        }
