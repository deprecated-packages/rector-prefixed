...
CODE_SAMPLE
    ...
    {
        if (!(bool) \_PhpScoper8b9c402c5f32\Nette\Utils\Strings::match($stringValue, self::JSON_STRING_REGEX)) {
            return \false;
        }
        try {
            return (bool) \_PhpScoper8b9c402c5f32\Nette\Utils\Json::decode($stringValue, \_PhpScoper8b9c402c5f32\Nette\Utils\Json::FORCE_ARRAY);
    ...
    {
        $stringValue = \_PhpScoper8b9c402c5f32\Nette\Utils\Strings::replace($stringValue, self::UNQUOTED_OBJECT_HASH_REGEX, '$1"$2"');
    ...
    {
        $array = \_PhpScoper8b9c402c5f32\Nette\Utils\Json::decode($stringValue, \_PhpScoper8b9c402c5f32\Nette\Utils\Json::FORCE_ARRAY);
    ...
    private function createAndReturnJsonEncodeFromArray(\PhpParser\Node\Expr\Assign $assign, \PhpParser\Node\Expr\Array_ $jsonArray) : \PhpParser\Node\Expr\Assign
    {
        ...
        $assign->expr = $this->createStaticCall('_PhpScoper8b9c402c5f32\\Nette\\Utils\\Json', 'encode', [$jsonDataVariable]);
        return $assign;
    }
    /**
