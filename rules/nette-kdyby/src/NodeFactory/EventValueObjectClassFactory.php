...
final class EventValueObjectClassFactory
{
     ...
     */
    private $nodeFactory;
    public function __construct(\Rector\CodingStyle\Naming\ClassNaming $classNaming, \Rector\Core\PhpParser\Node\NodeFactory $nodeFactory, \Rector\NetteKdyby\BlueprintFactory\VariableWithTypesFactory $variableWithTypesFactory)
    {
    ...
    private function createEventClassBuilder(string $className) : \Rector\Core\PhpParser\Builder\ClassBuilder
    {
        ...
        $classBuilder->extend(new \PhpParser\Node\Name\FullyQualified('_PhpScoperabd03f0baf05\\Symfony\\Contracts\\EventDispatcher\\Event'));
        return $classBuilder;
    }
    /**
     * @param Arg[] $args
    ...
    {
        $namespace = \_PhpScoperabd03f0baf05\Nette\Utils\Strings::before($className, '\\', -1);
