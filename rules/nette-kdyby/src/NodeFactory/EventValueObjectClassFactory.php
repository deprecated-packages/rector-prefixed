...
final class EventValueObjectClassFactory
{
    ...
    private function createEventClassBuilder(string $className) : \RectorPrefix20210118\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder
    {
        $shortClassName = $this->classNaming->getShortName($className);
        $classBuilder = new \RectorPrefix20210118\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder($shortClassName);
     ...
     * @param Arg[] $args
     */
    private function decorateWithConstructorIfHasArgs(\RectorPrefix20210118\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder $classBuilder, array $args) : void
    {
        if ($args === []) {
            return;
        }
    ...
    {
        $namespace = \RectorPrefix20210118\Nette\Utils\Strings::before($className, '\\', -1);
        $namespaceBuilder = new \RectorPrefix20210118\Symplify\Astral\ValueObject\NodeBuilder\NamespaceBuilder($namespace);
     ...
     */
    private function ensureVariablesAreUnique(array $variablesWithTypes, \RectorPrefix20210118\Symplify\Astral\ValueObject\NodeBuilder\ClassBuilder $classBuilder) : void
    {
        $usedVariableNames = [];
    ...
    {
        $methodBuilder = new \RectorPrefix20210118\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder(\Rector\Core\ValueObject\MethodName::CONSTRUCT);
