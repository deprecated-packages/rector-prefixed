...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if (!\RectorPrefix20210305\Nette\Utils\Strings::endsWith($oldRealPath, '.phpt')) {
            return null;
        }
    ...
    {
        // file suffix
        $newRealPath = \RectorPrefix20210305\Nette\Utils\Strings::replace($oldRealPath, self::PHPT_SUFFIX_REGEX, '.php');
        // Test suffix
        if (!\RectorPrefix20210305\Nette\Utils\Strings::endsWith($newRealPath, 'Test.php')) {
            return \RectorPrefix20210305\Nette\Utils\Strings::replace($newRealPath, self::PHP_SUFFIX_REGEX, 'Test.php');
        }
        return $newRealPath;
    }
}
