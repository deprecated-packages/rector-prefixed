...
final class BreakingVariableRenameGuard
{
    ...
    public function shouldSkipVariable(string $currentName, string $expectedName, \PhpParser\Node\FunctionLike $functionLike, \PhpParser\Node\Expr\Variable $variable) : bool
    {
        ...
        if (\_PhpScoper006a73f0e455\Nette\Utils\Strings::endsWith($currentName, \ucfirst($expectedName))) {
            return \true;
        }
    ...
    {
        return $this->nodeTypeResolver->isObjectType($node, \_PhpScoper006a73f0e455\Ramsey\Uuid\UuidInterface::class);
    }
    /**
     * @TODO Remove once ParamRenamer created
     * @param Param|Property $node
    ...
    private function isDateTimeAtNamingConvention(\PhpParser\Node $node) : bool
    {
        ...
        return (bool) \_PhpScoper006a73f0e455\Nette\Utils\Strings::match($currentName, self::AT_NAMING_REGEX . '');
    }
}
