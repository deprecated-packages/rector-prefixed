...
use RectorPrefix20210501\Webmozart\Assert\Assert;
/**
...
, [self::GROUP_NAMES_BY_SUFFIX => ['Repository']])]);
    ...
    public function configure(array $configuration) : void
    {
        ...
        \RectorPrefix20210501\Webmozart\Assert\Assert::allString($groupNamesBySuffix);
     ...
     */
    private function processGroupNamesBySuffix(\RectorPrefix20210501\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, \Rector\Core\ValueObject\Application\File $file, array $groupNamesBySuffix) : void
    {
        foreach ($groupNamesBySuffix as $groupNames) {
            // has class suffix
            $suffixPattern = '\\w+' . $groupNames . '(Test)?\\.php$';
            if (!\RectorPrefix20210501\Nette\Utils\Strings::match($smartFileInfo->getRealPath(), '#' . $suffixPattern . '#')) {
                continue;
            }
            ...
            if (\RectorPrefix20210501\Nette\Utils\Strings::match($smartFileInfo->getPath(), '#' . $groupNames . '$#')) {
                continue;
            }
    ...
    private function isLocatedInExpectedLocation(string $groupName, string $suffixPattern, \RectorPrefix20210501\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo) : bool
    {
        $expectedLocationFilePattern = $this->expectedFileLocationResolver->resolve($groupName, $suffixPattern);
        return (bool) \RectorPrefix20210501\Nette\Utils\Strings::match($smartFileInfo->getRealPath(), $expectedLocationFilePattern);
    }
    private function moveFileToGroupName(\RectorPrefix20210501\Symplify\SmartFileSystem\SmartFileInfo $fileInfo, \Rector\Core\ValueObject\Application\File $file, string $desiredGroupName) : void
    {
