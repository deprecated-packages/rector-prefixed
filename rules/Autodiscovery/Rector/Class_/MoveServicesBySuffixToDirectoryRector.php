...
, [self::GROUP_NAMES_BY_SUFFIX => ['Repository']])]);
    ...
    public function configure(array $configuration)
    {
        ...
        \RectorPrefix20210422\Webmozart\Assert\Assert::allString($groupNamesBySuffix);
     ...
     * @return void
     */
    private function processGroupNamesBySuffix(\RectorPrefix20210422\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, \Rector\Core\ValueObject\Application\File $file, array $groupNamesBySuffix)
    {
        foreach ($groupNamesBySuffix as $groupNames) {
            // has class suffix
            $suffixPattern = '\\w+' . $groupNames . '(Test)?\\.php$';
            if (!\RectorPrefix20210422\Nette\Utils\Strings::match($smartFileInfo->getRealPath(), '#' . $suffixPattern . '#')) {
                continue;
            }
            ...
            if (\RectorPrefix20210422\Nette\Utils\Strings::match($smartFileInfo->getPath(), '#' . $groupNames . '$#')) {
                continue;
            }
    ...
    private function isLocatedInExpectedLocation(string $groupName, string $suffixPattern, \RectorPrefix20210422\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo) : bool
    {
        $expectedLocationFilePattern = $this->expectedFileLocationResolver->resolve($groupName, $suffixPattern);
        return (bool) \RectorPrefix20210422\Nette\Utils\Strings::match($smartFileInfo->getRealPath(), $expectedLocationFilePattern);
    }
    /**
     * @return void
     */
    private function moveFileToGroupName(\RectorPrefix20210422\Symplify\SmartFileSystem\SmartFileInfo $fileInfo, \Rector\Core\ValueObject\Application\File $file, string $desiredGroupName)
    {
