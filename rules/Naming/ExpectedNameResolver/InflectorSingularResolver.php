...
final class InflectorSingularResolver
{
     ...
     */
    private $inflector;
    public function __construct(\RectorPrefix20210409\Doctrine\Inflector\Inflector $inflector)
    {
        $this->inflector = $inflector;
    }
    public function resolve(string $currentName) : string
    {
        $matchBy = \RectorPrefix20210409\Nette\Utils\Strings::match($currentName, self::BY_MIDDLE_REGEX);
        if ($matchBy) {
            return \RectorPrefix20210409\Nette\Utils\Strings::substring($currentName, 0, -\strlen($matchBy['by']));
        }
        ...
        $camelCases = \RectorPrefix20210409\Nette\Utils\Strings::matchAll($currentName, self::CAMELCASE_REGEX);
