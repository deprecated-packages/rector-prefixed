...
final class PropertyNaming
{
    ...
    {
        $matches = \RectorPrefix20210414\Nette\Utils\Strings::match($methodName, self::GET_PREFIX_REGEX);
        if ($matches === null) {
            return null;
        }
    ...
    public function getExpectedNameFromType(\PHPStan\Type\Type $type) : ?\Rector\Naming\ValueObject\ExpectedName
    {
        ...
        foreach (self::EXCLUDED_CLASSES as $excludedClass) {
            if (\RectorPrefix20210414\Nette\Utils\Strings::match($className, $excludedClass)) {
                return null;
            }
        }
        ...
        // remove "_"
        $shortClassName = \RectorPrefix20210414\Nette\Utils\Strings::replace($shortClassName, '#_#', '');
    ...
    private function resolveShortClassName(string $className) : string
    {
        if (\RectorPrefix20210414\Nette\Utils\Strings::contains($className, '\\')) {
            return \RectorPrefix20210414\Nette\Utils\Strings::after($className, '\\', -1);
        }
        return $className;
    }
    ...
    {
        // is SomeInterface
        if (\RectorPrefix20210414\Nette\Utils\Strings::endsWith($shortClassName, self::INTERFACE)) {
            $shortClassName = \RectorPrefix20210414\Nette\Utils\Strings::substring($shortClassName, 0, -\strlen(self::INTERFACE));
        }
        // is ISomeClass
        if ($this->isPrefixedInterface($shortClassName)) {
            $shortClassName = \RectorPrefix20210414\Nette\Utils\Strings::substring($shortClassName, 1);
        }
        // is AbstractClass
        if (\RectorPrefix20210414\Nette\Utils\Strings::startsWith($shortClassName, 'Abstract')) {
            $shortClassName = \RectorPrefix20210414\Nette\Utils\Strings::substring($shortClassName, \strlen('Abstract'));
        }
    ...
    private function prolongIfTooShort(string $shortClassName, string $className) : string
    {
        if (\in_array($shortClassName, ['Factory', 'Repository'], \true)) {
            $namespaceAbove = (string) \RectorPrefix20210414\Nette\Utils\Strings::after($className, '\\', -2);
            $namespaceAbove = (string) \RectorPrefix20210414\Nette\Utils\Strings::before($namespaceAbove, '\\');
    ...
    {
        if (!\RectorPrefix20210414\Nette\Utils\Strings::contains($fqn, '\\')) {
            return $fqn;
        }
        /** @var string $lastNamePart */
        $lastNamePart = \RectorPrefix20210414\Nette\Utils\Strings::after($fqn, '\\', -1);
        if (\RectorPrefix20210414\Nette\Utils\Strings::endsWith($lastNamePart, self::INTERFACE)) {
            return \RectorPrefix20210414\Nette\Utils\Strings::substring($lastNamePart, 0, -\strlen(self::INTERFACE));
        }
        return $lastNamePart;
    }
    private function removeInterfaceSuffixPrefix(string $className, string $shortName) : string
    {
        ...
        // starts with "I\W+"?
        if (\RectorPrefix20210414\Nette\Utils\Strings::match($shortName, self::I_PREFIX_REGEX)) {
            return \RectorPrefix20210414\Nette\Utils\Strings::substring($shortName, 1);
        }
        if (\RectorPrefix20210414\Nette\Utils\Strings::endsWith($shortName, self::INTERFACE)) {
            return \RectorPrefix20210414\Nette\Utils\Strings::substring($shortName, -\strlen(self::INTERFACE));
        }
        return $shortName;
    }
    /**
    ...
    private function isPrefixedInterface(string $shortClassName) : bool
    {
        ...
        if (!\RectorPrefix20210414\Nette\Utils\Strings::startsWith($shortClassName, 'I')) {
            return \false;
        }
    ...
    private function isBoolishMethodName(\PhpParser\Node\Stmt\ClassMethod $classMethod) : bool
    {
        ...
        return (bool) \RectorPrefix20210414\Nette\Utils\Strings::match($classMethodName, self::PREFIXED_CLASS_METHODS_REGEX);
    }
