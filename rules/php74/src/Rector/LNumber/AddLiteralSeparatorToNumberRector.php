...
final class AddLiteralSeparatorToNumberRector extends \Rector\Core\Rector\AbstractRector implements \Rector\Core\Contract\Rector\ConfigurableRectorInterface
{
    ...
    public function configure(array $configuration) : void
    {
        ...
        \RectorPrefix20210207\Webmozart\Assert\Assert::integer($limitValue);
...
, [self::LIMIT_VALUE => 1000000])]);
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if (\RectorPrefix20210207\Nette\Utils\Strings::contains($numericValueAsString, '.')) {
    ...
    private function shouldSkip(\PhpParser\Node $node, string $numericValueAsString) : bool
    {
        ...
        // already separated
        if (\RectorPrefix20210207\Nette\Utils\Strings::contains($numericValueAsString, '_')) {
            return \true;
        }
        ...
        // e+/e-
        if (\RectorPrefix20210207\Nette\Utils\Strings::match($numericValueAsString, '#e#i')) {
            return \true;
        }
        // too short
        return \RectorPrefix20210207\Nette\Utils\Strings::length($numericValueAsString) <= self::GROUP_SIZE;
    }
    /**
     * @return string[]
     */
