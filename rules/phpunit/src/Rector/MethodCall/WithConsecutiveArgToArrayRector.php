 ...
 */
final class WithConsecutiveArgToArrayRector extends \_PhpScopere8e811afab72\Rector\Core\Rector\AbstractRector
{
    /**
    ...
    {
        return new \_PhpScopere8e811afab72\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Split withConsecutive() arg to array', [new \_PhpScopere8e811afab72\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
class SomeClass
{
...
CODE_SAMPLE
    ...
    public function refactor(\_PhpScopere8e811afab72\PhpParser\Node $node) : ?\_PhpScopere8e811afab72\PhpParser\Node
    {
        ...
        // is a mock?
        if (!$this->isObjectType($node, '_PhpScopere8e811afab72\\PHPUnit\\Framework\\MockObject\\Builder\\InvocationMocker')) {
            return null;
        }
        ...
        foreach ($valueChunks as $valueChunk) {
            $node->args[] = new \_PhpScopere8e811afab72\PhpParser\Node\Arg($this->createArray($valueChunk));
        }
        return $node;
    }
    private function areAllArgArrayTypes(\_PhpScopere8e811afab72\PhpParser\Node\Expr\MethodCall $methodCall) : bool
    {
        foreach ($methodCall->args as $arg) {
            $argumentStaticType = $this->getStaticType($arg->value);
            if ($argumentStaticType instanceof \_PhpScopere8e811afab72\PHPStan\Type\ArrayType) {
                continue;
            }
            return \false;
        }
    ...
    private function inferMockedClassName(\_PhpScopere8e811afab72\PhpParser\Node\Expr\MethodCall $methodCall) : ?string
    {
        ...
        if ($assignToVariable->expr instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\MethodCall) {
    ...
    private function inferMockedMethodName(\_PhpScopere8e811afab72\PhpParser\Node\Expr\MethodCall $methodCall) : string
    {
    ...
    private function findRootVariableOfChainCall(\_PhpScopere8e811afab72\PhpParser\Node\Expr\MethodCall $methodCall) : ?\_PhpScopere8e811afab72\PhpParser\Node\Expr\Variable
    {
        $currentMethodCallee = $methodCall->var;
        while (!$currentMethodCallee instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\Variable) {
