...
final class RemoveDataProviderTestPrefixRector extends \Rector\Core\Rector\AbstractPHPUnitRector
{
    ...
    {
        return new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Data provider methods cannot start with "test" prefix', [new \RectorPrefix20201228\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    private function renameDataProviderAnnotationsAndCollectRenamedMethods(\PhpParser\Node\Stmt\Class_ $class) : void
    {
        foreach ($class->getMethods() as $classMethod) {
            ...
            foreach ($dataProviderTagValueNodes as $dataProviderTagValueNode) {
                ...
                if (!\RectorPrefix20201228\Nette\Utils\Strings::startsWith($oldMethodName, 'test')) {
                    continue;
                }
    ...
    {
        $newMethodName = \RectorPrefix20201228\Nette\Utils\Strings::substring($oldMethodName, \strlen('test'));
