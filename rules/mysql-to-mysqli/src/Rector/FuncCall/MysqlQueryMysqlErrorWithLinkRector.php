 ...
 */
final class MysqlQueryMysqlErrorWithLinkRector extends \_PhpScoper0a2ac50786fa\Rector\Core\Rector\AbstractRector
{
    /**
    ...
    {
        return new \_PhpScoper0a2ac50786fa\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Add mysql_query and mysql_error with connection', [new \_PhpScoper0a2ac50786fa\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
class SomeClass
{
    public function run()
    {
...
CODE_SAMPLE
    ...
    public function refactor(\_PhpScoper0a2ac50786fa\PhpParser\Node $node) : ?\_PhpScoper0a2ac50786fa\PhpParser\Node
    {
        foreach (self::FUNCTION_RENAME_MAP as $oldFunction => $newFunction) {
            ...
            if ($node->args === [] || !$this->isProbablyMysql($node->args[0]->value)) {
                ...
                $node->args = \array_merge([new \_PhpScoper0a2ac50786fa\PhpParser\Node\Arg($connectionVariable)], $node->args);
            }
            $node->name = new \_PhpScoper0a2ac50786fa\PhpParser\Node\Name($newFunction);
            return $node;
        }
        return null;
    }
    private function isProbablyMysql(\_PhpScoper0a2ac50786fa\PhpParser\Node\Expr $expr) : bool
    {
        if ($this->isObjectType($expr, 'mysqli')) {
            return \true;
        }
        $staticType = $this->getStaticType($expr);
        $resourceType = new \_PhpScoper0a2ac50786fa\PHPStan\Type\ResourceType();
    ...
    private function findConnectionVariable(\_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\FuncCall $funcCall) : ?\_PhpScoper0a2ac50786fa\PhpParser\Node\Expr
    {
        $connectionAssign = $this->betterNodeFinder->findFirstPrevious($funcCall, function (\_PhpScoper0a2ac50786fa\PhpParser\Node $node) : ?bool {
            if (!$node instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Assign) {
                return null;
            }
            return $this->isObjectType($node->expr, 'mysqli');
        });
        if (!$connectionAssign instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Assign) {
            return null;
        }
        return $connectionAssign->var;
    }
    private function removeExistingConnectionParameter(\_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\FuncCall $funcCall) : void
    {
    ...
    private function isUnionTypeWithResourceSubType(\_PhpScoper0a2ac50786fa\PHPStan\Type\Type $staticType, \_PhpScoper0a2ac50786fa\PHPStan\Type\ResourceType $resourceType) : bool
    {
