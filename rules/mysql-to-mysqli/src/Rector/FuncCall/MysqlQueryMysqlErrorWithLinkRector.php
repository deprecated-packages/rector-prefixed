 ...
 */
final class MysqlQueryMysqlErrorWithLinkRector extends \_PhpScopere8e811afab72\Rector\Core\Rector\AbstractRector
{
    /**
    ...
    {
        return new \_PhpScopere8e811afab72\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Add mysql_query and mysql_error with connection', [new \_PhpScopere8e811afab72\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
class SomeClass
{
    public function run()
    {
...
CODE_SAMPLE
    ...
    public function refactor(\_PhpScopere8e811afab72\PhpParser\Node $node) : ?\_PhpScopere8e811afab72\PhpParser\Node
    {
        foreach (self::FUNCTION_RENAME_MAP as $oldFunction => $newFunction) {
            ...
            if ($node->args === [] || !$this->isProbablyMysql($node->args[0]->value)) {
                ...
                $node->args = \array_merge([new \_PhpScopere8e811afab72\PhpParser\Node\Arg($connectionVariable)], $node->args);
            }
            $node->name = new \_PhpScopere8e811afab72\PhpParser\Node\Name($newFunction);
            return $node;
        }
        return null;
    }
    private function isProbablyMysql(\_PhpScopere8e811afab72\PhpParser\Node\Expr $expr) : bool
    {
        if ($this->isObjectType($expr, 'mysqli')) {
            return \true;
        }
        $staticType = $this->getStaticType($expr);
        $resourceType = new \_PhpScopere8e811afab72\PHPStan\Type\ResourceType();
    ...
    private function findConnectionVariable(\_PhpScopere8e811afab72\PhpParser\Node\Expr\FuncCall $funcCall) : ?\_PhpScopere8e811afab72\PhpParser\Node\Expr
    {
        $connectionAssign = $this->betterNodeFinder->findFirstPrevious($funcCall, function (\_PhpScopere8e811afab72\PhpParser\Node $node) : ?bool {
            if (!$node instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\Assign) {
                return null;
            }
            return $this->isObjectType($node->expr, 'mysqli');
        });
        if (!$connectionAssign instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\Assign) {
            return null;
        }
        return $connectionAssign->var;
    }
    private function removeExistingConnectionParameter(\_PhpScopere8e811afab72\PhpParser\Node\Expr\FuncCall $funcCall) : void
    {
    ...
    private function isUnionTypeWithResourceSubType(\_PhpScopere8e811afab72\PHPStan\Type\Type $staticType, \_PhpScopere8e811afab72\PHPStan\Type\ResourceType $resourceType) : bool
    {
