...
CODE_SAMPLE
    ...
    public function refactor($node) : ?\PhpParser\Node
    {
        ...
        // skip phar
        if (\RectorPrefix20210317\Nette\Utils\Strings::startsWith($includeValue, 'phar://')) {
            return null;
        }
        // skip absolute paths
        if (\RectorPrefix20210317\Nette\Utils\Strings::startsWith($includeValue, '/')) {
            return null;
        }
        // add preslash to string
        if (\RectorPrefix20210317\Nette\Utils\Strings::startsWith($includeValue, './')) {
            $node->expr->value = \RectorPrefix20210317\Nette\Utils\Strings::substring($includeValue, 1);
        } else {
