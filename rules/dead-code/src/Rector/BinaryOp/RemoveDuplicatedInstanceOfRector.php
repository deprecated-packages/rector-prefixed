...
CODE_SAMPLE
    ...
    private function resolveDuplicatedInstancesOf(\PhpParser\Node $node) : void
    {
        $this->duplicatedInstanceOfs = [];
        /** @var Instanceof_[] $instanceOfs */
        $instanceOfs = $this->betterNodeFinder->findInstanceOf([$node], \PhpParser\Node\Expr\Instanceof_::class);
        $instanceOfsByClass = [];
