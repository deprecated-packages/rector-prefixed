...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($methodCall, '_PhpScoper5edc98a7cce2\\Symfony\\Component\\PropertyInfo\\Extractor\\ReflectionExtractor')) {
            return \true;
        }
    ...
    {
        $classConstFetch = $this->createClassConstFetch('_PhpScoper5edc98a7cce2\\Symfony\\Component\\PropertyInfo\\Extractor\\ReflectionExtractor', 'MAGIC_GET');
        $magicSet = $this->createClassConstFetch('_PhpScoper5edc98a7cce2\\Symfony\\Component\\PropertyInfo\\Extractor\\ReflectionExtractor', 'MAGIC_SET');
        if (!$enableMagicCallExtractionValue) {
            return new \PhpParser\Node\Expr\BinaryOp\BitwiseOr($classConstFetch, $magicSet);
        }
        return new \PhpParser\Node\Expr\BinaryOp\BitwiseOr(new \PhpParser\Node\Expr\BinaryOp\BitwiseOr($this->createClassConstFetch('_PhpScoper5edc98a7cce2\\Symfony\\Component\\PropertyInfo\\Extractor\\ReflectionExtractor', 'MAGIC_CALL'), $classConstFetch), $magicSet);
    }
}
