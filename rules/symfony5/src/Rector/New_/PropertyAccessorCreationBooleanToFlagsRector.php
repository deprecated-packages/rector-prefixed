...
CODE_SAMPLE
    ...
    private function shouldSkip(\PhpParser\Node\Expr\New_ $new) : bool
    {
        ...
        if (!$this->isName($new->class, '_PhpScoperfce0de0de1ce\\Symfony\\Component\\PropertyAccess\\PropertyAccessor')) {
            return \true;
        }
    ...
    {
        $classConstFetch = $this->createClassConstFetch('_PhpScoperfce0de0de1ce\\Symfony\\Component\\PropertyAccess\\PropertyAccessor', 'MAGIC_GET');
        $magicSet = $this->createClassConstFetch('_PhpScoperfce0de0de1ce\\Symfony\\Component\\PropertyAccess\\PropertyAccessor', 'MAGIC_SET');
        if (!$currentValue) {
            return new \PhpParser\Node\Expr\BinaryOp\BitwiseOr($classConstFetch, $magicSet);
        }
        return new \PhpParser\Node\Expr\BinaryOp\BitwiseOr(new \PhpParser\Node\Expr\BinaryOp\BitwiseOr($this->createClassConstFetch('_PhpScoperfce0de0de1ce\\Symfony\\Component\\PropertyAccess\\PropertyAccessor', 'MAGIC_CALL'), $classConstFetch), $magicSet);
    }
}
