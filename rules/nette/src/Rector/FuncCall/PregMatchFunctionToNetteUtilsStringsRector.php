 ...
 */
final class PregMatchFunctionToNetteUtilsStringsRector extends \_PhpScoper0a2ac50786fa\Rector\Nette\Rector\FuncCall\AbstractPregToNetteUtilsStringsRector
{
    /**
    ...
    {
        return new \_PhpScoper0a2ac50786fa\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Use Nette\\Utils\\Strings over bare preg_match() and preg_match_all() functions', [new \_PhpScoper0a2ac50786fa\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
class SomeClass
{
    public function run()
    {
...
CODE_SAMPLE
    ...
    private function refactorFuncCall(\_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\FuncCall $funcCall) : ?\_PhpScoper0a2ac50786fa\PhpParser\Node\Expr
    {
        ...
        if ($parentNode instanceof \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\Assign) {
            if ($methodName === 'matchAll') {
                // use count
                return new \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\FuncCall(new \_PhpScoper0a2ac50786fa\PhpParser\Node\Name('count'), [new \_PhpScoper0a2ac50786fa\PhpParser\Node\Arg($matchStaticCall)]);
            }
            return null;
        }
        // assign
     ...
     * @return Arg[]
     */
    private function compensateMatchAllEnforcedFlag(string $methodName, \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\FuncCall $funcCall, array $args) : array
    {
        if ($methodName !== 'matchAll') {
            return $args;
        }
        ...
        $constFetch = new \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\ConstFetch(new \_PhpScoper0a2ac50786fa\PhpParser\Node\Name('PREG_SET_ORDER'));
        $minus = new \_PhpScoper0a2ac50786fa\PhpParser\Node\Expr\BinaryOp\Minus($constFetch, new \_PhpScoper0a2ac50786fa\PhpParser\Node\Scalar\LNumber(1));
        $args[] = new \_PhpScoper0a2ac50786fa\PhpParser\Node\Arg($minus);
        return $args;
    }
}
