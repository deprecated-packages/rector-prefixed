 ...
 */
final class PregFunctionToNetteUtilsStringsRector extends \_PhpScopere8e811afab72\Rector\Nette\Rector\FuncCall\AbstractPregToNetteUtilsStringsRector
{
    /**
    ...
    {
        return new \_PhpScopere8e811afab72\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Use Nette\\Utils\\Strings over bare preg_split() and preg_replace() functions', [new \_PhpScopere8e811afab72\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
class SomeClass
{
    public function run()
    {
...
CODE_SAMPLE
    ...
    private function refactorIdentical(\_PhpScopere8e811afab72\PhpParser\Node\Expr\BinaryOp\Identical $identical) : ?\_PhpScopere8e811afab72\PhpParser\Node\Expr\Cast\Bool_
    {
        ...
        if ($identical->right instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\FuncCall) {
            ...
            if ($refactoredFuncCall !== null && $this->isValue($identical->left, 1)) {
                return new \_PhpScopere8e811afab72\PhpParser\Node\Expr\Cast\Bool_($refactoredFuncCall);
            }
        }
        return null;
    }
     ...
     */
    private function processSplit(\_PhpScopere8e811afab72\PhpParser\Node\Expr\FuncCall $funcCall, \_PhpScopere8e811afab72\PhpParser\Node\Expr\StaticCall $matchStaticCall) : \_PhpScopere8e811afab72\PhpParser\Node\Expr
    {
     ...
     */
    private function compensateNetteUtilsSplitDelimCapture(\_PhpScopere8e811afab72\PhpParser\Node\Expr\StaticCall $staticCall) : \_PhpScopere8e811afab72\PhpParser\Node\Expr\StaticCall
    {
        $patternValue = $this->getValue($staticCall->args[1]->value);
        if (!\is_string($patternValue)) {
            return $staticCall;
        }
        $match = \_PhpScopere8e811afab72\Nette\Utils\Strings::match($patternValue, self::SLASH_REGEX);
        if ($match === null) {
            return $staticCall;
        }
        $constFetch = new \_PhpScopere8e811afab72\PhpParser\Node\Expr\ConstFetch(new \_PhpScopere8e811afab72\PhpParser\Node\Name('PREG_SPLIT_DELIM_CAPTURE'));
        $bitwiseAnd = new \_PhpScopere8e811afab72\PhpParser\Node\Expr\BinaryOp\BitwiseAnd(new \_PhpScopere8e811afab72\PhpParser\Node\Scalar\LNumber(0), new \_PhpScopere8e811afab72\PhpParser\Node\Expr\BitwiseNot($constFetch));
        $staticCall->args[2] = new \_PhpScopere8e811afab72\PhpParser\Node\Arg($bitwiseAnd);
        return $staticCall;
    }
}
