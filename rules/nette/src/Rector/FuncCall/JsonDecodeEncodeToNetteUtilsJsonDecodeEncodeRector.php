    ...
    private function refactorJsonEncode(\PhpParser\Node\Expr\FuncCall $funcCall) : \PhpParser\Node\Expr\StaticCall
    {
        ...
        if (isset($args[1])) {
            ...
            if ($this->isName($secondArgumentValue, 'JSON_PRETTY_PRINT')) {
                $classConstFetch = $this->createClassConstFetch('_PhpScoper5b8c9e9ebd21\\Nette\\Utils\\Json', 'PRETTY');
                $args[1] = new \PhpParser\Node\Arg($classConstFetch);
            }
        }
        return $this->createStaticCall('_PhpScoper5b8c9e9ebd21\\Nette\\Utils\\Json', 'encode', $args);
    }
    private function refactorJsonDecode(\PhpParser\Node\Expr\FuncCall $funcCall) : \PhpParser\Node\Expr\StaticCall
    {
        ...
        if (isset($args[1])) {
            ...
            if ($this->isFalse($secondArgumentValue)) {
                ...
                $classConstFetch = $this->createClassConstFetch('_PhpScoper5b8c9e9ebd21\\Nette\\Utils\\Json', 'FORCE_ARRAY');
                $args[1] = new \PhpParser\Node\Arg($classConstFetch);
            }
        }
        return $this->createStaticCall('_PhpScoper5b8c9e9ebd21\\Nette\\Utils\\Json', 'decode', $args);
    }
}
