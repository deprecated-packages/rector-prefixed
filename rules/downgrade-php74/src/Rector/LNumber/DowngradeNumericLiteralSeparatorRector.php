...
final class DowngradeNumericLiteralSeparatorRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \RectorPrefix20201227\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Remove "_" as thousands separator in numbers', [new \RectorPrefix20201227\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
class SomeClass
{
    public function run()
    {
...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
         ...
         * Then, add it again.
         */
        if ($node instanceof \PhpParser\Node\Scalar\DNumber && !\RectorPrefix20201227\Nette\Utils\Strings::contains($node->value, '.')) {
            $node->value .= '.0';
        }
        return $node;
    }
