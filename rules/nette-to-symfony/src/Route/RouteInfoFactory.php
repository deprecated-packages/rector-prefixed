...
namespace _PhpScopere8e811afab72\Rector\NetteToSymfony\Route;
...
final class RouteInfoFactory
{
    ...
    public function __construct(\_PhpScopere8e811afab72\Rector\NodeNameResolver\NodeNameResolver $nodeNameResolver, \_PhpScopere8e811afab72\Rector\NodeCollector\NodeCollector\ParsedNodeCollector $parsedNodeCollector, \_PhpScopere8e811afab72\Rector\Core\PhpParser\Node\Value\ValueResolver $valueResolver)
    {
        $this->nodeNameResolver = $nodeNameResolver;
        $this->valueResolver = $valueResolver;
        $this->parsedNodeCollector = $parsedNodeCollector;
    }
    public function createFromNode(\_PhpScopere8e811afab72\PhpParser\Node $node) : ?\_PhpScopere8e811afab72\Rector\NetteToSymfony\ValueObject\RouteInfo
    {
     ...
     */
    private function createRouteInfoFromArgs(\_PhpScopere8e811afab72\PhpParser\Node $node, array $methods = []) : ?\_PhpScopere8e811afab72\Rector\NetteToSymfony\ValueObject\RouteInfo
    {
        ...
        if ($targetNode instanceof \_PhpScopere8e811afab72\PhpParser\Node\Expr\ClassConstFetch) {
            return $this->createForClassConstFetch($node, $methods, $routePath);
        }
        if ($targetNode instanceof \_PhpScopere8e811afab72\PhpParser\Node\Scalar\String_) {
     ...
     * @param string[] $methods
     */
    private function createForClassConstFetch(\_PhpScopere8e811afab72\PhpParser\Node $node, array $methods, string $routePath) : ?\_PhpScopere8e811afab72\Rector\NetteToSymfony\ValueObject\RouteInfo
    {
        ...
        if ($this->nodeNameResolver->isName($controllerMethodNode->name, 'class')) {
            ...
            if (\method_exists($presenterClass, 'run')) {
                return new \_PhpScopere8e811afab72\Rector\NetteToSymfony\ValueObject\RouteInfo($presenterClass, 'run', $routePath, $methods);
            }
        }
        return null;
    }
    private function createForString(\_PhpScopere8e811afab72\PhpParser\Node\Scalar\String_ $string, string $routePath) : ?\_PhpScopere8e811afab72\Rector\NetteToSymfony\ValueObject\RouteInfo
    {
        $targetValue = $string->value;
        if (!\_PhpScopere8e811afab72\Nette\Utils\Strings::contains($targetValue, ':')) {
            return null;
        }
        ...
        return new \_PhpScopere8e811afab72\Rector\NetteToSymfony\ValueObject\RouteInfo($controllerClass, $methodName, $routePath, []);
    }
}
