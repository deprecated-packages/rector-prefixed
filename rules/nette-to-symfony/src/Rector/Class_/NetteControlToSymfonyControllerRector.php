...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        $node->extends = new \PhpParser\Node\Name\FullyQualified(\RectorPrefix20210206\Symfony\Bundle\FrameworkBundle\Controller\AbstractController::class);
    ...
    {
        if (!\RectorPrefix20210206\Nette\Utils\Strings::endsWith($content, $suffix)) {
            return $content;
        }
        return \RectorPrefix20210206\Nette\Utils\Strings::substring($content, 0, -\RectorPrefix20210206\Nette\Utils\Strings::length($suffix));
    }
    private function processRenderMethod(\PhpParser\Node\Stmt\ClassMethod $classMethod) : void
    {
        ...
        if ($this->isAtLeastPhpVersion(\Rector\Core\ValueObject\PhpVersionFeature::SCALAR_TYPES)) {
            $classMethod->returnType = new \PhpParser\Node\Name\FullyQualified(\RectorPrefix20210206\Symfony\Component\HttpFoundation\Response::class);
        }
