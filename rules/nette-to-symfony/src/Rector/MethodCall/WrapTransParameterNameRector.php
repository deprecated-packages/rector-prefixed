 ...
 */
final class WrapTransParameterNameRector extends \_PhpScoper0a2ac50786fa\Rector\Core\Rector\AbstractRector
{
    /**
     * @var string
    ...
    {
        return new \_PhpScoper0a2ac50786fa\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Adds %% to placeholder name of trans() method if missing', [new \_PhpScoper0a2ac50786fa\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node->var, '_PhpScoper0a2ac50786fa\\Symfony\\Component\\Translation\\TranslatorInterface')) {
            return null;
        }
        ...
        foreach ($parametersArrayNode->items as $arrayItem) {
            ...
            if (\_PhpScoper0a2ac50786fa\Nette\Utils\Strings::match($arrayItem->key->value, self::BETWEEN_PERCENT_CHARS_REGEX)) {
                continue;
            }
            $arrayItem->key = new \_PhpScoper0a2ac50786fa\PhpParser\Node\Scalar\String_('%' . $arrayItem->key->value . '%');
        }
        return $node;
    }
}
