...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node->var, '_PhpScoperabd03f0baf05\\Nette\\Http\\Request')) {
            return null;
        }
        if (!$this->isName($node->name, 'getHeader')) {
            return null;
        }
        $requestName = $this->classMethodManipulator->addMethodParameterIfMissing($node, \_PhpScoperabd03f0baf05\Symfony\Component\HttpFoundation\Request::class, ['request', 'symfonyRequest']);
