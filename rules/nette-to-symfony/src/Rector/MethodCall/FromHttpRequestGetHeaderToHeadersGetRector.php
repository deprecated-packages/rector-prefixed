...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node->var, '_PhpScoper267b3276efc2\\Nette\\Http\\Request')) {
            return null;
        }
        if (!$this->isName($node->name, 'getHeader')) {
            return null;
        }
        $requestName = $this->classMethodManipulator->addMethodParameterIfMissing($node, \_PhpScoper267b3276efc2\Symfony\Component\HttpFoundation\Request::class, ['request', 'symfonyRequest']);
