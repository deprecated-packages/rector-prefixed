...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node->var, '_PhpScoperbd5d0c5f7638\\Nette\\Http\\Request')) {
            return null;
        }
        if (!$this->isName($node->name, 'getHeader')) {
            return null;
        }
        $requestName = $this->classMethodManipulator->addMethodParameterIfMissing($node, \_PhpScoperbd5d0c5f7638\Symfony\Component\HttpFoundation\Request::class, ['request', 'symfonyRequest']);
