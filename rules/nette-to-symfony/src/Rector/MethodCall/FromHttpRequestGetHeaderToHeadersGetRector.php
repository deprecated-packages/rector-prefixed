...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node->var, '_PhpScoperfce0de0de1ce\\Nette\\Http\\Request')) {
            return null;
        }
        if (!$this->isName($node->name, 'getHeader')) {
            return null;
        }
        $requestName = $this->classMethodManipulator->addMethodParameterIfMissing($node, \_PhpScoperfce0de0de1ce\Symfony\Component\HttpFoundation\Request::class, ['request', 'symfonyRequest']);
