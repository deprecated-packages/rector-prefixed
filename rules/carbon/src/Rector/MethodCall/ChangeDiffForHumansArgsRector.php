...
final class ChangeDiffForHumansArgsRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \Symplify\RuleDocGenerator\ValueObject\RuleDefinition('_PhpScoper5edc98a7cce2\\Change methods arguments of diffForHumans() on Carbon\\Carbon', [new \Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
use Carbon\Carbon;
...
CODE_SAMPLE
    ...
    {
        if (!$this->isOnClassMethodCall($node, '_PhpScoper5edc98a7cce2\\Carbon\\Carbon', 'diffForHumans')) {
            return null;
        }
        ...
        if ($this->isTrue($secondArgValue)) {
            $node->args[1]->value = $this->createClassConstFetch('_PhpScoper5edc98a7cce2\\Carbon\\CarbonInterface', 'DIFF_ABSOLUTE');
            return $node;
        }
        if ($this->isFalse($secondArgValue)) {
            $node->args[1]->value = $this->createClassConstFetch('_PhpScoper5edc98a7cce2\\Carbon\\CarbonInterface', 'DIFF_RELATIVE_AUTO');
            return $node;
        }
        return null;
    }
