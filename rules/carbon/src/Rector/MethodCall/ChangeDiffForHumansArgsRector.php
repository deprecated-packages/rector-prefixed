...
final class ChangeDiffForHumansArgsRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \RectorPrefix20201227\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('RectorPrefix20201227\\Change methods arguments of diffForHumans() on Carbon\\Carbon', [new \RectorPrefix20201227\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
use Carbon\Carbon;
...
CODE_SAMPLE
    ...
    {
        if (!$this->isOnClassMethodCall($node, 'RectorPrefix20201227\\Carbon\\Carbon', 'diffForHumans')) {
            return null;
        }
        ...
        if ($this->isTrue($secondArgValue)) {
            $node->args[1]->value = $this->createClassConstFetch('RectorPrefix20201227\\Carbon\\CarbonInterface', 'DIFF_ABSOLUTE');
            return $node;
        }
        if ($this->isFalse($secondArgValue)) {
            $node->args[1]->value = $this->createClassConstFetch('RectorPrefix20201227\\Carbon\\CarbonInterface', 'DIFF_RELATIVE_AUTO');
            return $node;
        }
        return null;
    }
