...
namespace _PhpScoper2a4e7ab1ecbc\Rector\CakePHP\Rector\Namespace_;
...
use _PhpScoper2a4e7ab1ecbc\Rector\CakePHP\Naming\CakePHPFullyQualifiedClassNameResolver;
 ...
 */
final class AppUsesStaticCallToUseStatementRector extends \_PhpScoper2a4e7ab1ecbc\Rector\Core\Rector\AbstractRector
{
    /**
     * @var CakePHPFullyQualifiedClassNameResolver
     */
    private $cakePHPFullyQualifiedClassNameResolver;
    public function __construct(\_PhpScoper2a4e7ab1ecbc\Rector\CakePHP\Naming\CakePHPFullyQualifiedClassNameResolver $cakePHPFullyQualifiedClassNameResolver)
    {
        $this->cakePHPFullyQualifiedClassNameResolver = $cakePHPFullyQualifiedClassNameResolver;
    }
    public function getRuleDefinition() : \_PhpScoper2a4e7ab1ecbc\Symplify\RuleDocGenerator\ValueObject\RuleDefinition
    {
        return new \_PhpScoper2a4e7ab1ecbc\Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Change App::uses() to use imports', [new \_PhpScoper2a4e7ab1ecbc\Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
...
CODE_SAMPLE
     ...
     * @return StaticCall[]
     */
    private function collectAppUseStaticCalls(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) : array
    {
        /** @var StaticCall[] $appUsesStaticCalls */
        $appUsesStaticCalls = $this->betterNodeFinder->find($node, function (\_PhpScoper2a4e7ab1ecbc\PhpParser\Node $node) : bool {
     ...
     * @param Use_[] $uses
     */
    private function refactorFile(\_PhpScoper2a4e7ab1ecbc\Rector\Core\PhpParser\Node\CustomNode\FileWithoutNamespace $fileWithoutNamespace, array $uses) : ?\_PhpScoper2a4e7ab1ecbc\Rector\Core\PhpParser\Node\CustomNode\FileWithoutNamespace
    {
        $hasNamespace = $this->betterNodeFinder->findFirstInstanceOf($fileWithoutNamespace, \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\Namespace_::class);
        // already handled above
        if ($hasNamespace !== null) {
            return null;
        }
        $hasDeclare = $this->betterNodeFinder->findFirstInstanceOf($fileWithoutNamespace, \_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Stmt\Declare_::class);
        if ($hasDeclare !== null) {
    ...
    private function createFullyQualifiedNameFromAppUsesStaticCall(\_PhpScoper2a4e7ab1ecbc\PhpParser\Node\Expr\StaticCall $staticCall) : string
    {
     ...
     * @param Use_[] $uses
     */
    private function refactorFileWithDeclare(\_PhpScoper2a4e7ab1ecbc\Rector\Core\PhpParser\Node\CustomNode\FileWithoutNamespace $fileWithoutNamespace, array $uses) : \_PhpScoper2a4e7ab1ecbc\Rector\Core\PhpParser\Node\CustomNode\FileWithoutNamespace
    {
        $newStmts = [];
        ...
        return new \_PhpScoper2a4e7ab1ecbc\Rector\Core\PhpParser\Node\CustomNode\FileWithoutNamespace($newStmts);
    }
}
