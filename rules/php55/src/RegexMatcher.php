...
final class RegexMatcher
{
    ...
    public function resolvePatternExpressionWithoutEIfFound(\PhpParser\Node\Expr $expr) : ?\PhpParser\Node\Expr
    {
        if ($expr instanceof \PhpParser\Node\Scalar\String_) {
            ...
            $modifiers = \RectorPrefix20210109\Nette\Utils\Strings::after($pattern, $delimiter, -1);
            if (!\RectorPrefix20210109\Nette\Utils\Strings::contains($modifiers, 'e')) {
                return null;
            }
    ...
    {
        $modifiersWithoutE = \RectorPrefix20210109\Nette\Utils\Strings::replace($modifiers, '#e#', '');
        return \RectorPrefix20210109\Nette\Utils\Strings::before($pattern, $delimiter, -1) . $delimiter . $modifiersWithoutE;
    }
    private function matchConcat(\PhpParser\Node\Expr\BinaryOp\Concat $concat) : ?\PhpParser\Node\Expr
    {
        ...
        $matches = \RectorPrefix20210109\Nette\Utils\Strings::match($lastItem->value, self::LETTER_SUFFIX_REGEX);
        if (!isset($matches['modifiers'])) {
            return null;
        }
        if (!\RectorPrefix20210109\Nette\Utils\Strings::contains($matches['modifiers'], 'e')) {
            return null;
        }
        // replace last "e" in the code
        $lastItem->value = \RectorPrefix20210109\Nette\Utils\Strings::replace($lastItem->value, self::LAST_E_REGEX, '$1$2');
        return $concat;
    }
}
