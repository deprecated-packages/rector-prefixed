...
use RectorPrefix20210321\Symplify\SetConfigResolver\ConfigResolver;
...
final class RectorConfigsResolver
{
    ...
    {
        $this->configResolver = new \RectorPrefix20210321\Symplify\SetConfigResolver\ConfigResolver();
        $rectorSetProvider = new \Rector\Set\RectorSetProvider();
        $this->setAwareConfigResolver = new \RectorPrefix20210321\Symplify\SetConfigResolver\SetAwareConfigResolver($rectorSetProvider);
    }
    /**
     * @return SmartFileInfo[]
     */
    public function resolveFromConfigFileInfo(\RectorPrefix20210321\Symplify\SmartFileSystem\SmartFileInfo $configFileInfo) : array
    {
    ...
    public function provide() : \Rector\Core\ValueObject\Bootstrap\BootstrapConfigs
    {
        ...
        if (\in_array($argvInput->getFirstArgument(), ['generate', 'g', 'create', 'c'], \true)) {
            ...
            if (\file_exists($rectorRecipeFilePath)) {
                $configFileInfos[] = new \RectorPrefix20210321\Symplify\SmartFileSystem\SmartFileInfo($rectorRecipeFilePath);
            }
        }
        ...
        if ($argvInput->getFirstArgument() === 'sync-annotation-parser') {
            $configFileInfos[] = new \RectorPrefix20210321\Symplify\SmartFileSystem\SmartFileInfo(__DIR__ . '/../../utils/doctrine-annotation-parser-syncer/config/config.php');
        }
