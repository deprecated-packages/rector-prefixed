...
final class MinimalVersionChecker
{
    ...
    public function __construct(string $installedPhpVersion, \Rector\Core\Configuration\MinimalVersionChecker\ComposerJsonParser $composerJsonParser)
    {
        $this->installedPhpVersion = $installedPhpVersion;
        $this->composerJsonParser = $composerJsonParser;
        $this->phpVersionFactory = new \Rector\Core\Util\PhpVersionFactory();
    }
    public function check() : void
    {
        ...
        if ($intInstalledPhpVersion < $intMinimumPhpVersion) {
            throw new \Rector\Core\Exception\Application\PhpVersionException(\sprintf('PHP version %s or higher is required, but you currently have %s installed.', $minimumPhpVersion, $this->installedPhpVersion));
        }
    }
}
