...
final class IfManipulator
{
     ...
     */
    public function matchIfNotNullReturnValue(If_ $if)
    {
        $stmts = $if->stmts;
        if (count($stmts) !== 1) {
            return null;
     ...
     */
    public function matchIfNotNullNextAssignment(If_ $if)
    {
        if ($if->stmts === []) {
            return null;
        }
     ...
     * @return \PhpParser\Node\Expr|null
     */
    public function matchIfValueReturnValue(If_ $if)
    {
        $stmts = $if->stmts;
     ...
     * @return \PhpParser\Node\Expr|null
     */
    private function matchComparedAndReturnedNode(NotIdentical $notIdentical, Return_ $return)
    {
