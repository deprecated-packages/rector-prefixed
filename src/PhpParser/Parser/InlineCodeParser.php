...
final class InlineCodeParser
{
    ...
    public function __construct(\Rector\Core\PhpParser\Printer\BetterStandardPrinter $betterStandardPrinter, \Rector\NodeTypeResolver\NodeScopeAndMetadataDecorator $nodeScopeAndMetadataDecorator, \PhpParser\Parser $parser, \RectorPrefix20210506\Symplify\SmartFileSystem\SmartFileSystem $smartFileSystem)
    {
    ...
    public function parse(string $content) : array
    {
        ...
        $content = \RectorPrefix20210506\Nette\Utils\Strings::match($content, self::OPEN_PHP_TAG_REGEX) ? $content : '<?php ' . $content;
        $content = \RectorPrefix20210506\Nette\Utils\Strings::match($content, self::ENDING_SEMI_COLON_REGEX) ? $content : $content . ';';
    ...
    public function stringify(\PhpParser\Node\Expr $expr) : string
    {
        ...
        if ($expr instanceof \PhpParser\Node\Scalar\Encapsed) {
            ...
            // use \$ → $
            $expr = \RectorPrefix20210506\Nette\Utils\Strings::replace($expr, self::PRESLASHED_DOLLAR_REGEX, '$');
            // use \'{$...}\' → $...
            return \RectorPrefix20210506\Nette\Utils\Strings::replace($expr, self::CURLY_BRACKET_WRAPPER_REGEX, '$1');
        }
