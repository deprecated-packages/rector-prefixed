...
final class InlineCodeParser
{
    ...
    public function parse(string $content) : array
    {
        ...
        $content = \RectorPrefix20210314\Nette\Utils\Strings::startsWith($content, '<?php ') ? $content : '<?php ' . $content;
        $content = \RectorPrefix20210314\Nette\Utils\Strings::endsWith($content, ';') ? $content : $content . ';';
    ...
    public function stringify(\PhpParser\Node\Expr $expr) : string
    {
        ...
        if ($expr instanceof \PhpParser\Node\Scalar\Encapsed) {
            ...
            // use \$ → $
            $expr = \RectorPrefix20210314\Nette\Utils\Strings::replace($expr, self::PRESLASHED_DOLLAR_REGEX, '$');
            // use \'{$...}\' → $...
            return \RectorPrefix20210314\Nette\Utils\Strings::replace($expr, self::CURLY_BRACKET_WRAPPER_REGEX, '$1');
        }
