...
final class InlineCodeParser
{
    ...
    public function parse(string $content) : array
    {
        ...
        $content = \RectorPrefix20210215\Nette\Utils\Strings::startsWith($content, '<?php ') ? $content : '<?php ' . $content;
        $content = \RectorPrefix20210215\Nette\Utils\Strings::endsWith($content, ';') ? $content : $content . ';';
    ...
    public function stringify($content) : string
    {
        ...
        if ($content instanceof \PhpParser\Node\Scalar\Encapsed) {
            ...
            // use \$ → $
            $content = \RectorPrefix20210215\Nette\Utils\Strings::replace($content, self::PRESLASHED_DOLLAR_REGEX, '$');
            // use \'{$...}\' → $...
            return \RectorPrefix20210215\Nette\Utils\Strings::replace($content, self::CURLY_BRACKET_WRAPPER_REGEX, '$1');
        }
