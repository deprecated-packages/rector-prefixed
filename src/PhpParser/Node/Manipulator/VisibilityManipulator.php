...
use _PhpScoper0a6b37af0871\Rector\Core\Exception\InvalidNodeTypeException;
...
final class VisibilityManipulator
{
     ...
     * @var string[]
     */
    private const ALLOWED_NODE_TYPES = [\_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\ClassMethod::class, \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Property::class, \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\ClassConst::class, \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Class_::class];
    /**
     * @var string
     */
    ...
    public function makeNonStatic(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : void
    {
        ...
        $node->flags -= \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Class_::MODIFIER_STATIC;
    }
    /**
    ...
    public function makeNonFinal(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : void
    {
        ...
        $node->flags -= \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Class_::MODIFIER_FINAL;
    }
    /**
     ...
     * @param ClassMethod|Property|ClassConst $node
     */
    public function removeOriginalVisibilityFromFlags(\_PhpScoper0a6b37af0871\PhpParser\Node $node) : void
    {
        ...
        if ($node->isPublic()) {
            $node->flags -= \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Class_::MODIFIER_PUBLIC;
        }
        if ($node->isProtected()) {
            $node->flags -= \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Class_::MODIFIER_PROTECTED;
        }
        if ($node->isPrivate()) {
            $node->flags -= \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Class_::MODIFIER_PRIVATE;
        }
    }
    /**
     * @param ClassMethod|Property|ClassConst $node
    ...
    public function changeNodeVisibility(\_PhpScoper0a6b37af0871\PhpParser\Node $node, string $visibility) : void
    {
        if ($visibility === 'public') {
            ...
            throw new \_PhpScoper0a6b37af0871\Rector\Core\Exception\ShouldNotHappenException(\sprintf('Visibility "%s" is not valid. Use one of: ', \implode('", "', self::ALLOWED_VISIBILITIES)));
        }
    }
    /**
     * @param ClassMethod|Property|ClassConst $node
     ...
     * @param ClassMethod|Property|ClassConst $node
     */
    private function replaceVisibilityFlag(\_PhpScoper0a6b37af0871\PhpParser\Node $node, string $visibility) : void
    {
     ...
     */
    private function addVisibilityFlag(\_PhpScoper0a6b37af0871\PhpParser\Node $node, string $visibility) : void
    {
        $this->ensureIsClassMethodOrProperty($node, __METHOD__);
        if ($visibility === 'public') {
            $node->flags |= \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Class_::MODIFIER_PUBLIC;
        ...
        if ($visibility === self::ABSTRACT) {
            $node->flags |= \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Class_::MODIFIER_ABSTRACT;
        }
        if ($visibility === self::FINAL) {
            $node->flags |= \_PhpScoper0a6b37af0871\PhpParser\Node\Stmt\Class_::MODIFIER_FINAL;
        }
    }
    private function ensureIsClassMethodOrProperty(\_PhpScoper0a6b37af0871\PhpParser\Node $node, string $location) : void
    {
        ...
        throw new \_PhpScoper0a6b37af0871\Rector\Core\Exception\InvalidNodeTypeException(\sprintf('"%s" only accepts "%s" types. "%s" given.', $location, \implode('", "', self::ALLOWED_NODE_TYPES), \get_class($node)));
    }
}
