...
final class IfManipulator
{
    ...
    public function matchIfNotNullNextAssignment(\PhpParser\Node\Stmt\If_ $if) : ?\PhpParser\Node\Expr\Assign
    {
        ...
        if (!$if->cond instanceof \PhpParser\Node\Expr\BinaryOp\NotIdentical || !$this->isNotIdenticalNullCompare($if->cond)) {
            return null;
        }
        $insideIfNode = $if->stmts[0];
        if (!$insideIfNode instanceof \PhpParser\Node\Stmt\Expression || !$insideIfNode->expr instanceof \PhpParser\Node\Expr\Assign) {
            return null;
        }
    ...
    public function isIfCondUsingAssignIdenticalVariable(\PhpParser\Node $if, \PhpParser\Node $assign) : bool
    {
        ...
        return $if->cond instanceof \PhpParser\Node\Expr\BinaryOp\Identical && $this->betterStandardPrinter->areNodesEqual($this->getIfVar($if), $assign->var);
    }
    public function isIfCondUsingAssignNotIdenticalVariable(\PhpParser\Node\Stmt\If_ $if, \PhpParser\Node $node) : bool
    {
        ...
        return $if->cond instanceof \PhpParser\Node\Expr\BinaryOp\NotIdentical && !$this->betterStandardPrinter->areNodesEqual($this->getIfVar($if), $node->var);
    }
    ...
    private function isIfWithElse(\PhpParser\Node\Stmt\If_ $if) : bool
    {
        if ($if->else === null) {
            return \false;
        }
