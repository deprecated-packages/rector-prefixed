...
use RectorPrefix20210404\Symplify\Astral\ValueObject\NodeBuilder\PropertyBuilder;
/**
...
final class NodeFactory
{
    ...
    {
        $methodBuilder = new \RectorPrefix20210404\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder($name);
        $methodBuilder->makePublic();
        return $methodBuilder->getNode();
    }
    public function createParamFromNameAndType(string $name, ?\PHPStan\Type\Type $type) : \PhpParser\Node\Param
    {
        $paramBuilder = new \RectorPrefix20210404\Symplify\Astral\ValueObject\NodeBuilder\ParamBuilder($name);
        if ($type !== null) {
    ...
    {
        $propertyBuilder = new \RectorPrefix20210404\Symplify\Astral\ValueObject\NodeBuilder\PropertyBuilder($name);
        ...
        $phpDocInfo->addPhpDocTagNode(new \PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagNode('injet', new \PHPStan\PhpDocParser\Ast\PhpDoc\GenericTagValueNode('')));
        return $property;
    }
    ...
    public function createGetterClassMethodFromNameAndType(string $propertyName, ?\PhpParser\Node $typeNode) : \PhpParser\Node\Stmt\ClassMethod
    {
        ...
        $methodBuilder = new \RectorPrefix20210404\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder($getterMethod);
        $methodBuilder->makePublic();
    ...
    {
        $paramBuilder = new \RectorPrefix20210404\Symplify\Astral\ValueObject\NodeBuilder\ParamBuilder($propertyMetadata->getName());
