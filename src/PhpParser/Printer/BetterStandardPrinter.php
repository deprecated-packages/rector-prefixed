...
final class BetterStandardPrinter extends \PhpParser\PrettyPrinter\Standard
{
    ...
    public function printFormatPreserving(array $stmts, array $origStmts, array $origTokens) : string
    {
        ...
        if (\count($stmts) !== \count($origStmts) && !(bool) \RectorPrefix20210116\Nette\Utils\Strings::match($content, self::NEWLINE_END_REGEX)) {
            $content .= $this->nl;
        }
        return $content;
    }
    ...
    protected function pArray(array $nodes, array $origNodes, int &$pos, int $indentAdjustment, string $parentNodeType, string $subNodeName, $fixup) : ?string
    {
        ...
        return \RectorPrefix20210116\Nette\Utils\Strings::replace($content, self::EXTRA_SPACE_BEFORE_NOP_REGEX, '');
    }
    /**
    ...
    {
        return "'" . \RectorPrefix20210116\Nette\Utils\Strings::replace($string, self::QUOTED_SLASH_REGEX, '\\\\$0') . "'";
    }
    /**
    ...
    protected function pExpr_Closure(\PhpParser\Node\Expr\Closure $closure) : string
    {
        ...
        return \RectorPrefix20210116\Nette\Utils\Strings::replace($closureContent, self::USE_REGEX, '$1 (');
    }
    /**
    ...
    protected function pStmt_ClassMethod(\PhpParser\Node\Stmt\ClassMethod $classMethod) : string
    {
        ...
        return \RectorPrefix20210116\Nette\Utils\Strings::replace($content, self::REPLACE_COLON_WITH_SPACE_REGEX, '$1: ');
    }
    /**
    ...
    protected function pStmt_Declare(\PhpParser\Node\Stmt\Declare_ $declare) : string
    {
        ...
        return \RectorPrefix20210116\Nette\Utils\Strings::replace($declareString, '#\\s+#', '');
    }
    /**
    ...
    private function detectTabOrSpaceIndentCharacter(array $stmts) : void
    {
        ...
        foreach ($stmts as $stmt) {
            ...
            $whitespaces = \count(\RectorPrefix20210116\Nette\Utils\Strings::matchAll($fileInfo->getContents(), self::FOUR_SPACE_START_REGEX));
            $tabs = \count(\RectorPrefix20210116\Nette\Utils\Strings::matchAll($fileInfo->getContents(), '#^\\t#m'));
            // tab vs space
