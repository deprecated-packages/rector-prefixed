...
final class ContentPatcher
{
    ...
    {
        $matchesValidAnnotation = \RectorPrefix20210108\Nette\Utils\Strings::matchAll($originalContent, $validAnnotationRegex);
        if ($matchesValidAnnotation === []) {
            return $content;
        }
        $matchesInValidAnnotation = \RectorPrefix20210108\Nette\Utils\Strings::matchAll($content, $invalidAnnotationRegex);
    ...
    {
        $matchNoDuplicateComments = \RectorPrefix20210108\Nette\Utils\Strings::matchAll($originalContent, self::VALID_NO_DUPLICATE_COMMENT_REGEX);
    ...
    {
        $validAnnotation = \RectorPrefix20210108\Nette\Utils\Strings::replace($validAnnotation, self::SPACE_REGEX, '');
        $invalidAnnotation = \RectorPrefix20210108\Nette\Utils\Strings::replace($invalidAnnotation, self::SPACE_REGEX, '');
        if ($validAnnotationRegex !== self::VALID_ANNOTATION_ROUTE_REGEX) {
            $validAnnotation = \RectorPrefix20210108\Nette\Utils\Strings::replace($validAnnotation, self::STAR_QUOTE_PARENTHESIS_REGEX, '');
            $invalidAnnotation = \RectorPrefix20210108\Nette\Utils\Strings::replace($invalidAnnotation, self::STAR_QUOTE_PARENTHESIS_REGEX, '');
            if ($validAnnotationRegex === self::VALID_ANNOTATION_ROUTE_LOCALIZATION_REGEX) {
                $validAnnotation = \RectorPrefix20210108\Nette\Utils\Strings::replace($validAnnotation, self::ROUTE_LOCALIZATION_REPLACE_REGEX, '');
                $invalidAnnotation = \RectorPrefix20210108\Nette\Utils\Strings::replace($invalidAnnotation, self::ROUTE_LOCALIZATION_REPLACE_REGEX, '');
            }
            return $validAnnotation !== $invalidAnnotation;
        }
        $validAnnotation = \RectorPrefix20210108\Nette\Utils\Strings::replace($validAnnotation, self::ROUTE_VALID_REGEX, '');
        $invalidAnnotation = \RectorPrefix20210108\Nette\Utils\Strings::replace($invalidAnnotation, self::ROUTE_INVALID_REGEX, '');
        return $validAnnotation !== $invalidAnnotation;
    }
}
