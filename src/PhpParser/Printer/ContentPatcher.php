...
final class ContentPatcher
{
    ...
    public function cleanUpDuplicateContent(string $content) : string
    {
        foreach (self::MAY_DUPLICATE_FUNC_CALLS as $mayDuplicateFuncCall) {
            $matches = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::match($content, \sprintf(self::CODE_MAY_DUPLICATE_REGEX, $mayDuplicateFuncCall));
            if ($matches === null) {
                $matches = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::match($content, \sprintf(self::CODE_MAY_DUPLICATE_NO_BRACKET_REGEX, $mayDuplicateFuncCall));
            }
            if ($matches === null) {
                continue;
            }
            $firstMatch = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::replace($matches[0], self::SPACE_REGEX, '');
            $secondMatch = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::replace($matches[1], self::SPACE_REGEX, '');
    ...
    {
        $matchesValidAnnotation = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::matchAll($originalContent, $validAnnotationRegex);
        if ($matchesValidAnnotation === []) {
            return $content;
        }
        $matchesInValidAnnotation = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::matchAll($content, $invalidAnnotationRegex);
    ...
    {
        $validAnnotation = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::replace($validAnnotation, self::SPACE_REGEX, '');
        $invalidAnnotation = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::replace($invalidAnnotation, self::SPACE_REGEX, '');
        if ($validAnnotationRegex !== self::VALID_ANNOTATION_ROUTE_REGEX) {
            $validAnnotation = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::replace($validAnnotation, self::STAR_QUOTE_PARENTHESIS_REGEX, '');
            $invalidAnnotation = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::replace($invalidAnnotation, self::STAR_QUOTE_PARENTHESIS_REGEX, '');
            if ($validAnnotationRegex === self::VALID_ANNOTATION_ROUTE_LOCALIZATION_REGEX) {
                $validAnnotation = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::replace($validAnnotation, self::ROUTE_LOCALIZATION_REPLACE_REGEX, '');
                $invalidAnnotation = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::replace($invalidAnnotation, self::ROUTE_LOCALIZATION_REPLACE_REGEX, '');
            }
            return $validAnnotation !== $invalidAnnotation;
        }
        $validAnnotation = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::replace($validAnnotation, self::ROUTE_VALID_REGEX, '');
        $invalidAnnotation = \_PhpScoper88fe6e0ad041\Nette\Utils\Strings::replace($invalidAnnotation, self::ROUTE_INVALID_REGEX, '');
        return $validAnnotation !== $invalidAnnotation;
    }
}
