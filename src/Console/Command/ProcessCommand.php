...
use RectorPrefix20210215\Symplify\PackageBuilder\Console\ShellCode;
...
final class ProcessCommand extends \Rector\Core\Console\Command\AbstractCommand
{
    ...
    protected function configure() : void
    {
        ...
        $this->addArgument(\Rector\Core\Configuration\Option::SOURCE, \RectorPrefix20210215\Symfony\Component\Console\Input\InputArgument::OPTIONAL | \RectorPrefix20210215\Symfony\Component\Console\Input\InputArgument::IS_ARRAY, 'Files or directories to be upgraded.');
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_DRY_RUN, 'n', \RectorPrefix20210215\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'See diff of changes, do not save them to files.');
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_AUTOLOAD_FILE, 'a', \RectorPrefix20210215\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'File with extra autoload');
        $this->addOption(\Rector\Core\Configuration\Option::MATCH_GIT_DIFF, null, \RectorPrefix20210215\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Execute only on file(s) matching the git diff.');
        $names = $this->outputFormatterCollector->getNames();
        $description = \sprintf('Select output format: "%s".', \implode('", "', $names));
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_OUTPUT_FORMAT, 'o', \RectorPrefix20210215\Symfony\Component\Console\Input\InputOption::VALUE_OPTIONAL, $description, \Rector\ChangesReporting\Output\ConsoleOutputFormatter::NAME);
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_NO_PROGRESS_BAR, null, \RectorPrefix20210215\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Hide progress bar. Useful e.g. for nicer CI output.');
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_NO_DIFFS, null, \RectorPrefix20210215\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Hide diffs of changed files. Useful e.g. for nicer CI output.');
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_OUTPUT_FILE, null, \RectorPrefix20210215\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Location for file to dump result in. Useful for Docker or automated processes');
        $this->addOption(\Rector\Core\Configuration\Option::CACHE_DEBUG, null, \RectorPrefix20210215\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Debug changed file cache');
        $this->addOption(\Rector\Core\Configuration\Option::OPTION_CLEAR_CACHE, null, \RectorPrefix20210215\Symfony\Component\Console\Input\InputOption::VALUE_NONE, 'Clear unchaged files cache');
    }
    protected function execute(\RectorPrefix20210215\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210215\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        ...
        if ($this->errorAndDiffCollector->getErrors() !== []) {
            return \RectorPrefix20210215\Symplify\PackageBuilder\Console\ShellCode::ERROR;
        }
        // inverse error code for CI dry-run
        if ($this->configuration->isDryRun() && $this->errorAndDiffCollector->getFileDiffsCount()) {
            return \RectorPrefix20210215\Symplify\PackageBuilder\Console\ShellCode::ERROR;
        }
        return \RectorPrefix20210215\Symplify\PackageBuilder\Console\ShellCode::SUCCESS;
    }
    /**
     * @param string[] $paths
    ...
    private function findPhpFileInfos(array $paths) : array
    {
        ...
        $phpFileInfos = \array_filter($phpFileInfos, function (\RectorPrefix20210215\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo) : bool {
            return !\RectorPrefix20210215\Nette\Utils\Strings::endsWith($smartFileInfo->getPathname(), '.blade.php');
        });
