...
use RectorPrefix20210117\Composer\XdebugHandler\XdebugHandler;
...
use RectorPrefix20210117\Symplify\PackageBuilder\Console\Command\CommandNaming;
use RectorPrefix20210117\Symplify\SmartFileSystem\SmartFileInfo;
use Throwable;
final class ConsoleApplication extends \RectorPrefix20210117\Symfony\Component\Console\Application
{
    /**
     * @var string
     */
     ...
     */
    public function __construct(\Rector\Core\Configuration\Configuration $configuration, \Rector\Core\Bootstrap\NoRectorsLoadedReporter $noRectorsLoadedReporter, \RectorPrefix20210117\Symplify\PackageBuilder\Console\Command\CommandNaming $commandNaming, array $commands = [])
    {
        try {
    ...
    public function doRun(\RectorPrefix20210117\Symfony\Component\Console\Input\InputInterface $input, \RectorPrefix20210117\Symfony\Component\Console\Output\OutputInterface $output) : int
    {
        // @fixes https://github.com/rectorphp/rector/issues/2205
        $isXdebugAllowed = $input->hasParameterOption('--xdebug');
        if (!$isXdebugAllowed) {
            $xdebugHandler = new \RectorPrefix20210117\Composer\XdebugHandler\XdebugHandler('rector', '--ansi');
            $xdebugHandler->check();
            unset($xdebugHandler);
        }
        ...
        $dumpCommands = [\RectorPrefix20210117\Symplify\PackageBuilder\Console\Command\CommandNaming::classToName(\Rector\Utils\NodeDocumentationGenerator\Command\DumpNodesCommand::class)];
        ...
        if ($this->shouldPrintMetaInformation($input)) {
            ...
            if ($configFilePath) {
                $configFileInfo = new \RectorPrefix20210117\Symplify\SmartFileSystem\SmartFileInfo($configFilePath);
    ...
    public function renderThrowable(\Throwable $throwable, \RectorPrefix20210117\Symfony\Component\Console\Output\OutputInterface $output) : void
    {
    ...
    protected function getDefaultInputDefinition() : \RectorPrefix20210117\Symfony\Component\Console\Input\InputDefinition
    {
    ...
    private function shouldPrintMetaInformation(\RectorPrefix20210117\Symfony\Component\Console\Input\InputInterface $input) : bool
    {
    ...
    private function removeUnusedOptions(\RectorPrefix20210117\Symfony\Component\Console\Input\InputDefinition $inputDefinition) : void
    {
        $options = $inputDefinition->getOptions();
        unset($options['quiet'], $options['no-interaction']);
        $inputDefinition->setOptions($options);
    }
    private function addCustomOptions(\RectorPrefix20210117\Symfony\Component\Console\Input\InputDefinition $inputDefinition) : void
    {
        $inputDefinition->addOption(new \RectorPrefix20210117\Symfony\Component\Console\Input\InputOption(\Rector\Core\Configuration\Option::OPTION_CONFIG, 'c', \RectorPrefix20210117\Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED, 'Path to config file', $this->getDefaultConfigPath()));
