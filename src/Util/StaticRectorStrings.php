...
final class StaticRectorStrings
{
    ...
    public static function isInArrayInsensitive(string $checkedItem, array $array) : bool
    {
        foreach ($array as $item) {
            if (\RectorPrefix20210128\Nette\Utils\Strings::lower($item) === \RectorPrefix20210128\Nette\Utils\Strings::lower($checkedItem)) {
                return \true;
            }
        }
        return \false;
    ...
    public static function removePrefixes(string $value, array $prefixesToRemove) : string
    {
        foreach ($prefixesToRemove as $prefixToRemove) {
            if (\RectorPrefix20210128\Nette\Utils\Strings::startsWith($value, $prefixToRemove)) {
                $value = \RectorPrefix20210128\Nette\Utils\Strings::substring($value, \RectorPrefix20210128\Nette\Utils\Strings::length($prefixToRemove));
            }
        }
        return $value;
    }
    ...
    public static function removeSuffixes(string $value, array $suffixesToRemove) : string
    {
        foreach ($suffixesToRemove as $prefixToRemove) {
            if (\RectorPrefix20210128\Nette\Utils\Strings::endsWith($value, $prefixToRemove)) {
                $value = \RectorPrefix20210128\Nette\Utils\Strings::substring($value, 0, -\RectorPrefix20210128\Nette\Utils\Strings::length($prefixToRemove));
            }
        }
        return $value;
    }
    ...
    public static function constantToDashes(string $string) : string
    {
        ...
        return \RectorPrefix20210128\Nette\Utils\Strings::replace($string, self::UNDERSCORE_REGEX, '-');
    }
    ...
    private static function camelCaseToGlue(string $input, string $glue) : string
    {
        ...
        $matches = \RectorPrefix20210128\Nette\Utils\Strings::matchAll($input, self::CAMEL_CASE_SPLIT_REGEX);
        $parts = [];
