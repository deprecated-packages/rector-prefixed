...
final class StaticRectorStrings
{
    ...
    public static function camelCaseToUnderscore(string $input) : string
    {
        ...
        $matches = \RectorPrefix20210326\Nette\Utils\Strings::matchAll($input, self::CAMEL_CASE_SPLIT_REGEX);
        $parts = [];
    ...
    public static function removePrefixes(string $value, array $prefixesToRemove) : string
    {
        foreach ($prefixesToRemove as $prefixToRemove) {
            if (\RectorPrefix20210326\Nette\Utils\Strings::startsWith($value, $prefixToRemove)) {
                $value = \RectorPrefix20210326\Nette\Utils\Strings::substring($value, \RectorPrefix20210326\Nette\Utils\Strings::length($prefixToRemove));
            }
        }
        return $value;
    }
    ...
    public static function removeSuffixes(string $value, array $suffixesToRemove) : string
    {
        foreach ($suffixesToRemove as $suffixToRemove) {
            if (\RectorPrefix20210326\Nette\Utils\Strings::endsWith($value, $suffixToRemove)) {
                $value = \RectorPrefix20210326\Nette\Utils\Strings::substring($value, 0, -\RectorPrefix20210326\Nette\Utils\Strings::length($suffixToRemove));
            }
        }
        return $value;
    }
