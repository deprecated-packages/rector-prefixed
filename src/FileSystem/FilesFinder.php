...
use RectorPrefix20210411\Symfony\Component\Finder\Finder;
use RectorPrefix20210411\Symfony\Component\Finder\SplFileInfo;
use RectorPrefix20210411\Symplify\Skipper\SkipCriteriaResolver\SkippedPathsResolver;
...
final class FilesFinder
{
    ...
    public function __construct(\Rector\Core\FileSystem\FilesystemTweaker $filesystemTweaker, \RectorPrefix20210411\Symplify\SmartFileSystem\Finder\FinderSanitizer $finderSanitizer, \RectorPrefix20210411\Symplify\SmartFileSystem\FileSystemFilter $fileSystemFilter, \RectorPrefix20210411\Symplify\Skipper\SkipCriteriaResolver\SkippedPathsResolver $skippedPathsResolver)
    {
    ...
    public function findInDirectoriesAndFiles(array $source, array $suffixes) : array
    {
        ...
        foreach ($files as $file) {
            $smartFileInfos[] = new \RectorPrefix20210411\Symplify\SmartFileSystem\SmartFileInfo($file);
        }
    ...
    private function findInDirectories(array $directories, array $suffixes) : array
    {
        ...
        $finder = \RectorPrefix20210411\Symfony\Component\Finder\Finder::create()->followLinks()->files()->size('> 0')->in($absoluteDirectories)->name($suffixesPattern)->sortByName();
    ...
    private function addFilterWithExcludedPaths(\RectorPrefix20210411\Symfony\Component\Finder\Finder $finder) : void
    {
        $excludePaths = $this->skippedPathsResolver->resolve();
        if ($excludePaths === []) {
            return;
        }
        $finder->filter(function (\RectorPrefix20210411\Symfony\Component\Finder\SplFileInfo $splFileInfo) use($excludePaths) : bool {
            ...
            foreach ($excludePaths as $excludePath) {
                ...
                if (\RectorPrefix20210411\Nette\Utils\Strings::match($realPath, '#' . \preg_quote($excludePath, '#') . '#')) {
                    return \false;
                }
    ...
    {
        // ends with *
        if (\RectorPrefix20210411\Nette\Utils\Strings::match($path, self::ENDS_WITH_ASTERISK_REGEX)) {
            return '*' . $path;
        }
        // starts with *
        if (\RectorPrefix20210411\Nette\Utils\Strings::match($path, self::STARTS_WITH_ASTERISK_REGEX)) {
            return $path . '*';
        }
        return $path;
    }
