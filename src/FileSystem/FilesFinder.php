...
use _PhpScoper267b3276efc2\Symfony\Component\Finder\Finder;
use _PhpScoper267b3276efc2\Symfony\Component\Finder\SplFileInfo;
...
final class FilesFinder
{
    ...
    private function findInDirectories(array $directories, array $suffixes) : array
    {
        ...
        $finder = \_PhpScoper267b3276efc2\Symfony\Component\Finder\Finder::create()->followLinks()->files()->in($absoluteDirectories)->name($suffixesPattern)->sortByName();
    ...
    private function addFilterWithExcludedPaths(\_PhpScoper267b3276efc2\Symfony\Component\Finder\Finder $finder) : void
    {
        $excludePaths = $this->skippedPathsResolver->resolve();
        if ($excludePaths === []) {
            return;
        }
        $finder->filter(function (\_PhpScoper267b3276efc2\Symfony\Component\Finder\SplFileInfo $splFileInfo) use($excludePaths) : bool {
            ...
            foreach ($excludePaths as $excludePath) {
                ...
                if (\_PhpScoper267b3276efc2\Nette\Utils\Strings::match($realPath, '#' . \preg_quote($excludePath, '#') . '#')) {
                    return \false;
                }
    ...
    {
        // ends with *
        if (\_PhpScoper267b3276efc2\Nette\Utils\Strings::match($path, self::ENDS_WITH_ASTERISK_REGEX)) {
            return '*' . $path;
        }
        // starts with *
        if (\_PhpScoper267b3276efc2\Nette\Utils\Strings::match($path, self::STARTS_WITH_ASTERISK_REGEX)) {
            return $path . '*';
        }
        return $path;
    }
