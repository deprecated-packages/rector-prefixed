...
use _PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantArrayTypeAndMethod;
 ...
 */
final class ConstantArrayTypeToCallReflectionResolver implements \_PhpScoper0a2ac50786fa\Rector\Core\Contract\PHPStan\Reflection\TypeToCallReflectionResolver\TypeToCallReflectionResolverInterface
{
    /**
    ...
    {
        return $type instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantArrayType;
    }
    /**
     ...
     */
    private function findTypeAndMethodName(\_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantArrayType $constantArrayType) : ?\_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantArrayTypeAndMethod
    {
        ...
        $objectWithoutClassType = new \_PhpScoper0a2ac50786fa\PHPStan\Type\ObjectWithoutClassType();
        if ($classOrObjectType instanceof \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantStringType) {
            $value = $classOrObjectType->getValue();
            if (!$this->reflectionProvider->hasClass($value)) {
                return \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantArrayTypeAndMethod::createUnknown();
            }
            $classReflection = $this->reflectionProvider->getClass($value);
            $type = new \_PhpScoper0a2ac50786fa\PHPStan\Type\ObjectType($classReflection->getName());
        ...
        if (!$trinaryLogic->no()) {
            return \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantArrayTypeAndMethod::createConcrete($type, $methodType->getValue(), $trinaryLogic);
        }
        return null;
    }
    private function areKeyTypesValid(\_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantArrayType $constantArrayType) : bool
    {
        $keyTypes = $constantArrayType->getKeyTypes();
        if (\count($keyTypes) !== 2) {
            return \false;
        }
        if ($keyTypes[0]->isSuperTypeOf(new \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantIntegerType(0))->no()) {
            return \false;
        }
        return !$keyTypes[1]->isSuperTypeOf(new \_PhpScoper0a2ac50786fa\PHPStan\Type\Constant\ConstantIntegerType(1))->no();
    }
}
