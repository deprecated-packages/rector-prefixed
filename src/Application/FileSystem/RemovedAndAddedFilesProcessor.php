...
use _PhpScoper0a6b37af0871\Rector\Core\PhpParser\Printer\NodesWithFileDestinationPrinter;
...
final class RemovedAndAddedFilesProcessor
{
    ...
    public function __construct(\_PhpScoper0a6b37af0871\Rector\Core\Configuration\Configuration $configuration, \_PhpScoper0a6b37af0871\Symplify\SmartFileSystem\SmartFileSystem $smartFileSystem, \_PhpScoper0a6b37af0871\Rector\Core\PhpParser\Printer\NodesWithFileDestinationPrinter $nodesWithFileDestinationPrinter, \_PhpScoper0a6b37af0871\Rector\Core\Application\FileSystem\RemovedAndAddedFilesCollector $removedAndAddedFilesCollector, \_PhpScoper0a6b37af0871\Symfony\Component\Console\Style\SymfonyStyle $symfonyStyle, \_PhpScoper0a6b37af0871\Rector\Core\PhpParser\Printer\BetterStandardPrinter $betterStandardPrinter)
    {
    ...
    private function processMovedFiles() : void
    {
        foreach ($this->removedAndAddedFilesCollector->getMovedFiles() as $movedFile) {
            if ($this->configuration->isDryRun() && !\_PhpScoper0a6b37af0871\Rector\Testing\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
    ...
    private function printFileMoveWarning(\_PhpScoper0a6b37af0871\Rector\FileSystemRector\Contract\MovedFileInterface $movedFile, string $verb) : void
    {
        $message = \sprintf('File "%s" %s moved to "%s"', $movedFile->getOldPathname(), $verb, $movedFile->getNewPathname());
        $this->symfonyStyle->warning($message);
    }
    private function resolveFileContentFromMovedFile(\_PhpScoper0a6b37af0871\Rector\FileSystemRector\Contract\MovedFileInterface $movedFile) : string
    {
        if ($movedFile instanceof \_PhpScoper0a6b37af0871\Rector\FileSystemRector\ValueObject\MovedFileWithContent) {
            return $movedFile->getFileContent();
        }
        if ($movedFile instanceof \_PhpScoper0a6b37af0871\Rector\FileSystemRector\ValueObject\MovedFileWithNodes) {
            return $this->betterStandardPrinter->prettyPrintFile($movedFile->getNodes());
        }
        throw new \_PhpScoper0a6b37af0871\Rector\Core\Exception\NotImplementedYetException(\get_class($movedFile));
    }
}
